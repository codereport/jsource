...
typedef struct {
    ...
    A* tnextpushp;   // pointer to empty slot in allocated-block stack.  When low bits are 00..00, pointer to previous
                     // block of pointers.  Chain in first block is 0
    ...
    I modifiercounter;  // incremented whenever anything happens that could alter modifier lookup: assignment/deletion
                        // of a modifier, or any change to locales or path
...
#define PMCTRBSTKREQD 2              // set if we MUST create a stack entry for each named call
    ...
    A sf;                            /* for $:     $                                     */
    L* assignsym;  // symbol-table entry for the symbol about to be assigned   $   scaf  need to use LX when
                   // muvltithreaded
    D cct;         // complementary comparison tolerance $
                   // ----- end of cache line 1
    A nvra;    // data blocks that are in execution somewhere - always non-virtual, always rank 1, AS[0] holds current
               // pointer
    ...
    struct {
        I ballo;  // negative number of bytes in free pool, but with zero-point biased so that - means needs garbage
                  // collection
    ...
    C asgzomblevel;  // 0=do not assign zombie name before final assignment; 1=allow premature assignment of complete
                     // result; 2=allow premature assignment even of incomplete result   $
...
#define RECSTATERECUR \
    3  // JE is running and waiting for a prompt, and the host has made a recursive call to JDo (which must not prompt)
       // 2 bytes here
    ...
    A* tstackcurr;  // current allocation, holding NTSTACK bytes+1 block for alignment.  First entry points to
                    // next-lower allocation   $
    ...
    union {
        struct {
            D determ;  // determinant of the triangular matrix, if the matrix to be inverted was B01 or INT.  Set to 0
                       // to suppress INT rounding
        } minv;
        struct {
    ...
    LS callstack[1 + NFCALL];  // named fn calls: stack.  Usually only a little is used; the rest overflows onto a new
                               // DRAM page
