...
void
rlexit(int c) {
    if (!hist && histfile[0]) write_history(histfile);
}

char*
Jinput_rl(char* prompt) {
    static char* line = 0;
    if (hist) {
        char* s;
        hist        = 0;
...
void
addargv(int argc, char* argv[], char* d) {
    C *p, *q;
    I i;
...
main(int argc, char* argv[]) {
    ...
    if (!getrlimit(RLIMIT_STACK, &lim)) {
        if (lim.rlim_cur != RLIM_INFINITY && lim.rlim_cur < J_STACK) {
            lim.rlim_cur = (lim.rlim_max == RLIM_INFINITY) ? J_STACK
                           : (lim.rlim_max < J_STACK)      ? lim.rlim_max
                                                           : J_STACK;
    ...
    if (!norl && _isatty(_fileno(stdin))) breadline = readlineinit();
    ...
    if (!norl) { rl_readline_name = "jconsole"; /* argv[0] varies too much*/ }
