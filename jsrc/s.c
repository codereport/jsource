...
jtsymext(J jt, B b) {
    A x, y;
    I j, m, n /*obsolete ,s[2]*/, *v, xn, yn;
    L *u;
    if (b) {
        y  = jt->symp;
        j  = allosize(y) + NORMAH * SZI;
        yn = AN(y);
        n  = yn / symcol;
    }  // extract allo size from header (approx)
    else {
        j  = ((I)1) << 12;
        yn = 0;
        n  = 1;
    }                  // n is # rows in chain base + old values
    m = j << 1;        /* new size in bytes           */
    m -= AKXR(0);      /* less array overhead         */
    m /= symcol * SZI; /* new # rows                  */
    xn = m * symcol;   /* new pool array atoms        */
    GATV0(x, INT, xn, 0);
    v = (I *)LAV0(x);                    /* new pool array              */
...
// if tailx==0, append at head (immediately after *hv); if tailx!=0, append to tail.  If queue is empty, tail is always
// 0 result is new symbol
L *
jtsymnew(J jt, LX *hv, LX tailx) {
    LX j;
    L *u, *v;
    ...
    if (tailx) {
        L *t = tailx + LAV0(jt->symp);
        // appending to tail.  Queue is known to be nonempty
        u->next = 0;
        t->next = j;  // it's always the end: point to next & prev, and chain from prev
    } else {
        // appending to head.
        if (u->next = *hv) {
            v = *hv + LAV0(jt->symp);
        }         // chain old queue to u; if not empty, backchain old head to new one, clear old head flag,
        *hv = j;  // set new head
    }
    return u;
...
// Reset the fields in the deleted blocks.  If any modifiers are deleted, increment modifiercounter to reset name
// lookups This is used only for freeing local symbol tables, thus does not need to clear the name/path
extern void
jtsymfreeha(J jt, A w) {
    ...
    // loop through each hash chain, clearing the blocks in the chain.  Do not clear chain 0, which holds x/y bucket
    // numbers
    I ortypes = 0;
        ...
        if (k = *aprev) {
            ...
            do {
                ...
                if (jtsympv[k].val) {
                    ...
                    // if the value was abandoned to an explicit definition, we took usecount 8..1  -> 1 ; revert that.
                    // Can't change an ACPERMANENT! otherwise decrement the usecount
                    SYMVALFA(jtsympv[k]);
            ...
            *aprev = 0;  // only the PERMANENT survive
            ...
            if (k) {
                ...
                do {
                    aprev = &jtsympv[k].next;  // save last item we processed here
                    ortypes |=
                      AT(jtsympv[k].val);  // value must exist here, since the block wouldn't be created unless assigned
                    fr(jtsympv[k].name);
                    fa(jtsympv[k].val);
                    jtsympv[k].name = 0;
                    jtsympv[k].val  = 0;
  ...
  A jtsympool(J jt, A w) {
    ...
    for (i = 0; i < n; ++i) {                            // for each chain-base in locales pool
        for (j = v[i]; j; j = LAV0(jt->symp)[j].next) {  // j is index to named local entry; process the chain
            ...
            RZ(yv[j] = yv[LXAV0(x)[0]] = aa =
                 jtincorp(jt, jtsfn(jt, SFNSIMPLEONLY, LOCNAME(x))));  // install name in the entry for the locale
            RZ(q = jtsympoola(jt, x));
            u = AV(q);
            DO(AN(q), yv[u[i]] = aa;);
        }
    }
    n = jtcountnl(jt);
    for (i = 0; i < n; ++i)
        if (x = jtindexnl(jt, i)) { /* per numbered locales */
...
L *
jtprobedel(J jt, I l, C *string, UI4 hash, A g) {
    RZ(g);
    LX *asymx = LXAV0(g) + SYMHASH(hash, AN(g) - SYMLINFOSIZE);  // get pointer to index of start of chain
    while (1) {
        LX delblockx = *asymx;
        if (!delblockx) return 0;  // if chain empty or ended, not found
        L *sym = LAV0(jt->symp) + delblockx;
        IFCMPNAME(NAV(sym->name),
                  string,
                  l,  // (1) exact match - if there is a value, use this slot, else say not found
                  {
                      SYMVALFA(*sym);
                      sym->val = 0;  // decr usecount in value; remove value from symbol
                      if (!(sym->flag & LPERMANENT)) {
                          *asymx = sym->next;
                          fr(sym->name);
                          sym->name              = 0;
                          sym->flag              = 0;
                          sym->sn                = 0;
                          sym->next              = LAV0(jt->symp)[0].next;
                          LAV0(jt->symp)[0].next = delblockx;
                      }
                      return sym;
                  }
                  // if match, bend predecessor around deleted block, return address of match (now deleted but still
                  // points to value)
        )
...
L *
jtprobe(J jt, I l, C *string, UI4 hash, A g) {
    RZ(g);
    LX symx = LXAV0(g)[SYMHASH(hash, AN(g) - SYMLINFOSIZE)];  // get index of start of chain
    while (1) {
        if (!symx) return 0;  // if chain empty or ended, not found
        L *sym = LAV0(jt->symp) + symx;
        IFCMPNAME(
          NAV(sym->name),
          string,
          l,
          return sym->val ? sym : 0;)  // (1) exact match - if there is a value, use this slot, else say not found
...
L *
jtprobelocal(J jt, A a, A locsyms) {
    NM *u;
    I b, bx;
    ...
    if ((b = u->bucket) != 0) {
        if (0 > (bx = ~u->bucketx)) {
            // positive bucketx (now negative); that means skip that many items and then do name search.  This is set
            // for words that were recognized as names but were not detected as assigned-to in the definition If no new
            // names have been assigned since the table was created, we can skip this search, since it must fail (this
            // is the path for words in z eg)
            if (!(AR(locsyms) & LNAMEADDED)) return 0;
            LX lx = LXAV0(locsyms)[b];  // index of first block if any
            I m   = u->m;
            C *s  = u->s;  // length/addr of name from name block
            while (0 > ++bx) { lx = LAV0(jt->symp)[lx].next; }
            // Now lx is the index of the first name that might match.  Do the compares
            while (lx) {
                L *l = lx + LAV0(jt->symp);  // symbol entry
                IFCMPNAME(NAV(l->name), s, m, return l->val ? l : 0;)
                lx = l->next;
            }
            return 0;  // no match.
        } else {
    ...
    if (b = u->bucket) {
        ...
        if (0 > (bx = ~u->bucketx)) {
            ...
            while (lx) {
                ...
                IFCMPNAME(NAV(l->name), s, m, return l;)
                tx = lx;
                lx = l->next;
            }
            // not found, create new symbol.  If tx is 0, the queue is empty, so adding at the head is OK; otherwise add
            // after tx
            RZ(l = jtsymnew(jt, &LXAV0(jt->locsyms)[b], tx));
            ras(a);
            l->name = a;  // point symbol table to the name block, and increment its use count accordingly
            AR(jt->locsyms) |=
              LNAMEADDED;  // Mark that a name has been added beyond what was known at preprocessing time
            return l;
        } else {
        ...
        RZ(l);
        AR(jt->locsyms) |= (~l->flag) & LPERMANENT;  // Mark that a name has been added beyond what was known at
                                                     // preprocessing time, if the added name is not PERMANENT
        return l;
    }
}
...
jtprobeis(J jt, A a, A g) {
    ...
    m        = u->m;
    s        = u->s;
    hv       = LXAV0(g) + SYMHASH(u->hash, AN(g) - SYMLINFOSIZE);  // get bucket number among the hash tables
    if (tx = *hv) {                                                /* !*hv means (0) empty slot    */
        v = tx + sympv;
        while (1) {
            u = NAV(v->name);
            IFCMPNAME(u, s, m, return v;)  // (1) exact match - may or may not have value
            if (!v->next) break;           /* (2) link list end */
            v = (tx = v->next) + sympv;
        }
    }
    // not found, create new symbol.  If tx is 0, the queue is empty, so adding at the head is OK; otherwise add after
    // tx
    RZ(v = jtsymnew(jt, hv, tx));
    ras(a);
    v->name = a;  // point symbol table to the name block, and increment its use count accordingly
    return v;
...
static A
jtlocindirect(J jt, I n, C *u, UI4 hash) {
    A x, y;
    C *s, *v, *xv;
    I k, xn;
    A g;           // current locale.
    ...
    while (u < s) {
        ...
        while ('_' != *--v)
            ;
        ++v;  // v->start of last indirect locative
        k = s - v;
        ...
        if (AT(y) & (INT | B01)) {
            g = jtfindnl(jt, BIV0(y));
            ASSERT(g != 0, EVLOCALE);  // if atomic integer, look it up
        } else {
            ASSERTN(BOX & AT(y), EVDOMAIN, jtnfs(jt, k, v));  // verify box
            x = AAV(y)[0];
            if ((((I)AR(x) - 1) & -(AT(x) & (INT | B01))) < 0) {
                // Boxed integer - use that as bucketx
                g = jtfindnl(jt, BIV0(x));
                ASSERT(g != 0, EVLOCALE);  // boxed integer, look it up
            } else {
                xn = AN(x);
                xv = CAV(x);                                         // x->boxed contents, xn=length, xv->string
...
jtdllsymaddr(J jt, A w, C flag) {
    ...
    for (i = 0; i < n; ++i) {
        x = wv[i];
        v = jtsyrd(jt, jtnfs(jt, AN(x), CAV(x)), jt->locsyms);
        ASSERT(v != 0, EVVALUE);
        y = v->val;
...
A
jtdllsymget(J jt, A w) {
    return jtdllsymaddr(jt, w, 0);
}
A
jtdllsymdat(J jt, A w) {
    return jtdllsymaddr(jt, w, 1);
}
...
jtsymbrd(J jt, A w) {
    L *v;
    if (!w) return 0;
    ASSERTN(v = jtsyrd(jt, w, jt->locsyms), EVVALUE, w);
    return v->val;
}

// look up name w, return value unless locked or undefined; then return just the name
A
jtsymbrdlocknovalerr(J jt, A w) {
    A y;
    L *v;
    if (!(v = jtsyrd(jt, w, jt->locsyms))) {
        // no value.  Could be undefined name (no error) or some other error including value error, which means error
        // looking up an indirect locative If error, abort with it; if undefined, return a reference to the undefined
        // name
...
B
jtredef(J jt, A w, L *v) {
    A f;
    DC c, d;
    // find the most recent DCCALL, exit if none
    d = jt->sitop;
    while (d && !(DCCALL == d->dctype && d->dcj)) d = d->dclnk;
    if (!(d && DCCALL == d->dctype && d->dcj)) return 1;
    if (v == (L *)d->dcn) {  // if the saved stabent (from the unquote lookup) matches the name being assigned...
        ...
        d->dcf = w;
        // If we are redefining the executing explicit definition during debug, remember that.  We will use it to reload
        // the definition. Reassignment outside of debug waits until the name is off the stack
        if (CCOLON == FAV(w)->id) {
            jt->redefined  = (I)v;
            jt->cxspecials = 1;
        }
    ...
    c = d;
    while (c = c->dclnk) { ASSERTN(!(DCCALL == c->dctype && v == (L *)c->dcn), EVSTACK, c->dca); }
    return 1;
...
L *
jtprobeisquiet(J jt, A a, A locsyms) {
    A g;  // locsyms is used in the call, but is not needed in the function
    I n   = AN(a);
    NM *v = NAV(a);
    I m   = v->m;  // n is length of name, v points to string value of name, m is length of non-locale part of name
    if (n == m) {
        g = jt->global;
    ...
    }                            // if locative, find the locale for the assignment; error is not fatal
...
jtsymbis(J jt, A a, A w, A g) {
    ...
    if (jt->assignsym) {
        ASSERT(((I)g & ASGNLOCAL || NAV(a)->flag & (NMLOC | NMILOC) || !jtprobelocal(jt, a, jt->locsyms)),
               EVDOMAIN)    //  if global assignment not to locative, verify non locally defined
        e = jt->assignsym;  // point to the symbol-table entry being assigned
        CLEARZOMBIE         // clear until next use.
    } else {
        A jtlocal = jt->locsyms;
        n         = AN(a);
        NM *v     = NAV(a);
        m = v->m;  // n is length of name, v points to string value of name, m is length of non-locale part of name
        if (n == m)
            ASSERT(!(g == jt->global && jtprobelocal(jt, a, jtlocal)),
                   EVDOMAIN)  // if non-locative, give error if there is a local
        ...
        if (AT(w) & FUNC && (FAV(w)->fgh[0])) {
            if (FAV(w)->id == CCOLON) FAV(w)->flag |= VNAMED;
            if (jt->uflags.us.cx.cx_c.glock) FAV(w)->flag |= VLOCK;
        }
        // If the value is a function created by n : m, this becomes a named function; if running a locked function,
        // this is locked too. kludge  these flags are modified in the input area (w), which means they will be
        // improperly set in the result of the assignment (ex: (nm =: 3 : '...') y).  There seems to be no ill effect,
        // because VNAMED isn't used much.
    }
    ...
    if (x) {
        xaf = AFLAG(x);
        xt  = AT(x);  // if assigned, get the actual flags, from the name and the old value
        if ((e->flag & LWASABANDONED) != 0) {
            // Reassigning an x/y that was abandoned into this execution.  We did not increment the value when we
            // started, so we'd better not decrement now. However, we did change 8..1 to 1, and if the 1 is still there,
            // we set it back to 8..1 so that the caller can see that the value is unincorporated. The case where x==w
            // is of interest (it comes up in x =. x , 5).  In that case we will not change the usecount of x/w below,
            // so we have to keep the ABANDONED status in sync with the usecount.  The best thing is to keep both
            // unchanged, so that we can continue to inplace x
            AC(x) |= (ACINPLACE & (AC(x) - 1 - (x != w)));  // apply ABANDONED: 1 -> 8..1 but only if we are going to
                                                            // replace x; we don't want 8..1 in an active name
            e->flag &=
              ~((x != w)
                << LWASABANDONEDX);  // turn off abandoned flag after it has been applied, but only if we replace x
            xaf = AFNVRUNFREED;      // ignore other flags; set xaf as if x==0 to avoid any other usecount changes
        }
    } else {
        xaf = AFNVRUNFREED;
        xt  = 0;
    }  // If name is not assigned, indicate that it is not read-only or memory-mapped.  Also set 'impossible' code of
       // unfreed+not NVR
    if (!(AFNJA & xaf)) {
        // Normal case of non-memory-mapped assignment.
        // If we are assigning the same data block that's already there, don't bother with changing use counts or
        // anything else
        if (x != w) {
            if (((xt | AT(w)) & (VERB | CONJ | ADV)) != 0) {
                // When we assign to, or reassign, a modifier, invalidate all the lookups of modifiers that are extant
                // It's a pity that we have to do this for ALL assignments, even assignments to uv.  If we don't, a
                // reference to a local modifier may get passed in, and it will still be considered valid even though
                // the local names have disappeared.  Maybe we could avoid this if the local namespace has no defined
                // modifiers - but then we'd have to keep up with that...
                ++jt->modifiercounter;
            ...
            // Increment the use count of the value being assigned, to reflect the fact that the assigned name will
            // refer to it. This realizes any virtual value, and makes the usecount recursive if the type is recursible
            realizeifvirtual(w);
            ra(w);
            // If this is a reassignment, we need to decrement the use count in the old name, since that value is no
            // longer used. But if the value of the name is 'out there' in the sentence (coming from an earlier
            // reference), we'd better not delete that value until its last use. For simplicity, we defer ALL deletions
            // till the end of the sentence.  We put the to-be-deleted value onto the NVR stack if it isn't there
            // already, and free it.  If the value is already on the NVR stack and has been deferred-freed, we decrement
            // the usecount here to mark the current free, knowing that the whole block won't be freed till later.  By
            // deferring all deletions we don't have to worry about whether local values are on the stack; and that
            // allows us to avoid putting local values on the NVR stack at all. ABANDONED values can never be NVR (which
            // are never inplaceable), so they will be flagged as !NVR,!UNFREED,ABANDONED
            if ((xaf & AFNVRUNFREED) != 0) {  // x is 0, or unfreed on the NVR stack, or abandoned.  Do not fa().  0 is
                                              // probably the normal case (assignment to unassigned name)
                if ((xaf & AFNVR) != 0) {
                    AFLAG(x) = (xaf &= ~AFNVRUNFREED);
                }     // If unfreed on the NVR stack, mark as to-be-freed on the stack.  This defers the deletion
                      // x=0 case, and LABANDONED case, go through quietly making no change to the usecount of x
            } else {  // x is non0 and either already marked as freed on the NVR stack or must be put there now, or
                      // VIRTUAL
                if (!(xaf & (AFNVR | AFVIRTUAL))) {
                    // (1) the value in x is not on the NVR stack.  But it may still be at large in the sentence,
                    // because we don't push local names onto the NVR stack.  So, we defer the deletion until the end of
                    // the sentence, by adding the name to the NVR stack.  We could avoid this if we knew this is a
                    // final assignment (2) the value is not VIRTUAL.  The only way for an assigned value to be VIRTUAL
                    // is for it to be an initial assignment to x/y.  And to get here the value must not have been
                    // adandoned.  So the usecount was raised on assignment (otherwise we would have gone through the
                    // no-fa special case).  So it is safe to fa() immediately then
                    A nvra = jt->nvra;
                    if ((I)(jt->parserstackframe.nvrtop + 1U) > AN(nvra))
                        RZ(nvra = jtextnvr(jt));  // Extend nvr stack if necessary.  copied from parser
                    AAV1(nvra)
                    [jt->parserstackframe.nvrtop++] = x;  // record the place where the value was protected (i. e. this
                                                          // sentence); it will be freed when this sentence finishes
                    AFLAG(x) |= AFNVR;                    // mark the value as protected in NVR stack
                } else {
                    // already NVR+FREED or VIRTUAL: free this time, knowing the real free will happen later,  We know
                    // usecount>1, but it may be PERMANENT; decrement it if not
                    fadecr(x)
                }
            }
        ...
        if (x != w) {  // replacing name with different mapped data.  If data is the same, just leave it alone
            ...
            AR(x) = (RANKT)wr;
            MCISH(AS(x), AS(w), wr);
            memcpy(AV(x), AV(w), m);  // copy in the data
        }
    }
    e->sn = jt->slisti;                 // Save the script in which this name was defined
    if (jt->stch != 0) e->flag |= LCH;  // update 'changed' flag if enabled - no harm in marking locals too
    return e;                           // return the block for the assignment
} /* a: name; w: value; g: symbol table */

// assign symbol and free values immediately
// assignment within a sentence requires that values linger on a bit: till the end of the sentence or sometimes till
// printing is complete Values awaiting deletion are accumulated within the NVR stack till the sentence ends.  If there
// is an assignment not in a sentence, such as for for_x. or from sockets or DLLs, we have to finish the deletion
// immediately so that the NVR stack doesn't overflow
L *
jtsymbisdel(J jt, A a, A w, A g) {
    ...
    while (currtop > nvrtop) {  // the only new blocks must be ones that were newly added to the nvr stack by symbis
        ...
        AFLAG(delval) &= ~(AFNVR | AFNVRUNFREED);
        fa(delval);  // in case the block survives, indicate that it is off the stack now; then reduce usecount
    }
