...
L*jtprobe(J jt,I l,C*string,UI4 hash,A g){
 ...
 while(1){
  ...
  IFCMPNAME(NAV(sym->name),string,l,return sym->val?sym:0;)     // (1) exact match - if there is a value, use this slot, else say not found
...
// if(b&&jt->local&&(e=probe(NAV(a)->m,NAV(a)->s,NAV(a)->hash,jt->local))){av=NAV(a); return e;}  // return if found local
...
A jtsybaseloc(J jt,A a) {I m,n;NM*v;
 ...
 return NMILOC&v->flag?locindirect(n-m-2,2+m+v->s,(UI4)v->bucketx):stfindcre(n-m-2,1+m+v->s,v->bucketx);
}
...
B jtredef(J jt,A w,L*v){A f;DC c,d;
 ...
 d=jt->sitop; while(d&&!(DCCALL==d->dctype&&d->dcj))d=d->dclnk; if(!(d&&DCCALL==d->dctype&&d->dcj))return 1;
...
L* jtsymbis(J jt,A a,A w,A g){A x;I m,n,wn,wr,wt;L*e;
 ...
 if(!(AFNJA&xaf)){
  ...
  if(x!=w){
   ...
   if((xaf&AFNVRUNFREED)!=0){  // x is 0, or unfreed on the NVR stack, or abandoned.  Do not fa().  0 is probably the normal case (assignment to unassigned name)
    if((xaf&AFNVR)!=0){AFLAG(x)=(xaf&=~AFNVRUNFREED);} // If unfreed on the NVR stack, mark as to-be-freed on the stack.  This defers the deletion
  ...
  if(x!=w){  // replacing name with different mapped data.  If data is the same, just leave it alone
   ...
   AT(x)=wt; AN(x)=wn; AR(x)=(RANKT)wr; MCISH(AS(x),AS(w),wr);
