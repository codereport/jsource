...
DD NOTE: DD is not supported on special sentences: iep, xep, dbssexec, dbtrap.  This is mostly because I don't
...
//   if the vertical boxing character is Unicode, discard leading SP or | characters (which might have been error
//   typeout)
A
jtinpl(J jt, B b, I n, C* s) {
    C c;
    I k = 0;
...
jtjgets(J jt, C* p) {
    ...
    *jt->adbreak = 0;
    ...
    if (jt->dcs) {           // DCSCRIPT debug type - means we are reading from file (or string)  for 0!:x
        ++jt->dcs->dcn;
        j = jt->dcs->dcix;  // increment line# and fetch current start index
        y = jt->dcs->dcy;
        n = AN(y);
        s = UAV(y);
...
jdo(J jt, C* lp) {
    ...
    jt->jerr = 0;
    jt->etxn = 0; /* clear old errors */
    // The named-execution stack contains information on resetting the current locale.  If the first named execution
    // deletes the locale it is running in, that deletion is deferred until the locale is no longer running, which is
    // never detected because there is no earlier named execution to clean up. To prevent the stack from growing
    // indefinitely, we reset it here.  We reset the callstack only if it was 0, so that a recursive immex will have its
    // deletes handled by the resumption of the name that was interrupted.
...
C*
getlocale(J jt) {
    A y = jtlocname(jt, mtv);
    y   = AAV(y)[0];
    return CAV(jtstr0(jt, y));
}

// Front-ends can call any functions exposed by JE, but the callback function for 11!:0 only calls jga to allocate a new
// literal array for returning result. A front-end knows nothing how J memory pool works and it won't try to free or pop
// memory itself. This was just a design decision. eg, jqt front-end uses another alternative, it stores result in a
// static variable and pass its address back to JE so that no jga is needed.
//
...
// after callback function completed, the callback allocate an literal array inside JE memory pool and fill up the
// content of array. The array allocated by jga is post processed inside jtwd and becomes the return value of jtwd, ie,
// the result of calling 11!:0 in J script.
//
// This may be confusing because the actual callback functions do not appear anywhere inside JE source.
A
jtwd(J jt, A w, A self) {
    A z  = 0;
    C* p = 0;
    D* pd;
    I e, *pi, t;
    ...
    if (BETWEENO(t, 2000, 3000) && AN(w) && !(LIT + C2T + C4T + INT & AT(w))) {  // 2000<=t<3000
        switch (UNSAFE(AT(w))) {
            case B01: RZ(w = jtvi(jt, w)); break;
            case FL:
                pd = DAV(w);
                {
                    A wsav = w;
                    GATV0(w, INT, AN(wsav), AR(wsav));
                }
                pi = AV(w);
    ...
    // Calling the Host takes us out of BUSY state, back to IDLE.  (If for some reason we are prompting, that is not
    // affected) That is, while we are waiting for the reply we are back to interruptible state.  If the interrupt
    // issues another wd, that too is interruptible, for as many levels as needed.
    ...
    jt->recurstate |=
      RECSTATEBUSY;      // wd complete, go back to normal running state, BUSY normally or RECUR if a prompt is pending
    if (!e) return mtm;  // e==0 is MTM
    ASSERT(e <= 0, e);   // e>=0 is EVDOMAIN etc
    if (SMOPTPOLL & jt->smoption) {
        jt->recurstate = RECSTATEPROMPT;
        z              = (A)((polltype)(jt->smpoll))(jt, (int)t, (int)e);
        jt->recurstate = RECSTATEBUSY;
        RZ(z);
    }  // alternate way to get result aftercallback, but not yet used in any front-end
    if (SMOPTNOJGA & jt->smoption)
        z = jtca(jt, z);  // front-end promised not to use Jga to allocate memory, but not yet used in any front-end
    if (e == -2) {        // e==-2 is lit pairs
        ...
        A x = z;
        RZ(df1(z, x, jtcut(jt, ds(CBOX), num(-2))));
...
int _stdcall JDo(J jt, C* lp) {
    int r;
    UI savcstackmin, savcstackinit, savqtstackinit;
    if (jt->recurstate > RECSTATEIDLE) {
        ...
        savcstackmin = jt->cstackmin, savcstackinit = jt->cstackinit,
        savqtstackinit = jt->qtstackinit;  // save stack pointers over recursion, in case the host resets them
        ASSERT(!(jt->recurstate & (RECSTATEBUSY & RECSTATERECUR)), EVCTRL)  // fail if BUSY or RECUR
        CLEARZOMBIE  // since we are executing a surprise call, the verb that is prompting might have set assignsym,
                     // which is about to be invalidated.  Clear that.
    }
    if (jt->cstacktype == 2) {
    ...
    if (--jt->recurstate > RECSTATEIDLE) {  // return to IDLE or PROMPT state
        ...
        jt->cstackmin = savcstackmin, jt->cstackinit = savcstackinit,
        jt->qtstackinit = savqtstackinit;  // restore stack pointers after recursion
    }
...
A _stdcall JGetA(J jt, I n, C* name) {
    ...
    jt->jerr = 0;
    A* old   = jt->tnextpushp;
    if (!(x = jtsymbrdlock(jt, jtnfs(jt, n, name)))) {
        jtjsignal(jt, EVILNAME);  // look up the name, error if invalid
    } else if (FUNC & AT(x)) {
        jtjsignal(jt, EVDOMAIN);  // verify the value is not adv/verb/conj
    } else {
        ...
        if (z = jtbinrep1(jt, x)) {
            ...
            }  // allocate block if needed, and account for its space
            if (jt->iomalloc) {
                memcpy(jt->iomalloc, z, replen);
                z =
                  (A)jt
                    ->iomalloc;  // normal case: block exists, move the data, set the return address to the malloc block
            } else {
                jt->iomalloclen = 0;
                z               = 0;
...
void _stdcall JSM(J jt, void* callbacks[]) {
    ...
    if (SMOPTMTH & jt->smoption) {
        ...
        // If the user is giving us a better view of the stack through jt->qtstackinit, use it.  We get just the
        // top-of-stack address so we have to guess about the bottom, using our view of the minimum possible stack we
        // have. if cstackmin is 0, the user has turned off stack checking and we honor that decision
        if (jt->qtstackinit && jt->cstackmin)
            jt->cstackmin = (jt->cstackinit = jt->qtstackinit) - (CSTACKSIZE - CSTACKRESERVE);
    }
}
...
void _stdcall JSMX(J jt, void* out, void* wd, void* in, void* poll, I opts) {
    ...
    if (SMOPTMTH & jt->smoption) {
        ...
        jt->smpoll      = 0;
        jt->smoption    = (~SMOPTPOLL) & jt->smoption; /* smpoll not used */
        if (jt->qtstackinit && jt->cstackinit)
            jt->cstackmin = (jt->cstackinit = jt->qtstackinit) - (CSTACKSIZE - CSTACKRESERVE);
    }
}
...
C* _stdcall JGetLocale(J jt) {
    ...
    }                     // allocate & copy, and account for its space
...
int _stdcall JGetM(J jt, C* name, I* jtype, I* jrank, I* jshape, I* jdata) {
    ...
    if (strlen(name) >= sizeof(gn)) {
        jtjsignal(jt, z = EVILNAME);
    } else if (valid(name, gn)) {
        jtjsignal(jt, z = EVILNAME);
    } else if (!(a = jtsymbrdlock(jt, jtnfs(jt, strlen(gn), gn)))) {
        jtjsignal(jt, z = EVDOMAIN);
    } else if (FUNC & AT(a)) {
        jtjsignal(jt, z = EVDOMAIN);
    } else {
        *jtype  = AT(a);
...
static int
setterm(J jt, C* name, I* jtype, I* jrank, I* jshape, I* jdata) {
    A a;
    I k = 1, i, n;
    char gn[256];
    ...
    switch (CTTZNOFLAG(*jtype)) {
        case LITX:
        case B01X: n = sizeof(char); break;

        case C2TX: n = sizeof(unsigned short); break;

        case C4TX: n = sizeof(unsigned int); break;

        case INTX:
        case SBTX: n = sizeof(I); break;

        case FLX: n = sizeof(double); break;

        case CMPXX: n = 2 * sizeof(double); break;

        default: return EVDOMAIN;
    }
