...
#define EWOV \
    50  // integer overflow from the old routines that do not support recovery.  Anything >= EWOV is an overflow.  Leave
        // at 50 until asm routines are retired
#define EWOV1 51  // integer overflow that can be recovered because the float version has already been saved
#define EWOVIP \
    52  // overflow, but recoverable even though executed in place.  It starts here, but identifies the routine it came
        // from:
...
#define EXIGENTERROR                                                                                                 \
    (EMSK(EVALLOC) | EMSK(EVATTN) | EMSK(EVBREAK) | EMSK(EVINPRUPT) | EMSK(EVFACE) | EMSK(EVWSFULL) | EMSK(EVTIME) | \
     EMSK(EVSTACK) | EMSK(EVSYSTEM))  // errors that always create failure
