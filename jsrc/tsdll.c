...
CDPROC char _stdcall cbasic(char* a, char b, char* c) sum CDPROC wc _stdcall wbasic(wc* a, wc b, wc* c) sum CDPROC uc
  _stdcall ubasic(uc* a, uc b, uc* c) sum CDPROC BYTE _stdcall bbasic(BYTE* a, BYTE b, BYTE* c) sum CDPROC
  short _stdcall sbasic(short* a, short b, short* c) sum CDPROC int _stdcall ibasic(int* a, int b, int* c) sum CDPROC I
  _stdcall xbasic(I* a, I b, I* c) sum CDPROC D _stdcall dbasic(D* a, D b, D* c) sum CDPROC F
  _stdcall fbasic(F* a, F b, F* c) sum
...
#define fsum                       \
  ...
  CDPROC D _stdcall jbasic(double_complex* a, double_complex* b, double_complex* c) dsum CDPROC F
  _stdcall zbasic(float_complex* a, float_complex* b, float_complex* c) fsum

  // test pointer result
  static char cd[] = "test";
CDPROC char* _stdcall pc() {
    return cd;
}

// test in place convert down/up
CDPROC void _stdcall downup(I n, short* a, int* b, float* c) {
    I i;
    for (i = 0; i < n; ++i) {
        a[i] = -a[i];
        b[i] = -b[i];
...
CDPROC BYTE _stdcall bipbpb(int c, BYTE* p, BYTE* pr) {
    BYTE f = 0;
    int i;
    for (i = 0; i < c; ++i) { *pr = f += (BYTE)p[i]; }
    return f;
}

CDPROC double _stdcall dcomplex0(int c, double* j) {
    return j[c];
}
CDPROC float _stdcall fcomplex0(int c, float* j) {
    return j[c];
}
CDPROC void _stdcall dcomplex1(int c, double_complex* j, double* a, double* b) {
    I i;
    for (i = 0; i < c; ++i) {
        a[i] = creal(j[i]);
        b[i] = -cimag(j[i]);
...
CDPROC void _stdcall fcomplex1(int c, float_complex* j, float* a, float* b) {
    I i;
    for (i = 0; i < c; ++i) {
        a[i] = crealf(j[i]);
        b[i] = -cimagf(j[i]);
...
CDPROC float _stdcall f() {
    return (float)1.5;
}
CDPROC double _stdcall d() {
    return (double)1.5;
}

CDPROC D _stdcall dd(D d0) {
    return d0 * 2;
}
CDPROC D _stdcall ddd(D d0, D d1) {
    return d0 + d1;
}
CDPROC D _stdcall dddd(D d0, D d1, D d2) {
    return d0 + d1 + d2;
}

CDPROC D _stdcall dx0(I a, D b) {
    return a + b;
}
CDPROC D _stdcall dx1(D a, I b) {
    return a + b;
}
CDPROC D _stdcall dx2(I a, D b, I c) {
    return a + b + c;
}
CDPROC D _stdcall dx3(D a, I b, D c) {
    return a + b + c;
}
CDPROC D _stdcall dx4(I a, D b, I c, D d) {
    return a + b + c + d;
}
CDPROC D _stdcall dx5(D a, I b, D c, I d) {
    return a + b + c + d;
}
CDPROC D _stdcall dx6(I a, D b, I c, D d, I e) {
    return a + b + c + d + e;
}
CDPROC D _stdcall dx7(D a, I b, D c, I d, D e) {
    return a + b + c + d + e;
}
...
CDPROC D _stdcall di9d9a(
  I ia, I ib, I ic, I id, I ie, I iF, I ig, I ih, I ii, D a, D b, D c, D d, D e, D f, D g, D h, D i) {
    return ia + ib + ic + id + ie + iF + ig + ih + ii + a + b + c + d + e + f + g + h + i;
}

CDPROC D _stdcall di9d9b(
  I ia, I ib, I ic, I id, I ie, I iF, I ig, I ih, D a, D b, D c, D d, D e, D f, D g, D h, D i, I ii) {
    return ia + ib + ic + id + ie + iF + ig + ih + ii + a + b + c + d + e + f + g + h + i;
}
...
CDPROC float _stdcall fff(float f0, float f1) {
    return f0 + f1;
}
...
CDPROC int _cdecl altinci(int i) {
    return ++i;
}

// '0 procaddress ...'
CDPROC I _stdcall xbasic_add() {
    return (I)xbasic;
}

// '1 procindex ...'
typedef I(_stdcall* PROC)();
CDPROC I _stdcall objxxx(void* obj, I a, I b) {
    return a + b;
}
CDPROC D _stdcall objddd(void* obj, D a, D b) {
    return a + b;
}
// PROC vtable[]={&(PROC)objxxx,&(PROC)objddd};
CDPROC PROC vtable[] = {(PROC)objxxx, (PROC)objddd};
CDPROC PROC* object  = vtable;
CDPROC I _stdcall obj_add() {
    return (I)&object;
}
