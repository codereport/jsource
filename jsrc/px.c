...
jtevc(J jt, A a, A w, C* s) {
    A z;
    return df2(z, a, w, jtcolon(jt, num(2), jtcstr(jt, s)));
}
...
jtexec1(J jt, A w) {
    A z;
    if (AT(w) & NAME) {
        z = jtnameref(jt, w, jt->locsyms);  // the case ".@'name' which is the fastest way to refer to a deferred name
    } else {
        F1RANK(1, jtexec1, UNUSED_VALUE);
        A savself = jt->sf;  // in case we are in a recursion, preserve the restart point
        STACKCHKOFL FDEPINC(1);
        z        = jtparse(jt, jtddtokens(jt, jtvs(jt, w), 4 + 1 + (AN(jt->locsyms) > 1)));
        jt->asgn = 0;
        FDEPDEC(1);  // replace DDs, but require that they be complete within the string (no jgets)
        jt->sf = savself;
    }
...
jtimmex(J jt, A w) {
    ...
    // the normal condition and u./v. will rely on it.  This is not needed for a recursive call, but it doesn't hurt
    // either, because if AKGST has been set it will already hold jt->global.  Of course, any event code must restore
    // everything to its previous state, including locales
    AKGST(jt->locsyms) = jt->global;  // in case the sentence has operators, set a locale for it
    STACKCHKOFL FDEPINC(1);
    z = jtparse(jt, AT(w) & BOX ? w : jttokens(jt, w, 1 + (AN(jt->locsyms) > 1)));
    FDEPDEC(1);
...
jtimmea(J jt, A w) {
    A t, z, z1;
    RZ(w = jtddtokens(jt, w, 4 + 1 + (AN(jt->locsyms) > 1)));
...
jtapplystr(J jt, A a, A w) {
    ...
    z = CALL1(FAV(fs)->valencefns[0], w, fs);
    FDEPDEC(d);
    EPILOG(z);
