...
A jteval(J jt,C*s){return jtparse(jt,jttokens(jt,jtcstr(jt,s),1+(AN(jt->locsyms)>1)));}

A jtev1(J jt,    A w,C*s){A z; return df1(z,  w,jteval(jt,s));}  // parse *s and apply to w
A jtev2(J jt,A a,A w,C*s){A z; return df2(z,a,w,jteval(jt,s));}  // parse *s and apply to a and w
A jteva(J jt,    A w,C*s){A z; return df1(z,  w,jtcolon(jt,num(1),   jtcstr(jt,s)));}
A jtevc(J jt,A a,A w,C*s){A z; return df2(z,a,w,jtcolon(jt,num(2),jtcstr(jt,s)));}
...
// ". y
 ...
 if(AT(w)&NAME){z=jtnameref(jt,w,jt->locsyms);  // the case ".@'name' which is the fastest way to refer to a deferred name
  ...
  STACKCHKOFL FDEPINC(1); z=jtparse(jt,jtddtokens(jt,jtvs(jt,w),4+1+(AN(jt->locsyms)>1))); jt->asgn=0; FDEPDEC(1);  // replace DDs, but require that they be complete within the string (no jgets)
  jt->sf=savself;
 }
...
// execute w, which is either a string or the boxed words of a string (as if from jttokens(jt,))
 ...
 if(!w)return 0;  // if no string, return error
 ...
 STACKCHKOFL FDEPINC(1); z=jtparse(jt,AT(w)&BOX?w:jttokens(jt,w,1+(AN(jt->locsyms)>1))); FDEPDEC(1);
 if(z&&!jt->asgn)jtjpr(jt,z);
 return z;
}

// execute for assert: check result for all 1
 A jtimmea(J jt, A w){A t,z,z1;
 RZ(w=jtddtokens(jt,w,4+1+(AN(jt->locsyms)>1))); z=jtimmex(jt,w);   // check for DD, but don't allow continuation read
 ASSERT(jt->asgn||!z||!(AT(z)&NOUN)||(t=eq(num(1),z),
     all1(AT(z)&SPARSE?df1(z1,t,jtatop(jt,jtslash(jt,ds(CSTARDOT)),ds(CCOMMA))):t)),EVASSERT);  // apply *./@, if sparse
 return z;
}
...
// convert the gerund (i.e  AR) in w into a verb
 ...
 DO(n, x=wv[i]; RZ(*v++=(y=cex(x,jtfx,0L))?y:jtexg(jt,x)););  // if the AR can be converted to an A, do so; otherwise it should be a list of ARs, recur on each
 return jtparse(jt,z);
}
...
L* jtjset(J jt,C*name,A x){return symbisdel(jtnfs(jt,(I)strlen(name),name),x,jt->global);}
 ...
 RZ(fs=jtparse(jt,jttokens(jt,jtvs(jt,a),1+(AN(jt->locsyms)>1))));
