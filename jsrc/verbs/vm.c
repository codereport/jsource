...
static I
jtcirx(J jt, I n, I k, D* z, D* x) {
    D p, t;
    NAN0;
    switch (k) {
        default: ASSERTWR(0, EWIMAG);
        case 0: DQ(n, t = *x++; ASSERTWR(ABS(t) <= 1.0, EWIMAG); *z++ = sqrt(1.0 - t * t);); break;
        case 1:; DQ(n, t = *x++; ASSERTWR(ABS(t) < THMAX, EVLIMIT); *z++ = sin(t);); break;
        case 2:; DQ(n, t = *x++; ASSERTWR(ABS(t) < THMAX, EVLIMIT); *z++ = cos(t);); break;
        case 3:; DQ(n, t = *x++; ASSERTWR(ABS(t) < THMAX, EVLIMIT); *z++ = tan(t);); break;
        ...
        case 7:;
            ...
            break;
        case -1:; DQ(n, t = *x++; ASSERTWR(-1.0 <= t && t <= 1.0, EWIMAG); *z++ = asin(t);); break;
        case -2:; DQ(n, t = *x++; ASSERTWR(-1.0 <= t && t <= 1.0, EWIMAG); *z++ = acos(t);); break;
        case -3:; DQ(n, *z++ = atan(*x++);); break;
        case -4:
            DQ(n, t = *x++; ASSERTWR(t <= -1.0 || 1.0 <= t, EWIMAG); *z++ = t < -1e8 || 1e8 < t ? t
                                                                            : t == -1
                                                                              ? 0
                                                                              : (t + 1) * sqrt((t - 1) / (t + 1)););
            break;
        case -5:
            p = log(2.0);
            DQ(n, t = *x++; *z++ = 1.0e8 < t ? p + log(t) : -7.8e3 > t ? -(p + log(-t)) : log(t + sqrt(t * t + 1.0)););
            break;
        case -6:
            p = log(2.0);
            DQ(n, t = *x++; ASSERTWR(1.0 <= t, EWIMAG); *z++ = 1.0e8 < t ? p + log(t) : log(t + sqrt(t * t - 1.0)););
            break;
        case -7:
            DQ(n, t = *x++; ASSERTWR(-1.0 <= t && t <= 1.0, EWIMAG); *z++ = 0.5 * log((1.0 + t) / (1.0 - t)););
            break;
...
A
jtlogar2(J jt, A a, A w) {
    A z;
    I t;
    RE(t = maxtype(AT(a), AT(w)));
    jt->xmode = XMEXACT;
    return jtatomic2(
      JTIPAW, logar1(w), logar1(a), ds(CDIV));  // better to multiply by recip, but not much, & it makes 0 ^. 0 not fail
}
...
jtroot(J jt, A a, A w) {
    ...
    RE(t = maxtype(AT(a), AT(w)));
    A ma = a;
    if (TYPESNE(t, AT(a))) RZ(ma = jtcvt(jt, t, a));
    A mw = w;
    if (TYPESNE(t, AT(w))) RZ(mw = jtcvt(jt, t, w));
...
A
jtjdot2(J jt, A a, A w) {
    return plus(a, tymes(a0j1, w));
}
A
jtrdot1(J jt, A w) {
    return expn1(jtjdot1(jt, w));
}
A
jtrdot2(J jt, A a, A w) {
    return tymes(a, jtrdot1(jt, w));
}

A
jtpolar(J jt, A w) {
    A z;
    return jtcvt(jt, SPARSE & AT(w) ? SFL : FL, df2(z, jtv2(jt, 10L, 12L), w, jtqq(jt, ds(CCIRCLE), jtv2(jt, 1L, 0L))));
}
...
jtrect(J jt, A w) {
    ...
    t = AT(w);
    r = AR(w);
    RESETRANK;  // Run as infinite rank
    ASSERT(!AN(w) || t & NUMERIC, EVDOMAIN);
    if (t & CMPX) {
        GATV(z, FL, 2 * AN(w), 1 + r, AS(w));
        AS(z)[r] = 2;
        memcpy(AV(z), AV(w), AN(z) * sizeof(D));
        return z;
    } else if ((t & SPARSE) != 0) {
        b = 1 && t & SCMPX;
        GASPARSE(z, b ? SFL : t, 1, 1 + r, AS(w));
        AS(z)[r] = 2;
        wp       = PAV(w);
        zp       = PAV(z);
        if (b) {
            e = SPA(wp, e);
            c = *ZAV(e);
            ASSERT(FFEQ(c.re, c.im), EVSPARSE);
            SPB(zp, e, jtscf(jt, c.re));
        } else
