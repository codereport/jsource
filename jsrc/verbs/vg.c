...
#define GBEGIN(L)                                                \
    I olt                        = jt->workareas.compare.complt; \
    jt->workareas.compare.complt = L
...
#define CXCHG2(v0, v1)                    \
    {                                     \
        void *v2 = v0;                    \
        B t      = COMPFN(compn, v0, v1); \
        v0       = (!t) ? v1 : v0;        \
        v1       = (!t) ? v2 : v1;        \
    }

// Comparison functions.  Do one comparison before the loop for a fast exit if it differs.
// On VS this sequence, where a single byte is returned, creates a CMP/JE/SETL sequence, performing only one (fused)
// compare #define COMPGRADE(T,t) T av=*a, bv=*b; if(av!=bv) return av t bv; while(--n){++a; ++b; av=*a, bv=*b;
// if(av!=bv) return av t bv;} return a<b;
#define COMPGRADE(T, t)               \
    do {                              \
        T av = *a, bv = *b;           \
        if (av != bv) return av t bv; \
        if (!--n) break;              \
        ++a;                          \
        ++b;                          \
    } while (1);                      \
    return a < b;
static __forceinline B
compiu(I n, I *a, I *b) {
    COMPGRADE(I, <)
}
static __forceinline B
...
static void
jtmsortitems(J jt, void **(*sortfunc)(), I n, void **zv, void **xv) {
    // Initialize the result area to be 'in', with pointers to the input items
    C *item    = jt->workareas.compare.compv;
    C *item0   = item;
    I inc      = jt->workareas.compare.compk;
    void **zvv = (void **)zv;
    DQ(n, *zvv++ = item; item += inc;)
    // Do the grade
    void **sortres = (*sortfunc)(
      jt->workareas.compare.comp, jt->workareas.compare.compusejt ? (I)jt : jt->workareas.compare.compn, zv, n, xv);
    // Convert the result to item numbers in the main result area.
    I shift = CTTZI(inc);  // bit number of LSB
    if (inc == ((I)1) << shift) {
        DQ(n, *zv++ = (void *)((I)((C *)*sortres++ - item0) >> shift););  // use shift for power of 2 itemsize
    } else {
        // Not a power of 2.  Use rounding multiply to avoid divide throughput
        D recipinc =
          (D)n / ((D)n * (D)inc -
                  0.25);  // make sure the result is correct after truncation, by shading the factor to the high side
...
void
jtmsort(J jt, I n, void **zv, void **xv) {
    // Do the grade
    void **sortres = jmsortmincomp(
      jt->workareas.compare.comp, jt->workareas.compare.compusejt ? (I)jt : jt->workareas.compare.compn, zv, n, xv);
...
static struct {
  ...
  [B01X]  = {{compcd, jmsort}, {compcu, jmsort}},
  [LITX]  = {{compcd, jmsort}, {compcu, jmsort}},
  [INTX]  = {{0, jmsortid}, {0, jmsortiu}},
  [FLX]   = {{0, jmsortdd}, {0, jmsortdu}},
  [CMPXX] = {{0, jmsortdd}, {0, jmsortdu}},
  [BOXX]  = {{compr, jmsortmincomp}, {compr, jmsortmincomp}},
  [XNUMX] = {{compxd, jmsortmincomp}, {compxu, jmsortmincomp}},
  [RATX]  = {{compqd, jmsortmincomp}, {compqu, jmsortmincomp}},
  [C2TX]  = {{compud, jmsort}, {compuu, jmsort}},
  [C4TX]  = {{comptd, jmsort}, {comptu, jmsort}},
  [SBTX]  = {{compcd, jmsort}, {compcu, jmsort}}};
...
jtgrx(J jt, I m, I ai, I n, A w, I *zv) {
    ...
    jt->workareas.compare.compn     = ai << ((t >> CMPXX) & 1);
    jt->workareas.compare.compv     = CAV(w);
    jt->workareas.compare.comp      = sortroutines[CTTZ(t)][SGNTO0(jt->workareas.compare.complt)].comproutine;
...
I
grcol4(I d, I c, UI4 *yv, I n, I *xv, I *zv, const I m, US *u, I flags) {
    I split;
    US *v;
    I ct00, ctff;
    ...
    if (flags & 8) {
        // Here we know that the previous pass found all the values were sign-extensions.  So we just clear the first
        // and last values, and the flag
        flags &= ~8;
    } else {
        memset(c + yv, C0, d * sizeof(*yv));
    }  // full clear if the fast option is not selected
    // increment the bucket for each input value
    v = u;
    DQ(n, ++yv[*v]; v += m;);
    // If all the values are sign-extensions (which will happen often) we can short-circuit much of the processing,
    // including the rolling sum Do this only when there is data/index to copy, to save us the trouble of having a case
    // for synthesizing the index
    if (xv && ((ct00 = yv[0]) + (ctff = yv[65535]) == n)) {
        ...
        if ((-ct00 & -ctff) < 0) {
            S *vs = (S *)u;  // since we know they're sign-extensions, extend when we load
            // There are both positive & negative sign-extensions.  Simulate the full processing, but without the
            // fetches from the table We need the output index for 00 and ff values, which will depend on the up/split
            // values Copy 00 values into index 0 if up && !split  or !up && split; otherwise to ctff copy ff values
            // into index 0 if !up && !split  or  up && split; otherwise to ct00 So, zero the appropriate index & use
            // ctff as the pos index, ct00 as the neg
            {
                I writeposto0 = (((flags >> 1) ^ flags) & 1);
                ctff &= (writeposto0 -= 1);
                ct00 &= ~writeposto0;
            }  // if up^split, generate 1,0,ff, clearing ctff; otherwise 0,ff,00, clearing ct00
            ...
            if (flags & 4) {
                ...
                if (2 == m) {
                    C4 *zvc = (C4 *)zv;
                    C4 *xvc = (C4 *)xv + n;
                    DP(n, I vv = *vs; I writeval = ct00; ct00 -= vv; writeval = (vv += 1) ? ctff : writeval; ctff += vv;
                       zvc[writeval] = xvc[i];
                       vs += m;)
                }  // result and intermediates are 4 bytes
                else {
                    D *zvc = (D *)zv;
                    D *xvc = (D *)xv + n;
                    DP(n, I vv = *vs; I writeval = ct00; ct00 -= vv; writeval = (vv += 1) ? ctff : writeval; ctff += vv;
                       zvc[writeval] = xvc[i];
                       vs += m;)
                }  // result and intermediates are 8 bytes
            } else {
                // copy the index, refer through it to get the data.  xv[i] is the initial index mapped to current i. We
                // fetch the current word for that index, call that xvv Depending on whether xvv is ffff or 0000, move
                // the index xv[i] to the selected output location, and increment whichever pointer it was moved to.
                xv += n;
                DP(n, I xvv = vs[m * xv[i]]; I writeval = ct00; ct00 -= xvv; writeval = (xvv += 1) ? ctff : writeval;
                   ctff += xvv;
                   zv[writeval] = xv[i];)
            }
        } else {
            // All the sign-extensions are the same.  All there is to do is copy the input to the output.  could try to
            // save the copy with pointer tricks? The length, as below, depends on the operation mode
            I csiz = m << LGSZUS;
            csiz   = flags & 4 ? csiz : SZI;
            memcpy(zv, xv, n * csiz);
        }
    } else {
        UI4 k;
        ...
        UI4 *t = yv + c + (tct & (REPSGN(tinc) | -split)) +
                 REPSGN(tinc);  // starting position: based on up/split: 00: +d-1  01: +d/2-1  10: +0  11: +d/2
        UI4 s = 0;
        do {
            DP(tct, k = *t; *t = s; s += k; t += tinc;) t -= tinc * d;
        } while (--split >= 0);  // 1 iteration if not split, 2 if split
        ...
        if (flags & 4) {
            ...
            if (!xv) {
                ...
                xv += n;
                DP(n, I buckno = (UI4)v[m * xv[i]]; I buckval = yv[buckno]; zv[buckval] = xv[i];
                   yv[buckno] = (UI4)(buckval += 1);)
            }
        }
...
I
grcol2(I d, I c, US *yv, I n, I *xv, I *zv, const I m, US *u, I flags) {
    I split;
    US *v;
    I ct00, ctff;
    yv[0] = yv[65535] = 0;
    ...
    if (xv && ((ct00 = yv[0]) + (ctff = yv[65535]) == n)) {
        ...
        US s   = 0;
        do { DP(tct, k = *t; *t = s; s += k; t += tinc;) t -= tinc * d; } while (--split >= 0);
        v = u;
        if (flags & 4) {
            if (2 == m) {
                C4 *zvc = (C4 *)zv;
            ...
            if (!xv) {
                ...
                xv += n;
                DP(n, I buckno = v[m * xv[i]]; I buckval = (US)yv[buckno]; zv[buckval] = xv[i];
                   yv[buckno] = (US)(buckval += 1);)
            }
        }
    }
    return flags;
...
// We interpret the input as integer form so that we can hide the item number in an infinity without turning it into a
// NaN
static B
jtgrdq(J jt, I m, I ai, I n, A w, I *zv) {
    ...
    CTLZI(n - 1, hbit);
    ++hbit;
    I itemmask = ((I)1 << hbit) - 1;  // mask where the item number will go
    ...
    while (--m >= 0) {
        ...
        sortiq1(zv, n);
        // pass through the result area, removing the upper bits.  If consecutive values have the same upper bits, go
        // through them, replacing the upper bits with the actual bits from the input (after honoring sign/direction
        // bits), and then re-sort the result in place. remove the upper bits from that sorted result
        I nextv = zv[0];  // always has first value with a new key
        I i;
        for (i = 0; i < n - 1; ++i) {
            I currv = nextv;
            if (((nextv = zv[i + 1]) ^ currv) & ~itemmask) {
                zv[i] = currv & itemmask;  // normal case with no repetition
            } else {                       // reprocess the repeated block
                I j = i;
                do {
                    I v = wv[zv[j] & itemmask] ^ sortdown63;
                    v ^= (UI)REPSGN(v) >> 1;
                    v     = (v == 0) ? -1 : v;
                    zv[j] = ((v & itemmask) << hbit) +
                            (zv[j] & itemmask);  // fetch original v, reconstitute; get itemmask in uppper bits
                } while (!(++j == n || (((nextv = zv[j]) ^ currv) & ~itemmask)));
                sortiq1(zv + i, j - i);  // sort the collision area in place.  j points to first item beyond the
                                         // collision area, and nextv is its value
                while (i < j) {
                    zv[i] &= itemmask;
                    ++i;
                }
        ...
        // We handled all the starting items up to the next-last.  The last item may be unprocessed, if it was not part
        // of a batch.  Handle it now.
        zv[n - 1] &= itemmask;
...
static B
jtgrd(J jt, I m, I ai, I n, A w, I *zv) {
    A x, y;
    int b;
    D *v, *wv;
    I *g, *h, nneg, *xv;
    ...
    {
        I use4 = n > 65535;
        grcol  = use4 ? (I(*)(I, I, void *, I, I *, I *, const I, US *, I))grcol4
                      : (I(*)(I, I, void *, I, I *, I *, const I, US *, I))grcol2;
        GATV0(y, INT, ((65536 * sizeof(US)) >> LGSZI) << use4, 1);
        yv = AV(y);
    }
    GATV0(x, INT, n, 1);
    xv = AV(x);  // allocate a ping-pong buffer for the result
    while (--m >= 0) {
        I colflags;               // loop over each cell of input
        ...
        // If we are doing all negative values, just change the direction and return the sorted values without reversal
        {
            v    = wv;
            nneg = 0;
            DQ(n, nneg += (((US *)v)[FPMSBWDX] >> 15); ++v;);
            b = 0 < nneg && nneg < n;
        }
        // set the ping-pong buffer pointers so we will end up with the output in zv.  If b is set, we have 5 passes
        // (one final sign-correction pass); if not, 4 h is the even-numbered output buffer.  if b is off, we start
        // writing to x and finish in z; if b is set, we start writing to z, finish the 4th pass in x, then the postpass
        // ends in z
        g = b ? xv : zv;
        h = b ? zv : xv;
        // sort from LSB to MSB.  Sort in the order requested UNLESS all the values are negative; then reverse the order
        // and save the postpass
        colflags =
          grcol(65536,
                0L,
                yv,
                ...
                sizeof(D) / sizeof(US),
                u,
                ((1 - jt->workareas.compare.complt) ^ ((nneg == n) << 1)));  // 'up' in bit 1, inverted if all neg
        ...
        // for the MSB, which is signed, do the same thing, but to save a few cycles tell grcol if all the values are
        // negative or nonnegative.  grcol will save the time for clearing the unused half of the buffer.  If all neg,
        // call with 32768,32768; if all nonneg, 32768,0; otherwise 65536,0
        ...
        if (b) {
            ...
            I j, m, *u, *v, *vv;
            I nneg0;
            // the input contained a mixture of neg/nonneg.  They are sorted into unsigned order in *xv.  Reorder them
            // to have neg first, in reversed order, followed by positive first, merge +0 and -0, which are widely
            // separated in the result (each at the beginning of their areas).  The merged result must be in ascending
            // order of index
            if (colflags & 2) {  // values were sorted up, & so now contain +0,  +, -0,  -
                I npos0;
                u = xv + (n - nneg);
                for (npos0 = -(n - nneg); npos0 < 0 && wv[u[npos0]] == 0; ++npos0)
                    ;
                npos0 += (n - nneg);  // Count +0
                u = xv + n;
                for (nneg0 = -nneg; nneg0 < 0 && wv[u[nneg0]] == 0; ++nneg0)
                    ;
                nneg0 += nneg;  // Count -0
                u      = zv + nneg - nneg0;
                I ppos = 0, pneg = 0;
                DQ(npos0 + nneg0,
                   *u++ = (pneg >= nneg0 || (ppos < npos0 && xv[ppos] < xv[n - nneg + pneg]))
                            ? xv[ppos++]
                            : xv[n - nneg + pneg++];)  // merge
                // Copy the positives
                ...
                for (npos0 = n - nneg; npos0 > 0 && wv[u[npos0]] == 0; --npos0)
                    ;
                npos0 = (n - nneg) - npos0;  // Count +0
                u     = xv - 1;
                for (nneg0 = nneg; nneg0 > 0 && wv[u[nneg0]] == 0; --nneg0)
                    ;
                nneg0  = nneg - nneg0;  // Count -0
                u      = zv + n - nneg - npos0;
                I ppos = 0, pneg = 0;
                DQ(npos0 + nneg0,
                   *u++ = (pneg >= nneg0 || (ppos < npos0 && xv[n - npos0 + ppos] < xv[nneg - nneg0 + pneg]))
                            ? xv[n - npos0 + ppos++]
                            : xv[nneg - nneg0 + pneg++];)  // merge
                // Copy the positives
                ICPY(zv, xv + nneg, n - nneg - npos0);  //  /: copy the positives to the beginning of result area
                u = zv + n - nneg + nneg0;
                v = xv + nneg - nneg0;
            }
            // BUG: should copy down in other order to avoid pointing past valid region.  As written will run over into
            // the shape... but that will always be positive, at least copy in the negatives, reversing the order.  Here
            // u->place to put negatives, v->last+1 negative as we copy, we have to keep equal keys in the original
            // order, i. e. don't reverse them.
            j = 0;
            d = wv[*(v - 1)];
            // At start of this loop, d has the last of a set of (possibly only 1) equal values, j has the index of the
            // output location where the set will be stored.  v scans back through the negatives, looking for a new
            // value, and when it finds one copies out all the old values.
            DO(
              nneg - nneg0, --v; if (d != wv[*v]) {
                  vv = 1 + v;
                  m  = i - j;
                  DO(m, *u++ = *vv++;);
                  j = i;
            ...
            // We have to push out the last set by hand because *v is outside the input area.  That wouldn't pose a
            // problem except when there is rank; then *v may be the same value as d and a change would be missed
...
static B
jtgri1(J jt, I m, I ai, I n, A w, I *zv) {
    A x, y;
    I *wv;
    I i, *xv;
    US *u;
    ...
    for (i = 0; i < m; ++i) {
        ...
        colflags = grcol(
          65536, 0L, yv, n, 0L, xv, sizeof(I) / sizeof(US), u, 1 - jt->workareas.compare.complt);  // move 'up' to bit 1
        ...
        grcol(
          65536, 0L, yv, n, xv, zv, sizeof(I) / sizeof(US), u += WDINC, colflags | 1);  // the 1 means 'handle sign bit'
        wv += c;
        zv += n;  // advance to next input/output area
    }
    return 1;
} /* grade"r w on integer w where c==n */

static B
jtgru1(J jt, I m, I ai, I n, A w, I *zv) {
    A x, y;
    C4 *wv;
    I i, *xv;
    US *u;
    ...
    for (i = 0; i < m; ++i) {
        ...
        u = (US *)wv + INTLSBWDX;  // point to LSB
        colflags =
          grcol(65536, 0L, yv, n, 0L, xv, sizeof(C4) / sizeof(US), u, 1 - jt->workareas.compare.complt);  // move 'up'
                                                                                                          // to bit 1
...
// NaN
static B
jtgriq(J jt, I m, I ai, I n, A w, I *zv) {
    GBEGIN(-1);                  // subsorts will always be ascending
    ...
    ++hbit;
    I itemmask   = ((I)1 << hbit) - 1;  // mask where the item number will go
    I itemmsb    = (I)1 << (BW - 1 - hbit);
    I itemsigmsk = 2 * -itemmsb;  // get bit at place we will shift into sign bit, and a mask for all higher bits
    // Loop over each grade
    I *wv = IAV(w);  // we interpret the floats in w as if they were integers.
    while (--m >= 0) {
        ...
        DO(n, I v = wv[i] ^ gradedown; if (siglost |= itemsigmsk & ((v & -itemmsb) + itemmsb)) break;
           zv[i] = (v << hbit) + i;)
        ...
        if (!siglost) {
            ...
            // We encountered significance when we tried to shift the values to make room for the indexes.  We will have
            // to sort and then go back to re-sort equal partitions.  At least there is a lot of significance and
            // probably not many collisions First, install the item number in the LSBs
            DO(n, I v = wv[i] ^ gradedown; zv[i] = (v & ~itemmask) + i;)
            // sort the result area in place
            sortiq1(zv, n);
            // pass through the result area, removing the upper bits.  If consecutive values have the same upper bits,
            // go through them, replacing the upper bits with the actual bits from the input (after honoring
            // sign/direction bits), and then re-sort the result in place. remove the upper bits from that sorted result
            I nextv = zv[0];  // always has first value with a new key
            I i;
            for (i = 0; i < n - 1; ++i) {
                I currv = nextv;
                if (((nextv = zv[i + 1]) ^ currv) & ~itemmask) {
                    ...
                    I j = i;
                    do {
                        I v   = wv[zv[j] & itemmask] ^ gradedown;
                        zv[j] = ((v & itemmask) << hbit) +
                                (zv[j] & itemmask);  // fetch original v, reconstitute; get itemmask in upper bits
                    } while (!(++j == n || (((nextv = zv[j]) ^ currv) & ~itemmask)));
                    sortiq1(zv + i, j - i);  // sort the collision area in place.  j points to first item beyond the
                                             // collision area, and nextv is its value
                    while (i < j) {
                        zv[i] &= itemmask;
                        ++i;
                    }           // the item numbers are guaranteed right after this sort
                    i = j - 1;  // pick up after the batch
                }
            }
            // We handled all the starting items up to the next-last.  The last item may be unprocessed, if it was not
            // part of a batch.  Handle it now.
            zv[n - 1] &= itemmask;
        }
        // advance to next sort
        wv += n;
...
static B
jtgri(J jt, I m, I ai, I n, A w, I *zv) {
    A x, y;
    B up;
    I e, i, *v, *wv, *xv;
    UI4 *yv, *yvb;
    I c = ai * n;
    wv  = AV(w);
    // select algorithm based on size & range.  To develop models for the different algorithms, modify the code here to
    // force one choice & run test lines. Test line for lists for dependence on length of list:
    // (((20) 6!:2 '/: a'&(4 : 'x [ a =: y ?@$ y'))"0  %.  (*: ,. (* 2&^.) ,. ] ,. 1:)) <. 10 ^ 3 + 10 %~ i. 30  //
    // tweak range to places of interest Results 3/2/2017 on Ivy Bridge: smallrange 6.5e_14 5.5e_9 _8e_8 2e_4
    ...
    // for lists, smallrange beats radix if range<~70000; smallrange beats merge if range<n*65; radix beats merge if
    // n>1250 (or more depending on how well sign-detection works)
    ...
    // for atoms, smallrange wins if the range is less than (2n lgn) unless that exceeds L2; then 4n.  quicksort wins
    // otherwise, except for length 5500-500000 when there are <= two bytes of significance: then radix wins for lists,
    // smallrange always beats radix, but loses to merge if the range is too high.  We assess the max acceptable range
    // as (80>>keylength)*(n), smaller if the range would exceed cache size
    CR rng;
    if (ai == 1) {                                       // for atoms, usually use smallrange or quicksort
        if (n < 10) return jtgriq(jt, m, ai, n, w, zv);  // for short lists just use qsort
        UI4 lgn3;
        CTLZI(n, lgn3);
        lgn3 = (UI4)((lgn3 * 8) - 8 + (n >> (lgn3 - 3)));  // approx lg(n)<<3
        rng  = condrange(wv,
                        AN(w),
                        IMAX,
                        IMIN,
                        MIN((L2CACHESIZE >> LGSZI),
                            (n * lgn3) >> (3 - 1)));  // let range go up to 2n lgn, but never more than L2 size
        if (!rng.range) {
            if (!BETWEENC(n, 5500, 500000)) return jtgriq(jt, m, ai, n, w, zv);  // quicksort except for 5500-500000
            // in the middle range, we still use quicksort if the atoms have more than 2 bytes of significance.  We just
            // spot-check rather than running condrange, because the main appl is sorting timestamps, which are ALL big
            DO(10,
      ...
      if (ai <= 6) {
        rng =
          condrange(wv,
                    ...
                    IMAX,
                    IMIN,
                    (MIN(((ai * n < (L2CACHESIZE >> LGSZI)) ? 16 : 4), 80 >> ai)) * n);  // test may overflow; OK   TUNE
    } else
        rng.range = 0;  // if smallrange impossible
    ...
    yvb = C4AV(y);
    yv  = yvb - rng.min;
    up  = SGNTO0(jt->workareas.compare.complt);
    ...
    for (i = 0; i < m; ++i) {  // Loop over each cell of w
        ...
        if (up) {
            UI4 k, s = 0, *yvi = yvb + rng.range;
            DP(rng.range, k = yvi[i]; yvi[i] = s; s += k;)
        } else {
            UI4 k, s = 0;
            DQ(rng.range, k = yvb[i]; yvb[i] = s; s += k;);
        }
        // refetch each input and take its position from the +/\ list.  After taking a position, add 1 so the next
        // identical value gets the next position store the index of the fetched input value into the indicated position
        ...
        for (e = ai - 2; 0 <= e; --e) {  // loop d-1 times
            ...
            // *v is the base of the vector to process, which has stride d.  Process the elements in the order given by
            // the previous grade.
            {
                I *vv = v;
                DQ(n, ++yv[*vv]; vv += ai;);
            ...
            // process the input atoms in the order of the grade so far, and find the result position as above.  Move
            // the original index for the item into the result
            {
                I *gg = xv + n;
                DP(n, zv[yv[v[ai * gg[i]]]++] = gg[i];);
            }
        }
...
static B
jtgru(J jt, I m, I ai, I n, A w, I *zv) {
    A x, y;
    B up;
    I e, i, *xv;
    UI4 *yv, *yvb;
    ...
    CR rng;
    if (ai <= 6) {
        rng = condrange4(wv, AN(w), -1, 0, (MIN(((ai * n < (L2CACHESIZE >> LGSZI)) ? 16 : 4), 80 >> ai)) * n);  //  TUNE
    } else
        rng.range = 0;
    if (!rng.range)
...
#define DOCOL1(p, iicalc0, iicalc1, ind, vinc)      \
    {                                               \
        ...
        memset(yv, C0, ps);                         \
        v = vv;                                     \
        DQ(n, ++yv[iicalc0]; v += ai;);             \
        if (up)                                     \
            DO(p, k = yv[i]; yv[i] = s; s += k;)    \
        else                                        \
            DQ(p, k = yv[j]; yv[j--] = s; s += k;); \
        v = vv;                                     \
        DO(n, h[yv[iicalc1]++] = ind; vinc;);       \
    }

#define DOCOL4(p, iicalc0, iicalc1, ind, vinc)          \
    {                                                   \
        I *g, *h, ii, j = p - 1, k, s = 0;              \
        UC *v;                                          \
        if (b) {                                        \
            g = xv;                                     \
            h = zv;                                     \
        ...
        memset(yv, C0, ps);                             \
        v = vv;                                         \
        DQ(n, IND4(iicalc0); ++yv[ii]; v += ai;);       \
        if (up)                                         \
            DO(p, k = yv[i]; yv[i] = s; s += k;)        \
        else                                            \
            DQ(p, k = yv[j]; yv[j--] = s; s += k;);     \
        v = vv;                                         \
        DO(n, IND4(iicalc1); h[yv[ii]++] = ind; vinc;); \
    }

static B
jtgrb(J jt, I m, I ai, I n, A w, I *zv) {
    A x;
    B b, up;
    I i, p, ps, q, *xv, yv[16];
    UC *vv, *wv;
...
static B
jtgrc(J jt, I m, I ai, I n, A w, I *zv) {
    A x;
    B b, q, up;
    I e, i, p, ps, *xv, yv[256];
    UC *vv, *wv;
    UI4 lgn;
    CTLZI(n, lgn);
...
A
jtgrade1p(J jt, A a, A w) {
    PROLOG(0074);
    A x, z;
    I n, *s, *xv, *zv;
    s                               = AS(w);
    n                               = s[0];
    jt->workareas.compare.compn     = s[1] - 1;
    jt->workareas.compare.compk     = SZI * s[1];
    jt->workareas.compare.comp      = compp;
    jt->workareas.compare.compsyv   = AV(a);
    jt->workareas.compare.compv     = CAV(w);
...
jtgr1(J jt, A w) {
    ...
    if (((wn - 1) | (n - 2)) < 0) return jtreshape(jt, shape(jt, z), IX(n));
...
jtgrade1(J jt, A w) {
    ...
    GBEGIN(-1);
    RZ(w);
    z = SPARSE & AT(w) ? jtgrd1sp(jt, w) : jtgr1(jt, w);
    GEND return z;
}
A
jtdgrade1(J jt, A w) {
...
// Since grade2 pulls from a, mark a as non-pristine.  But since there can be no repeats, transfer a's pristinity to
// result if a is inplaceable We do this in jtgr2 because it has a branch where all boxed values go
A
jtgrade2(J jt, A a, A w) {
    FPREFIP;
    A z;
    GBEGIN(-1);
    RZ(z = SPARSE & AT(w) ? jtgrd2sp(jt, a, w) : jtgr2(jtinplace, a, w));
    GEND return z;
}
A
jtdgrade2(J jt, A a, A w) {
    FPREFIP;
    A z;
    GBEGIN(1);
    RZ(z = SPARSE & AT(w) ? jtgrd2sp(jt, a, w) : jtgr2(jtinplace, a, w));
...
#define SORT4                                              \
    switch (n) {                                           \
        ...
        case 3:                                            \
            if (OSGT(0, 1)) XC(0, 1);                      \
            if (OSGT(1, 2))                                \
                if (OSGT(0, 2)) P3(2, 0, 1) else XC(1, 2); \
            break;                                         \
        case 4:                                            \
            if (OSGT(0, 1)) XC(0, 1);                      \
            if (OSGT(2, 3)) XC(2, 3);                      \
    ...
    {                                                                        \
        ...
        while (1) {                                                          \
            if (4 >= n) {                                                    \
                u = tv;                                                      \
                SORT4;                                                       \
                return ATOMF(jt, tv[j]);                                     \
            } /* stop loop on small partition */                             \
            p0 = tv[(I)(qv[i] * n)];                                         \
            --i;                                                             \
            p1 = tv[(I)(qv[i] * n)];                                         \
            --i;                                                             \
            ...
            if (p0 > p1) {                                                   \
                ...
                p0 = p1;                                                     \
                p1 = q;                                                      \
            } /* create pivots p0, p1 selected from input, with p0 <= p1  */ \
            {                                                                \
                m0 = m1 = 0;                                                 \
                v       = tv;                                                \
                DQ(n, m0 += *v < p0; m1 += *v < p1; ++v;);                   \
            } /* count m0: # < p0; and m1: # < p1  */                        \
            {                                                                \
                I l = 0, h = m0;                                             \
                l = j >= m0 ? m0 : l;                                        \
                h = j >= m0 ? m1 : h;                                        \
                ...
                h = j >= m1 ? n : h;                                         \
                m = h - l;                                                   \
            } /* calc size of partition holding the result */                \
            if (t)                                                           \
                u = v = tv;                                                  \
            else {                                                           \
                GA(t, wt, m + 1, 1, 0);                                      \
                u = tv = (T *)AV(t);                                         \
                v      = wv;                                                 \
            } /* allow for 1 overstore */                                    \
            if (j < m0) {                                                    \
                DQ(n, *u = *v; u += *v < p0; ++v;);                          \
            } else if (j < m1) {                                             \
                DQ(n, *u = *v; u += (p0 <= *v) & (*v < p1); ++v;);           \
                j -= m0;                                                     \
            } else if (m1) {                                                 \
                DQ(n, *u = *v; u += p1 <= *v; ++v;);                         \
                j -= m1;                                                     \
                ...
                m = u - tv;                                                  \
                n -= m;                                                      \
                if (j < n) return ATOMF(jt, p1);                             \
                j -= n;                                                      \
            } /* pivots both low; use > to split */                          \
            n = m;                                                           \
        }                                                                    \
    }

A
jtordstat(J jt, A a, A w) {
    A q, t = 0;
    I j, m, m0, m1, n, wt;
    D *qv;
    ...
    wt  = AT(w);
    RE(j = jti0(jt, a));
    if (((4 - n) & ((AR(a) | (1 ^ AR(w))) - 1) & (-(wt & FL + INT))) >= 0)
        return jtfrom(
          jt, a, jtgrade2(jt, w, w));  // if not int/float, or short, or not (atom a and list w), do full grade
    if ((UI)j >= (UI)n) {
        j += n;
