...
// /: and \: with IRS support
 A jtgr1(J jt, A w){PROLOG(0075);A z;I c,f,ai,m,n,r,*s,t,wn,wr,zn;
 if(!w) return 0;
...
// We do this in jtgr2 because it has a branch where all boxed values go
 A jtgrade2 (J jt,A a,A w){F2PREFIP;A z;GBEGIN(-1); if(!(a && w)) return 0; RZ(z=SPARSE&AT(w)?grd2sp(a,w):jtgr2(jtinplace,a,w)); GEND
 return z;}
 A jtdgrade2(J jt,A a,A w){F2PREFIP;A z;GBEGIN( 1); if(!(a && w)) return 0; RZ(z=SPARSE&AT(w)?grd2sp(a,w):jtgr2(jtinplace,a,w)); GEND
 return z;}
