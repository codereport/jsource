...
static X jtxlog2(J jt,X a,X w){D c,d,x,y;I an,*av,j,k,m,n,wn,*wv;X p,q;
 ...
 if(2<an)return rifvsdebug(jtxlog2sub(jt,a,w));
 ASSERT(0<=c,EWIMAG);
 ...
 if     (0<j){--m; RZ(p=xdiv(p,a,XMEXACT)); j=xcompare(p,w); if(0<j)return rifvsdebug(jtxlog2sub(jt,a,w));}
 else if(0>j){++n; RZ(q=xtymes(p,a));       k=xcompare(q,w); if(0>k)return rifvsdebug(jtxlog2sub(jt,a,w));}
