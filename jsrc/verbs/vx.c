...
I jtxcompare(J jt,X a,X w){I*av,j,m,n,x,y,*wv;int s,t;
 ...
 X jtxsgn(J jt,    X w){I x=XDIG(w); return jtxc(jt,SGN(x));}
...
X jtxdiv(J jt,X a,X w,I mode){PROLOG(0096);B di;I an,*av,c,c0,d,e,k,s,u[2],u1,wn,*wv,yn;X q,r,y;
 ...
 if(c==XPINF||c==XNINF){ASSERT(!di,EVNAN); return jtvci(jt,0<c*d?XPINF:XNINF);}
 if(di)return iv0;
 if(1==wn&&d){I*v;
 ...
 c=XDIG(a); if(0>c)RZ(a=jtnegate(jt,a));
 d=XDIG(w); if(0>d)RZ(w=jtnegate(jt,w));
...
static X jtxexp(J jt,X w,I mode){I k,m;X s,y;
 ...
 m=(I)(2.718281828*jtxint(jt,w)); k=2; s=jtxplus(jt,iv1,w); y=w;
 ...
 if(0>d){
  ...
  r=jt->xmode==XMCEIL; return jtxc(jt,0<c?r:1&e?r-1:r);
 }
 t=a; z=iv1; m=jt->xmod?XAV(jt->xmod)[0]:0;
 if(!m||1>jtxcompare(jt,w,jtxc(jt,IMAX))){
  ASSERT(m||2>=AN(w),EVLIMIT);
  RE(e=jtxint(jt,w));
  if(m)while(e){if(1&e)RZ(z=jtxrem(jt,m,jtxtymes(jt,z,t))); RZ(t=jtxrem(jt,m,jtxsq(jt,t))); e>>=1;}
  else while(e){if(1&e)RZ(z=       jtxtymes(jt,z,t) ); RZ(t=       jtxsq(jt,t) ); e>>=1;}
 }else{B b;I n,*u,*v;X e;
  RZ(e=jtca(jt,w)); n=AN(e); v=AV(e);
  while(n){
 ...
 switch(jt->xmode){
  ...
  case XMFLR:  if(-1==p){--AV(x)[0]; return jtxstd(jt,x);}else return x;
  case XMCEIL: if( 1==p){++AV(x)[0]; return jtxstd(jt,x);}else return x;
  case XMEXACT:
   if(!p)return x;
   AV(x)[0]+=p; RZ(x=jtxstd(jt,x));
...
static D jtxlogd1(J jt,X w){ASSERT(0<=XDIG(w),EWIMAG); return jtxlogabs(jt,w);}
static Z jtxlogz1(J jt,X w){Z z; z.re=jtxlogabs(jt,w); z.im=0>XDIG(w)?PI:0.0; return z;}
