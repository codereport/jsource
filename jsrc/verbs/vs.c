...
static A jtsparse1a(J jt,A s,A a,A e,A y,A x){A z;B*b;I an,*av,et,r,*sv,t,*v;P*p;
 ...
 if(y==mark){
  ...
  if(n&&0<=j){
   m=jtaii(jt,x); k=m<<bplg(t);
   RZ(q=jtgrade1(jt,y)); qv=AV(q);
...
A jtsparseit(J jt,A w,A a,A e){PROLOG(0091);A ax,c,x,y,z;B b,*cv;I cm,cn,m,n,r,*s,t,*u,*v,wn;P*p;
 ...
 if(b)s=AS(w); else{RZ(q=jtover(jt,a,jtless(jt,IX(r),a))); RZ(s1=jtfrom(jt,q,shape(jt,w))); s=AV(s1);}
 RE(n=jtprod(jt,r,s));
 GA(z,t,n,r,s); zv=CAV(z); xv=CAV(x); 
 if(1<an)RZ(y=jtbase2(jt,vec(INT,an,s),y)); yv=AV(y);
 k=bpnoun(t); ck=k*jtaii(jt,x); mvc(k*n,zv,k,AV(e));
 ...
 if(all1(jteps(jt,a,a1))){I*s;  /* old is subset of new */
  ...
  RZ(y=jtstitch(jt,jtrepeat(jt,jtsc(jt,c),y),jtreshape(jt,jtv2(jt,c*m,k),jtabase2(jt,vec(INT,k,v),IX(c)))));
  RZ(p=jtgrade1(jt,jtover(jt,a,jtless(jt,a1,a))));
  if(jtequ(jt,p,IX(AN(p))))SPB(zp,i,jtrepeat(jt,q,y))
  else{y=jtfromr(jt,p,jtrepeat(jt,q,y)); q=jtgrade1(jt,y); SPB(zp,i,jtfrom(jt,q,y)); SPB(zp,x,jtfrom(jt,q,x));}
  return z;
 }
 if(all1(jteps(jt,a1,a))){A x1,y1;B*pv;C*s,*t;I g,h,*iv,n;  /* new is subset of old */
  ...
  RZ(p=jtover(jt,jtless(jt,a,a1),jtdaxis(jt,r,a))); v=AV(p);
  *u=n; j=1; DQ(AN(p), u[j++]=ws[*v++];); RE(h=jtprod(jt,1+r-d,u));
  ...
  k=bpnoun(AT(x)); g=k*jtaii(jt,x); h=k*jtaii(jt,x1); mvc(k*AN(x1),t,k,AV(e));
  DO(m, memcpy(t+g*iv[i],s,g); s+=g; if(b[i]){ICPY(v,u+i*c,d); v+=d; t+=h;});
  SPB(zp,i,y1); SPB(zp,x,jtcant2(jt,jtincrem(jt,jtindexof(jt,p,jtdaxis(jt,r,a1))),x1));
  return z;
 }
...
static A jtaxbytes(J jt,A a,A w){A a1,e,p,q,x;B*b;I c,d,j,m,n=0,r,*u,*v,*ws,wt;P*wp;
 ...
 else         {           a=mtv;       RZ(e=jtselm(jt,wt)); x=w;         c=0;}
 if(all1(jteps(jt,a,a1))){    /* old is subset of new */
  RZ(p=jteps(jt,jtdaxis(jt,r,a),a1)); b=BAV(p);
  v=c+AS(x); DO(AN(p), if(!b[i])u[j++]=v[i];);
  RZ(q=irs2(jtcant2(jt,plus(jtsc(jt,c),jtdgrade1(jt,p)),x),jtreshape(jt,vec(INT,j,u),e),0L,j,j,jtmatch));
...
static A jtsparsen1(J jt, A w){A*u,z;P*p;
 ...
 u[0]=shape(jt,w); u[1]=jtca(jt,SPA(p,a)); u[2]=jtca(jt,SPA(p,e));
 RE(0); return z;
}
 ...
 switch(k){
  ...
  case 1:  ASSERT(!q,EVDOMAIN); q=jtsparsep1(jt,w); PRISTCLRF(w); return q;
  ...
  case 2:
   ...
   if(q)return jtreaxis(jt,q,w); else if(b)return jtrat(jt,SPA(p,a)); else{ASSERT(STYPE(t)!=0,EVDOMAIN); return IX(AR(w));}
  case 3:  return q?jtrezero(jt,q,w):jtrat(jt,SPA(p,e));  // ? there jtrat(jt,)s don't protect anything?  SPA is as permanent as w
