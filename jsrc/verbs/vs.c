...
B jtscheck(J jt,A w){A a,e,x,y;I k1,k2,r,*s,t;P*p;D rkblk[16];
 ...
 ASSERTSYS(equ(behead(shape(x)),jtfrom(jt,jtless(jt,IX(r),a),shape(w))),"scheck x shape");
 return 1;
...
static A jtsparse1a(J jt,A s,A a,A e,A y,A x){A z;B*b;I an,*av,et,r,*sv,t,*v;P*p;
 ...
 if(y==mark){
  ...
  if(n&&0<=j){
   ...
   GA(x1,t,  AN(x),AR(x),AS(x)); xv=CAV(x1); xu=CAV(x);
   memcpy(xv,xu+k**qv,k);
   for(i=0;i<n;++i){
    ++qv; v=yu+an**qv;
    DO(an, if(yv[i]<v[i]){yv+=an; ICPY(yv,v,an); xv+=k;
                    memcpy(xv,xu+k**qv,k); break;});
   }
