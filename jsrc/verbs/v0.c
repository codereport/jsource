...
static CFR(jtcfrd, D, FL, dplus, dtymes, dnegate) static CFR(jtcfrx, X, XNUM, xplus, xtymes, negate) static CFR(
  jtcfrq, Q, RAT, qplus, qtymes, QNEGATE)

  static A jtrsort(J jt, A w) {
    A t, z;
    PUSHCCT(1.0 - FUZZ)
    RZ(t = jtover(jt, mag(w), jtcant1(jt, jtrect(jt, w))));
    A tt;
    RZ(IRS2(t, t, 0L, 1L, 1L, jtindexof, tt));
    z = jtdgrade2(jt, w, jtcant1(jt, IRS2(tt, t, 0L, 1L, 1L, jtfrom, z)));
    POPCCT
    return z;
}
...
jtcfr(J jt, A w) {
    ...
    AF tf;
    tf = (AF)jtcfrd;
    tf = t & CMPX ? (AF)jtcfrz : tf;
    tf = t & XNUM ? (AF)jtcfrx : tf;
    tf = t & RAT ? (AF)jtcfrq : tf;
    return (*tf)(jt, c, r);
...
jtnewt(J jt, I m, Z* a, Z x, I n) {
    ...
    for (i = 0; i < n; ++i) {
        ...
        v     = a + m;
        j     = m;
        DQ(m, p = jtzplus(jt, *v, jtztymes(jt, x, p)); c.re = (D)j--;
           q = jtzplus(jt, jtztymes(jt, c, *v), jtztymes(jt, x, q));
           --v;);
...
static B
jtdeflateq(J jt, B k, I m, Q* v, Q x) {
    Q q, r, *u;
    u = v + m;
    q = *u--;
    DQ(m, r = *u--; q = jtqplus(jt, r, jtqtymes(jt, q, x)););
    RE(0);
    if (!(QEQ(q, zeroQ))) return 0;
    u = v + m;
    q = *u--;
    DQ(m, r = *u; *u-- = q; q = jtqplus(jt, r, jtqtymes(jt, q, x)););
    return 1;
} /* deflate by x which may or may not be a root. result is 1 iff x is a root. k is ignored. */

static void
jtdeflate(J jt, B k, I m, Z* v, Z x) {
    if (k) {
        Z q, r;
        v += m;
        q = *v--;
        DQ(m, r = *v; *v-- = q; q = jtzplus(jt, r, jtztymes(jt, q, x)););
    } else {
        D a, b, d, p, q, r;
        a = 2 * x.re;
...
#define CSZ1 11
static Z
jtlaguerre(J jt, I m, Z* a, Z x) {
    D ax, e;
    I i, j;
    Z b, c, d, dx, g, g2, h, p, q, s, sq, y, zm, zm1;
    ...
    for (i = 0;; ++i) {
        ...
        sq = jtzsqrt(jt, jtztymes(jt, zm1, jtzminus(jt, jtztymes(jt, zm, h), g2)));
        p  = jtzplus(jt, g, sq);
        q  = jtzminus(jt, g, sq);
        s  = zmag(p) > zmag(q) ? p : q;
        y  = x;
        dx = ZNZ(s) ? jtzdiv(jt, zm, s)
                    : jtzpow(jt,
                             jtznegate(jt, jtzdiv(jt, a[0], a[m])),
                             zrj0(1.0 / (D)m));  // Normal step if s!=0; random step if s=0
        x  = jtzminus(jt, x, dx);                // advance to new position
        if (zmag(jtzminus(jt, x, y)) <= EPS * zmag(x))
            return x;  // if we didn't move much, call it converged.  We hope it's a root.
        ...
        if (!--kicktimer) {
            kicktimer = CSZ1;
            x         = jtzplus(jt, x, jtztymes(jt, dx, zrj0(cyclefracs[(i >> 3) & 8])));
        }
    }
}  // Press et al., "Numerical Recipes in C" with additions from 2d edition

static Q
jtmultiple(J jt, D x, Q m) {
    A y;
    Q q1, q2, q1r2;
    q1r2.n = iv1;
...
static Q
jtmaxdenom(J jt, I n, Q* v) {
    Q z;
    X *u, x, y;
    u = 1 + (X*)v;
    x = *u;                                                             // u-> &1st denominator, x=&1st denominator
    DQ(n - 1, u += 2; y = *u; if (-1 == jtxcompare(jt, x, y)) x = y;);  // x=&largest denominator
    z.n = x;
    z.d = iv1;
    return z;  // set denominator as a rational number
...
jtrfcq(J jt, I m, A w, A* zz, A* ww) {
    ...
    if (!(wt & RAT)) RZ(w = jtcvt(jt, RAT, w));
    wv  = QAV(w);
    rdx = jtmaxdenom(jt, 1 + m, wv);  // rdx = max denominator in the polynomial, in rational form
    RZ(x = jtcvt(jt, CMPX, w));
    xv = ZAV(x);  // set x = complex form of w, xv->first complex coeff
    RZ(y = jttake(jt, jtsc(jt, 1 + m), x));
    makewritable(y);
    yv = ZAV(y);  // y = complex form with degree m, yv->first coeff.  These are modified by deflate[q]() and must not
                  // be virtual
    RZ(q = jttake(jt, jtsc(jt, 1 + m), w));
    makewritable(q);
    qv = QAV(q);  // q = rational form with degree m, qv->first coeff
    GATV0(z, RAT, m, 1);
    zv = QAV(z);  // allocate space for exact rational roots, zv->first result location
    i = j = 0;
    ...
    while (i < m) {
        ...
        if (b) {
            AN(q) = AS(q)[0] = 1 + m - j;
            rdx              = jtmaxdenom(jt, 1 + m - j, qv);
            RZ(y = jtcvt(jt, CMPX, q));
            yv = ZAV(y);
            i  = j;
        } else {
            D c, d;
...
jtrfcz(J jt, I m, A w) {
    ...
    GATV0(y, CMPX, 1 + m, 1);
    yv = ZAV(y);
    memcpy(yv, xv, (1 + m) * sizeof(Z));
    GATV0(z, CMPX, m, 1);
    zv = ZAV(z);
    if (2 == m) {
        Z a2, b, c, d, z2 = {2, 0};
        a2    = jtztymes(jt, z2, xv[2]);
        b     = jtznegate(jt, xv[1]);
        c     = xv[0];
...
jtrfc(J jt, A w) {
    ...
    return link(jtfrom(jt, jtsc(jt, m), w), jtrsort(jt, r));
}
...
jtmnomx(J jt, I m, A w) {
    ...
    if (BOX & AT(w)) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            zv[i] = jtincorp(jt, m <= 1 ? x : jtreshape(jt, s, x));
        }
        RE(z);
        RZ(z = jtope(jt, z));
    }
...
jtpoly2a(J jt, A a, A w) {
    ...
    RZ(e = jtcant1(jt, IRS1(a, 0L, 1L, jtbehead, e)));
    RZ(x = jtmnomx(jt, m, w));
    if (1 == m) {
        A er;
        RZ(er = jtravel(jt, e));
        return jtpdt(jt, ATOMIC2(jt, x, er, rkblk, 0L, 2L, CEXP), c);
    } else {
        A z;
        return jtpdt(jt, df2(z, x, e, jtdot(jt, jtslash(jt, ds(CSTAR)), ds(CEXP))), c);
    }  // scaf need agreement check?
} /* multinomial: (<c,.e0,.e1,.e2) p. <x0,x1,x2, left argument opened */

// x p. y    Supports IRS on the y argument; supports inplace
A
jtpoly2(J jt, A a, A w, A self) {
    FPREFIP;
    A c, za;
    I b;
    D *ad, d, p, *x, u, *z;
    ...
    Z *az, e, q, *wz, y, *zz;
    {
        ...
        RESETRANK;  // cell-rank of a
        if (((1 - acr) | (acr - AR(a))) < 0) {
            return rank2ex(a, w, self, MIN(acr, 1), 0, acr, MIN(AR(w), jtr & RMAX), jtpoly2);
        }  // loop if multiple cells of a
    }
    an = AN(a);
    at = AT(a);
    b  = BOX & at;  // b if mplr/roots form or multinomial; otherwise coeff
    n  = AN(w);
    wt = AT(w);
    ...
    if (b) {
        ...
        if (!AR(a)) RZ(a = jtravel(jt, a));  // treat atomic a as list
    }
    t = maxtyped(at, wt);
    if (b) t = maxtyped(t, AT(c));
    if (!(t & XNUM + RAT)) t = maxtyped(t, FL);  // promote B01/INT to FL
    ...
    if (b) {
        ...
        // coeffs: discard trailing 0 coeffs (to avoid 0*_ as our first action), extract high-order coeff.  Leave
        // constant coeff always
        for (; an > 1; --an) {
            if (t & CMPX ? ad[2 * (an - 1)] || ad[2 * (an - 1) + 1] : ad[an - 1]) break;
        }  // stop on nonzero
    }
    j = 0;  // Set j=1 if there is an infinity in the coeffs/roots.  In that case we can't use Horner's rule (could do
            // this only if !b&&FL?)
    if (t & FL + CMPX) {
        DO(
          t & FL ? an : an + an, u = ad[i]; if (fabs(u) == inf) {
              j = 1;
              break;
          });
    }
    if ((-postfn & ((-b) | (1 - (an ^ 2)) | ((t & FL) - 1))) < 0)
        return jtupon2cell(jt, a, w, self);  // revert if ^@:p. but not powers (postfn not 0, and a boxed or degree not
                                             // 1 or 2 or type not FL).  an is final here
    ...
    if (((j - 1) & ((t & XNUM + RAT + SPARSE) - 1)) < 0) {
        ...
        if (postfn)
            return jtupon2cell(
              jt, a, w, self);  // revert if there is a postfn, and we are using the eval path.  type must be FL
    }
    switch (b) {
        // coeffs: d/e are not set
        case 0: return df2(za, w, a, jteval(jt, "(^/i.@#) +/ .* ]"));  // XNUM/RAT/SPARSE
        case 1:
            NAN0;          // FL
            switch (an) {  // special cases for linear, quadratic, cubic
                ...
                case 3: {
                    D c0 = ad[0], c1 = ad[1], c2 = ad[2];
                    DQ(n, u = *x++; *z++ = c0 + u * (c1 + u * c2););
                } break;
                case 4: {
                    D c0 = ad[0], c1 = ad[1], c2 = ad[2], c3 = ad[3];
                    DQ(n, u = *x++; *z++ = c0 + u * (c1 + u * (c2 + u * c3)););
                } break;
                default: DO(n, p = ad[an - 1]; u = *x++; DQ(an - 1, p = ad[i] + u * p;); *z++ = p;); break;
            }
            NAN1;
            break;  // Horner's rule.  First multiply is never 0*_
        case 2:
            NAN0;
            DQ(n, q = zeroZ; y = *wz++; j = an; DQ(an, q = jtzplus(jt, az[--j], jtztymes(jt, y, q));); *zz++ = q;);
            NAN1;
            break;  // CMPX
        // mult/roots: d/e are set
        case 3: return tymes(c, df2(za, jtnegate(jt, a), w, jteval(jt, "*/@(+/)")));
        case 4:
            NAN0;
            DO(n, p = d; u = *x++; DO(an, p *= u - ad[i];); *z++ = p;);
            NAN1;
            break;
        case 5:
            NAN0;
            DO(n, q = e; y = *wz++; DO(an, q = jtztymes(jt, q, jtzminus(jt, y, az[i]));); *zz++ = q;);
            NAN1;
            break;
    }
    return za;
