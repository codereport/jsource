...
#define RANKINGLOOP(T)              \
    {                               \
        T *v = (T *)wv;             \
        DQ(n, ++yu[*v++];);         \
        RANKINGSUMSCAN(rng.range);  \
        v = (T *)wv;                \
        DQ(n, *zv++ = yu[*v++]++;); \
    }

static A
jtrankingb(J jt, A w, I wf, I wcr, I m, I n, I k) {
    A z;
    C *wv;
    I i, j, p, t, *zv;
    TTYPE yv[16];
...
jtranking(J jt, A w) {
    A y, z;
    C *wv;
    I icn, i, k, m, n, t, wcr, wf, wn, wr, *ws, wt, *zv;
    CR rng;
    TTYPE *yv, *yu;
    wr  = AR(w);
    wcr = (RANKT)jt->ranks;
    ...
    wn = AN(w);
    ws = AS(w);
    SETICFR(w, wf, wcr, n);  // n=#cells in w; m is number of atoms in w
    if (wn) {
        PROD(m, wf, ws);
    ...
    if (icn == 1 && wt & INT + C4T) {  // items are individual INTs or C4Ts
        ...
        // small-range indexing is range*4.5 (.5 to clear, 2 to read) + n*6 (4 to increment, 2 to write).  So range can
        // be as high as n*lg(n)*4/4.5 - n*6/4.5 approximate lg(n) with bit count.  And always use small-range if range
        // is < 256
        UI4 lgn;
        CTLZI(wn, lgn);
        ...
        rng.range = shortrange[wt & (B01 + LIT)][k];
        rng.min   = 0;  // if B01, must be 1 byte; otherwise 2^(8*k)
    } else
        rng.range = 0;
    if (!rng.range) {
        I *yv;
        // small-range not possible.  Do the grade and install each value into its location
        RZ(IRS1(w, 0L, wcr, jtgrade1, y));
        yv = AV(y);
        GATV(z, INT, m * n, 1 + wf, ws);
        if (!wcr) AS(z)[wf] = 1;
        zv = AV(z);
    ...
    for (i = 0; i < m; ++i) {
        ...
        switch (k) {
            ...
            case sizeof(C4): RANKINGLOOP(C4); break;
