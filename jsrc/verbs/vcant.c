...
static A jtcants(J jt,A a,A w,A z){A a1,q,y;B*b,*c;I*u,wr,zr;P*wp,*zp;
 ...
 SPB(zp,x,jtfrom(jt,q,jtcanta(jt,over(zeroionei(0),increm(grade1(jtless(jt,a,a1)))),SPA(wp,x))));
 return z;
}    /* w is sparse */
...
static A jtcanta(J jt,A a,A w){A m,s,t,z;C*wv,*zv;I*av,j,*mv,r,*sv,*tv,wf,wr,*ws,zn,zr,ms[4],ss[4],ts[4];
 ...
 if((SPARSE&AT(w))!=0){GASPARSE(z,AT(w),1,zr,sv); return cants(a,w,z);}  // if sparse, go to sparse transpose code.
