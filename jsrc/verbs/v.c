...
jtnatoms(J jt, A w) {
    FPREFIP;
    A z;
    if ((AT(w) & SPARSE) != 0) return df1(z, shape(jt, w), jtslash(jt, ds(CSTAR)));
    return jtsc(jt, AN(w));
}  // */@$  #@,
...
jtravel(J jt, A w) {
    ...
    r = (RANKT)jt->ranks;
    r = AR(w) < r ? AR(w) : r;
    f = AR(w) - r;  // r=effective rank (jt->rank is effective rank from irs1), f=frame
    if (!(AT(w) & SPARSE)) {
        if (r == 1)
        ...
        if (ASGNINPLACESGN(SGNIF((I)jtinplace, JTINPLACEWX) & (-r), w) &&
            ...
            // rather than relying on the self-virtual-block code in virtual() because we can do it for indirect types
            // also, since we know we are not changing the number of atoms We preserve pristinity between the input and
            // the output
            AR(w)    = (RANKT)(1 + f);
            AS(w)[f] = m;
            return w;
    ...
    if (memchr(b + f, C1, r)) {
        if (memchr(b + f, C0, r)) {
            memset(b + f, C1, r);
            RZ(w = jtreaxis(jt, jtifb(jt, AR(w), b), w));
            wp = PAV(w);
            x  = SPA(wp, x);
        } else
            RZ(x = jtca(jt, SPA(wp, x)));
        a = jtifb(jt, 1 + f, b);
        makewritable(a)  // avoid readonly block
          GATV0(c, INT, r, 1L);
        v = r + AV(c);
        j = AR(w);
        m = 1;
        ...
        RZ(x = IRS1(SPA(wp, x), 0L, r, jtravel, y0));
...
static A
jtlr2(J jt, RANK2T ranks, A a, A w) {
...
jtiota(J jt, A w) {
    ...
    if (AT(w) & XNUM + RAT) return jtcvt(jt, XNUM, jtiota(jt, jtvi(jt, w)));
    RZ(w = jtvi(jt, w));
    n = AN(w);
    v = AV(w);
    if (1 == n) {
...
jtjico1(J jt, A w) {
    ...
    RZ(y = jtcvt(jt, FL, jtrect(jt, w)));
    v = DAV(y);
    d = *v;  // convert to complex, d=real part of value
    RE(m = v[1] ? jti0(jt, jtcvt(jt, INT, jttail(jt, y)))
                : jti0(jt, tymes(mag(w), num(2))));  // m=#steps: imaginary part if nonzero; otherwise 2*|w
...
A
jtnum1(J jt, A w, A self) {
    return FAV(self)->fgh[2];
}
A
jtnum2(J jt, A a, A w, A self) {
    return FAV(self)->fgh[2];
}

A
jtfromr(J jt, A a, A w) {
    A z;
    return IRS2(a, w, 0, RMAX, 1L, jtfrom, z);
}  // no agreement check because left rank is infinite - no frame  {"_ 1
A
jtrepeatr(J jt, A a, A w) {
    A z;
    return IRS2(a, w, 0, RMAX, 1L, jtrepeat, z);
}  // #"_ 1

A
jttaker(J jt, I n, A w) {
    A a, z;
    RZ(a = jtsc(jt, n));
    return IRS2(a, w, 0, RMAX, 1L, jttake, z);
}  // n {."1 w
A
jtdropr(J jt, I n, A w) {
    A a, z;
    RZ(a = jtsc(jt, n));
    return IRS2(a, w, 0, RMAX, 1L, jtdrop, z);
}  // n }."1 w
...
jtcharmap(J jt, A w, A x, A y) {
    ...
    memset(bb, C0, 256);
    if (n < AN(y))
        memset(zz, *(n + UAV(y)), 256);  // bb is array telling which input chars are in x; zz is result char to map for
                                         // given input byte.  If not exact mapping, init z to the 'not found' char
    DQ(
      n, c = *--u; zz[c] = *--v; k -= (I)bb[c] ^ 1;
      bb[c] =
        1;);  // mark characters in x, and count down to see if we hit all 256.  Note earliest mapped character for each
    GATV(z, LIT, wn, AR(w), AS(w));
    v = UAV(z);
    u = UAV(w);
    if (((k - 1) & (n - AN(y))) >= 0)
