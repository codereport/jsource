...
static B jtopes1(J jt,B**zb,A*za,A*ze,I*zm,A cs,A w){A a,e=0,q,*wv,x;B*b;I i,k,m=0,n,*v,wcr;P*p;
 ...
 for(i=0;i<n;++i)
  if(q=wv[i],SPARSE&AT(q)){
   ...
   if(!e)e=SPA(p,e); else ASSERT(jtequ(jt,e,SPA(p,e)),EVSPARSE);
...
static A jtopes(J jt,I zt,A cs,A w){A a,d,e,sh,t,*wv,x,x1,y,y1,z;B*b;C*xv;I an,*av,c,dk,dt,*dv,i,j,k,m,m1,n,
 ...
 RE(i=jtmult(jt,m,xc)); GA(x,dt, i,1+c,s); xv=CAV(x); mvc(m*xk,xv,dk,AV(e));
 RE(i=jtmult(jt,m,yc)); GATV0(y,INT,i,2L); v=AS(y); *v=m; v[1]=yc; yv=AV(y); memset(yv,C0,SZI*i);
 for(i=p=0;i<n;++i){
