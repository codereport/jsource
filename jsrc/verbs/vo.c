...
static C *copyresultcell(J jt, C *z, C *w, I *sizes, I rf, I *s){I wadv;I r=rf>>1;
 ...
 if(r==0){
  ...
  wadv=sizes[0];
  if(rf&1){DO(wadv>>LGSZI, A a=((A*)w)[i]; ra(a); ((A*)z)[i]=a;)}else{
      memcpy(z,w,wadv);}
  return wadv+w;
 }
...
static A jtopes(J jt,I zt,A cs,A w){A a,d,e,sh,t,*wv,x,x1,y,y1,z;B*b;C*xv;I an,*av,c,dk,dt,*dv,i,j,k,m,m1,n,
 ...
 for(i=p=0;i<n;++i){
  ...
  if(memcmpne(1+AS(x1),1+s,SZI*c)){*s=m1; povtake(jt,sh,x1,xv);} else
...
// ; y
 ...
 if(!(AFLAG(w)&AFUNIFORMITEMS)) {  // normal case
  ...
  for(i=0;i<n;++i){
   y=v[i]; if(AN(y)){if(TYPESNE(t,AT(y)))RZ(y=jtcvt(jt,t,y)); d=AN(y)<<klg;
       memcpy(zu,AV(y),d); zu+=d;}
  }
 }else{
