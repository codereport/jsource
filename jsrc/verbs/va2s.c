...
static A
jtvaspc(J jt, A a, A w, C id, VF ado, I cv, I t, I zt, I af, I acr, I wf, I wcr, I f, I r) {
    A q;
    I *as, *v, *ws;
    as = AS(a);
    ws = AS(w);
...
static A
jtvasp0(J jt, A a, A w, VF ado, I cv, I t, I zt) {
    A e, x, xx, y, z, ze, zx;
    B b;
    I n;
    P *p, *zp;
    ...
    GA(ze, zt, 1, 0, 0);
    I rc;
    ASSERT(EVOK == (rc = ((AHDR2FN *)ado)((I)1, (I)1, b ? AV(e) : AV(y), b ? AV(y) : AV(e), AV(ze), jt)), rc);
    GA(zx, zt, n, AR(x), AS(x));
    if (n)
        ASSERT(EVOK == (rc = ((AHDR2FN *)ado)(n ^ (b - 1), (I)1, b ? AV(x) : AV(y), b ? AV(y) : AV(x), AV(zx), jt)),
               rc);  // was !b
    if (cv & VRI + VRD) {
        RZ(ze = jtcvz(jt, cv, ze));
        RZ(zx = jtcvz(jt, cv, zx));
    }
    GASPARSE(z, STYPE(AT(zx)), 1, AR(xx), AS(xx));
    zp = PAV(z);
...
static B
jtvaspeqprep(J jt, A a, A w, I t, I f, I r, A *ae, A *ay, A *ax, A *we, A *wy, A *wx, A *za) {
    A aa, e, q, x, wa;
    B *b, sa, sw;
    I n, *v;
    P *p;
    ...
    q                   = jtifb(jt, n, b);
    makewritable(q) *za = q;  // avoid readonly result
    if (!sa || !jtequ(jt, q, aa)) RZ(a = jtreaxis(jt, q, a));
    if (!sw || !jtequ(jt, q, wa)) RZ(w = jtreaxis(jt, q, w));
    p   = PAV(a);
    *ae = e = SPA(p, e);
    *ay     = SPA(p, i);
    *ax = x = SPA(p, x);
...
#define FLUSH                                                                        \
    if (d) {                                                                         \
        c = d * yc;                                                                  \
        ICPY(zyv, u - c, c);                                                         \
        I lrc = ((AHDR2FN *)ado)((I)1, d * xc, axv - d * ak, wxv - d * wk, zxv, jt); \
        rc    = lrc < rc ? lrc : rc;                                                 \
        zxv += d * zk;                                                               \
        zyv += c;                                                                    \
        d = 0;                                                                       \
...
static A
jtvaspeq(J jt, A a, A w, C id, VF ado, I cv, I t, I zt, I f, I r) {
    ...
    while (m > i && n > j) {
        ...
        if (0 > c) {
            ...
            if (wb) {
                ICPY(zyv, u, yc);
                I lrc = ((AHDR2FN *)ado)(xc, (I)1, axv, wev, zxv, jt);
                rc    = lrc < rc ? lrc : rc;
                zxv += zk;
                zyv += yc;
            }
            ...
            if (ab) {
                ICPY(zyv, v, yc);
                I lrc = ((AHDR2FN *)ado)(~xc, (I)1, aev, wxv, zxv, jt);
                rc    = lrc < rc ? lrc : rc;
                zxv += zk;
                zyv += yc;
            }
    ...
    if (wb && m > i) {
        c = m - i;
        ICPY(zyv, u, c * yc);
        I lrc = ((AHDR2FN *)ado)(c * xc, (I)1, axv, wev, zxv, jt);
        rc    = lrc < rc ? lrc : rc;
    } else if (ab && n > j) {
        c = n - j;
        ICPY(zyv, v, c * yc);
        I lrc = ((AHDR2FN *)ado)(~(c * xc), (I)1, aev, wxv, zxv, jt);
        rc    = lrc < rc ? lrc : rc;
    }
    GA(ze, zt, 1, 0, 0);
    I lrc = ((AHDR2FN *)ado)((I)1, (I)1, aev, wev, AV(ze), jt);
    rc    = lrc < rc ? lrc : rc;
    ASSERT((rc & 255) == 0, rc);
    if (cv & VRI + VRD) {
        A e, x;
