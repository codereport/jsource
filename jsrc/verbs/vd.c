...
static A
jtnorm(J jt, A w) {
    return sqroot(jtpdt(jt, w, conjug(w)));
}
...
jtrinvip(J jt, A w, I n, I ncomp) {
    ...
    ai = jtrinvip(
      jt,
      jttake(jt, jtv2(jt, m, m), w),
      m,
      ncomp);  // take inverse of w00  kludge could use faux block to avoid take overhead esp for 2x2 FL results
    ...
    if (AT(w) & SPARSE) {
        z = jtover(jt, jtstitch(jt, ai, bx), jttake(jt, jtv2(jt, n - m, -n), di));  // should copy this over w, inplace
    } else {
        // copy in the pieces, line by line, writing over the input area
        I leftlen  = m << bplg(AT(w));
        I rightlen = (n - m) << bplg(AT(w));
        z          = w;
        void *zr   = voidAV(z);  // reuse input area, set pointer to output
        // copy top part: ai,.bx
        void *leftr = voidAV(ai), *rightr = voidAV(bx);  // input pointers
        DQ(m, memcpy(zr, leftr, leftlen); zr = (C *)zr + leftlen; leftr = (C *)leftr + leftlen;
           memcpy(zr, rightr, rightlen);
           zr     = (C *)zr + rightlen;
           rightr = (C *)rightr + rightlen;)
        // copy bottom part: 0,.di
        rightr = voidAV(di);
        DQ(n - m, memset(zr, C0, leftlen); zr = (C *)zr + leftlen; memcpy(zr, rightr, rightlen);
           zr     = (C *)zr + rightlen;
           rightr = (C *)rightr + rightlen;)
...
jtrinv(J jt, A w) {
    ...
    return jtrinvip(
      ...
      AT(w) & FL
        ? 2
        : 0);  // take the inverse.  Since it runs in place, clone w.  For float, reduce overhead at bottom of recursion
}
...
jtqrr(J jt, A w) {
    ...
    tv = AAV(t0);
    q0 = *tv++;
    r0 = *tv;                                         // point to Q and return of w0  pxm mxm  w0t
...
#define verifyinplace(to, from) \
    if (to != from) { memcpy(CAV(to), CAV(from), AN(to) << bplg(AT(to))); }
...
jtltqip(J jt, A w) {
    ...
    A q0;
    fauxblock(virtwq0);
    D *w0v = DAV(w);  // q0 & q1 data
    I rw   = AS(w)[0];
    I cl   = AS(w)[1];  // # rows, # columns
                        // handle case of 2 rows
    if (rw <= 2) {
        // If not FL length 2, handle any length 1
        if (rw <= 1) {  // just 1 row
            // Use a faux-virtual block to take the norm of w, so that we leave w inplaceable for when we normalize it
            // in place
            fauxvirtual(q0, virtwq0, w, 1, ACUC1);
            AS(q0)[0] = cl;
            ...
            RZ(t = jttable(jt, t));
            realizeifvirtual(t);
            return t;  // this is real, so it is also the adjoint of L
        }
    }
    // continue for blocks that must be subdivided
    I m   = rw >> 1;
    I tom = (0x01222100 >> ((rw & 7) << 2)) & 3;
    m     = (m + tom < rw) ? m + tom : m;  // Minimize number of wasted multiply slots, processing in batches of 4
    // construe w as w0 w1
    fauxvirtual(q0, virtwq0, w, 2, ACUC1 | ACINPLACE);
    AS(q0)[0] = m;
    AS(q0)[1] = cl;
    AN(q0)    = m * cl;
    ...
    A q1;
    fauxblock(virtwq1);
    fauxvirtual(q1, virtwq1, w, 2, ACUC1 | ACINPLACE);
    AK(q1) += (m * cl) << bplg(AT(w));
    AS(q1)[0] = rw - m;
    AS(q1)[1] = cl;
    AN(q1)    = (rw - m) * cl;
    // calculate w1 - (w1 q0*) q0
    {
        ...
        RZ(z = minusA(q1, jtpdt(jt, y, q0)));
        verifyinplace(q1, z);  // w1 - (w1 q0*) q0   n-mxmxp
    }
    RZ(l1 = jtltqip(jt, q1));  //  get QR of   w1 - (w1 q0*) q0
    // copy in the pieces, line by line
    I leftlen  = m << bplg(AT(w));
    I rightlen = (rw - m) << bplg(AT(w));
    GA(z, AT(w), rw * rw, 2, AS(w));
    AS(z)[1] = rw;
    void *zr = voidAV(z);  // allocate result, set pointer to output
    ...
    rightr = voidAV(l1);
    DQ(rw - m, memset(zr, C0, leftlen); zr = (C *)zr + leftlen; memcpy(zr, rightr, rightlen); zr = (C *)zr + rightlen;
       rightr = (C *)rightr + rightlen;)
...
jtqr(J jt, A w) {
    ...
    if (FL & AT(r)) {
        D *v = DAV(r);
        DQ(n, x = ABS(*v); if (x < c) c = x; if (x > d) d = x; v += n1;);
    } else {
        Z *v = ZAV(r);
        DQ(n, x = zmag(*v); if (x < c) c = x; if (x > d) d = x; v += n1;);
    }
...
jtlq(J jt, A w) {
    ...
    if (FL & AT(l)) {
        D *v     = DAV(l);
        D determ = jt->workareas.minv.determ;
        DQ(
          n, x = ABS(*v); if (determ != 0) {
              determ *= x;
              if (determ > 1e20) determ = 0.0;
          } if (x < c) c = x;
          if (x > d) d   = x;
          v += n1;);
        jt->workareas.minv.determ = determ;
    } else {
        Z *v = ZAV(l);
        DQ(n, x = zmag(*v); if (x < c) c = x; if (x > d) d = x; v += n1;);
    }
...
jticor(J jt, A w) {
    ...
    D recipd = 1 / d;
    v        = DAV(w);
    DO(AN(w), v[i] = jround(d * v[i]) * recipd;);  // force each value to multiple of recip of determinant, then divide
    return w;
}
...
jtminv(J jt, A w) {
    ...
    if (AN(w) && t & RAT + XNUM) {
        ...
        z = jtdrop(
          ...
          jtgausselm(
            jt,
            jtstitch(jt, y, jtreshape(jt, jtv2(jt, n, n), jttake(jt, jtsc(jt, 1 + n), jtxco1(jt, jtscf(jt, 1.0)))))));
        if (2 > wr)
            z = tymes(jtreshape(jt, mtv, z), w);
        else if (m > n)
            z = jtpdt(jt, z, q);
    } else {
        // not RAT/XNUM.  Calculate inverse as R^-1 Q^-1 after taking QR decomp & using Q^-1=Q*
        if (t & B01 + INT && 2 == wr && m == n)
            jt->workareas.minv.determ =
              1.0;  // if taking inverse of square int, allow setting up for correction afterward
        z = jtlq(jt, w);
