...
/* 21 !  */ {
 {{(VF)leBB,VCVTIP+VB+VIP            }, {(VF)binDD,VCVTIP+VD+VDD+VRI+VIP}, {(VF)binDD,VCVTIP+VD+VDD+VIP}, 
  ...
  {(VF)binZZ,VCVTIP+VZ+VZZ}, {(VF)0,VCVTIP+VX+VXX}, {(VF)binQQ,VCVTIP+VX+VQQ}, {0,0}}, 
  &rpsle},
...
static A jtva2(J jt,AD * RESTRICT a,AD * RESTRICT w,AD * RESTRICT self,UI allranks){  // allranks is argranks/ranks
 ...
 // We detect agreement error before domain error
 {
  if((allranks&RANK2TMSK)==0){ // rank 0 0 means no outer frames, sets up faster
   ...
   if(jtinplace!=0){  // If not sparse... This block isn't needed for sparse arguments, and may fail on them.
...
// this shows the fix   if(ICMP(as+af,ws+wf,MIN(acr,wcr))){if(1||zn)ASSERT(0,EVLENGTH)else r = 0;}
 ...
 if(jtinplace!=0){   // if not sparse...
  // Not sparse.
   ...
   if(((I)jtinplace&VARGMSK)!=0)if(zn>0){I t=atype((I)jtinplace);  // input conversion required (rare), and the result is not empty
...
//                                                 frame loc     shape of long frame             len of long frame           cellshape loc              cellshape     longer cellen 
  ...
  {I rc=EVOK;
   ...
   I i=mf; I jj=nf; lp000: {I lrc=((AHDR2FN*)aadocv->f)(n,m,av,wv,zv,jt); rc=lrc<rc?lrc:rc; if(--i!=0){ I jj1=--jj; jj=jj<0?nf:jj; av+=aawwzk[1+REPSGN(jj1)]; wv+=aawwzk[3+REPSGN(jj1)]; zv+=aawwzk[4]; goto lp000; }}  // jj1 is -1 on the last inner iter, where we use outer incr
   }
...
// +/@:*"1 with IRS
 ...
 if(((ar-acr)|(wr-wcr))==0){  // normal case
  ...
  I zn = ndpo*ndpi*nfro; RE(zn=jtmult(jt,zn,nfri));  // no error possible till we extend the shape
...
#define SETCONPTR2(n) A conptr=num(n); A conptr3=numvr(n); conptr=AT(w)&FL?conptr3:conptr;   // used for 2, when the only options are INT/FL
 ...
 A jtnot   (J jt, A w){ SETCONPTR(1) return AT(w)&B01+SB01?eq(num(0),w):minus(conptr,w);}
 A jtnegate(J jt, A w){if(!w) return 0; SETCONPTR(0) return minus(conptr,w);}
 A jtdecrem(J jt, A w){ SETCONPTR(1) IPSHIFTWA; return minus(w,conptr);}
 ...
 A jtsquare(J jt, A w){ return tymes(w,w);}   // leave inplaceable in w only  ?? never inplaces
 ...
 if(!jt->jerr){
  ...
  if(!((t=UNSAFE(at|wt))&(NOUN&~(B01|INT|FL)))){
   ...
   VA2 retva2;  retva2.cv=VB; // where we build the return value   cv indicates no input conversion, boolean result
    if(((UC)FAV(self)->id&~1)==CEQ){I opcode;  // CEQ or CNE
