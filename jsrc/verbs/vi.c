...
#define XMV(hash,exp,inc,stmt)      \
 ...
 else       XDO(hash,exp,inc,if(m==hj){ \
              memcpy(zc,v,k); zc+=k; stmt;});
...
#define TMV(FXY,FYY,expa,expw,prop)   \
 switch(4*bx+2*b+(I )(k==sizeof(D))){                                  \
  default: DO(c, FXY(expa,expw); if(prop){                                        \
                  memcpy(zc,v,k); zc+=k;}; v+=cn;);            break;  \
  case 1:  DO(c, FXY(expa,expw); if(prop)*zd++=*(D*)v;         ++v;  ); zc=(C*)zd; break;  \
  case 2:  DO(c, FYY(expa,expw); if(prop){                                        \
                  memcpy(zc,v,k); zc+=k;}; v+=cn;);            break;  \
...
static A jtiobs(J jt,I mode,I m,I n,I c,I k,I acr,I wcr,I ac,I wc,I ak,I wk,A a,A w,A*hp,A z){A*av,h=*hp,*wv,y;B b,bk,*yb,*zb;C*zc;I acn,*hu,*hv,l,m1,md,s,wcn,*zi,*zv;
 ...
 for(l=0;l<ac;++l,av+=acn,wv+=wcn,hv+=m){  // loop for each result in a
  ...
  if(b)switch(md){  // self-indexes
   ...
   case INUB:         BSLOOPAA(hi++,yb[p]=1,yb[q]=0,yb[q]=1  ); DO(m, if(yb[i]){
             memcpy(zc,av+i*n,k); zc+=k;}); ZCSHAPE; break;
   ...
   case ILESS:        BSLOOPAW(if(-2< q){
           memcpy(zc,u,k); zc+=k;}); ZCSHAPE;    break;
...
A jtindexofsub(J jt,I mode,A a,A w){PROLOG(0079);A h=0,hi=mtv,z;B mk=w==mark,th;fauxblockINT(zfaux,1,0);
 ...
 if(!(mk||m&&n&&zn&&(th>0))){
  ...
  case ILESS:   if(m)AN(z)=*AS(z)=0; else
          memcpy(AV(z),AV(w),k1*AN(w)); return z;
