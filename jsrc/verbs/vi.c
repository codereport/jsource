...
hicnz(I k, UC* v) {
    UI HASHINIT(z);
    UC c;
    DQ(k, c = *v++; if (c && c != 255) HASHSTEP(z, c););
    return z;
}
...
hic2(I k, UC* v) {
    UI HASHINIT(z);
    DQ(
      k >> 1, HASHSTEP(z, v[0]); if (*(v + 1)) { HASHSTEP(z, v[1]); } v += 2;);
    return z;
}
...
hic4(I k, UC* v) {
    UI HASHINIT(z);
    DQ(
      k >> 2, HASHSTEP(z, v[0]); if (*(v + 2) || *(v + 3)) {
          HASHSTEP(z, v[1]);
          HASHSTEP(z, v[2]);
          HASHSTEP(z, v[3]);
      } else if (*(v + 1)) { HASHSTEP(z, v[1]); } v += 4;);
    return z;
}

// Hash a single unsigned INT
#define hicw(v) (10495464745870458733U * *(UI*)(v))
// Hash a single double, using only the bits in ctmask.  -0 is hashed differently than +0.  Should we take the sign bit
// out of ct?  Only if ct=0?
//  not required for tolerant comparison, but if we tried to do tolerant comparison through the fast code it would help
static UI
jthid(J jt, D d) {
    return 10495464745870458733U * (jt->ctmask & *(I*)&d);
}
...
jthiau(J jt, A y) {
    ...
    switch (CTTZ(AT(y))) {
        ...
        case XNUMX:
            u = XAV(y);
            DQ(m, x = *u++; v = UAV(x); z += hicnz(AN(x) * SZI, UAV(x)););
            return z;
...
static B
jteqx(J jt, I n, X* u, X* v) {
    DQ(n, if (!jtequ(jt, *u, *v)) return 0; ++u; ++v;);
    return 1;
}
static B
jteqq(J jt, I n, Q* u, Q* v) {
    DQ(n, if (!QEQ(*u, *v)) return 0; ++u; ++v;);
    return 1;
}
static B
jteqd(J jt, I n, D* u, D* v) {
    DQ(n, if (!TEQ(*u, *v)) return 0; ++u; ++v;);
    return 1;
}
static B
jteqz(J jt, I n, Z* u, Z* v) {
    DQ(n, if (!jtzeq(jt, *u, *v)) return 0; ++u; ++v;);
    return 1;
}
...
#define IODECL(T)                                                                                                \
    ...
    C* zc;                                                                                                       \
    D t1 = 1.0;                                                                                                  \
    I t = sizeof(T), acn = ak / t, cn = k / t, hj, *hv = AV(h), j, l, p = AN(h), wcn = wk / t, *zi, *zv = AV(z); \
    T *av = (T*)AV(a), *v, *wv = (T*)AV(w);                                                                      \
    UI pm = p
...
// Routines to build the hash table from a.  hash calculates the hash function, usually referring to v (the input) and
// possibly other names.  exp is the comparison routine.  should use _1 for empty?
#define XDOA(hash, exp, inc)                                             \
    {                                                                    \
        v = av;                                                          \
        DO(m, j = (hash) % pm; FIND(exp); if (m == hj) hv[j] = i; inc;); \
    }
#define XDQA(hash, exp, dec)                                             \
    {                                                                    \
        v = av + cn * (m - 1);                                           \
        DQ(m, j = (hash) % pm; FIND(exp); if (m == hj) hv[j] = i; dec;); \
    }

// Routines to look up an item of w.  hash calculates the hash function, usually referring to v (the input) and possibly
// other names.  exp is the comparison routine.  stmt is executed after the hash lookup and must check whether *hj
// (->prev data) matches the new data in *v
#define XDO(hash, exp, inc, stmt)                       \
    {                                                   \
        v = wv;                                         \
        DO(cm, j = (hash) % pm; FIND(exp); stmt; inc;); \
    }
#define XDQ(hash, exp, dec, stmt)                       \
    {                                                   \
        v = wv + cn * (c - 1);                          \
        DQ(cm, j = (hash) % pm; FIND(exp); stmt; dec;); \
    }
// special lookup routines to move the data rather than store its index, used for nub/match
#define XMV(hash, exp, inc, stmt)        \
    if (k == SZI) {                      \
        XDO(                             \
          hash, exp, inc, if (m == hj) { \
              *zi++ = *(I*)v;            \
              stmt;                      \
          });                            \
        zc = (C*)zi;                     \
...
// *hh is the hash table we have selected, p is the number of hash entries we need, m is the maximum+1 value that needs
// to be stored in an entry md gives information about the type of entry, in particular if it is bits or packed bits
// Main result is in *hh, notably currentlo/currentindexofst.  Other fields have been adjusted to account for the
...
hashallo(IH* RESTRICT hh, UI p, UI m, I md) {
    ...
    if (md & IIMODBITS) {
        hh->currentlo        = 0;
        hh->currentindexofst = 0;  // Bits always start at 0
        ...
        // Round p up to multiple of I, then convert to hashtable entries. Must do this for endian reasons, to avoid a
        // hole in the invalid region
        p = (p + (SZI - 1)) & -SZI;  // round up to number of bytes that need to be cleared to clear Is
        // Clear the bits before returning.  We init packed bits to 0, but byte-bits to a value that depends on the
    ...
    // Not bits. We have a choice of allocating on the left or the right.  Allocating on the left creates cache
    // coherence but tends to waste address points.  Calculate the cost of each and choose.
    ...
    if (!(md & IIMODFORCE0)) {
        ...
        UI indexceil =
          maxindex -
          m;  // max value that can be accepted in the table already.  We will write from indexceil to maxindex
        ...
        // in the amount of (new left index end-right index end-future m)*(width-currenthi).  We estimate future values
        // to equal current ones. (3) now, if we have to clear the invalid area (1 store per word cleared) BUT: if the
        // cleared invalid area covers the entire p, we use m rather than left-index+m for calculating the other costs
        // The cost of a unit of index space is 1 store per (width*maxindex)/(m*p) index-space unit, with m and p
        // rounded up if large; we will approximate as 2*(m*p)/(width*maxindex)

        // Cost of allocating on the right is
        // (p*m) units of index space for the allocated area, plus (currenthi*((right index+m)-left index)) units of
        // index space lost on the left, plus cache expense of p<.currenthi cache words The cost of a cache word is
        // reckoned at 1/2 store per word

        // That's not worth figuring out for every call.  So, we will just allocate on the right as long as the request
        // fits vertically and horizontally, and there is not invalid region on the right, EXCEPT when m<4096 (about the
        // size of L1 cache) and there is no invalid region on the left: this last exception to keep the cache footprint
        // small for short requests
        if (p <= maxn - hh->currenthi && hh->previousindexend < indexceil && hh->invalidhi < hh->currenthi &&
            !(m < 4096 && hh->invalidlo >= p)) {  // the right side is a possibility
            ...
            hh->currentlo = hh->currenthi;
            hh->currenthi += p;  // set return value (starting position) and partition
            UI startx            = hh->previousindexend;
            hh->currentindexofst = startx;
            hh->previousindexend = (startx += m);  // set return value (starting index) and allocated index space
            hh->currentindexend  = MAX(
              startx,
              hh->currentindexend);  // since currentindexend speaks for the whole left side, we may have to push it up
            // Since the partition (currenthi) is kept to the right of the invalid region, there is no need to clear
            // invalid data
            ...
            hh->currentindexofst = startx;
            hh->currentindexend  = (startx += m);  // set return value (starting index) and allocated index space
            // If (part of) the return area needs to be cleared, clear it.  If we clear the entire left side, reduce the
            // left index
            ...
            if (nclrhsh > 0) {                  // if there is something to clear
                memset(hh->data.UC + (hh->invalidlo << hh->hashelelgsize),
                       C0,
    ...
    if (md & IINOTALLOCATED) {
        // Cannot allocate as is: the region must be initialized.  set BASE0 in this case, and since we initialize,
        // initialize to the most useful value
        md |= IIMODBASE0;  // if we clear the region, mention that so that we get the fastest code
        // Clear the entries of the first allocation to m.  Use fullword stores (should use cache-line stores).  Our
        // allocations are always multiples of fullwords, so it is safe to overfill with fullword stores
        UI storeval = m;
        if (hh->hashelelgsize == 1) storeval |= storeval << 16;
        if (SZI > 4) storeval |= storeval << 32;  // Pad store value to 64 bits, dropping excess on smaller machines
        ...
        // Clear everything past the first allocation to 0, indicating 'not touched yet'.  But we can elide this if it
        // is already 0, which we can tell by examining the partition pointer and the right-hand index.  This is
        // important if FORCE0 was set for i."r: we will repeatedly reset the base, and we need to avoid clearing the
        // whole buffer for any time after the first. We also don't want to clear the whole buffer if we were trying to
        // save a little time by setting BASE0 in an argument that uses only a little of the table. We have to make sure
        // we are not moving the partition to the left: that might expose some uninitialized values on the right side
        if (md & IIMODFORCE0) {
            // Putting that together: if FORCE0 is set, we just clear the left side and leave the right alone, except
            // that we may move the partition right.
            if (p >= hh->currenthi) {
                // Moving the partition right (or not at all).  That doesn't uncover anything
                hh->currenthi = p;  // set the new partition pointer
                hh->currentindexend =
                  MAX(hh->previousindexend, 1 + m);  // keep the left-side max index no less than the right-wide
            } else {
                // We didn't clear as far as the partition.  The left side still contains old data.
                if (hh->currentindexend < 1 + m)
                    hh->currentindexend = 1 + m;  // Don't lower the index if there is uncleared data on the left
            }
        } else {
            // Not FORCE0: we are clearing because we have to.  Clear everything.  But we can save clearing the right
            // side if it's already clear.
            I clrtopoint =
              (hh->previousindexend != 1) ? hh->datasize : hh->currenthi << hh->hashelelgsize;  // high+1 entry to clear
            I clrfrompoint = p << hh->hashelelgsize;                                            // offset to clear from
            if ((clrtopoint -= clrfrompoint) > 0) {
                memset(hh->data.UC + clrfrompoint, C0, clrtopoint);
            }                   // clear the region to 0
            hh->currenthi = p;  // set return value (starting position) and partition
            hh->currentindexend =
              1 + m;  // set return value (starting index) and allocated index space.  Leave 0 for 'not found'
        ...
        hh->invalidlo        = IMAX;
        hh->invalidhi        = 0;  // clear invalidity region
    }
    return md;
}
...
// should change hash so as not to require clearing it - consider leaving the table allocated permanently to reduce
// cache thrashing and save initialization. Could use subsets of it - power of 2??  Choose good size to reduce cache
// footprint of accessed area.  For prehashing, copy the hash to the
...
#define IOFX(T, f, hash, exp, inc, dec)                                                                            \
    A f(J jt, I mode, I m, I n, I c, I k, I acr, I wcr, I ac, I wc, I ak, I wk, A a, A w, A* hp, A z) {            \
        ...
        b     = a == w && ac == wc &&                                                                              \
            (mode == IIDOT || mode == IICO || mode == INUBSV || mode == INUB || mode == INUBI || mode == IFORKEY); \
        zb = (B*)zv;                                                                                               \
        zc = (C*)zv;                                                                                               \
        zi = zv;                                                                                                   \
        cm = w == mark ? 0 : c;                                                                                    \
        for (l = 0; l < ac; ++l, av += acn, wv += wcn) {                                                           \
            if (mode < IPHOFFSET) {                                                                                \
                DO(p, hv[i] = m;);                                                                                 \
                if (!b) {                                                                                          \
                    if (mode == IICO) XDQA(hash, exp, dec) else XDOA(hash, exp, inc);                              \
                }                                                                                                  \
            }                                                                                                      \
            switch (md) {                                                                                          \
                case IIDOT:                                                                                        \
                    if (b) {                                                                                       \
                        XDO(hash, exp, inc, *zv++ = m == hj ? (hv[j] = i) : hj);                                   \
                    } else                                                                                         \
                        XDO(hash, exp, inc, *zv++ = hj);                                                           \
                    break;                                                                                         \
                case IFORKEY: {                                                                                    \
                    I nuniq = 0;                                                                                   \
                    XDO(                                                                                           \
                ...
                case IICO:                                                                                         \
                    if (b) {                                                                                       \
                        zi = zv += c;                                                                              \
                        XDQ(hash, exp, dec, *--zi = m == hj ? (hv[j] = i) : hj);                                   \
                    } else                                                                                         \
                        XDO(hash, exp, inc, *zv++ = hj);                                                           \
                    break;                                                                                         \
                case INUBSV: XDO(hash, exp, inc, *zb++ = m == hj ? (hv[j] = i, 1) : 0); break;                     \
                case INUB:                                                                                         \
                    XMV(hash, exp, inc, hv[j] = i);                                                                \
                    ZCSHAPE;                                                                                       \
                    break;                                                                                         \
                case ILESS:                                                                                        \
                    XMV(hash, exp, inc, 0);                                                                        \
                    ...
                    break;                                                                                         \
                case INUBI:                                                                                        \
                    XDO(hash, exp, inc, if (m == hj)* zi++ = hv[j] = i);                                           \
                    ZISHAPE;                                                                                       \
                    break;                                                                                         \
...
// functions for building the hash table for tolerant comparison.  expa is the function for detecting matches on a
// values
...
// find a tolerant match for *v.  Check a threshold below and a threshold above, and set il and ir to the lower/upper
// buckets matched
...
// same idea, but also throw in an exact match on the value itself (rounded to a bucket value).  Used for reflexive
// searches, in which we have not initialized the hash table.  We first add an (exact) entry for the current value, and
// then search for tolerant matches We have to add an entry for the current value always, because a hashed value may be
// tolerantly equal to y but not tequal some other value tequal to y.  -0 will always get hashed as +0, and possibly -0
// as well
#define TFINDYY(expa, expw)                       \
    {                                             \
        ...
        MASK(dx, x);                              \
        j = jx = HID(dx) % pm;                    \
        PUSHCCT(1.0) FIND(expa);                  \
        POPCCT if (m == hj) hv[j] = i;            \
        MASK(dl, x* tl);                          \
        j = dl.d == dx.d ? jx : HID(dl) % pm;     \
        FIND(expw);                               \
        il = ir = hj;                             \
        MASK(dr, x* tr);                          \
        if (dr.d != dl.d) {                       \
            j = dr.d == dx.d ? jx : HID(dr) % pm; \
            FIND(expw);                           \
            ir = hj;                              \
        }                                         \
    }
...
#define TMV(FXY, FYY, expa, expw, prop)                                              \
    switch (4 * bx + 2 * b + (I)(k == sizeof(D))) {                                  \
        default:                                                                     \
            ...
            break;                                                                   \
        case 1:                                                                      \
            DO(c, FXY(expa, expw); if (prop)* zd++ = *(D*)v; ++v;);                  \
            zc = (C*)zd;                                                             \
            break;                                                                   \
        case 2:                                                                      \
            DO(                                                                      \
              c, FYY(expa, expw); if (prop) {                                        \
                  memcpy(zc, v, k);                                                  \
                  zc += k;                                                           \
              };                                                                     \
              v += cn;);                                                             \
...
#define IOFT(T, f, FA, FXY, FYY, expa, expw)                                                                       \
    A f(J jt, I mode, I m, I n, I c, I k, I acr, I wcr, I ac, I wc, I ak, I wk, A a, A w, A* hp, A z) {            \
        ...
        bx   = 1 && BOX & AT(a);                                                                                   \
        jx   = 0;                                                                                                  \
        dl.d = dr.d = dx.d = x = 0.0;                                                                              \
        for (l = 0; l < ac; ++l, av += acn, wv += wcn) {                                                           \
            if (mode < IPHOFFSET) {                                                                                \
                DO(p, hv[i] = m;);                                                                                 \
                if (bx || !b) {                                                                                    \
                    v = av;                                                                                        \
                    PUSHCCTIF(1.0, !(mode == INUBSV || mode == INUB || mode == INUBI || mode == IFORKEY))          \
                    if (IICO == mode) {                                                                            \
                        v += e;                                                                                    \
                        DQ(m, FA(expa); v -= cn;);                                                                 \
                    } else                                                                                         \
            ...
            switch (md) {                                                                                          \
                ...
                case IFORKEY: {                                                                                    \
                    I nuniq = 0;                                                                                   \
                    TDO(FXY, FYY, expw, expw, il = MIN(il, ir); nuniq += (il - i == 0); *zv = il; zv[il - i]++;    \
                        ++zv;);                                                                                    \
                    AM(h) = nuniq;                                                                                 \
                    break;                                                                                         \
                }                                                                                                  \
                case IICO:                                                                                         \
                    zv += c;                                                                                       \
                    TDQ(FXY, FYY, expa, expw, *--zv = m == il ? ir : m == ir ? il : MAX(il, ir));                  \
                    zv += c;                                                                                       \
                    break;                                                                                         \
                case INUBSV: TDO(FXY, FYY, expw, expw, *zb++ = i == MIN(il, ir)); break;                           \
                case INUB:                                                                                         \
                    TMV(FXY, FYY, expw, expw, i == MIN(il, ir));                                                   \
                    ZCSHAPE;                                                                                       \
                    break;                                                                                         \
                case ILESS:                                                                                        \
                    TMV(FXY, FYY, expa, expw, m == il && m == ir);                                                 \
                    ZCSHAPE;                                                                                       \
                    break;                                                                                         \
                case INUBI:                                                                                        \
                    TDO(FXY, FYY, expw, expw, if (i == MIN(il, ir))* zi++ = i;);                                   \
                    ZISHAPE;                                                                                       \
                    break;                                                                                         \
                ...
                case II0EPS:                                                                                       \
                    s = c;                                                                                         \
                    TDO(                                                                                           \
                      FXY, FYY, expa, expw, if (m == il && m == ir) {                                              \
                          s = i;                                                                                   \
                          break;                                                                                   \
                      });                                                                                          \
                    *zi++ = s;                                                                                     \
                ...
                case ISUMEPS:                                                                                      \
                    s = 0;                                                                                         \
                    TDO(FXY, FYY, expa, expw, if (m > il || m > ir)++ s);                                          \
                    *zi++ = s;                                                                                     \
                    break;                                                                                         \
                case IANYEPS:                                                                                      \
                    s = 0;                                                                                         \
                ...
                case IALLEPS:                                                                                      \
                    ...
                    *zb++ = 1 && s;                                                                                \
                    break;                                                                                         \
                case IIFBEPS:                                                                                      \
                    TDO(FXY, FYY, expa, expw, if (m > il || m > ir)* zi++ = i);                                    \
                    ZISHAPE;                                                                                       \
                    break;                                                                                         \
            }                                                                                                      \
        }                                                                                                          \
...
// *********************** new version **************************
// Macros to convert bit index to byte index and bit#.  We address to the byte to avoid rounding problems, as long as
// the compiler doesn't extend
#define BYTENO(b) ((b) >> 3)
#define BITNO(b) ((b)&7)
...
// This version for I values (which have a partial table); use the table only if the value is represented there.  vv is
// the not-found value.  Install it in default position Since the table is not FULL, the range scan of must have
...
#define SCOZ(T, Ttype, vv)                          \
    {                                               \
        ...
        while (zi != zie) {                         \
            ...
            if (v > max) hv = def;                  \
            I hvv;                                  \
            if ((hvv = (I)*hv - zi0) < 0) hvv = vv; \
            zv[zi] = (Ttype)hvv;                    \
            ++zi;                                   \
        }                                           \
    }
/* printf("v=%d, hv=0x%p, def=0x%p, zi0=%d, *hv=%d, hvv=%d\n",v,hv,def,zi0,*hv,hvv); */
// this version is used when the result vector is known to be full (out-of-range not possible).  Omit the out-of-bounds
...
#define SCOZF(T, Ttype, vv)                                  \
    {                                                        \
        ...
        while (zi != zie) {                                  \
            --zie;                                           \
            I hvv;                                           \
            if ((hvv = (I)hu[mwv[zie]] - zi0) < 0) hvv = vv; \
            zv[zie] = (Ttype)hvv;                            \
        }                                                    \
    }
// these versions are used if the table is known to start at offset 0.  Omit the not-found check, since m was already
// loaded; and zi is 0
...
// for e. -. u@e. - for each item of w, see if it is in the value table.  Set v to the value read from the table (1 if
...
#define SCOWP(T, bitdef, stmt)                       \
    {                                                \
        ...
        for (i = -c; i < 0; ++i) {                   \
            ...
            if (x < min) hv = def;                   \
            if (x > max) hv = def;                   \
            UC v = ((*hv >> BITNO(x)) & 1) ^ bitdef; \
            stmt                                     \
        }                                            \
    }
// packed full version
#define SCOWPF(T, bitdef, stmt)                        \
    {                                                  \
        I i;                                           \
        T* mwv = wv + c;                               \
        for (i = -c; i < 0; ++i) {                     \
...
#define SMCASE0(casename, text) \
    case casename: text break;

#define SMCASEF(casename, text) \
    case IIMODFULL + casename: text break;

#define SMCASEP(casename, text) \
    case IIMODPACK + casename: text break;

#define SMCASE1(casename, text) \
    case IIMODFULL + IIMODPACK + casename: text break;

#define SMFULLPACK(T, bitdef, casename, decl, action)     \
    SMCASE0(casename, {SDO(T, bitdef) decl SCOW action})  \
    SMCASEF(casename, {SDO(T, bitdef) decl SCOWF action}) \
    SMCASEP(casename, {SDOP(T) decl SCOWP action}) SMCASE1(casename, {SDOP(T) decl SCOWPF action})

#define SMFULLPACQ(T, bitdef, casename, decl, action)     \
    SMCASE0(casename, {SDO(T, bitdef) decl SCQW action})  \
    SMCASEF(casename, {SDO(T, bitdef) decl SCQWF action}) \
    SMCASEP(casename, {SDOP(T) decl SCQWP action}) SMCASE1(casename, {SDOP(T) decl SCQWPF action})

// EPS builds a full-size result and thus can scan FULLs in either order; we choose backwards
#define SMFULLPACKEPS(T, bitdef, casename, action)                \
    SMCASE0(casename, {SDO(T, bitdef) DCLZVO(B, 1) SCOW action})  \
    SMCASEF(casename, {SDO(T, bitdef) DCLZVQ(B, 1) SCQWF action}) \
    SMCASEP(casename, {SDOP(T) DCLZVO(B, 1) SCOWP action}) SMCASE1(casename, {SDOP(T) DCLZVQ(B, 1) SCQWPF action})
...
#define IOFSMALLRANGE(f, T, Ttype)                                                                                    \
    A f(J jt, I mode, I m, I n, I c, I k, I acr, I wcr, I ac, I wc, I ak, I wk, A a, A w, A* hp, A z) {               \
        ...
        for (l = 0; l < ac; ++l, av += m, wv += e) {                                                                  \
            if (!(mode & (IPHOFFSET | IPHCALC))) {                                                                    \
                mode = hashallo(hh, p, m, mode);                                                                      \
            } /* set parms for this loop - only if not prehashing or using prehashed table, which always start at     \
                 offset 0 */                                                                                          \
            switch (mode & (IIOPMSK | IIMODFULL | IIMODPACK |                                                         \
                            IIMODBASE0)) { /* We know the setting of IIMODBITS without looking */                     \
                default:                                                                                              \
                    ASSERTSYS(0, "switch failure in i.");                                                             \
                    /* a lot of the l*c in the cases below could be removed because the verbs lack IRS, but we're     \
                     * leaving them in for now */                                                                     \
                    /* reflexives, which include nubs, do not require a FULL version.  i.~ and i:~ don't need PACK    \
                     * versions either, since they deal in full indexes only */                                       \
                ...
                } break; /* scan sequentially; if prev value present, rewrite it, otherwise write index */            \
                ...
                case IIMODBASE0 + IIMODREFLEX + IICO: {                                                               \
                    ...
                    do {                                                                                              \
                        ...
                        --zie;                                                                                        \
                                                                                                                      ...
                                                                                                                      \
                    /* NUB types use Boolean masks but do not depend on FULL, since they cannot miss.  But they must  \
                     * support PACK. */                                                                               \
                ...
                case IIMODFULL + IIDOT: {                                                                             \
                    SDQA(T, Ttype);                                                                                   \
                    SCOZF(T, Ttype, m);                                                                               \
                } break;                                                                                              \
                case IIMODBASE0 + IIDOT: {                                                                            \
                    SDQA0(T, Ttype);                                                                                  \
                    SCOZ0(T, Ttype, m);                                                                               \
                } break;                                                                                              \
                case IIMODBASE0 + IIMODFULL + IIDOT: {                                                                \
                    SDQA0(T, Ttype);                                                                                  \
                    SCOZF0(T, Ttype, m);                                                                              \
                } break;                                                                                              \
                case IICO: {                                                                                          \
                ...
                case IIMODBASE0 + IICO: {                                                                             \
                    SDOA(T, Ttype);                                                                                   \
                    SCOZ0(T, Ttype, m);                                                                               \
                } break;                                                                                              \
                case IIMODBASE0 + IIMODFULL + IICO: {                                                                 \
                    SDOA(T, Ttype);                                                                                   \
                    SCOZF0(T, Ttype, m);                                                                              \
                } break;                                                                                              \
                    /* Boolean indexes from here on.  These must support FULL and PACK */                             \
                    SMFULLPACKEPS(                                                                                    \
                      T, 0, IEPS, (T, 0, zv[i] = v;)) /* EPS scans FULL args backwards for cache coherence */         \
                    SMFULLPACK(T, 1, ILESS, DCLZVO(T, 0) T* zv0 = zv;, (T, 1, *zv = mwv[i]; zv += v;);                \
                               *AS(z) = zv - zv0;                                                                     \
                               AN(z)  = n * (zv - zv0);)                                                              \
                    SMFULLPACK(T, 0, II0EPS, DCLZVO(I, 0) I s = c;,                                                   \
                                                                  (                                                   \
                                                                    T,                                                \
                                                                    0,                                                \
                                                                    if (!v) {                                         \
                               ...
                               *zv++ = s;)                                                                            \
                    SMFULLPACK(T, 0, II1EPS, DCLZVO(I, 0) I s = c;,                                                   \
                                                                  (                                                   \
                                                                    T,                                                \
                                                                    0,                                                \
                                                                    if (v) {                                          \
                               ...
                               *zv++ = s;)                                                                            \
                    SMFULLPACQ(T, 0, IJ0EPS, DCLZVQ(I, 0) I s = c;,                                                   \
                                                                  (                                                   \
                                                                    T,                                                \
                                                                    0,                                                \
                                                                    if (!v) {                                         \
                               ...
                               *zv++ = s;)                                                                            \
                    SMFULLPACQ(T, 0, IJ1EPS, DCLZVQ(I, 0) I s = c;,                                                   \
                                                                  (                                                   \
                                                                    T,                                                \
                                                                    0,                                                \
                                                                    if (v) {                                          \
                               ...
                               *zv++ = s;)                                                                            \
                    SMFULLPACK(T, 0, IANYEPS, DCLZVO(B, 0) B s = 0;,                                                  \
                                                                   (                                                  \
                                                                     T,                                               \
                                                                     0,                                               \
                                                                     if (v) {                                         \
                               ...
                               *zv++ = s;)                                                                            \
                    SMFULLPACK(T, 0, IALLEPS, DCLZVO(B, 0) B s = 1;,                                                  \
                                                                   (                                                  \
                                                                     T,                                               \
                                                                     0,                                               \
                                                                     if (!v) {                                        \
                    ...
                    SMFULLPACK(T, 0, IIFBEPS, DCLZVO(I, 0) I* zv0 = zv;, (T, 0, *zv = i + c; zv += v;);               \
                               *AS(z) = AN(z) = zv - zv0;)                                                            \
            }                                                                                                         \
        }                                                                                                             \
        return z;                                                                                                     \
  ...
  static IOFSMALLRANGE(jtio12, UC, US) static IOFSMALLRANGE(jtio14,
                                                            UC,
                                                            UI4)  // 1-byte items, using small/large hashtable
  static IOFSMALLRANGE(jtio22, US, US) static IOFSMALLRANGE(jtio24,
                                                            US,
                                                            UI4)            // 2-byte items, using small/large hashtable
...
#define SCDO(T, xe, exp)                                                                                         \
    {                                                                                                            \
        ...
        switch (mode) {                                                                                          \
            case IIDOT: {                                                                                        \
                T* av = (T*)u + m;                                                                               \
                DQ(ac, DQ(c, x = (xe); j = -m; while (j < 0 && (exp))++ j; *zv++ = j + m; wv += q;); av += p;    \
                   if (1 == wc) wv = v0;);                                                                       \
            } break;                                                                                             \
            case IFORKEY: {                                                                                      \
                T* av   = (T*)u + m;                                                                             \
                I nuniq = 0;                                                                                     \
                DQ(ac,                                                                                           \
                   DO(c, x = (xe); j = -m; while (j < 0 && (exp))++ j; *zv = j += m; nuniq += (j - i) == 0;      \
                      zv[j - i]++;                                                                               \
                      zv++;                                                                                      \
                      wv += q;);                                                                                 \
                   AM(z) = nuniq;                                                                                \
            ...
            case IICO: {                                                                                         \
                T* av = (T*)u;                                                                                   \
                DQ(ac, DQ(c, x = (xe); j = m - 1; while (0 <= j && (exp))-- j; *zv++ = 0 > j ? m : j; wv += q;); \
                   av += p;                                                                                      \
                   if (1 == wc) wv = v0;);                                                                       \
            } break;                                                                                             \
            case IEPS: {                                                                                         \
                T* av = (T*)u + m;                                                                               \
                DQ(ac, DQ(c, x = (xe); j = -m; while (j < 0 && (exp))++ j; *zb++ = j < 0; wv += q;); av += p;    \
                   if (1 == wc) wv = v0;);                                                                       \
            } break;                                                                                             \
        }                                                                                                        \
    }
...
// should scrap this and do a recursive hash on the entire box, and use the normal hash code.  would require managing -0
// in the hash for float/complex

// return 1 if a is boxed, and ct==0, and a contains a box whose contents are boxed, or complex, or numeric with more
// than one atom
static B
jtusebs(J jt, A a, I ac, I m) {
    A *av, x;
    I t;
...
//  for duplicates as matches are found, and mark when a check has been made.  Duplicate removal might be reasonable for
//  prehashing
static A
jtnodupgrade(J jt, A a, I acr, I ac, I acn, I ad, I n, I m, B b, B bk) {
    A *av, h, *u, *v;
    I *hi, *hu, *hv, l, m1, q;
    RZ(IRS1(a, 0L, acr, jtgrade1, h));
    hv = AV(h) + bk * (m - 1);
    av = AAV(a);
    // if not self-index, close up the duplicates
    if (!b)
        for (l = 0; l < ac; ++l, av += acn, hv += m) {  // for each item of the overall result
            ...
            // if the list was shortened, replace the last position with 1-(length of shortened list).  This will be
            // detected and the sign changed to give (length of list)-1.  0 is OK too, indicating a 1-element list
            if (bk) {
                hu = --hi;
                DQ(
...
// should not bother testing for equality if ascending order and q<p (since grade would preserve order if equal), or if
...
q is input element# that will have result index i, v->it; if *u=*v, v is a duplicate: map the result back to u \
(index=p) if *u!=*p, advance u/p to v/q and use q as the result index */                                                \
    }
...
//
static A
jtiobs(J jt, I mode, I m, I n, I c, I k, I acr, I wcr, I ac, I wc, I ak, I wk, A a, A w, A* hp, A z) {
    A *av, h = *hp, *wv, y;
    B b, bk, *yb, *zb;
    C* zc;
    I acn, *hu, *hv, l, m1, md, s, wcn, *zi, *zv;
    bk = mode == IICO || mode == IJ0EPS || mode == IJ1EPS || mode == IPHICO || mode == IPHJ0EPS || mode == IPHJ1EPS;
    b  = a == w && ac == wc &&
        (mode == IIDOT || mode == IICO || mode == INUB || mode == INUBSV || mode == INUBI || mode == IFORKEY);
    ...
    for (l = 0; l < ac; ++l, av += acn, wv += wcn, hv += m) {  // loop for each result in a
        // m1=index of last item, which may be less than m if there were discarded duplicates (signaled by last index
        // <0)
        s  = hv[bk ? 1 - m : m - 1];
        m1 = 0 > s ? -s : m - 1;
        hu = hv - m1 * bk;
        if (b) switch (md) {  // self-indexes
                ...
                case IFORKEY: {
                    I nuniq = 0;
                    BSLOOPAA(hi++, ++nuniq; zv[p] = p + 1, zv[q] = p; zv[p]++, ++nuniq; zv[q] = (p = q) + 1);
                    zv += m;
                    AM(h) = nuniq;
                    break;
                }
            ...
            switch (md) {  // searches, by binary search
...
jtutype(J jt, A w, I c) {
    ...
    DO(c, t = 0; DO(
         m, x = wv[i]; if (AN(x)) {
             if (t) RZ(TYPESEQ(t, AT(x))) else {
                     t = AT(x);
                     if (t & FL + CMPX + BOX) return 0;
                 }
         }););
    return t;
} /* return type if opened atoms of cells of w has uniform type (but not one that may contain -0), else 0. c is # of
     cells */
...
// We keep this version, using CMOV, because on Ivy Bridge CMOVs execute at one per clock (latency 2) and thus cannot
// update a single min/max every 2 cycles - and there is no other way to update faster than one per clock.  On
// Broadwell, CMOVs execute 2 per clock with latency 1 and thus just can update both min and max each clock with 2
// copies.  This is faster than taken branches which retire only 0.5 branches per clock
#define ASSESSBLOCKSIZE 64  // every so many inputs, check for range too large
CR
condrange(I* s, I n, I min, I max, I maxrange) {
    CR ret;
    I i, min0, min1, max0, max1;
    I x;
    // Unroll loop once to keep the compares rolling
    if (!n) goto fail;
    min0 = min1 = min;
    max0 = max1 = max;              // initial values
    if (n & 1) min1 = max1 = *s++;  // if odd number of words, take first word to even it up
    if (n >>= 1) {                  // n=#pairs of words left
        --n;
        i = n & (ASSESSBLOCKSIZE / 2 - 1);
        n >>= 5;  // do a block of compares to get on boundary; n=#64-words blocks left, i=size-1 of this block
        do {      // We keep this short loop separate because it always finishes with a misprediction.
            x    = s[0];
            max0 = (x > max0) ? x : max0;
            min0 = (x < min0) ? x : min0;  // this generates CMOVcc in VS
            x    = s[1];
            max1 = (x > max1) ? x : max1;
            min1 = (x < min1) ? x : min1;
        ...
        while (n--) {  // Do the remaining 64-word blocks
            ...
            max0 = (max1 > max0) ? max1 : max0;
            min0 = (min1 < min0) ? min1 : min0;
            if ((UI)(max0 - min0) >= (UI)maxrange) goto fail;
    ...
    if ((UI)(max0 - min0) >= (UI)maxrange) goto fail;
    ret.min   = min0;
    ret.range = max0 - min0 + 1;  // because the tests succeed, this will give the proper range
    return ret;
fail:
    ret.min = ret.range = 0;
    return ret;
...
CR
condrange4(C4* s, I n, I min, I max, I maxrange) {
    CR ret;
    I i;
    C4 min0, min1, max0, max1;
    C4 x;
    // Unroll loop once to keep the compares rolling
    if (!n) goto fail;
    min0 = min1 = (C4)min;
    max0 = max1 = (C4)max;          // initial values
    if (n & 1) min1 = max1 = *s++;  // if odd number of words, take first word to even it up
    if (n >>= 1) {                  // n=#pairs of words left
        --n;
        i = n & (ASSESSBLOCKSIZE / 2 - 1);
    ...
    if ((UI)(max0 - min0) >= (UI)maxrange) goto fail;
    ret.min   = min0;
    ret.range = (I)((UI)(max0 - min0) + 1);  // because the tests succeed, this will give the proper range
    return ret;
fail:
    ret.min = ret.range = 0;
    return ret;
...
// Same for US types
CR
condrange2(US* s, I n, I min, I max, I maxrange) {
    CR ret;
    I i;
    US min0, min1, max0, max1;
    US x;
    // Unroll loop once to keep the compares rolling
    if (!n) goto fail;
    min0 = min1 = (US)min;
    max0 = max1 = (US)max;          // initial values
    if (n & 1) min1 = max1 = *s++;  // if odd number of words, take first word to even it up
    if (n >>= 1) {                  // n=#pairs of words left
        --n;
        i = n & 31;
...
// This is the routine that analyzes the input, allocates result area and hashtable, and vectors to the correct action
// routine
...
A
jtindexofsub(J jt, I mode, A a, A w) {
    PROLOG(0079);
    A h = 0, hi = mtv, z;
    B mk = w == mark, th;
    fauxblockINT(zfaux, 1, 0);
    I ac, acr, af, ak, an, ar, *as, at, datamin, f, f1, k, k1, n, r, *s, t, wc, wcr, wf, wk, wn, wr, *ws, wt, zn;
    UI c, m, p;
    if (!(a && w)) return 0;
    ...
    wcr = wr < wcr ? wr : wcr;
    wf  = wr - wcr;
    RESETRANK;  // note: mark is an atom
    as = AS(a);
    at = AT(a);
    an = AN(a);
    ws = AS(w);
    ...
    if (mk) {
        ...
        r  = acr - 1;
        f1 = wcr - r;
    }       // if w is omitted (for prehashing), use info from a
        ...
        if (0 > f1 || ICMP(as + af + 1, ws + wf + f1, r)) {
            ...
            if ((wf - af) > 0 && af) {
                f1 += wf - af;
                wf = af;
            }  // see below for discussion about long frame in w
            I witems =
              wr > r
                ? ws[0]
                : 1;  // # items of w, in case we are doing i.&0 eg on result of e., which will have that many items
            m  = acr ? as[af] : 1;
            f0 = MAX(0, f1);
            RE(zn = mult(jtprod(jt, f, s), jtprod(jt, f0, ws + wf)));
            switch (mode) {
                case IIDOT:
                case IICO:
                    GATV(z, INT, zn, f + f0, s);
                    if (af) MCISH(f + AS(z), ws + wf, f0);
                    v = AV(z);
                    DQ(zn, *v++ = m;);
                    return z;
                case IEPS:
                    GATV(z, B01, zn, f + f0, s);
                    if (af) MCISH(f + AS(z), ws + wf, f0);
                    memset(BAV(z), C0, zn);
                    return z;
                case ILESS: return w;
                ...
                case IJ0EPS: return jtsc(jt, witems - 1);
                case INUBSV:
                case INUB:
                case INUBI: ASSERTSYS(0, "indexofsub");  // impossible
            }
        }
    }
    // f is len of frame of a (or of w if a has no frame); s->shape of a (or of w is a has no frame)
    // r is rank of an item of a cell of a (i. e. rank of a target item), f1 is len of frame of A CELL OF w with respect
    // to target cells, in other words the frame of the results each cell of w will produce
    ...
    k1 = bpnoun(t);  // m=length of target axis; the common type; k1=#bytes/atom of common type
    // Now that we have audited the shape of the cells of a/w to make sure they have commensurate items, we need to
    // revise the frame of w if it has the longer frame.  This can happen only where IRS is supported, namely ~: i. i:
    // e. . For those verbs, we get the effect of repeating a cell of a by having a macrocell of w, which is then broken
    // into target-cell sizes. We do this only if af!=0, because we have already set up to repeat cells of a if af=0
    if ((wf - af) > 0 && af) {
        f1 += wf - af;
        wf = af;
    }
    if (an && wn) {
        ...
        PROD(n, acr - 1, as + af + 1);
        k = n * k1;  // n=number of atoms in a target item; k=number of bytes in a target item
        PROD(ac, af, as);
        PROD(wc, wf, ws);
        PROD(c, f1, ws + wf);  // ?c=#cells in a & w;  c=#target items (and therefore #result values) in a result-cell
        RE(zn = mult(af ? ac : wc, c));  // #results is results/cell * number of cells; number of cells comes from ac if
                                         // a has frame, otherwise w.  If both have frame, a's must be longer, use it
        ak = (acr ? as[af] * k : k) & REPSGN(1 - ac);
        wk = (c * k) & REPSGN(1 - wc);  // # bytes in a cell, but 0 if there are 0 or 1 cells
        if (!af)
            c = zn;  // if af=0, wc may be >1 if there is w-frame.  In that case, #result/a-cell must include the #
                     // w-cells.  This has been included in zn
    } else {
        // An argument is empty.  We must beware of overflow in counting cells.  Just do it the old slow way
        n = acr ? jtprod(jt, acr - 1, as + af + 1) : 1;
        RE(zn = mult(jtprod(jt, f, s), jtprod(jt, f1, ws + wf)));
        k  = n * k1;
        ac = jtprod(jt, af, as);
        ak = ac ? k1 * an / ac : 0;  // ac = #cells of a
        wc = jtprod(jt, wf, ws);
        wk = wc ? k1 * wn / wc : 0;
        c  = 1 < ac ? wk / k : zn;
        wk *= 1 < wc;
    ...
    th      = HOMO(at, wt);
    jt->min = 0;  // are args compatible? clear return values from irange
    ...
    if (mk) {
        fauxINT(z, zfaux, 1, 0)
    }  // if prehashed, we must create an area that can hold at least one stored result
    else
        switch (mode & IIOPMSK) {
            I q;
            case IIDOT:
            case IFORKEY:
            case IICO:
                GATV(z, INT, zn, f + f1, s);
                if (af) MCISH(f + AS(z), ws + wf, f1);
                break;
            case INUBSV:
                GATV(z, B01, zn, f + f1 + !acr, s);
                if (af) MCISH(f + AS(z), ws + wf, f1);
                if (!acr) *(AS(z) + AR(z) - 1) = 1;
                break;
            case INUB:
                q = m + 1;
                GA(z, t, mult(q, jtaii(jt, a)), MAX(1, wr), ws);
                *AS(z) = q;
                break;  // +1 because we speculatively overwrite.  Was MIN(m,p) but we don't have the range yet
            case ILESS: GA(z, t, AN(w), MAX(1, wr), ws); break;
            case IEPS:
                GATV(z, B01, zn, f + f1, s);
                if (af) MCISH(f + AS(z), ws + wf, f1);
                break;
            case INUBI:
                q = m + 1;
            ...
            case IJ0EPS:
            case IJ1EPS:
                if (wr > MAX(ar, 1)) return jtsc(jt, wr > r ? ws[0] : 1);
                GAT0(z, INT, 1, 0);
                break;
            // ([: I. e.) ([: +/ e.) ([: +./ e.) ([: *./ e.) work only if e. produces rank 0 or 1.  Nonce error
            // otherwise
            case IIFBEPS:
                ASSERT(wr <= MAX(ar, 1), EVNONCE);
                GATV0(z, INT, c + 1, 1);
                break;  // +1 because we speculatively overwrite
            case IANYEPS:
            case IALLEPS:
                ASSERT(wr <= MAX(ar, 1), EVNONCE);
                GAT0(z, B01, 1, 0);
    ...
    if (!(mk || m && n && zn && (th > 0))) {
        ...
        switch (mode & IIOPMSK) {
            ...
            // We also handle the case of i.&0@:e. when the rank of w is more than 1 greater than the rank of a cell of
            // a; in that case the search always fails
            case IIDOT: return jtreshape(jt, shape(jt, z), jtsc(jt, n ? m : 0));
            case IFORKEY: {
                z = jtreshape(jt, shape(jt, z), jttake(jt, jtsc(jt, m), jtsc(jt, m)));
                RZ(z = jtmkwris(jt, z));
                AM(z) = !!m;
                return z;
            ...
            case II1EPS: return jtsc(jt, n && zn ? witems : 0L);
    ...
    // NOTE: from here on we may add modifiers to mode, indicating FULL/BITS/PACK etc.  These flags are needed in the
    // action routine, and must be preserved if the resulting hashtable is saved as part of a prehash.  They are not
    // valid on input to this routine.
    ...
    // m*number of results.  The cost of small-range hashing is at best 8 cycles per atom added to the table and 5
    // cycles per lookup. (full hashing is considerably more expensive)
    if (1 == acr && (1 == wc || ac == wc) && a != w && !mk && ((D)m * (D)zn < (4 * m) + 2.5 * (D)zn) &&
        (mode == IIDOT || mode == IICO || mode == IEPS ||
         (mode == IFORKEY && ((0x100000 & ((UI4*)&jt->cct)[1]) > ((at | wt) & (FLX | CMPX | BOX)))))) {
        ...
        UI booladj =
          (mode & (IIOPMSK & ~(IIDOT ^ IICO)))
            ? 5
            : 0;  // init table length not found; booladj = 5 if boolean hashvalue is OK, 0 if full index needed
        ...
        if (t & BOX)
            ...
            mode |= IIMODFULL;
            fn = jtio12;
        }  // 1-byte ops, just use small-range code: checking takes too much time
        else {
            // We might switch over to small-range mode, if the sizes are right.  See how big the hash table would be
            // for full hashing figure out whether we should use small-range matching or hashing.  We use small-range
            // code if: type is exact and length is 1 or 2 bytes; or type is exact numeric, length is 4 bytes or 8
            // bytes, and the (range of the data)*(length of rangecell) is less than 2.1*(length of data)*(length of
            // hashcell)
            ...
            // If the allocated range includes all the possible values for the input, set IIMODFULL to indicate that
            // fact
            if (2 == k) {
                // if the actual range of the data exceeds p, we revert to hashing.  All 2-byte types are exact
                CR crres = condrange2(USAV(a),
                                      (AN(a) * k1) / sizeof(US),
                                      -1,
                                      0,
                                      MIN((UI)(IMAX - 5) >> booladj, p) << booladj);  // get the range
                if (crres.range) {
                    datamin = crres.min;
                    // If the range is close to the max, we should consider widening the range to use the faster FULL
                    // code.  We do this only for boolean hashes, because in the current allocation going all the way to
                    // 65536 kicks us into the longer hashtable (questionable decision).  Otherwise we should just
                    // promote any non-Boolean, because the actual cache footprint won't change. The cost of promoting a
                    // Boolean is 1 store (1 clock) per word cleared, for (65536-range)>>booladj bytes (if booladj!=0)
                    // [or (65536-range) hashtable entries if booladj==0] The savings is 4 ops (2 clocks) per word
                    // searched
                    if (booladj && ((UI)(65536 - crres.range) >> booladj) < (c << (LGSZI + 1))) {
                        p       = 65536;
                        datamin = 0;
                    } else {
                        p = crres.range;
                    }  // this underestimates the benefit for prehashes
                    if (p == 65536) mode |= IIMODFULL;
                    fn = jtio22;  // This qualifies for small-range processing
                } else {
                    booladj = 0;
                }  // Turn off booladj if small-range processing not engaged
            }
            if (!fn) {  // if we don't have it yet, it will be a hash.  Decide which one
                if (cvtsneeded & 1)
                    RZ(a = jtcvt0(jt, a));  // Convert negative 0 to positive 0. Should do this in the hash
                if (cvtsneeded & 2) RZ(w = jtcvt0(jt, w));
                if (k == SZI && !(t & FL)) {  // non-float, might be INT or SBT
                    if (t & INT + SBT) {      // same here, for I types
                        CR crres = condrange(
                          AV(a), (AN(a) * k1) >> LGSZI, IMAX, IMIN, MIN((UI)(IMAX - 5) >> booladj, p) << booladj);
                        if (crres.range) {
                            datamin = crres.min;
                            p       = crres.range;
                            fn      = jtio42;
                            ...
                            booladj = 0;
                            fn      = jtioi;
                        }  // leave p as is; clear booladj since not small-range; select integer hashing
                    } else {
                        booladj = 0;
                        fn      = jtioi;
                    }
                } else {
                    fn = b || t & B01 + JCHAR + INT + SBT ? jtioc
                         : 1 == n                         ? (t & FL ? jtiod1 : jtioz1)
                         : t & FL                         ? jtiod
                                                          : jtioz;
                }  // select other hashing
            }
        }

        // if a hashtable will be needed, allocate it.  It is NOT initialized
        // the hashtable is INT unless we have selected small-range hashing AND we are not looking for the index with i.
        // or i:; then boolean is enough
        if (fn == jtio12 || fn == jtio22 || fn == jtio42) {
            IH* hh;
            if ((mode & IIOPMSK) == IFORKEY) {
                // We are processing on behalf of key, and we decided to do small-range processing.  Key can do better
                // by creating and processing the small-range table itself, so we will let it do that.  We return a
                // special short block (LSB=1) that indicates the length of the key (AN) and the start and range of the
                // keys (AK and AM)
                A z;
                GAT0(z, INT, 1, 0);
                AN(z) = k;
                AK(z) = datamin;
                AM(z) = p;             // allocate and return
                return (A)((I)z + 1);  // return the tagged address
            }
            // make sure we have a hashtable of the requisite size.  p has the number of entries, booladj indicates
            // whether they are 1 bit each. if the #entries fits in a US, use the short table.  But bits always use the
            // long table

            // See if the size/range of w allows use of one of the faster loops.  The options are FULL (which saves the
            // 4 instructions per atom of w that would be spent range-checking w) and BASE0 (which clears the hashtable,
            // at a cost of 1 cycle per 2/4 entries, or 4x that if we use fast instructions) First check FULL, which is
            // always the right decision if possible - except for self-classify which assumes FULL, or prehash which
            // doesn't go through w at all
            if (a != w && !mk && !(mode & IIMODFULL)) {
                CR crres;
                I allowrange;  // where we will build the max allowed range of w
                if (h = jt->idothash1) {
                    allowrange = IHAV(h)->datasize >> IHAV(h)->hashelelgsize;
                } else {
                    allowrange = 0;
                }  // current max capacity of large hash
                // always allow a little bit larger than the range of a, to make sure we expand the hashtable if a
                // little more would be enough. but never increase the range if that would exceed the L2 cache - just
                // pay the 4 instructions
                if (k == 2) {
                    allowrange = MIN(MAX(L2CACHESIZE / (LGSZUS), (I)p),
                                     MAX(allowrange, (I)(p + (p >> 3))));  // allowed range, with expansion
                    crres = condrange2(USAV(w), (AN(w) * bp(AT(w))) >> LGSZS, datamin, datamin + p - 1, allowrange);
                } else {
                    allowrange = MIN(MAX(L2CACHESIZE / (LGSZUI4), (I)p),
                                     MAX(allowrange, (I)(p + (p >> 3))));  // allowed range, with expansion
            ...
            if (p < (65536 - ((NORMAH * SZI + sizeof(IH)) / sizeof(US))) && booladj == 0 && m < 65536) {
                // using the short table.  Allocate it if it hasn't been allocated yet, or if this is prehashing, where
                // we will build a separate table. It would be nice to use the main table for m&i. to avoid having to
                // clear a custom table, since m&i. may never get assigned to a name; but if it IS assigned, the main
                // table may be too big, and we don't have any good way to trim it down If the sizes are such that we
                // should clear this table to save 3 clocks per atom of w, say so.  The clearing is done in hashallo
                // Clearing also saves 1 clock per input word
                mode |= ((c * 3 + m) < (p >> (LGSZI - LGSZUS - 1)))
                        << IIMODFORCE0X;  // 3 cycles per atom of w, 1 cycle per atom of m, versus 2/4 cycle per atom to
                                          // clear (without wide insts)
                if (mk || !(h = jt->idothash0)) {
                    ...
                    if (mk) {
                        // The table is being used for prehashing.  Clear the data area (only the part we will use), and
                        // also the values used as return values from hashallo, to wit the allocated position and index
                        mode |=
                          IIMODBASE0 |
                          IIMODFORCE0;  // we are surely initializing this table now, & it stays that way on every use
                        // It's OK to round the fill up to the length of an I
                        UI fillval = m | (m << 16);
                        if (SZI > 4) fillval |= fillval << 32;
                        I fillct = (p + (((((I)1) << (LGSZI - LGSZUS)) - 1))) >> (LGSZI - LGSZUS);
                        DO(fillct, hh->data.UI[i] = fillval;)
                        hh->currentlo        = 0;
                        hh->currentindexofst = 0;  // clear the parms.  Leave index 0 for not found
                    } else {
                        // not prehashing.  Fill in the remaining fields, remember this block for later use, and make
                        // its allocation permanent
                        jt->idothash0       = h;
                        hh->invalidlo       = IMAX;
                        hh->invalidhi       = 0;                              // none of this is ever used for bits
                ...
                // First, make a decision for Boolean tables.  If the table will be Boolean, decide whether to use
                // packed bits or bytes, and represent that information in mode and booladj.
                if (booladj) {
                    if (p > MAXBYTEBOOL) {
                        mode |= IIMODPACK | IIMODBITS;
                    } else {
                        mode |= IIMODBITS;
                        booladj = 5 - 3;
                    }  // set MODBITS as a flag to hashallo
                } else {
                    // If the sizes are such that we should clear this table to save 3 clocks per atom of w, say so. The
                    // clearing is done in hashallo.  Only for non-bits.
                    mode |= ((c * 3 + m) < (p << (1 - (LGSZI - LGSZUI4))))
                            << IIMODFORCE0X;  // 3 cycles per atom of w, 1 cycle per atom of m, versus 2/2 cycle per
                                              // atom to clear (without wide insts)
                }
                I psizeinbytes = ((p >> booladj) + 4) *
                                 sizeof(UI4);  // Get length of table in bytes.  We add 4 UI4s to the request:
                                               // for small-range to round up to an even word of an I, and possibly
                                               // padding leading/trailing bytes for packed bits; for hashing, we need a
                                               // sentinel at the beginning and the end The 4 UI4s give plenty of slack
                if (mk || !((h = jt->idothash1) && IHAV(h)->datasize >= psizeinbytes)) {
                    // if we have to reallocate, free the old one
                    if (!mk && h) {
                        fr(h);
                        jt->idothash1 = 0;
                    }  // free old, and clear pointer in case of allo error
                    ...
                    if (mk) {
                        ...
                        if (booladj) {  // convert bit count to words, rounded up
                            fillval = (mode & (IIMODPACK + IIOPMSK)) <= INUBI;
                            fillval |= fillval << 8;
                            fillval |= fillval
                                       << 16;  // fill packed bits with 0, byte-bits with 0 except for ~. ~: I.@~. -.
                        } else {               // convert UI4 count to words, rounded up
                            mode |= IIMODBASE0 | IIMODFORCE0;  // we are surely initializing this table now, & it stays
                                                               // that way on every use.  Only for non-Boolean
                            fillval = m;
                        }                                       // fill bits with 0; fill full hashes with m
                        if (SZI > 4) fillval |= fillval << 32;  // fill entire words
                        UI fillct =
                          (p + (((2LL << (LGSZI - LGSZUI4)) << booladj) - 1)) >>
                          (booladj + LGSZI - LGSZUI4);  // Round bits/UI4 up to SZI, then convert to count of Is.  We
                                                        // add 2 SZIs because we must pad packed bits on both ends
                        DO(fillct, hh->data.UI[i] = fillval;)
                        hh->currentlo        = 0;
                        hh->currentindexofst = 0;  // clear the parms.  This will never go through hashallo, so
                                                   // right-side and upper info not needed
                    } else {
                        // not prehashing.  Fill in the remaining fields, remember this block for later use, and make
                        // its allocation permanent
                        jt->idothash1       = h;
                        hh->invalidlo       = IMAX;
                        hh->invalidhi       = 0;  // none of this is invalid because it held bitmasks
            ...
            hh            = IHAV(h);
            hh->datamin   = datamin;
            hh->datarange = p;  // max will be inferred
        } else {
            if (fn != jtiobs) GATV0(h, INT, p, 1);
        }  // hash allocation old-style, now always INT
        ...
        if (fn == jtioc) {
            ...
            // Allocate bitmask (as a B01) for each byte in an item of rimatand, init to true.  This will indicate which
            // bytes need to be indexed should get rid of this - cheaper to hash than to check for need-to-hash
            GATV0(x, B01, k, 1);
            b = BAV(x);
            memset(b, C1, k);
            q = k;
            u = CAV(a);
            v = u + k;  // q = #bytes that have all identical values.  v point to current item, starting at second
            DO(ac * (m - 1),
               DO(
                 k,
                 if (u[i] != *v && b[i]) {
               ...
               if (!q) break;);  // Check for differing byte.  Exit loop if all different.   should reverse b[i] test
                                 // error - should be ac*m-1
            // Convert the mask of varying bytes into the list of indexes of varying bytes, and set a pointer to that
            // list for use in the indexing routine
            if (q) {
                jt->hin = k - q;
                GATV0(hi, INT, k - q, 1);
                jt->hiv = d = AV(hi);
        ...
        if (mk) {
            ...
            switch (mode & IIOPMSK) {
                ...
                case IANYEPS:
                case IALLEPS: ztype = PREHRESBAN; break; /* boolean scalar      */
                case ISUMEPS: ztype = PREHRESIAN; break;
                case II0EPS:
                case II1EPS:
                case IJ0EPS:
                case IJ1EPS: ztype = PREHRESIA; break;    /* integer scalar      */
    ...
    I forkeyresult = AM(z);
    RZ(z = EPILOGNORET(z));
    // Since EPILOG may have rewritten AM, and IFORKEY never returns to the parser, we can store the FORKEY result in
    // AM.
...
A
jtindexofprehashed(J jt, A a, A w, A hs) {
    A h, hi, *hv, x, z;
    AF fn;
    I ar, *as, at, c, f1, k, m, mode, n, r, t, *xv, wr, *ws, wt, ztype;
    ...
    switch (ztype) {
        // the N endings are types that do not produce correct results if the result of e. has rank >1.  We give nonce
    ...
    if (TYPESNE(t, wt)) RZ(w = jtcvt(jt, t, w)) else if (t & FL + CMPX) RZ(w = jtcvt0(jt, w));
...
jtless(J jt, A a, A w) {
    ...
    if (wr && r != wr) {
        ...
        *s    = jtprod(jt, k, ws);
        MCISH(1 + s, k + ws, r - 1);
    }  // bug: should test for error on the jtprod(jt,)
...
jtsclass(J jt, A w) {
    ...
    e = SPA(p, e);
    y = SPA(p, i);
    RZ(xy = jtstitch(jt, SPA(p, x), y));
    if (n > *AV(e)) RZ(xy = jtover(jt, xy, jtstitch(jt, e, jtless(jt, IX(n), y))));
    RZ(xy = jtgrade2(jt, xy, xy));
    v = AV(xy);
    c = *AS(xy);
    m = j = -1;
    DQ(
...
#define IOCOLFT(T, f, hasha, hashl, hashr, exp)                                  \
    void f(J jt, I m, I c, I d, A a, A w, A z, A h) {                            \
        IOCOLDECL(T);                                                            \
        for (l = 0; l < c; ++l) {                                                \
            DO(p, hv[i] = m;);                                                   \
            v = av;                                                              \
            DO(m, j = (hasha) % pm; FIND(exp); if (m == hj) hv[j] = i; v += c;); \
            v = wv;                                                              \
            u = zv;                                                              \
            for (i = 0; i < d; ++i) {                                            \
                x = *(D*)v;                                                      \
...
#define JOCOLFT(T, f, hasha, hashl, hashr, exp)                                  \
    void f(J jt, I m, I c, I d, A a, A w, A z, A h) {                            \
        ...
        for (l = 0; l < c; ++l) {                                                \
            DO(p, hv[i] = m;);                                                   \
            v = av + c * (m - 1);                                                \
            DQ(m, j = (hasha) % pm; FIND(exp); if (m == hj) hv[j] = i; v -= c;); \
            v = wv;                                                              \
            u = zv;                                                              \
            for (i = 0; i < d; ++i) {                                            \
                x = *(D*)v;                                                      \
...
static IOCOLFT(D, jtiocold, jthid(jt, *v), jthid(jt, tl* x), jthid(jt, tr* x), !TEQ(*v, av[c * hj])) static IOCOLFT(
  Z, jtiocolz, jthid(jt, *(D*)v), jthid(jt, tl* x), jthid(jt, tr* x), !jtzeq(jt, *v, av[c * hj]))

  static JOCOLFT(D, jtjocold, jthid(jt, *v), jthid(jt, tl* x), jthid(jt, tr* x), !TEQ(*v, av[c * hj])) static JOCOLFT(
    Z, jtjocolz, jthid(jt, *(D*)v), jthid(jt, tl* x), jthid(jt, tr* x), !jtzeq(jt, *v, av[c * hj]))
  ...
  A jtiocol(J jt, I mode, A a, A w) {
    ...
    ar = AR(a);
    m  = *AS(a);
    c  = jtaii(jt, a);  // a: at=type ar=rank m=#items c=#atoms in an item
    wt = AT(w);
    wr = AR(w);
    ws = AS(w);
    d  = 1;
