 ...
 *zs=*as+*ws; t=maxtype(at,wt);
 ...
 if(*zb){
  ...
  while(i<m||j<n){I cmp;
   ...
   switch(cmp){
    case -1: ICPY(yv,u,c); u+=c; ++i;
        memcpy(xv,   av,ak); av+=ak; break;
    case  0: ICPY(yv,u,c); u+=c; ++i;
        memcpy(xv,   av,ak); av+=ak; ++p;  /* fall thru */
    case  1: ICPY(yv,v,c); v+=c; ++j;
        memcpy(xv+ak,wv,wk); wv+=wk;
   }
   yv+=c; xv+=xk;
  }
