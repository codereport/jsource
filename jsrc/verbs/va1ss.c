...
A jtssingleton1(J jt, A w,A self){A z;
 ...
 case SSINGCASE(VA2CSTAR-VA2CMIN,SSINGENC(INT)): SSSTORENV((SSRDI(w)>0)-(SGNTO0(SSRDI(w))),z,INT,I) return z;
 ...
 case SSINGCASE(VA1CROOT-VA2CMIN,SSINGENC(INT)):
    ...
    jt->jerr=EWIMAG; return 0;   // otherwise fall through to normal code, returning complex
 case SSINGCASE(VA1CROOT-VA2CMIN,SSINGENC(FL)):
    if((wdv = SSRDD(w))>=0){SSSTORENVFL(sqrt(wdv),z,FL,D) return z;}   // return value if real
    jt->jerr=EWIMAG; return 0;   // otherwise fall through to normal code, returning complex


 case SSINGCASE(VA2CEXP-VA2CMIN,SSINGENC(B01)): SSSTORENV(SSRDB(w)?2.71828182845904523536:1.0,z,FL,D) return z;
 ...
 case SSINGCASE(VA2CSTILE-VA2CMIN,SSINGENC(INT)):
    wiv = SSRDI(w); wiv=(I)((UI)(wiv^REPSGN(wiv))-(UI)REPSGN(wiv)); if(wiv>=0){SSSTORENV(wiv,z,INT,I)}else SSSTORE(-(D)IMIN,z,FL,D) return z;
 case SSINGCASE(VA2CSTILE-VA2CMIN,SSINGENC(FL)):
    wdv = SSRDD(w); wdv=ABS(wdv); SSSTORENVFL(wdv,z,FL,D) return z;
 ...
 case SSINGCASE(VA2CCIRCLE-VA2CMIN,SSINGENC(INT)):
    SSSTORE(PI*(D)SSRDI(w),z,FL,D) return z;
 case SSINGCASE(VA2CCIRCLE-VA2CMIN,SSINGENC(FL)):
    SSSTORENVFL(PI*SSRDD(w),z,FL,D) return z;
 ...
 case SSINGCASE(VA2CPLUS-VA2CMIN,SSINGENC(FL)):
  return w;  // conjugating reals is a NOP
