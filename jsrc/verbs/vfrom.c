...
jtcatalog(J jt, A w) {
    ...
    A b, *wv, x, z, *zv;
    C *bu, *bv, **pv;
    I *cv, i, j, k, m = 1, n, p, *qv, r = 0, *s, t = 0, *u;
    ...
    DO(
      n, x = wv[i]; if (AN(x)) {
          ...
          RE(t = maxtype(t, p));
      });  // use vector maxtype; establish type of result
    t = t ? t : B01;
    k = bpnoun(t);  // if all empty, use boolean for result
    GA(b, t, n, 1, 0);
    bv = CAV(b);  // allocate place to build each item of result - one atom from each box.  bv->item 0
    GATV0(x, INT, n, 1);
    ...
    GATV0(z, BOX, m, r);
    zv = AAV(z);
    s  = AS(z);  // allocate result area
    // There is no need to turn off pristinity of w, because nothing was copied out by pointer (everything was copied &
    // then cloned) The result is certainly pristine if it is DIRECT
...
#define SETNDX(ndxvbl, ndxexp, limexp)                \
    {                                                 \
        ndxvbl = (ndxexp);                            \
        if ((UI)ndxvbl >= (UI)limexp) {               \
            ndxvbl += (limexp);                       \
            ASSERT((UI)ndxvbl < (UI)limexp, EVINDEX); \
        }                                             \
    }  // if ndxvbl>p, adding p can never make it OK
#define SETNDXRW(ndxvbl, ndxexp, limexp)              \
    {                                                 \
        ndxvbl = (ndxexp);                            \
        if ((UI)ndxvbl >= (UI)limexp) {               \
            (ndxexp) = ndxvbl += (limexp);            \
            ASSERT((UI)ndxvbl < (UI)limexp, EVINDEX); \
        }                                             \
    }  // this version write to input if the value was negative
...
#define IFROMLOOP(T)                                           \
    {                                                          \
        ...
        if (1 == an) {                                         \
            ...
            DQ(m, DO(an, SETJ(av[i]); *x++ = v[j];); v += p;); \
    }
...
jtifrom(J jt, A a, A w) {
    ...
    RESETRANK;
    if (ar > acr)
        return rank2ex(a, w, UNUSED_VALUE, acr, wcr, acr, wcr, jtifrom);  // split a into cells if needed.  Only 1 level
                                                                          // of rank loop is used
    // From here on, execution on a single cell of a (on matching cell(s) of w, or all w).  The cell of a may have any
    // rank
    an = AN(a);
    wn = AN(w);
    ws = AS(w);
    ...
    // items, where 0 { empty is an index error!  In that case, we set wr to 0, in effect making it an atom (since
    // failing exec on fill-cell produces atomic result)
    // if(an==0 && wn==0 && ws[wf]==0)wcr=wr=0;     defer this pending analysis
    // If w is empty, portions of its shape may overflow during calculations, and there is no data to move (we still
    // need to audit for index error, though).  If w is not empty, there is no need to check for such overflow.  So we
    // split the computation here. Either way, we need   zn: #atoms in result   p: #items in a cell of w
    p       = wcr ? ws[wf] : 1;
    ...
    if (wn) {
        ...
        if (((zn - 2) | (wr - 2) | -(wf | (wflag & (AFNJA)))) >=
            0) {  // zn>1 and not (frame or NJA) and rank>1.  Don't take the time creating a virtual block for a list.
                  // User should use ;.0 for that  NJAwhy
            // result is more than one atom and does not come from multiple cells.  Perhaps it should be virtual.  See
            // if the indexes are consecutive
            I index0 = av[0];
            index0 += REPSGN(index0) & p;  // index of first item
            // check the last item before checking the middle.
            I indexn = av[an - 1];
            indexn += REPSGN(indexn) & p;
            if (indexn == index0 + an - 1) {
                I indexp = index0;
                DO(
                  an - 1, indexn = av[1 + i]; indexn += REPSGN(indexn) & p; if (indexn != indexp + 1) {
                      indexn = p;
                      break;
                  } indexp = indexn;);
            } else
            ...
            if ((index0 | (p - indexn - 1)) >= 0) {  // index0>0 and indexn<=p-1
                ...
                AN(z) = zn;
                I *s  = AS(z);
                MCISH(s, as, ar) MCISH(s + ar, ws + 1, wr - 1) return z;
            }
        }
    ...
    GA(z, AT(w), zn, ar + wr - (I)(0 < wcr), 0);  // result-shape is frame of w followed by shape of a followed by shape
                                                  // of item of cell of w; start with w-shape, which gets the frame
    MCISH(AS(z), AS(w), wf);
    MCISH(&AS(z)[wf], AS(a), ar);
    ...
    switch (k) {
        ...
        default:
            ...
            // the end of any type that is not a word-multiple
            {
                ...
                if (1 == an) {
                    v += j * k;
                    DQ(m, u = v; JMCR(x, u, k + (SZI - 1), loop1, 0, endmask) x += k; v += pq;);
                } else
                    DQ(m, DO(an, SETJ(av[i]); u = v + j * k; JMCR(x, u, k + (SZI - 1), loop2, 0, endmask) x += k;);
                       v += pq;)
            }
            break;
    }
...
jtbfrom(J jt, A a, A w) {
    ...
    RESETRANK;
    if (ar > acr)
        return rank2ex(a, w, UNUSED_VALUE, acr, wcr, acr, wcr, jtbfrom);  // use rank loop if multiple cells of a
    an = AN(a);
    wn = AN(w);
    ws = AS(w);
    // If a is empty, it needs to simulate execution on a cell of fills.  But that might produce domain error, if w has
    ...
    if (wn) {
        // If there is data to move, we also need m: #cells of w   k: #bytes in an items of a cell of w   wk: #bytes in
        // a cell of w
        PROD(m, wf, ws);
    ...
    switch (k + k + (I)(1 == an)) {
        ...
        default:
            if (1 == an) {
                wv += k * *av;
                DQ(m, memcpy(zv, wv, k); zv += k; wv += wk;);
            }

            else
                DQ(m, b = av; DQ(an, memcpy(zv, wv + k * *b++, k); zv += k;); wv += wk;);
    }
    return z;
} /* a{"r w for boolean a */
...
A
jtfrombu(J jt, A a, A w, I wf) {
    FPREFIP;
    A p, q, z;
    I ar, *as, h, m, r, *u, *v, wcr, wr, *ws;
    ar  = AR(a);
    as  = AS(a);
    h   = as[ar - 1];  // h is length of the index list, i. e. number of axes of w that disappear during indexing
    wr  = AR(w);
    ws  = AS(w);
    wcr = wr - wf;
    if ((-AN(a) & -AN(w)) >= 0) {  // empty array, either a or w
        ...
        MCISH(AS(z), AS(w), wf)
        MCISH(AS(z) + wf, AS(a), ar - 1) MCISH(AS(z) + wf + ar - 1, AS(w) + wf + h, wcr - h) return z;
    }
    fauxblockINT(pfaux, 4, 1);
    fauxINT(p, pfaux, h, 1) v = AV(p) + h;
    u                         = ws + wf + h;
    m                         = 1;
    DQ(h, *--v = m; m *= *--u;);  // m is number of items in the block of axes that index into w
    ...
    if (r == wr) {
        ...
        MCISH(v, ws, wf);
        v[wf] = m;
        MCISH(v + wf + 1, ws + wf + h, wcr - h); /* q is jtreshape(jt,.,w) */
    }
...
// if the opened boxes have contents with the same item shape (treating atoms as same as singleton lists), create an
// array of all the indexes; put that into *ind and return 1. otherwise return 0
B
jtaindex(J jt, A a, A w, I wf, A *ind) {
    A *av, q, z;
    I an, ar, c, j, k, t, *u, *v, *ws;
    an   = AN(a);
    *ind = 0;
    if (!an) return 0;
    ws = wf + AS(w);
    ...
    av = AAV(a);
    q  = av[0];
    c  = AN(q);  // q=addr, c=length of first box
    if (!c) return 0;
    ASSERT(c <= AR(w) - wf, EVLENGTH);
    GATV(z, INT, an * c, 1 + ar, AS(a));
    AS(z)[ar] = c;
    v         = AV(z);  // allocate array for result
    for (j = 0; j < an; ++j) {
        q = av[j];
        t = AT(q);
...
static B
jtaindex1(J jt, A a, A w, I wf, A *ind) {
    A z;
    I c, i, k, n, t, *v, *ws;
    n    = AN(a);
    t    = AT(a);
    *ind = 0;
    if (AR(a) == 0) return 0;  // revert to normal code for atomic a
    ws = wf + AS(w);
    c  = AS(a)[AR(a) - 1];                                   // c = length of 1-cell
    if (((n - 1) | (c - 1) | SGNIF(t, BOXX)) < 0) return 0;  // revert to normal code for empty or boxed a
    ASSERT(c <= AR(w) - wf, EVLENGTH);
    PROD(n, AR(a) - 1, AS(a));
    v = AV(a);  // n now=number of 1-cells of a   v=running pointer through a
    ...
    if (t & INT) {  // if it's INT already, we don't need to move it.
        ...
        i = 1;  // if not INT to begin with, we must convert
    if (i == 0) {
        z = a;  // If all indexes OK, return the original block
    } else {
        // There was a negative index.  Allocate a new block for a and copy to it.
        RZ(z = t & INT ? jtca(jt, a) : jtcvt(jt, INT, a));
        v = AV(z);
...
jtafrom2(J jt, A p, A q, A w, I r) {
    ...
    if (AN(w)) {
        ...
        // e=length of axis corresponding to q  n=#_2-cells in a cell of w   m=#cells of w (frame*size of 2-cell*(#
        // _2-cells = pn*qn))
        PROD(m, wf, ws);
        PROD(d, r - 2, ws + wf + 2);
        e = ws[1 + wf];
        n = e * ws[wf];
        DPMULDE(qn, d * m, zn) DPMULDE(pn, zn, zn);
    } else {
        zn = 0;
    }
...
// general boxed a
static A
jtafrom(J jt, A a, A w) {
    PROLOG(0073);
    A c, ind, p = 0, q, *v, y = w;
    B bb = 1;
    I acr, ar, i = 0, j, m, n, pr, *s, t, wcr, wf, wr;
    ar  = AR(a);
    acr = jt->ranks >> RANKTX;
    acr = ar < acr ? ar : acr;
    wr  = AR(w);
    ...
    if (ar) {  // if there is an array of boxes
        ...
        return wr == wcr ? rank2ex(a, w, UNUSED_VALUE, 0L, wcr, 0L, wcr, jtafrom) :  // if a has frame, rank-loop over a
                 df2(p,
                     IRS1(a, 0L, acr, jtbox, c),
                     IRS1(w, 0L, wcr, jtbox, ind),
                     jtamp(jt, ds(CLBRACE), ds(COPE)));  // (<"0 a) {&> <"0 w
    }
    c = AAV(a)[0];
    t = AT(c);
    SETIC(c, n);
    v = AAV(c);  // B prob not reqd
    s = AS(w) + wr - wcr;
    ...
    if (wcr == wr) {
        for (i = m = pr = 0; i < n; ++i) {
            p = jtafi(jt, s[i], v[i]);
            if (!(p && (((AN(p) ^ 1) - 1) & -(AT(p) & NUMERIC)) < 0))
                break;  // if 1 selection from numeric axis, do selection here, by adding offset to selected cell
            pr += AR(p);
            RANKT rsav = AR(p);
            AR(p)      = 0;
            I px;
            PRODX(px, wcr - i - 1, 1 + i + s, jti0(jt, p)) m += px;
            AR(p) = rsav;  // kludge but easier than creating a fauxvirtual block
        }
    }
    if (i) {
        I *ys;
    ...
    // take remaining axes 2 at a time, properly handling omitted axes (ds(CACE)).  First time through p is set if there
    // has been no error
    for (; i < n; i += 2) {
        j = 1 + i;
        if (!p) p = jtafi(jt, s[i], v[i]);
    ...
    // We have to make sure that a virtual NJA block does not become the result, because x,y and x u}y allow modifying
    // those even when the usecount is 1.  Realize in that case
    RE(y);
    if (AFLAG(y) & AFNJA) { RZ(y = jtca(jt, y)); }
...
jtfrom(J jt, A a, A w) {
    ...
    at = AT(a);
    if (!((AT(a) | AT(w)) & (SPARSE))) {
        // if B01|INT|FL atom { INT|FL|BOX array, and no frame, just pluck the value.  If a is inplaceable and not
        // unincorpable, use it If we turn the result to BOX it will have the original flags, i. e. it will be
        // nonrecursive.  Thus fa will not free the contents, which do not have incremented usecount (and are garbage on
        // error)
        if (!((AT(a) & (NOUN & ~(B01 | INT | FL))) + (AT(w) & (NOUN & ~(INT | FL | BOX))) + AR(a) +
              (SGNTO0((((RANKT)jt->ranks - AR(w)) | (AR(w) - 1)))) + (AFLAG(w) & AFNJA))) {  // NJAwhy
            I av;                                                                            // selector value
            if (AT(a) & (B01 | INT)) {
                av = BIV0(a);  // INT index
            } else {           // FL index
                ...
                av = (I)f;
                ASSERT(f == af || FFIEQ(f, af),
                       EVDOMAIN);  // if index not integral, complain.  IMAX/IMIN will fail presently.  We rely on
                                   // out-of-bounds conversion to peg out one side or other (standard violation)
            }
            I wr1 = AR(w) - 1;
            if (wr1 <= 0) {  // w is atom or list, result is atom
                // Get the area to use for the result: the a input if possible, else an INT atom
                if ((SGNIF(jtinplace, JTINPLACEAX) & AC(a) & SGNIFNOT(AFLAG(a), AFUNINCORPABLEX)) < 0)
                    z = a;
                else {
                    GAT0(z, INT, 1, 0)
                }
                ...
                SETNDX(j, av, AN(w));
                IAV(z)[0] = IAV(w)[j];
                // Here we transferred an atom out of w.  We must mark w non-pristine.  If it was inplaceable, we can
                // transfer the pristine status.  We overwrite w because it is no longer in use
                ...
                AN(z) = m;
                MCISH(AS(z), ws + 1, wr1)
                // When we create a virtual block we do not actually copy anything out of w, so it remains pristine. The
                // result is not.
            }
        } else {
            ...
            A (*fn)(J, A, A);
            fn        = jtifrom;
            fn        = at & BOX ? jtafrom : fn;
            fn        = at & (AN(a) != 1) ? jtbfrom : fn;
            jtinplace = fn != jtifrom ? jt : jtinplace;
            z         = (*fn)(jtinplace, a, w);
            // Here we transferred out of w.  We must mark w non-pristine.  Since there may have been duplicates, we
            // cannot mark z as pristine.  We overwrite w because it is no longer in use
            PRISTCLRF(w)
        }
    } else if (!((AT(a) | AT(w)) & (NOUN & ~SPARSE))) {
        z = jtfromss(jt, a, w);
    }  // sparse cases
    else if (AT(w) & SPARSE) {
        z = at & BOX ? jtfrombs(jt, a, w) : jtfromis(jt, a, w);
    } else {
        z = jtfromsd(jt, a, w);
    }
    return z;
...
A
jtsfrom(J jt, A a, A w) {
    if (!(SPARSE & AT(w))) {
        // Not sparse.  Verify the indexes are numeric and not empty
        if (((AN(a) - 1) | (AR(a) - 2) | ((AT(a) & NUMERIC) - 1)) >= 0) {
            A ind;  // a is an array with rank>1 and numeric.  Rank 1 is unusual & unimportant & we'll ignore it
            ...
            if (ind = jtcelloffset(jt, w, a)) {
                ...
                // We could do this with ifrom, but it validates the input and checks for virtual, neither of which is
                // germane here.  Also, we would have to reshape w into an array of cells.  Easier just to copy the data
                ...
                MCISH(AS(z), AS(ind), AR(ind))
                  MCISH(AS(z) + AR(ind), AS(w) + AS(a)[AR(a) - 1], AR(w) - AS(a)[AR(a) - 1])  // shape from ind, then w
    ...
    A z;
    return jtfrom(jt, IRS1(a, 0L, 1L, jtbox, z), w);
} /* (<"1 a){w */
...
A
jtmap(J jt, A w) {
    return jtmapx(jt, ds(CACE), w);
}

// extract the single box a from w and open it.  Don't mark it no-inplace.  If w is not boxed, it had better be an atom,
// and we return it after auditing the index
static A
jtquicksel(J jt, A a, A w) {
    I index;
...
A
jtfetch(J jt, A a, A w) {
    A *av, z;
    I n;
    FPREFIP;
    F2RANKW(1, RMAX, jtfetch, UNUSED_VALUE);  // body of verb applies to rank-1 a, and must turn pristine off if used
                                              // higher, since there may be repetitions.
    if (!(BOX & AT(a))) {
        // look for the common special case scalar { boxed vector.  This path doesn't run EPILOG
        if (((AT(w) >> BOXX) & 1) >=
            (2 * AR(a) +
             AR(w))) {  // a is an atom, w is atom or list of boxes   AR(a)==0 && (AR(w)==0 || (AR(w)==1 && AT(w)&BOX))
    ...
    z = w;
    DO(
      n, A next = av[i]; if (((AT(z) >> BOXX) & 1) >= (2 * (AR(next) + (AT(next) & BOX)) + AR(z))) {
          RZ(z = jtquicksel(jt, next, z))
      }  // next is unboxed atom, z is boxed atom or list, use fast indexing  AR(next)==0 && !(AT(next)&BOX) &&
         // (AR(z)==0 || (AR(z)==1 && AT(z)&BOX))
      else {
          RZ(z = jtafrom(jt, jtbox(jt, next), z));
          ASSERT(((i + 1 - n) & -AR(z)) >= 0, EVRANK);
          if (((AR(z) - 1) & SGNIF(AT(z), BOXX)) < 0) RZ(z = jtope(jt, z));
      }  // Rank must be 0 unless last; open if boxed atom
    );
    // Since the whole purpose of fetch is to copy one contents by address, we turn off pristinity of w
    PRISTCLRF(w)
