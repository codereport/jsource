...
static B
jtspsscell(J jt, A w, I wf, I wcr, A* zc, A* zt) {
    A c, t, y;
    B b;
...
static A
jtgrd1spz(J jt, A w, I wf, I wcr) {
    A z;
    I *ws, zn;
    ws = AS(w);
    RE(zn = jtprod(jt, wf + !!wcr, ws));
    GATV(z, INT, zn, 1 + wf, ws);
    if (!wcr) AS(z)[wf] = 1;
    return z;
} /* allocate result for grd1sp__ */

static void
sp1merge0(I n, I n1, I yc, I* zv, I* xv, I* yv, I* tv) {
...
#define ADVANCE(dv)                                  \
    {                                                \
        I j = wf;                                    \
        while (++dv[--j], dv[j] == ws[j]) dv[j] = 0; \
    }

static A
jtgrd1spss(J jt, A w, I wf, I wcr) {
    A c, d, t, x, y, z;
    ...
    y                             = SPA(wp, i);
    jt->workareas.compare.compsyv = yv = AV(y);
    jt->workareas.compare.compsyc = yc = *(1 + AS(y));
    x                                  = SPA(wp, x);
    jt->workareas.compare.compsxv      = CAV(x);
    jt->workareas.compare.compsxc      = jtaii(jt, x) * (wt & SCMPX ? 2 : 1);
    jt->workareas.compare.compswf      = wf;
    jt->workareas.compare.comp         = (CMP)(wt & SB01   ? compspssB
                                               : wt & SINT ? compspssI
                                               : wt & SFL  ? compspssD
                                                           : compspssZ);
...
static A
jtgrd1spsd(J jt, A w, I wf, I wcr) {
    A d, t, y, z;
...
static B
jtspdscell(J jt, A w, I wf, I wcr, A* zc, A* zt) {
    A c, t, y;
...
static A
jtgrd1spds(J jt, A w, I wf, I wcr) {
    A c, t, x, y, z;
    ...
    x                                  = SPA(wp, x);
    jt->workareas.compare.compsxv      = CAV(x);
    jt->workareas.compare.compsxc = p = jtaii(jt, x) * (wt & SCMPX ? 2 : 1);
    jt->workareas.compare.compn       = p / m;
    jt->workareas.compare.comp        = (CMP)(wt & SB01   ? compspdsB
                                              : wt & SINT ? compspdsI
                                              : wt & SFL  ? compspdsD
                                                          : compspdsZ);
    jt->workareas.compare.compusejt   = 1;
...
static A
jtgrd1spdd(J jt, A w, I wf, I wcr) {
    A x, z;
    I n, *ws;
    P* wp;
    wp = PAV(w);
...
static void
sp2merge0(I n, I n1, I yc, I* zyv, I* xv, I* yv, I* tv) {
...
static A
jtgrd2spss(J jt, A w, I wf, I wcr) {
    A c, t, x, y, z, zy;
    ...
    RZ(x = jtgrade1(jt, zy));
    SPB(zp, i, jtfrom(jt, x, zy));
    SPB(zp, x, jtfrom(jt, x, SPA(zp, x)));
    return z;
} /* sparse frame, sparse cell */

static A
jtgrd2spsd(J jt, A w, I wf, I wcr) {
    A x, z;
    P* zp;
    RZ(z = jtca(jt, w));
    zp = PAV(z);
...
jtgrd2sp(J jt, A a, A w) {
    ...
    if (c) {
        b = a == w;
        RZ(w = jtreaxis(jt, jtifb(jt, wr, wb), w));
        if (b) a = w;
    }
