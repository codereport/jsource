...
// a and w (which may be the same) are 2 nouns.  We pick the fill based  on the types of a/w (a first, then w, skipping
// a if empty) if jt->fill is set, we use that value instead we put one atom of fill into jt->fillv0 and point jt->fillv
// to that atom if w is not the same type as the fill, convert it.  The user has to handle a.
A
jtsetfv(J jt, A a, A w) {
    A q = jt->fill;
    I t;
    q    = q ? q : mtv;  // if no fill given, use empty vector
    I t2 = REPSGN(-AN(w)) & AT(w);
    t    = REPSGN(-AN(a)) & AT(a);
    t    = t ? t : t2;                                   // ignoring empties, use type of a then w
...
jtfillv(J jt, I t, I n, C *v) {
    ...
    switch (CTTZ(t)) {
        ...
        case SFLX:
        case SCMPXX:
        case SBTX: memset(v, C0, k * n); break;
...
jtrotsp(J jt, A a, A w) {
    ...
    wp = PAV(w);
    a  = SPA(wp, a);
    RZ(y = jtca(jt, SPA(wp, i)));
    SETIC(y, m);
    n = AN(a);
    RZ(a = jtpaxis(jt, wr, a));
    av = AV(a);
    RZ(q = jtfrom(jt, a, q));
    qv = AV(q);
    GASPARSE(z, AT(w), 1, wr, s);
    zp = PAV(z);
    by = 0;
    DO(
      n, if (qv[i]) {
...
#define ROF(r)                                          \
    if ((I)(r < -n) | (I)(r > n)) r = (r < 0) ? -n : n; \
    x = dk * ABS(r);                                    \
    y = e - x;                                          \
    j = 0 > r ? y : x;                                  \
    k = 0 > r ? x : y;
#define ROT(r)                               \
    if ((I)(r < -n) | (I)(r > n)) r = r % n; \
    x = dk * ABS(r);                         \
    y = e - x;                               \
    j = 0 > r ? y : x;                       \
    k = 0 > r ? x : y;
...
static void
jtrot(J jt, I m, I d, I n, I atomsize, I p, I *av, C *u, C *v) {
    I dk, e, k, j, r, x, y;
    e  = n * d * atomsize;
    dk = d * atomsize;  // e=#bytes per cell  dk=bytes per item
    if (jt->fill) {
        if (p <= 1) {
            r = p ? *av : 0;
            ROF(r);
            DQ(
              m,
              if (r < 0) {
                  mvc(k, v, atomsize, jt->fillv);
                  memcpy(k + v, u, j);
              } else {
                  memcpy(v, j + u, k);
                  mvc(j, k + v, atomsize, jt->fillv);
              } u += e;
              v += e;);
        } else {
            DO(
              m, r = av[i]; ROF(r); if (r < 0) {
                  mvc(k, v, atomsize, jt->fillv);
                  memcpy(k + v, u, j);
              } else {
                  memcpy(v, j + u, k);
        ...
        if (p <= 1) {
            r = p ? *av : 0;
            ROT(r);
            DQ(m, memcpy(v, j + u, k); memcpy(k + v, u, j); u += e; v += e;);
        } else {
            DO(m, r = av[i]; ROT(r); memcpy(v, j + u, k); memcpy(k + v, u, j); u += e; v += e;);
        }
    }
}
...
A
jtrotate(J jt, A a, A w) {
    A origw = w, y, z;
    B b;
    C *u, *v;
    I acr, af, ar, *av, d, k, m, n, p, *s, wcr, wf, wn, wr;
    FPREFIP;
    ...
    if (((1 - acr) | ((-af) & (-acr | (wf - 1)))) < 0)
        return df2(z,
                   a,
                   w,
                   jtqq(jt,
                        jtqq(jt, ds(CROT), jtv2(jt, 1L, RMAX)),
    ...
    wn = AN(w);
    s  = AS(w);
    k  = bpnoun(AT(w));  // set fill value if given
    GA(z, AT(w), wn, wr, s);
    v = CAV(z);
    if (!wn) return z;
    PROD(m, wf, s);
    ...
    // w is going to be replaced.  That makes it non-pristine; but if it is inplaceable it can pass its pristinity to
    // the result, as long as there is no fill
...
jtrevsp(J jt, A w) {
    ...
    m  = *(f + AS(w));
    wp = PAV(w);
    GASPARSE(z, AT(w), 1, wr, AS(w));
    zp = PAV(z);
    a  = SPA(wp, a);
    n  = AN(a);
    RZ(y = jtca(jt, SPA(wp, i)));
    x = SPA(wp, x);
    RZ(q = jtpaxis(jt, wr, a));
    v = AV(q);
    DO(
      wr, if (f == v[i]) {
          k = i;
...
jtreverse(J jt, A w) {
    ...
    r  = (RANKT)jt->ranks;
    r  = wr < r ? wr : r;
    f  = wr - r;                      // no RESETRANK - we don't call any primitive from here on
    ...
    switch (k) {
        default: {
            C *s = wv - k, *t;
            DQ(m, t = s += nk; DQ(n, memcpy(zv, t, k); zv += k; t -= k;););
        } break;
        case sizeof(C): {
            C *s = wv, *t, *u = zv;
            DQ(m, t = s += n; DQ(n, *u++ = *--t;););
...
static A
jtreshapesp0(J jt, A a, A w, I wf, I wcr) {
    A e, p, x, y, z;
    B *b, *pv;
    I c, d, r, *v, wr, *ws;
    P *wp, *zp;
    ...
    if (!wf) {
        if (r && c) {
            v = AV(y);
            DO(c, if (v[i]) return e;);
        }
        return AN(x) ? jtreshape(jt, mtv, x) : e;
    }
    GASPARSE(z, AT(w), 1, wf, ws);
    zp = PAV(z);
    SPB(zp, e, e);
    A bvec = jtifb(jt, wf, b);
    makewritable(bvec) SPB(zp, a, bvec);  // avoid readonly
    GATV0(p, B01, r, 1);
    pv = BAV(p);
    v  = AV(y);
    DO(r, *pv = 1; DO(
...
static A
jtreshapesp(J jt, A a, A w, I wf, I wcr) {
    A a1, e, t, x, y, z;
    B az, *b, wz;
    ...
    if (b[wf]) {
        ...
        v = AV(y);
        DQ(n, if (r <= *v) break; ++q; v += c;);
        GATV0(t, INT, q, 1);
        u = AV(t);
        v = v0 = AV(y);
        m = j = 0;
        DO(
          q, u[i] = m + *v; v += c; ++j; if (j == n) {
              j = 0;
              v = v0;
              m += ws[wf];
          });
...
jtreshape(J jt, A a, A w) {
    ...
    RESETRANK;
    if ((I)(1 < acr) | (I)(acr < ar)) {
        z = rank2ex(a, w, UNUSED_VALUE, MIN(acr, 1), wcr, acr, wcr, jtreshape);
        PRISTCLRF(w) return z;
    }  // multiple cells - must lose pristinity
    ...
    if (m <= n) {      // no wraparound
        if (c == 1) {  // if there is only 1 cell of w...
            // If no fill required, we can probably use a virtual result, or maybe even an inplace one.  Check for
            // inplace first.  Mustn't inplace an indirect that shortens the data, because then who would free the
            // blocks?  (Actually it would be OK if nonrecursive, but we are trying to exterminate those).  Since it
            // must be DIRECT, there's no question about PRISTINE, but that would be OK to transfer if inplaceable
            if (ASGNINPLACESGN(SGNIF((I)jtinplace, JTINPLACEWX) & (r - (wcr + 1)) & ((n - (m + 1)) | -(t & DIRECT)),
                               w)) {  //  inplace allowed, just one cell, result rank (an) <= current rank (so rank
                                      //  fits), usecount is right
                // operation is loosely inplaceable.  Copy in the rank, shape, and atom count.
                AR(w) = (RANKT)(r + wf);
                AN(w) = m;
                ws += wf;
                MCISH(ws, u, r) return w;  // Start the copy after the (unchanged) frame
            }
    ...
    } else if (filling = jt->fill != 0) {
        RZ(w = jtsetfv(jt, w, w));
        t = AT(w);
    }  // if fill required, set fill value.  Remember if we need to fill
    k = bpnoun(t);
    p = k * m;
    q = k * n;
    DPMULDE(c, m, zn);
    ...
    zv = CAV(z);
    wv = CAV(w);
    // We extracted from w, so mark it (or its backer if virtual) non-pristine.  Note that w was not changed above if it
    // was boxed nonempty.  z is never pristine, since it may have repeats
    PRISTCLRF(w)
    // w has been destroyed
    if (filling)
        DQ(c, mvc(q, zv, q, wv); mvc(p - q, q + zv, k, jt->fillv); zv += p; wv += q;)
    else
        DQ(c, mvc(p, zv, q, wv); zv += p; wv += q;);
    return z;
} /* a ($,)"r w */

A
jtreitem(J jt, A a, A w) {
    A y, z;
    I acr, an, ar, r, *v, wcr, wr;
    FPREFIP;
    if (!(a && w)) return 0;
    ar  = AR(a);
    acr = jt->ranks >> RANKTX;
    ...
    RESETRANK;
    if ((I)(1 < acr) | (I)(acr < ar)) {
        z = rank2ex(a, w, UNUSED_VALUE, MIN(acr, 1), wcr, acr, wcr, jtreitem);
        PRISTCLRF(w) return z;
    }  // multiple cells - must lose pristinity  // We handle only single operations here, where a has rank<2
    // acr<=ar; ar<=acr; therefore ar==acr here
    fauxblockINT(yfaux, 4, 1);
    if (1 >= wcr)
        y = a;  // y is atom or list: $ is the same as ($,)
    else {      // rank y > 1: append the shape of an item of y to x
        RZ(a = jtvi(jt, a));
        an                             = AN(a);
        acr                            = 1;  // if a was an atom, now it is a list
        fauxINT(y, yfaux, an + r, 1) v = AV(y);
        MCISH(v, AV(a), an);
        MCISH(v + an, AS(w) + wr - r, r);
    }
    return wr == wcr ? jtreshape(jtinplace, y, w)
                     : IRS2(y, w, 0L, acr, wcr, jtreshape, z);  // Since a has no frame, we dont have to check agreement
} /* a $"r w */
