...
static void jtrot(J jt,I m,I d,I n,I atomsize,I p,I*av,C*u,C*v){I dk,e,k,j,r,x,y;
 ...
 if(jt->fill){
  if(p<=1){r=p?*av:0;     ROF(r); DQ(m, if(r<0){mvc(k,v,atomsize,jt->fillv); memcpy(k+v,u,j);}else{memcpy(v,j+u,k); mvc(j,k+v,atomsize,jt->fillv);}        u+=e; v+=e;);}
  else{DO(m, r=av[i]; ROF(r);       if(r<0){mvc(k,v,atomsize,jt->fillv); memcpy(k+v,u,j);}else{memcpy(v,j+u,k); mvc(j,k+v,atomsize,jt->fillv);}            u+=e; v+=e;);}
 }else{
...
/* m   # cells
 ...
 if(((1-acr)|((-af)&(-acr|(wf-1))))<0)return df2(z,a,w,qq(qq(ds(CROT),jtv2(jt,1L,RMAX)),jtv2(jt,acr,wcr)));  // if multiple a-lists per cell, or a has frame and (a cell is not an atom or w has frame) handle rank by using " for it
...
static A jtreshapesp0(J jt,A a,A w,I wf,I wcr){A e,p,x,y,z;B*b,*pv;I c,d,r,*v,wr,*ws;P*wp,*zp;
 ...
 zp=PAV(z); SPB(zp,e,e); A bvec=jtifb(jt,wf,b); makewritable(bvec) SPB(zp,a,bvec);  // avoid readonly
...
static A jtreshapesp(J jt,A a,A w,I wf,I wcr){A a1,e,t,x,y,z;B az,*b,wz;I an,*av,c,d,j,m,*u,*v,wr,*ws;P*wp,*zp;
 ...
 if(b[wf]){I n,q,r,*v0;   /* sparse */
  ...
  RE(m=jtprod(jt,1+d,av)); SETIC(y,n); if(ws[wf]){q=n*(m/ws[wf]); r=m%ws[wf];} else {q=0; r=0;}
...
// correct   if(!(AFLAG(w)&(AFNJA))){RZ(z=virtual(w,0,r+wf)); AN(z)=m; I *zs=AS(z); DO(wf, *zs++=ws[i];); DO(r, zs[i]=u[i];) return z;}
 ...
 }else if(filling=jt->fill!=0){RZ(w=jtsetfv(jt,w,w)); t=AT(w);}   // if fill required, set fill value.  Remember if we need to fill
...
#define EXPAND(T)  \
 ...
 switch(wk){
  ...
  default:  
   ...
   for(i=p=0;i<an;++i)
    if(*av++)p+=wk; 
    else{if(p){memcpy(zv,wv,p); wv+=p; zv+=p; p=0;} zv+=wk;}
   if(p){memcpy(zv,wv,p);}
 }
 return z;
}    /* a&#^:_1 w or a&#^:_1!.f w */
