...
#define BXLOOP(T)                                                                                    \
    {                                                                                                \
        T *wv = (T*)AV(w), x;                                                                        \
        if (descend) {                                                                               \
            DQ(m, x = *wv++; I t = n; t = x >= 0 ? p : t; t = x >= 1 ? 0 : t; *zv++ = t;); /* p q */ \
        } else if (p < 0) {                                                                          \
            DQ(m, *zv++ = n * (1 < *wv++);) /*   q */                                                \
        } else if (q < 0) {                                                                          \
            DQ(m, *zv++ = n * (0 < *wv++);); /* p   */                                               \
        } else {                                                                                     \
            DQ(m, x = *wv++; I t = n; t = x <= 1 ? q : t; t = x <= 0 ? 0 : t; *zv++ = t;) /* p q */  \
        }                                                                                            \
    }

static B
jtiixBX(J jt, I n, I m, A a, A w, I* zv) {
    B *av, *b, descend;
    I p, q;
    av      = BAV(a);
    descend = av[0] > av[n - 1];
...
jtiixI(J jt, I n, I m, A a, A w, I* zv) {
    ...
    if (ascend) {
        ...
        x    = *u++;
        *v++ = j = 0;
        DQ(n - 1, ++j; y = *u++; ASSERT(BETWEENC(y, x, q), EVDOMAIN); DQ(y - x, *v++ = j;); x = y;);
    } else {
        u = av + n - 1;
        x = *u--;
        j = n;
    ...
    if (ascend)
        DQ(m, x = *wv++; *zv++ = x <= p ? 0 : q < x ? n : tv[x];)
    else
        DQ(m, x = *wv++; *zv++ = x >= q ? 0 : p > x ? n : tv[x];);
    return 1;
...
#define BSLOOP1x(CMP) /* if CMP is true, move q; otherwise p */                                                        \
    ...
    do {              /* empty lists handled earlier */                                                                \
        ...
        I k2;                                                                                                          \
        MID(k2, p2, q);                                                                                                \
        MID(k, p, q1); /* calculate new values for kpq depending on which way the compare turns out.  2=upper side (ie \
                          if p moved), 1=lower side (if q moved) */                                                    \
        k = CMP ? k : k2;                                                                                              \
        p = CMP ? p : p2;                                                                                              \
        q = CMP ? q1 : q; /* move k first to start next fetch */                                                       \
    } while (q >= p);
    ...
    while (p <= q) {                                \
        ...
        DQ(                                         \
          c, x = *u++; y = *v++; if (NE) {          \
              CMP;                                  \
              break;                                \
          }); /* make this compare fixed length? */ \
        if (b)                                      \
            q = k - 1;                              \
        else                                        \
            p = k + 1;                              \
...
#define BSLOOPNx(NE, CMP) /* if CMP is true, move q; otherwise p */                                                    \
    ...
    do { /* empty lists handled earlier */                                                                             \
        ...
        k = b ? k : k2;                                                                                                \
        p = b ? p : p2;                                                                                                \
        q = b ? q1 : q; /* move k first to start next fetch */                                                         \
    } while (q >= p);
...
#define BSLOOP(Ta, Tw)                                                                                        \
    {                                                                                                         \
        ...
        Tw* wv = (Tw*)AV(w);                                                                                  \
        if (c == 1) {                                                                                         \
            if (ge != 1) DQ(m, BSLOOP1x(x >= y); *zv++ = q + 1;) else DQ(m, BSLOOP1x(x <= y); *zv++ = q + 1;) \
        } else {                                                                                              \
            if (ge != 1)                                                                                      \
                DQ(m, Ta* u; Tw * v; BSLOOPNx(x != y, x > y); *zv++ = q + 1; wv += c;)                        \
                else DQ(m, Ta* u; Tw * v; BSLOOPNx(x != y, x < y); *zv++ = q + 1; wv += c;)                   \
        }                                                                                                     \
    }
...
#define BSLOOF(Ta, Tw, COMP)                                                        \
    {                                                                               \
        ...
        Tw *wv = (Tw*)AV(w), *v, y;                                                 \
        if (1 == c)                                                                 \
            DQ(m, BSLOOP1(b = ge != COMP(av[k], y)); *zv++ = 1 + q;)                \
        else                                                                        \
            DQ(m, BSLOOPN(cc = COMP(x, y), b = gt == cc); *zv++ = 1 + q; wv += c;); \
    }
...
jticap2(J jt, A a, A w) {
    ...
    as = AS(a);
    SETIC(a, n);
    r  = ar - 1 < 0 ? 0 : ar - 1;  // n=length of 1-cell of a, r=frame of a
    wr = AR(w);
    wt = AT(w);
    ws = AS(w);
    ASSERT(r <= wr, EVRANK);
    ...
    if (((m - 1) | (n - 1) | (c - 1)) < 0) {
        DQ(m, *zv++ = 0;);
        return z;
    }  // exit with zeros for empty args
    t = maxtyped(at, wt);
    if (1 == c) {  // the search is for atoms
        ...
        if (at & wt & INT) {
            ...
            UI r = IAV(a)[n - 1] - IAV(a)[0];
            r    = IAV(a)[n - 1] < IAV(a)[0] ? 0 - r : r;  // get range, which may overflow I but will stay within UI
            UI4 nlg;
            CTLZI(n, nlg);
            nlg =
              (nlg << 1) +
        ...
        case TT(BOXX, BOXX):
            for (j = 0, cm = c * m; j < cm; j += c) {
                ...
                while (p <= q) {
                    ...
                    I b = 1;
                    DO(
                      c, if (cc = jtcompare(jt, av[i + ck], wv[i + j])) {
                          b = gt == cc;
                          break;
                      });
                    if (b)
