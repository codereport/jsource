...
static A jtebarmat(J jt,A a,A w){A ya,yw,z;B b,*zv;C*au,*av,*u,*v,*v0,*wu,*wv;I*as,c,i,k,m,n,r,s,si,sj,t,*ws;
 ...
 if(t&B01+LIT+C2T+C4T+INT+SBT||1.0==jt->cct&&t&FL+CMPX)
  ...
  for(i=0;i<m;++i){
   DO(n, u=av; b=1; DO(si, memcpy(au,u,s); memcpy(wu,v,s); if(!equ(ya,yw)  ){b=0; break;} u+=s; v+=r;); v=v0+=k; zv[i]=b;);
   zv+=c; v=v0=wv+=r;
 }}
 return z;
...
static A jtebarvec(J jt,A a,A w){A y,z;B*zv;C*av,*wv,*yv;I an,k,n,s,t,wn;
 ...
 else{GA(y,t,an,AR(a),0); yv=CAV(y); DO(n, memcpy(yv,wv,s); zv[i]=equ(a,y); wv+=k;);}
 return z;
