...
#define MINUSVV(n,z,x,y)   {I u,v,w; DQ(n, u=*x; v=*y; w=u-v; *z=w; ++x; ++y; ++z; v^=u; u^=w; if(XANDY(u,v)<0)return EWOV;)}
#define TYMESVV(n,z,x,y)   {DPMULDECLS  DQ(n, DPMUL(*x,*y,z, return EWOV;); z++; x++; y++;)}


// overflow if (~ssign^vsign)&&(resultsign^vsign), i. e. input signs equal and result changes sign
#define PLUSP(n,z,x)      {I s=0,u,v;   DQ(n, u=~s; v=*x; s+=v; u^=v; v^=s; x++; *z++=s; if(XANDY(u,v)<0)return EWOV;);}
#define MINUSP(n,z,x)      {I s=0,u,v,p=0;   DQ(n, u=~s; v=*x^p; s+=v-p; p = ~p; u^=v; v^=s; *z++=s; x++; if(XANDY(u,v)<0)return EWOV;);}
#define TYMESP(n,z,x)      {DPMULDDECLS I s=1; DQ(n, DPMULD(*x,s,s,return EWOV;) *z++=s; x++;)}
...
#define TYMESR(n,z,x)      {I l=1; DPMULDDECLS DQ(n, DPMULD(l,*x,l,return EWOV;); x++;) *z=l;}

#define PLUSRV(n,z,x)     {I u,v,w; DQ(n, u=*x; v=*z; w=~u; u+=v; *z=u; ++x; ++z; w^=v; v^=u; if(XANDY(w,v)<0)return EWOV;)}
#define MINUSRV(n,z,x)     {I u,v,w; DQ(n, u=*x; v=*z; w=u-v; *z=w; ++x; ++z; v^=u; u^=w; if(XANDY(u,v)<0)return EWOV;)}
#define TYMESRV(n,z,x)     {DPMULDECLS DQ(n, DPMUL(*x,*z,z,return EWOV;); x++; z++;);}

#define PLUSS(n,z,x)      {I s=0,u,v; x+=n; z+=n; DQ(n, u=~s; v=*--x; s+=v; u^=v; v^=s; *--z=s; if(XANDY(u,v)<0)return EWOV;);}
