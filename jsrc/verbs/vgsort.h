...
// comp() is the comparison function and compn is a numeric parameter to it.  comp(op0,op1,compn) returns 1 if op0 comes
// before op1,
...
MERGEFNNAME(CMP comp, I compn, I bpi, T *lo, I lon, T *hi, I hin, T *wk) {
    ...
    do {
        ...
        if (!COMPFN(compn, PTRADD(lo, offsettotry - 1), hi))
            break;                 // if test value is not below the start of hi, stop looking
    ...
    if (orderedlen) {
        ...
        if (orderedlen == lon) {
            // Presorted list.  If the two lists are contiguous, just return their address.  Otherwise copy to be
            // contiguous
            if (loend != hi) { memcpy(loend, hi, hin * bpi); }
            return lo;  // return contiguous result
        }
        // Partial presort.  Because the presorted part is larger, leave it in place and copy the shorter remnant at the
        // end to the workarea; then change the pointers so we merge the moved remnant onto the end of the presorted
        // fragment
        wk    = lo;
        wkptr = PTRADD(wkptr, orderedlen);
        lo    = PTRADD(lo, orderedlen);
        lon -= orderedlen;
        memcpy(wkptr, lo, lon * bpi);  // move wkptr&lo to end of block; lon=length of remnant; move remnant to wk;
                                       // point wk to start of original input
        T *tmp = wkptr;
        loend  = PTRADD(wkptr, lon);
        wkptr  = lo;
        lo     = tmp;  // lo, loend point into workarea; wkptr-> input area
    }
    T *hiend = PTRADD(hi, hin);  // end+1 of hi
    // Perform the merge into wk[].
    do {
        // this version does not requires prediction of the test.  Faster, because we are mostly in cache and there is
        // not a long delay for any fetch We keep processing until one of the lists has been exhausted.  At that point,
        // we copy the other list to the output and return.
        if (lo != loend) {
            if (hi != hiend) {
                // Normal case. compare the leading keys, moving and incrementing the lower.  We do the address
                // arithmetic without branches to avoid misprediction
                I lomove = COMPFN(compn, lo, hi);  // 1 if we should move lo
                // Select the address to move from
                T *fromptr = lo;
                fromptr    = (lomove -= 1) ? hi : fromptr;
                ...
                hi = (void *)((C *)hi + (lomove &= bpi));
                lo = (void *)((C *)lo + (lomove ^= bpi));
                // copy to the output
...
// sort the values in *wv, using *wk as a work area of the same size.  in and wk are the main output and the workarea to
// use. On the first pass, where we order small blocks of the input, we draw from *wv and create sorted strings in *in.
// Thereafter we draw from *in.  We do this rather than copying the input into *wv to begin with, because that copy
...
GRADEFNNAME(CMP comp, I compn, I n, I bpi, T *in, T *wk, T *wv) {
    ...
    switch (n) {
        ...
        default:
            // sort the low and high halves, and then merge the results, giving as workarea whatever buffer does not
            // contain lo
            {
                I lohalf = n >> 1;
                I hihalf = n - lohalf;
                T *lo    = GRADEFNNAME(comp, compn, lohalf, bpi, in, wk, wv);
                T *hi =
                  GRADEFNNAME(comp, compn, hihalf, bpi, PTRADD(in, lohalf), PTRADD(wk, lohalf), PTRADD(wv, lohalf));
