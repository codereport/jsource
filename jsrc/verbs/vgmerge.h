...
MERGEFNNAME(CMP comp, I compn, void **lo, I lon, void **hi, I hin, void **wk) {
    ...
    do {
        ...
        if (!COMPFN(compn, lo[offsettotry - 1], hi[0]))
    ...
    if (orderedlen) {
        ...
        lo += orderedlen;
        lon -= orderedlen;
        MCIL(wkptr, lo, lon);  // move wkptr&lo to end of block; lon=length of remnant; move remnant to wk; point wk to
                               // start of original input
        void **tmp = wkptr;
        loend      = wkptr + lon;
        wkptr      = lo;
    ...
    do {
        if (COMPFN(compn, loaddr, hiaddr)) {
            ++lo;
            *wkptr++ = loaddr;
            if (lo != loend)
                loaddr = *lo;
            else {
                if (wkptr != hi) MCIL(wkptr, hi, hiend - hi);
                break;
            }
        } else {
            ++hi;
...
static void **
GRADEFNNAME(CMP comp, I compn, void *(in[]), I n, void *(wk[])) {
    void *a, *b, *c, *d, *e;
    switch (n) {
        case 0:
        case 1: return in;
        case 2:  // happens only if original input is 2 long
            if (!COMPFN(compn, in[0], in[1])) {
                void *tmp = in[0];
                in[0]     = in[1];
                in[1]     = tmp;
            }
        ...
        default:
            ...
            // contain lo
            {
                I lohalf = n >> 1;
                I hihalf = n - lohalf;
                void *lo = GRADEFNNAME(comp, compn, in, lohalf, wk);
                void *hi = GRADEFNNAME(comp, compn, in + lohalf, hihalf, wk + lohalf);
