...
// To reduce parameter overhead, we call matchsub with trailing parameters omitted if x==0.  This is fine for clang.  In
// MSVC, the parameter area is used as a workarea by the routine, and thus omitting the parms makes for a segfault.
...
#define EQV(T)                                                                          \
    {                                                                                   \
        ...
        switch (MCS(q, af, wf)) {                                                       \
            case MCS(1, 0, 1):                                                          \
                h = *u;                                                                 \
                if (b1) DQ(mn, *x++ = h == *v++;) else DQ(mn, *x++ = h != *v++;) break; \
            case MCS(1, 1, 0):                                                          \
                h = *v;                                                                 \
                if (b1) DQ(mn, *x++ = *u++ == h;) else DQ(mn, *x++ = *u++ != h;);       \
                break;                                                                  \
            case MCS(1, 1, 1):                                                          \
                if (b1) {                                                               \
                    if (1 == n)                                                         \
            ...
            case MCS(2, 1, 1):                                                          \
                if (1 == n)                                                             \
                    ...
                    DQ(m, DQ(n, QLOOP; v += q;); u += q;)                               \
                else                                                                    \
                    DQ(m, DQ(n, QLOOP; u += q;); v += q;);                              \
                break;                                                                  \
        }                                                                               \
    }
...
eqv(I af, I wf, I m, I n, I k, C* av, C* wv, B* RESTRICT x, B b1) {
    ...
    if (0 == (k & (SZI - 1)))
        ...
        if (af < wf)
            DQ(m, DQ(n, *x++ = (!!memcmp(av, wv, k)) ^ b1; wv += k;); av += k;)
        else
            DQ(m, DQ(n, *x++ = (!!memcmp(av, wv, k)) ^ b1; av += k;); wv += k;);
    }
    return xx[0];
...
jtequ(J jt, A a, A w) {
    ...
    if (a == w) return 1;
    if ((SPARSE & (AT(a) | AT(w))) != 0)
        if (AR(a) && AR(w)) {
            RZ(x = jtmatchs(jt, a, w));
            return BAV(x)[0];
        }
...
jteqf(J jt, A a, A w) {
    ...
    p = u->fgh[0];
    q = v->fgh[0];
    if (!((p == q || p && q && ((B(*)())jtmatchsub)(jt, p, q, 0 MATCHSUBDEFAULTS)))) return 0;
    p = u->fgh[1];
    q = v->fgh[1];
    if (!((p == q || p && q && ((B(*)())jtmatchsub)(jt, p, q, 0 MATCHSUBDEFAULTS)))) return 0;
    p = u->fgh[2];
    q = v->fgh[2];
    return (p == q || p && q && ((B(*)())jtmatchsub)(jt, p, q, 0 MATCHSUBDEFAULTS));
}
...
// match two values, returning b1 if match, 1^b1 if no match.  If the values are functions, that's all we return.  If
// the values are nouns, we store the match value(s) in *x.  x may be 0, if af and wf are 0 and m and n are 1.  In this
// case we don't store anything. but return the match status.  We use this when comparing boxed arrays or functions For
// recursive calls, x=0, af=wf=0, and m=n=1.  If there is frame, arguments have been exchanged so that a has the shorter
// frame, i. e. af<=wf, and a will be repeated b1 is the value to use for 'match' - 1 normally, but 0 for top level of
...
jtmatchsub(J jt, A a, A w, B* RESTRICT x, I af, I wf, I m, I n, I b1) {
    ...
    // m*n cannot be 0.  If this is a recursive call, m=n=1; while if it is the first call, empty m/n were handled at
    // the top level
    {
        af &= (-(x != 0));
        wf &= (-(x != 0));  // if x not given, use defaults for af, wf
        I p = AR(a) - af;
        I q = AR(w) - wf;
        // p=cell-rank of a; q=cell-rank of w; ?t=type;
        // c=#atoms in a cell, b is 1 if rank or cell-shape mismatches, or if cells are not empty and types are
        // incompatible We know that either there is no frame or both arguments are nonempty (Empty arguments with frame
        // can happen only at the top level and were handled there). Do all the tests for miscompare and combine the
        // results at the end We assume the caller checked for a==w and handled it, so we don't.
        I shapediff = p;  // see if ranks differ
        p           = q < p ? q : p;
        q ^= shapediff;
        TESTDISAGREE(
          shapediff, af + AS(a), wf + AS(w), p);  // now p is smaller rank; q=ranks differ; shapediff=shapes differ
        shapediff |= q;                           // shapes or ranks differ
        PROD(c, p, af + AS(a));                   // get c=length of a cell in atoms
        at = UNSAFE(AT(a));
        wt = UNSAFE(AT(w));                       // save types, now that register pressure is over
        p  = NEGIFHOMO(at, wt);                   // now p= neg if homogeneous args
        if (((shapediff - 1) & (-c) & p) >= 0) {  // skip compare if rank differ, or if shapes differ, or if inhomo, or
                                                  // if empty; not checking for identical args
            ...
            if (x)
                ...
                b1 = 1;
            return p ^ b1;  // write 'error' if writing enabled; return false
        }
    }
    ...
    switch (CTTZ(t)) {
        ...
        default:
            ...
            break;
        case CMPXX:
            if (1.0 != jt->cct) { INNERT(Z, zeq) break; }  // tolerant, must use complex distance
            INNERT(Z, ZEQCT0) break;
        case FLX:
            if (1.0 != jt->cct) INNERT(D, TEQ) else INNERT(D, DEQCT0) break;
...
jtmatchs(J jt, A a, A w) {
    ...
    c = 0;
    DO(r, c += b[i];);
    if (an < c || DENSE & AT(a)) RZ(a = jtreaxis(jt, jtifb(jt, r, b), a));
    ap = PAV(a);
    ae = SPA(ap, e);
    ax = SPA(ap, x);
    m  = *AS(ax);
    if (wn < c || DENSE & AT(w)) RZ(w = jtreaxis(jt, jtifb(jt, r, b), w));
    wp = PAV(w);
    we = SPA(wp, e);
    wx = SPA(wp, x);
    n  = *AS(wx);
    RZ(x = jtindexof(jt, SPA(ap, i), SPA(wp, i)));
    v = AV(x);
    GATV0(p, B01, m, 1);
    pv = BAV(p);
    GATV0(q, B01, n, 1);
...
jtmatch(J jt, A a, A w) {
    ...
    if (isatoms >= 0) {
        ...
        p = AR(a) - af;
        b = p == (AR(w) - wf) && !ICMP(af + AS(a), wf + AS(w), p);  // b =  shapes are the same
        // Allocate & return result
        GATV(z, B01, mn, wf, AS(w));
        memset(BAV(z), b ^ eqis0, mn);
        return z;
    }
...
A
jtnotmatch(J jt, A a, A w) {
    return jtmatch((J)((I)jt + 1), a, w);
} /* a -.@-:"r w */
