...
#define makewritable(x)             RZ(x=(AC(x)<(AFLAG(x)<<((BW-1)-AFROX)))?x:jtca(jt,x))  // OK if AC is 0x8..1 and AFRO is 0
...
#define equ(x,y)                    jtequ(jt,(x),(y)) // this macro is used in many other macros. Need to resolve the usage before removal.
...
#define fr(x)                       {if((x)!=0){I Zs = AC(x); if(!ACISPERM(Zs)){if(--Zs<=0)jtmf(jt,x);else AC(x)=Zs;}}}
...
#define ra0(x)                      {I tt=AT(x); FLAGT flg=AFLAG(x); if(((tt^flg)&RECURSIBLE)!=0){if((flg&AFVIRTUAL)!=0){RZ((x)=jtrealize(jt,x)); flg=AFLAG(x);} AFLAG(x)=flg|=(tt&RECURSIBLE); jtra((x),tt);}}
...
#define realizeifvirtual(x)         {if((AFLAG(x)&AFVIRTUAL)!=0)RZ((x)=jtrealize(jt,x));}
...
#define tpushcommon(x,suffix) {if(!ACISPERM(AC(x))){I tt=AT(x); A *pushp=jt->tnextpushp; *pushp++=(x); \
                              if(!((I)pushp&(NTSTACKBLOCK-1))){RZ(pushp=jttg(jt,pushp));} if(((tt^AFLAG(x))&TRAVERSIBLE)!=0)RZ(pushp=jttpush(jt,(x),tt,pushp)); jt->tnextpushp=pushp; suffix}}
#define tpush(x)              tpushcommon(x,)
// Internal version, used when the local name pushp is known to hold jt->tnextpushp
#define tpushi(x)                   {if(!ACISPERM(AC(x))){I tt=AT(x); *pushp++=(x); if(!((I)pushp&(NTSTACKBLOCK-1))){RZ(pushp=jttg(jt,pushp));} if(((tt^AFLAG(x))&TRAVERSIBLE!=0))RZ(pushp=jttpush(jt,(x),tt,pushp)); }}
// tpush1 is like tpush, but it does not recur to lower levels.  Used only for virtual block (which cannot be PERMANENT)
#define tpush1(x)                   {A *pushp=jt->tnextpushp; *pushp++=(x); if(!((I)pushp&(NTSTACKBLOCK-1))){RZ(pushp=jttg(jt,pushp));} jt->tnextpushp=pushp; }
