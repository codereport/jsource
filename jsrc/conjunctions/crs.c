/* Copyright 1990-2004, Jsoftware Inc.  All rights reserved.               */
/* Licensed use only. Any other use is in violation of copyright.          */
/*                                                                         */
/* Conjunctions: Rank on Sparse Arrays                                     */

#include "j.h"

static A
jtsprarg(J jt, I f, A x) {
    A q;
    B *b, c;
    I r;
    P* xp;
    r  = AR(x);
    xp = PAV(x);
    if (SPARSE & AT(x)) {
        c = 1;
        RZ(b = jtbfi(jt, r, SPA(xp, a), 1));
        DO(
          f, if (!b[i]) {
              c = 0;
              break;
          });
    } else {
        c = 0;
        GATV0(q, B01, r, 1);
        b = BAV(q);
        memset(b, C0, r);
    }
    memset(b, C1, f);
    return c || !r ? x : jtreaxis(jt, jtifb(jt, r, b), x);
} /* ensure frame axes are sparse */

static A
jtsprinit(J jt, I f, I r, I* s, I t, P* p) {
    A a, a1, z;
    I n, *u, *v;
    P* zp;
    GASPARSE(z, t, 1, r, f + s);
    zp = PAV(z);
    a  = SPA(p, a);
    n  = AN(a) - f;
    u  = f + AV(a);
    GATV0(a1, INT, n, 1);
    v = AV(a1);
    DO(n, v[i] = u[i] - f;);
    SPB(zp, a, a1);
    SPB(zp, e, jtca(jt, SPA(p, e)));
    SPB(zp, i, jtiota(jt, jtv2(jt, 0L, n)));  // empty so cannot be readonly
    SPB(zp, x, jtrepeat(jt, jfalse, SPA(p, x)));
    return z;
} /* initialize an argument cell */

static B*
jtspredge(J jt, A y, I f, I* zm) {
    A q;
    B* b;
    I c, m, n, *v;
    v = AS(y);
    n = v[0];
    c = v[1];
    m = n ? 1 : 0;
    GATV0(q, B01, n, 1);
    b = BAV(q);
    if (n) {
        if (f) {
            v = AV(y);
            DO(n - 1, if (b[i] = 1 && ICMP(v, v + c, f))++ m; v += c;);
        } else
            memset(b, C0, n);
        b[n - 1] = 1;
    }
    *zm = m;
    return b;
} /* cell boundaries per index matrix y */

static A
jtsprz(J jt, A z0, A y, A e, I f, I* s) {
    A a, a0, q, y0, z;
    B d;
    I c, et, h, m, n, r, t, *u, *v, zt;
    P *ep, *zp, *zq;
    RZ(z0 && y && e);
    ASSERT(AN(e), EVDOMAIN);
    if (SPARSE & AT(e)) {
        ep = PAV(e);
        ASSERT(all1(eq(SPA(ep, e), SPA(ep, x))), EVSPARSE);
        q = SPA(ep, e);
    } else {
        RZ(q = jtreshape(jt, mtv, e));
        ASSERT(all1(eq(q, e)), EVSPARSE);
    }
    if (!*AS(z0)) {
        t  = AT(q);
        zt = STYPE(t);
        GASPARSE(z, zt, 1L, f + AR(e), s);
        ICPY(f + AS(z), AS(e), AR(e));
        zp = PAV(z);
        SPB(zp, e, q);
        SPB(zp, a, mtv);
        GATR(q, INT, 0, 2, (I*)&zeroZ);
        SPB(zp, i, q);  // zeroZ is a pun for 0 0!!
        GA(q, t, 0L, 1 + AR(z), 0L);
        *AS(q) = 0;
        ICPY(1 + AS(q), AS(z), AR(z));
        SPB(zp, x, q);
        return z;
    }
    e  = q;
    zt = t = AT(z0);
    d      = t & SPARSE ? 0 : 1;
    if (d)
        zt = STYPE(t);
    else
        t = DTYPE(zt);
    et = AT(e);
    m  = maxtype(et, t);
    zt = STYPE(m);
    r  = AR(z0);
    GASPARSE(z, zt, 1, f + r - 1, s);
    ICPY(AS(z) + f, AS(z0) + 1, r - 1);
    zp = PAV(z);
    SPB(zp, e, TYPESEQ(m, et) ? e : jtcvt(jt, m, e));
    if (d) {
        SPB(zp, a, jtapvwr(jt, f, 0L, 1L));
        SPB(zp, i, y);
        SPB(zp, x, TYPESEQ(m, t) ? z0 : jtcvt(jt, m, z0));
        return z;
    }
    zq = PAV(z0);
    y0 = SPA(zq, i);
    v  = AS(y0);
    n  = v[0];
    c  = v[1];
    v  = AV(y0);
    ASSERT(jtequ(jt, e, SPA(zq, e)), EVNONCE);
    h = *AS(y);
    GATV0(q, INT, h, 1);
    u = AV(q);
    memset(u, C0, h * SZI);
    if (n) {
        h = -1;
        DO(
          n - 1, if (*v != *(v + c)) {
              u[*v] = i - h;
              h     = i;
          } v += c;);
        u[*v] = n - 1 - h;
    }
    SPB(zp, i, jtstitch(jt, jtrepeat(jt, q, y), jtdropr(jt, 1L, y0)));
    a0 = SPA(zq, a);
    v  = AV(a0);
    GATV0(a, INT, f + c - 1, 1);
    u = AV(a);
    DO(f, u[i] = i;);
    DO(c - 1, u[f + i] = v[1 + i] + f - 1;);
    SPB(zp, a, a);
    // memory leak if(TYPESEQ(m,t))ras(SPA(zq,x));
    SPB(zp, x, TYPESEQ(m, t) ? SPA(zq, x) : jtcvt(jt, m, SPA(zq, x)));
    return z;
} /* result processing */

A
jtsprank1(J jt, A w, A fs, I mr, AF f1) {
    PROLOG(0043);
    A q, wx, wy, wy1, ww, z, ze, zi, *zv;
    B* wb;
    I c, i, *iv, j, k, m, n, *v, wcr, wf, wr, *ws, wt, *wv;
    P *wp, *wq;
    wr = AR(w);
    ws = AS(w);
    efr(wcr, wr, mr);
    wf = wr - wcr;
    if (!wf) return CALL1(f1, w, fs);
    DO(wf, ASSERT(ws[i] != 0, EVNONCE););
    RZ(w = jtsprarg(jt, wf, w));
    wp = PAV(w);
    wx = SPA(wp, x);
    wy = SPA(wp, i);
    if (mr) {
        wt = AT(w);
        v  = AS(wy);
        n  = v[0];
        c  = v[1];
        wv = AV(wy);
        RZ(wy1 = jtdropr(jt, wf, wy));
        RZ(wb = jtspredge(jt, wy, wf, &m));
        RZ(ww = jtsprinit(jt, wf, wcr, ws, wt, wp));
        wq = PAV(ww);
        RZ(ze = CALL1(f1, ww, fs));
        GATV0(z, BOX, m, 1);
        zv = AAV(z);
        GATV0(zi, INT, m * wf, 2);
        iv   = AV(zi);
        v    = AS(zi);
        v[0] = m;
        v[1] = wf;
        for (i = j = 0; i < m; ++i) {
            k = 1 + (B*)memchr(wb + j, C1, n - j) - (wb + j);
            ICPY(iv, wv + j * c, wf);
            iv += wf;
            RZ(q = jtapv(jt, k, j, 1L));
            SPB(wq, i, jtfrom(jt, q, wy1));
            SPB(wq, x, jtfrom(jt, q, wx));
            RZ(zv[i] = jtincorp(jt, CALL1(f1, ww, fs)));
            j += k;
        }
        RZ(z = jtope(jt, z));
    } else {
        RZ(zi = jtca(jt, wy));
        RZ(z = jtrank1ex(jt, wx, fs, mr, f1));
        RZ(ze = CALL1(f1, SPA(wp, e), fs));
    }  // mr is 0
    z = jtsprz(jt, z, zi, ze, wf, ws);
    EPILOG(z);
} /* f"r w on sparse arrays */

static I
jtspradv(J jt, I n, B* b, I f, I r, I j, P* p, A* z) {
    A s, x;
    I k;
    P* q;
    k = n ? 1 + (B*)memchr(b + j, C1, n - j) - (b + j) : 0;
    x = SPA(p, x);
    if (r) {
        q = PAV(*z);
        RZ(s = jtapv(jt, k, j, 1L));
        SPB(q, i, jtfrom(jt, s, jtdropr(jt, f, SPA(p, i))));
        SPB(q, x, jtfrom(jt, s, x));
    } else
        RZ(*z = AN(x) ? jtfrom(jt, jtsc(jt, j), x) : jtca(jt, SPA(p, e)));
    return k;
} /* advance to the next cell */

static A
jtsprank2_0w(J jt, A a, A w, A fs, AF f2, I wf, I wcr) {
    PROLOG(0044);
    A we, ww, y, z, zi, *zv;
    B* wb;
    I f, *iv, j, *v, wc, wj, wk, wm, wn, *ws, wt, *wv;
    P* wp;
    f  = wf;
    ws = AS(w);
    RZ(w = jtsprarg(jt, wf, w));
    wt = AT(w);
    wp = PAV(w);
    y  = SPA(wp, i);
    v  = AS(y);
    wn = v[0];
    wc = v[1];
    wv = AV(y);
    RZ(wb = jtspredge(jt, y, wf, &wm));
    RZ(ww = jtsprinit(jt, wf, wcr, ws, wt, wp));
    RZ(we = wcr ? jtca(jt, ww) : SPA(wp, e));
    GATV0(z, BOX, MAX(1, wm), 1);
    zv = AAV(z);
    GATV0(zi, INT, f * MAX(1, wm), 2);
    iv   = AV(zi);
    v    = AS(zi);
    v[0] = wm;
    v[1] = f;
    RE(wj = wk = jtspradv(jt, wn, wb, wf, wcr, 0L, wp, &ww));
    j = 0;
    while (1) {
        ICPY(iv, wv, f);
        iv += f;
        RZ(zv[j++] = jtincorp(jt, CALL2(f2, a, ww, fs)));
        if (wj == wn) break;
        wv += wk * wc;
        RE(wk = jtspradv(jt, wn, wb, wf, wcr, wj, wp, &ww));
        wj += wk;
    }
    RZ(z = jtope(jt, z));
    AS(z)[0] = wm;  // we did one cell of aa to get the shape, but now we have to set back to correct # indexes
    z        = jtsprz(jt, z, zi, CALL2(f2, a, we, fs), f, ws);
    EPILOG(z);
}

static A
jtsprank2_a0(J jt, A a, A w, A fs, AF f2, I af, I acr) {
    PROLOG(0045);
    A aa, ae, y, z, zi, *zv;
    B* ab;
    I f, *iv, j, *v, ac, aj, ak, am, an, *as, at, *av;
    P* ap;
    f  = af;
    as = AS(a);
    RZ(a = jtsprarg(jt, af, a));
    at = AT(a);
    ap = PAV(a);
    y  = SPA(ap, i);
    v  = AS(y);
    an = v[0];
    ac = v[1];
    av = AV(y);
    RZ(ab = jtspredge(jt, y, af, &am));
    RZ(aa = jtsprinit(jt, af, acr, as, at, ap));
    RZ(ae = acr ? jtca(jt, aa) : SPA(ap, e));
    GATV0(z, BOX, MAX(1, am), 1);
    zv = AAV(z);
    GATV0(zi, INT, f * MAX(1, am), 2);
    iv   = AV(zi);
    v    = AS(zi);
    v[0] = am;
    v[1] = f;
    RE(aj = ak = jtspradv(jt, an, ab, af, acr, 0L, ap, &aa));
    j = 0;
    while (1) {
        ICPY(iv, av, f);
        iv += f;
        RZ(zv[j++] = jtincorp(jt, CALL2(f2, aa, w, fs)));
        if (aj == an) break;
        av += ak * ac;
        RE(ak = jtspradv(jt, an, ab, af, acr, aj, ap, &aa));
        aj += ak;
    }
    RZ(z = jtope(jt, z));
    AS(z)[0] = am;  // we did one cell of aa to get the shape, but now we have to set back to correct # indexes
    z        = jtsprz(jt, z, zi, CALL2(f2, ae, w, fs), f, as);
    EPILOG(z);
}

A
jtsprank2(J jt, A a, A w, A fs, I lr, I rr, AF f2) {
    PROLOG(0046);
    A aa, ae, we, ww, y, zi, z, *zv;
    B *ab, b, *wb;
    I ac, acr, af, aj, ak, am, an, ar, *as, at, *av, d, f, g, *ii, *iv, j, k, m, s, *u, *v, wc, wcr, wf, wj, wk, wm, wn,
      wr, *ws, wt, *wv;
    P *ap, *wp;
    STACKCHKOFL
    ar = AR(a);
    as = AS(a);
    efr(acr, ar, lr);
    af = ar - acr;
    wr = AR(w);
    ws = AS(w);
    efr(wcr, wr, rr);
    wf = wr - wcr;
    if (!af && !wf) return CALL2(f2, a, w, fs);
    DO(af, ASSERT(as[i] != 0, EVNONCE););
    DO(wf, ASSERT(ws[i] != 0, EVNONCE););
    if (!af) return jtsprank2_0w(jt, a, w, fs, f2, wf, wcr);
    if (!wf) return jtsprank2_a0(jt, a, w, fs, f2, af, acr);
    f = MIN(af, wf);
    g = MAX(af, wf);
    m = 1;
    if (f < g) {
        d = g - f;
        RZ(y = jtodom(jt, 2L, d, f + (af < wf ? ws : as)));
        ii = AV(y);
        m  = *AS(y);
    }
    ASSERT(!ICMP(as, ws, f), EVLENGTH);
    RZ(a = jtsprarg(jt, af, a));
    at = AT(a);
    ap = PAV(a);
    RZ(w = jtsprarg(jt, wf, w));
    wt = AT(w);
    wp = PAV(w);
    y  = SPA(ap, i);
    v  = AS(y);
    an = v[0];
    ac = v[1];
    av = an ? AV(y) : 0;
    RZ(ab = jtspredge(jt, y, af, &am));
    y  = SPA(wp, i);
    v  = AS(y);
    wn = v[0];
    wc = v[1];
    wv = wn ? AV(y) : 0;
    RZ(wb = jtspredge(jt, y, wf, &wm));
    RZ(aa = jtsprinit(jt, af, acr, as, at, ap));
    RZ(ae = acr ? jtca(jt, aa) : SPA(ap, e));
    RZ(ww = jtsprinit(jt, wf, wcr, ws, wt, wp));
    RZ(we = wcr ? jtca(jt, ww) : SPA(wp, e));
    b = af < wf;
    j = am * (af < wf ? m : 1) + wm * (af > wf ? m : 1);
    GATV0(z, BOX, MAX(1, j), 1);
    zv = AAV(z);
    GATV0(zi, INT, MAX(1, j) * g, 2);
    v    = AS(zi);
    v[0] = j;
    v[1] = g;
    iv   = AV(zi);
    RE(aj = ak = jtspradv(jt, an, ab, af, acr, 0L, ap, &aa));
    RE(wj = wk = jtspradv(jt, wn, wb, wf, wcr, 0L, wp, &ww));
    j = s = k = 0;
    u         = ii;
    y         = 0;
    v         = 0;
    if (af == wf)
        while (av || wv) {
            if (av && wv) DO(f, if (s = av[i] - wv[i]) break;) else s = av ? -1 : 1;
            if (0 == s) {
                RZ(zv[j++] = jtincorp(jt, CALL2(f2, aa, ww, fs)));
                ICPY(iv, av, f);
                iv += g;
            } else if (0 > s) {
                RZ(zv[j++] = jtincorp(jt, CALL2(f2, aa, we, fs)));
                ICPY(iv, av, f);
                iv += g;
            } else if (0 < s) {
                RZ(zv[j++] = jtincorp(jt, CALL2(f2, ae, ww, fs)));
                ICPY(iv, wv, f);
                iv += g;
            }
            if (0 >= s) {
                if (aj == an)
                    av = 0;
                else {
                    av += ak * ac;
                    RE(ak = jtspradv(jt, an, ab, af, acr, aj, ap, &aa));
                    aj += ak;
                }
            }
            if (0 <= s) {
                if (wj == wn)
                    wv = 0;
                else {
                    wv += wk * wc;
                    RE(wk = jtspradv(jt, wn, wb, wf, wcr, wj, wp, &ww));
                    wj += wk;
                }
            }
        }
    else
        while (av || wv) {
            if (av && wv && f) DO(f, if (s = av[i] - wv[i]) break;) else s = !f ? 0 : av ? -1 : 1;
            if (b && 0 < s || !b && 0 > s) {
                RZ(zv[j++] = jtincorp(jt, CALL2(f2, b ? ae : aa, b ? ww : we, fs)));
                ICPY(iv, b ? wv : av, g);
                iv += g;
                k = m;
            } else if (s) {
                DQ(m, RZ(zv[j++] = y = jtincorp(jt, y ? jtca(jt, y) : CALL2(f2, b ? aa : ae, b ? we : ww, fs)));
                   ICPY(iv, b ? av : wv, f);
                   ICPY(iv + f, u, d);
                   iv += g;
                   u += d;);
                u = ii;
                y = 0;
                v = 0;
            } else {
                while (ICMP(f + (b ? wv : av), u, d)) {
                    RZ(zv[j++] = y = jtincorp(jt, y ? jtca(jt, y) : CALL2(f2, b ? aa : ae, b ? we : ww, fs)));
                    ICPY(iv, wv, f);
                    ICPY(iv + f, u, d);
                    iv += g;
                    u += d;
                    ++k;
                }
                RZ(zv[j++] = jtincorp(jt, CALL2(f2, aa, ww, fs)));
                ICPY(iv, b ? wv : av, g);
                iv += g;
                u += d;
                ++k;
            }
            if (b && 0 <= s)
                if (wj == wn)
                    wv = v = 0;
                else {
                    v = wv;
                    wv += wk * wc;
                    RE(wk = jtspradv(jt, wn, wb, wf, wcr, wj, wp, &ww));
                    wj += wk;
                }
            else if (!b && 0 >= s)
                if (aj == an)
                    av = v = 0;
                else {
                    v = av;
                    av += ak * ac;
                    RE(ak = jtspradv(jt, an, ab, af, acr, aj, ap, &aa));
                    aj += ak;
                }
            if (b && (!s && !wv || v && ICMP(v, wv, f)) || !b && (!s && !av || v && ICMP(v, av, f))) {
                DQ(m - k, RZ(zv[j++] = y = jtincorp(jt, y ? jtca(jt, y) : CALL2(f2, b ? aa : ae, b ? we : ww, fs)));
                   ICPY(iv, b ? av : wv, f);
                   ICPY(iv + f, u, d);
                   iv += g;
                   u += d;);
                u = ii;
                y = 0;
                k = 0;
            }
            if (b && 0 >= s && (!v || ICMP(v, wv, f)))
                if (aj == an)
                    av = 0;
                else {
                    av += ak * ac;
                    RE(ak = jtspradv(jt, an, ab, af, acr, aj, ap, &aa));
                    aj += ak;
                }
            else if (!b && 0 <= s && (!v || ICMP(v, av, f)))
                if (wj == wn)
                    wv = 0;
                else {
                    wv += wk * wc;
                    RE(wk = jtspradv(jt, wn, wb, wf, wcr, wj, wp, &ww));
                    wj += wk;
                }
        }
    AN(z) = *AS(z) = *AS(zi) = j;
    AN(zi)                   = j * g;
    z                        = jtsprz(jt, jtope(jt, z), zi, CALL2(f2, ae, we, fs), g, g == af ? as : ws);
    EPILOG(z);
} /* a f"r w on sparse arrays */
