...
#define ZZFLAGWORD state
    ...
    // include its rank in the loop here, if its rank is not less than the outer rank (we would simply ignore it), but
    // we don't bother.  If its rank is smaller we can't ignore it because assembly might affect the order of fill.  But
    // if f is BOXATOP, there will be no fill, and we can safely use the smaller rank
    if ((FAV(fs)->flag2 & VF2BOXATOP1) != 0) {
        I mr = FAV(fs)->mr;
        rr   = rr < mr ? rr : mr;
        state |= (FAV(fs)->flag2 & VF2BOXATOP1) >>
                 (VF2BOXATOP1X -
                  ZZFLAGBOXATOPX);  // If this is BOXATOP, set so for loop.  Don't touch fs yet, since we might not loop
        state &= ~((FAV(fs)->flag2 & VF2ATOPOPEN1) >>
                   (VF2ATOPOPEN1X - ZZFLAGBOXATOPX));  // We don't handle &.> here; ignore it
        // if we are using the BOXATOP from f, we can also use the raze flags.  Set these only if BOXATOP to prevent us
        // from incorrectly marking the result block as having uniform items if we didn't go through the assembly loop
        // here
    ...
    if (mn) {
        ...
        fauxblock(virtwfaux);
        // if the original block was direct inplaceable, make the virtual block inplaceable.  (We can't do this for
        // indirect blocks because a virtual block is not marked recursive - rather it increments the usecount of the
        // entire backing block - and modifying the virtual contents would leave the usecounts invalid since the backing
        // block is always recursive (having been ra'd).  Maybe could do this if it isn't?) Don't pass WILLOPEN status -
        // we use that at this level
        fauxvirtual(virtw, virtwfaux, w, rr, ACUC1) MCISH(AS(virtw), AS(w) + wf, rr);
        AN(virtw) = wcn;
        // Calculate inplaceability.  Since the inplaceability of f1 was passed into f"r, we don't need to look it up:
        // we just pass the original jtinplace through, except that we remove WILLOPEN status which we are picking up at
        // this level
        jtinplace = (J)((I)jtinplace & ~(JTWILLBEOPENED + JTCOUNTITEMS));
        // Mark the virtual block as inplaceable only if w is fully inplaceable.  We have to turn of inplaceability in
        // the virtual block so that a non-inplaceable value might cause PRISTINE to be set.  We also require the type
        // to be right, with some allowances for &.>
        ...
        // mark the virtual block inplaceable; this will be ineffective unless the original w was direct inplaceable,
        // and inplacing is allowed by u
#define ZZDECL
#include "result.h"
        ZZPARMS(wf, mn, 1)
#define ZZINSTALLFRAME(optr) MCISHd(optr, AS(w), wf)
        for (i0 = mn; i0; --i0) {
            AC(virtw) =
              ACUC1 + ((state & ZZFLAGVIRTWINPLACE)
...
#include "result.h"
            ...
            // advance input pointer for next cell.  We keep the same virtual block because it can't be incorporated
            // into anything
            AK(virtw) += wk;
        }
...
#define ZZFLAGWORD state
    ...
    // Look for the forms we handle specially: <@:f (not here where rank=0)  <@f  f@>   and their combinations  <@(f@>)
    // f&.> (<@:f)@>  but not as combinations  (<@f)@> (unless f has rank _) <@:(f@>)   also using &
    ...
    if ((AN(w)) != 0) {  // if no cells, go handle fill before we advance over flags
        ...
        // RANKONLY verbs contain an invalid f1 pointer (it was used to get to a call to here).  We have to step over
        // the RANKONLY to get to what we can execute
        ...
        while (1) {  // loop collecting ATOPs
            I fstate = (FAV(fs)->flag2 & (VF2BOXATOP1 | VF2ATOPOPEN1)) >>
                       (VF2BOXATOP1X - ZZFLAGBOXATOPX);  // extract <@ and @> status bits from f
            if ((((fstate & state) - 1) & -fstate) >= 0)
                break;  // If this f overlaps with old, or it's not just a flag node, we have to stop
            if (fstate & ZZFLAGATOPOPEN1) {
                ...
                // <@: <@ <& <&:
                // Because the outermost rank is 0, <@f by itself is OK; but later, as in (<@f)@>, it is not.  <@:f is.
                // So check for infinite rank
                if (state & ZZFLAGATOPOPEN1 && FAV(fs)->mr < RMAX) break;  // not first, and not infinite rank: ignore
                // Advance fs to the g of <@g
                fs = FAV(fs)->fgh[1 + ((FAV(fs)->flag2 >> VF2ISCCAPX) & 1)];
                f1 = FAV(fs)->valencefns[0];
            }
        ...
        A *wav;  // virtwk is offset of virtual block/pointer to next box
                 // Normal case where there are cells.
        // RANKONLY verbs were handled in the caller to this routine, but fs might be RANKATOP.  In that case we could
        // include its rank in the loop here, if its rank is not less than the outer rank (we would simply ignore it),
        // but we don't bother.  If its rank is smaller we can't ignore it because assembly might affect the order of
        // fill.  But if f is BOXATOP, there will be no fill, and we can safely use the smaller rank
#define ZZDECL
#include "result.h"
        ZZPARMSNOFS(AR(w), mn)
        // if we are using the BOXATOP from f, we can also use the raze flags from the caller.  Set these only if
        // BOXATOP to prevent us from incorrectly marking the result block as having uniform items if we didn't go
        // through the assembly loop here
        ...
        // Allocate a non-in-place virtual block unless this is ATOPOPEN and w is boxed, in which case we will just use
        // the value of the A block
        fauxblock(virtwfaux);
        if ((SGNIF(state, ZZFLAGATOPOPEN1X) & SGNIF(AT(w), BOXX)) >= 0) {  // if not (ATOPOPEN and w boxed)...
            fauxvirtual(virtw, virtwfaux, w, 0, ACUC1);
            AN(virtw) = 1;
            state &= ~ZZFLAGATOPOPEN1;
            // Mark the virtual block as inplaceable only if w is fully inplaceable.  We have to turn of inplaceability
            // in the virtual block so that a non-inplaceable value might cause PRISTINE to be set.  We also require the
            // type to be right, with some allowances for &.>
            state |= (UI)(SGNIF((I)jtinplace, JTINPLACEWX) & ~((AT(w) & TYPEVIPOK) - (f1 != jteveryself)) & AC(w)) >>
                     (BW - 1 - ZZFLAGVIRTWINPLACEX);  // requires JTINPLACEWX==0.  Single flag bit
            // Init the inplaceability of virtw.  We do this here because in the loop we handle it only for low rank (i.
            // e. virtwfaux) so as to avoid inplacing ATOPOPEN. Thus, for higher rank we set it only this once.  It will
            // stay right unless it gets virtualed
...
#include "result.h"
            ...
            AC((A)virtwfaux) =
              ACUC1 + ((state & ZZFLAGVIRTWINPLACE)
                       << (ACINPLACEX -
                           ZZFLAGVIRTWINPLACEX));  // in case we created a virtual block from it, restore inplaceability
                                                   // to the UNINCORPABLE block for simplicity we apply this correction
                                                   // to the faux block, so as to ensure we don't do it when ATOPOPEN is
                                                   // active.  This means ranks higher than the max for the faux virtual
                                                   // block miss out on the reinitialization.  That's not so bad
            // advance input pointer for next cell.  We keep the same virtual block because it can't be incorporated
            // into anything
            if (!(state & ZZFLAGATOPOPEN1)) {
                AK(virtw) += wk;
            } else {
                virtw = *wav++;
            }
...
#include "result.h"
        ...
        if (!(FAV(fs)->flag2 & VF2BOXATOP1)) {
            ...
            // If we are executing a BOXATOP on a single cell, we know the result is going to be an atomic box.  We
            // don't bother executing the verb at all then. jmf.ijs unknowingly takes advantage of this fact, and would
            // crash if executed on an empty cell
            z = ds(CACE);  // cell 'returned' a:
        }
        GA(zz, AT(z), 0L, AR(w) + AR(z), 0L);
        zzs = AS(zz);
        MCISH(zzs, AS(w), AR(w));
        MCISH(zzs + AR(w), AS(z), AR(z));
    }
...
jtrank2ex(J jt, AD *RESTRICT a, AD *RESTRICT w, A fs, UI lrrrlcrrcr, AF f2) {
    ...
    if (!(a && w)) return 0;
    if ((UI)lrrr == (((UI)AR(a) << RANKTX) + AR(w))) {
    ...
    if (((AT(a) | AT(w)) & SPARSE) != 0)
        ...
        // being repeated; outercellct gives the number of (below lr) cells that are processed before an outer
        // repetition. The two repeats can be for either argument independently, depending on which frame is shorter.
...
#define ZZFLAGWORD state
    ...
    state |= (SGNTO0(-AN(a)) << STATEANOTEMPTYX) | (SGNTO0(-AN(w)) << STATEWNOTEMPTYX) |
             ((SGNTO0(-AN(a)) & SGNTO0(-AN(w))) << STATEAWNOTEMPTYX);
    // RANKONLY verbs were handled in the caller to this routine, but fs might be RANKATOP.  In that case we can include
    // its rank in the loop here, which will save loop setups
    if ((UI)(FAV(fs)->flag2 & (VF2RANKATOP2 | VF2BOXATOP2)) >
        (UI)REPSGN(lrrr - lcrrcr)) {  // if there are new ranks to include, provided ?r=?cr
        I lr = (UI)lrrr >> RANKTX, rr = lrrr & RANKTMSK;
        I t = (I)lr(fs);
        lr  = t < lr ? t : lr;
        t   = (I)rr(fs);
        rr  = t < rr ? t : rr;  // get the ranks if we accept the new cell
        state |= (FAV(fs)->flag2 & VF2BOXATOP2) >>
                 (VF2BOXATOP2X -
                  ZZFLAGBOXATOPX);  // If this is BOXATOP, set so for loop.  Don't touch fs yet, since we might not loop
        state &= ~((FAV(fs)->flag2 & VF2ATOPOPEN2W) >>
                   (VF2ATOPOPEN2WX - ZZFLAGBOXATOPX));  // We don't handle &.> here; ignore it
        // if we are using the BOXATOP from f, we can also use the raze flags.  Set these only if BOXATOP to prevent us
        // from incorrectly marking the result block as having uniform items if we didn't go through the assembly loop
        // here
        state |= (-state) & (I)jtinplace & (JTWILLBEOPENED | JTCOUNTITEMS);
    ...
    if ((I)((lrrr - lcrrcr) & (-((aofwof >> RANKTX) ^ (aofwof & RANKTMSK)))) >= 0) {
        los = 0;
        lof = aof = wof = 0;
        outerframect = outerrptct = 1;  // no outer frame unless it's needed
    } else {
        // outerframect is the number of cells in the shorter frame; outerrptct is the number of cells in the residual
        // frame find smaller/larger frame/shape, and indicate if a is the repeated arg (otherwise we assume w)
        wof = aofwof & RANKTMSK;
        aof = aofwof >> RANKTX;
        lof = aof;
        sof = wof;
        los = AS(a);
        lof = aof - wof < 0 ? wof : lof;
        sof = aof - wof < 0 ? aof : sof;
    ...
    {
        I ea = (afwf >> RANKTX) - aof, ew = (afwf & RANKTMSK) - wof;
        sif = lif = ea;
        lif       = ea - ew <= 0 ? ew : lif;
        sif       = ea - ew <= 0 ? sif : ew;  // lif=long inner frame, sif=short
        state |= (ea - ew) &
                 STATEINNERREPEATA;  // if w is longer, indicate repeating a.  The flag bit is far above the max rank
        lis    = AS(w) + wof;
        I *sis = AS(a) + aof;       // start out as the two inner shapes lis=w, sis=a
    ...
    if (innerrptct == 1) {
        innerrptct   = innerframect;
        innerframect = 1;
        state &= ~(STATEINNERREPEATW | STATEINNERREPEATA);
    }  // if only one loop needed, make it the inner, with no repeats

    // Get size of each argument cell in atoms.  If this overflows, there must be a 0 in the frame, & we will have
    // gone through the fill path (& caught the overflow)
    PROD(acn, (UI)lrrr >> RANKTX, AS(a) + (afwf >> RANKTX));
    PROD(wcn, lrrr & RANKTMSK, AS(w) + (afwf & RANKTMSK));
    // ?cn=number of atoms in a cell, ?k=#bytes in a cell
    ak = acn << bplg(AT(a));  // reshape below will catch any overflow
    wk = wcn << bplg(AT(w));

    // See how many cells are going to be in the result
    DPMULDE(outerframect, outerrptct, mn)
    DPMULDE(innerframect, mn, mn) DPMULDE(innerrptct, mn, mn)

      // allocate the virtual blocks that we will use for the arguments, and fill in the shape of a cell of each
      // The base pointer AK advances through the source argument.  But if an operand is empty (meaning that there are
      // no output cells), replace any empty operand with a cell of fills.  (Note that operands can have no atoms and
      // yet the result can have nonempty cells, if the cells are empty but the frame does not contain 0)
      //
      // If an arg is pristine, any cell of it must ipso facto be pristine, as long as it in inplaceable (& therefore
      // not repeated here)
      //
      // See which arguments we can inplace.  The key is that they have to be not repeated.  This means outerrptct=1,
      // and the specified argument not repeated in the inner loop.  Also, a and w mustn't be the same block (one cannot
      // be a virtual of the other unless the backer's usecount disables inplacing)
      fauxblock(virtwfaux);
    fauxblock(virtafaux);
    if ((mn | (state & STATEANOTEMPTY)) != 0) {
        // OK to inplace an arg if it's not the same as the other, not repeated, correct type (unless &.>), inplaceable
        // usecount
        state |= (UI)(SGNIF((a != w) & (outerrptct == 1), 0) & SGNIF((I)jtinplace, JTINPLACEAX) & AC(a) &
                      ~(((AT(a) & TYPEVIPOK) - (f2 != jtevery2self)) | SGNIF(state, STATEINNERREPEATAX))) >>
                 (BW - 1 - ZZFLAGVIRTAINPLACEX);  // requires JTINPLACEWX==0.  Single flag bit  sign=0 if (VIPOK or &.>)
        fauxvirtual(virta, virtafaux, a, (UI)lrrr >> RANKTX, ACUC1)
          MCISH(AS(virta), AS(a) + (afwf >> RANKTX), (UI)lrrr >> RANKTX);
        AN(virta) = acn;
        // Init the inplaceability of virtw.  We do this here because in the loop we handle it only for low rank (i. e.
        // virt[aw]faux) so as to avoid inplacing fill. Thus, for higher rank we set it only this once.  It will stay
    ...
    if ((mn | (state & STATEWNOTEMPTY)) != 0) {  // repeat for w
        state |= (UI)(SGNIF((a != w) & (outerrptct == 1), 0) & SGNIF((I)jtinplace, JTINPLACEWX) & AC(w) &
                      ~(((AT(w) & TYPEVIPOK) - (f2 != jtevery2self)) | SGNIF(state, STATEINNERREPEATWX))) >>
                 (BW - 1 - ZZFLAGVIRTWINPLACEX);  // requires JTINPLACEWX==0.  Single flag bit  sign=0 if (VIPOK or &.>)
        fauxvirtual(virtw, virtwfaux, w, lrrr & RANKTMSK, ACUC1)
          MCISH(AS(virtw), AS(w) + (afwf & RANKTMSK), lrrr & RANKTMSK);
        AN(virtw) = wcn;
        AC(virtw) = ACUC1 + ((state & ZZFLAGVIRTWINPLACE) << (ACINPLACEX - ZZFLAGVIRTWINPLACEX));
    } else {
    ...
    A zz = 0;  // place where we will build up the homogeneous result cells
    if (mn != 0) {
        I i0, i1, i2, i3;  // likely on single word fails
                           // Normal case where there are cells.
            ...
            for (i1 = outerrptct; i1; --i1) {  // make MOVEY? post-increment
                ...
                for (i2 = innerframect; i2; --i2) {
                    ...
                    for (i3 = innerrptct; i3; --i3) {
                        AC((A)virtafaux) =
                          ACUC1 +
                          ((state & ZZFLAGVIRTAINPLACE)
                           << (ACINPLACEX - ZZFLAGVIRTAINPLACEX));  // in case we created a virtual block from it,
                                                                    // restore inplaceability to the UNINCORPABLE block
                                                                    // - only if faux (thus not filler)
...
#include "result.h"
                        ...
                        // advance input pointers for next cell.  We keep the same virtual blocks because they can't be
                    ...
                    // advance input pointers for next cell.  We increment any block that was being held constant in the
                    // inner loop.  There can be only one such.  Such an arg is never inplaced
                    if ((state & STATEINNERREPEATA) != 0)
                        AK(virta) += ak;
                    else if ((state & STATEINNERREPEATW) != 0)
                        AK(virtw) += wk;
                }
            }
        }
...
#include "result.h"
        ...
        zzs = AS(zz);
        MCISH(zzs, los, lof);
        MCISH(zzs + lof, lis, lif);
        MCISH(zzs + lof + lif, AS(z), AR(z));
    }
...
#define ZZFLAGWORD state
    ...
    // Calculate strides for inner and outer loop.  Cell-size is known to be 1 atom.  The stride of the inner loop is 1
    // atom, except for a repeated value, of which there can be at most 1.  For a repeated value, we set the stride to 0
    // and remember the repetition count and stride
    ak = bpnoun(AT(a));
    wk = bpnoun(AT(w));  // stride for 1 atom
    if (ar >= wr) {      // a will be the long frame
        ...
        if (ar == wr) {  // no surplus frame: common case
            ict = mn;
            oct = 0;  // leave frames as is, set loop counters
        } else {
            PROD(oct, wr, as);
            PROD(ict, ar - wr, as + wr);
            wr = wk;
            wk = 0;  // set repeat counts, outer/inner strides
        }
    } else {
        mn = AN(w);
        PROD(oct, ar, ws);
        PROD(ict, wr - ar, ws + ar);
        as = ws;
        ar = wr;
        wr = ak;
        ak = 0;  // set repeat counts, outer/inner strides
    }
    ...
    // Look for the forms we handle specially: <@:f (not here where rank=0)  <@f  f@>   and their combinations  <@(f@>)
    // f&.> (<@:f)@>  but not as combinations  (<@f)@> (unless f has rank _) <@:(f@>)   also using & For the nonce, we
    // assume that VF2ATOPOPEN2A and VF2ATOPOPEN2W are always the same
    I state = ZZFLAGINITSTATE;
    ...
    if (mn != 0) {  // likely fails on single word
        // Collect flags <@ and @> from the nodes.  It would be nice to do this even on empty arguments, but that would
        // complicate our job in coming up with a fill-cell or argument cell, because we would have to keep track of
        // whether we passed an ATOPOPEN.  But then we could avoid executing the fill cell any time the is a BOXATOP,
        // even down the stack.  As it is, the only time we elide the execution is when BOXATOP occurs at the first
        // node, i.e. for an each that is not boxed

        // RANKONLY verbs contain an invalid f1 pointer (it was used to get to a call to here).  We have to step over
        // the RANKONLY to get to what we can execute
        while (FAV(fs)->flag2 & VF2RANKONLY2) {
            fs = FAV(fs)->fgh[0];
            f2 = FAV(fs)->valencefns[1];
        }

        while (1) {  // loop collecting ATOPs
            I fstate = (FAV(fs)->flag2 & (VF2BOXATOP2 | VF2ATOPOPEN2A | VF2ATOPOPEN2W)) >>
                       (VF2BOXATOP2X - ZZFLAGBOXATOPX);  // extract <@ and @> status bits from f
            if ((((fstate & state) - 1) & -fstate) >= 0)
                break;  // If this f overlaps with old, or it's not just a flag node, we have to stop
        ...
        if ((SGNIF(state, ZZFLAGATOPOPEN2WX) & SGNIF(AT(w), BOXX)) >= 0) {
            fauxvirtual(virtw, virtwfaux, w, 0, ACUC1);
            AN(virtw) = 1;
            state &= ~ZZFLAGATOPOPEN2W;
        } else {
            wav   = AAV(w);
            virtw = *wav;
        }
        if ((SGNIF(state, ZZFLAGATOPOPEN2AX) & SGNIF(AT(a), BOXX)) >= 0) {
            fauxvirtual(virta, virtafaux, a, 0, ACUC1);
            AN(virta) = 1;
            state &= ~ZZFLAGATOPOPEN2A;
        } else {
            aav   = AAV(a);
            virta = *aav;
        }
...
#include "result.h"
                ...
                if (!(state & ZZFLAGATOPOPEN2A)) {
                    AK(virta) += ak;
                } else {
                    aav   = (A *)((I)aav + ak);
                    virta = *aav;
                }
                if (!(state & ZZFLAGATOPOPEN2W)) {
                    AK(virtw) += wk;
                } else {
                    wav   = (A *)((I)wav + wk);
                    virtw = *wav;
...
#include "result.h"
        ...
        if (!(FAV(fs)->flag2 & VF2BOXATOP2)) {
            ...
            // If we are executing a BOXATOP on a single cell, we know the result is going to be an atomic box.  We
            // don't bother executing the verb at all then.
            z = ds(CACE);  // cell 'returned' a:
        }
        GA(zz, AT(z), 0L, ar + AR(z), 0L);
        zzs = AS(zz);
        MCISH(zzs, as, ar);
        MCISH(zzs + ar, AS(z), AR(z));  // allocate result, copy frame and shape
    }
...
A
jtirs1(J jt, A w, A fs, I m, AF f1) {
    A z;
    I wr;
    FPREFIP;
    ...
    wr += m;
    wr        = wr < 0 ? 0 : wr;
    wr        = m >= 0 ? m : wr;  // requested rank, after negative resolution, or ~0
...
A
jtirs2(J jt, A a, A w, A fs, I l, I r, AF f2) {
    A z;
    I ar, wr;
    FPREFIP;
    wr = AR(w);
    ...
    wr = wr < 0 ? 0 : wr;
    wr = r >= 0 ? r : wr;
    r = AR(w) - wr;  // wr=requested rank, after negative resolution, or ~0; r=frame of w, possibly negative if no frame
    ar = AR(a);
    l  = l >= ar ? (RANKT)~0 : l;
    ar += l;
    ar = ar < 0 ? 0 : ar;
    ar = l >= 0 ? l : ar;
    l = AR(a) - ar;  // ar=requested rank, after negative resolution, or ~0; l=frame of a, possibly negative if no frame
    l = MIN(r, l);
    l = l < 0 ? 0 : l;                          // get length of frame
...
cons2(J jt, A a, A w, A self) {
    V *sv = FAV(self);
    I lr2, rr2;
    efr(lr2, AR(a), (I)sv->localuse.lI4[1]);
    efr(rr2, AR(w), (I)sv->localuse.lI4[2]);
...
cycr2(J jt, A a, A w, A self) {
    ...
    efr(rr2, AR(w), (I)sv->localuse.lI4[2]);
    return rank2ex(
      a, w, self, lr2, rr2, lr2, rr2, FAV(self)->valencefns[1]);  // callback is to the cyclic-execution function
}

// Handle u"n y where u supports irs.  Since the verb may support inplacing even with rank (,"n for example), pass that
// through.
static A
rank1i(J jt, A w, A self) {
    FPREFIP;
    DECLF;  // this version when requested rank is positive
    I m       = sv->localuse.lI4[0];
    m         = m >= AR(w) ? ~0 : m;
    jt->ranks = (RANK2T)(m);  // install rank for called routine
    A z       = CALL1IP(f1, w, fs);
    jt->ranks = (RANK2T)~0;  // reset rank to infinite
    return z;
}
static A
rank1in(J jt, A w, A self) {
    FPREFIP;
    DECLF;  // this version when requested rank is negative
    I m       = sv->localuse.lI4[0] + AR(w);
    m         = m < 0 ? 0 : m;
    jt->ranks = (RANK2T)(m);  // install rank for called routine
    A z       = CALL1IP(f1, w, fs);
    jt->ranks = (RANK2T)~0;  // reset rank to infinite
...
static A
rank2i(J jt, A a, A w, A self) {
    FPREFIP;
    DECLF;  // this version when requested rank is positive
    I ar = sv->localuse.lI4[1];
    ar   = ar >= AR(a) ? (RANKT)~0 : ar;
    I af = AR(a) - ar;  // left rank
    I wr = sv->localuse.lI4[2];
    wr   = wr >= AR(w) ? (RANKT)~0 : wr;
    I wf = AR(w) - wr;  // right rank
    af   = wf < af ? wf : af;
    af   = af < 0 ? 0 : af;
...
static A
rank2in(J jt, A a, A w, A self) {
    FPREFIP;
    DECLF;  // this version when a requested rank is negative
    I wr = AR(w);
    I r  = sv->localuse.lI4[2];
    r    = r >= wr ? (RANKT)~0 : r;
    wr += r;
    wr   = wr < 0 ? 0 : wr;
    wr   = r >= 0 ? r : wr;
...
static A
rank1(J jt, A w, A self) {
    DECLF;
    I m, wr;
    wr = AR(w);
    efr(m, wr, (I)sv->localuse.lI4[0]);
    // We know that the first call is RANKONLY, and we consume any other RANKONLYs in the chain until we get to
    // something else.  The something else becomes the fs/f1 to rank1ex.  Until we can handle multiple fill
    // neighborhoods, we mustn't consume a verb of lower rank
    while (FAV(fs)->flag2 & VF2RANKONLY1) {
        I hm = FAV(fs)->localuse.lI4[0];
        efr(hm, m, hm);
        if (hm < m) break;  // if new rank smaller than old, abort
        m  = hm;
        fs = FAV(fs)->fgh[0];
        f1 = FAV(fs)->valencefns[0];
    }
...
static A
jtrank10atom(J jt, A w, A self) {
    A fs = FAV(self)->fgh[0];
    return (FAV(fs)->valencefns[0])(jt, w, fs);
}  // will be used only for no-frame executions.  Otherwise will be replaced by the flags loop
static A
jtrank10(J jt, A w, A self) {
    return jtrank1ex0(jt, w, self, jtrank10atom);
}  // pass inplaceability through.

static A
rank1q(J jt, A w, A self) {  // fast version: nonneg rank, no check for multiple RANKONLY
    I r  = AR(w);
...
// when the ranks are nonnegative and u is not RANKONLY
static A
rank2q(J jt, A a, A w, A self) {
    I ar = AR(a);
    ar   = ar > FAV(self)->localuse.lI4[1] ? FAV(self)->localuse.lI4[1] : ar;
    I wr = AR(w);
    wr   = wr > FAV(self)->localuse.lI4[2] ? FAV(self)->localuse.lI4[2] : wr;
    A fs = FAV(self)->fgh[0];
    return rank2ex(a, w, fs, ar, wr, ar, wr, FAV(fs)->valencefns[1]);
}

static A
rank2(J jt, A a, A w, A self) {
    DECLF;
    I ar, l = sv->localuse.lI4[1], r = sv->localuse.lI4[2], wr;
    ar = AR(a);
    efr(l, ar, l);
    wr = AR(w);
    efr(r, wr, r);
    if (((l - ar) | (r - wr)) < 0) {
        ...
        // We know that the current call is RANKONLY, and we consume any other RANKONLYs in the chain until we get to
        // something else.  The something else becomes the fs/f1 to rank1ex.  We have to stop if the new ranks will not
        // fit in the two slots allotted to them. This may lead to error until we support multiple fill neighborhoods
        while (FAV(fs)->flag2 & VF2RANKONLY2) {
            I hlr = FAV(fs)->localuse.lI4[1];
            I hrr = FAV(fs)->localuse.lI4[2];
            efr(hlr, llr, hlr);
            efr(hrr, lrr, hrr);               // fetch ranks of new verb, resolve negative, clamp against old inner rank
            if ((hlr ^ llr) | (hrr ^ lrr)) {  // if there is a new rank to insert...
                if ((l ^ llr) | (r ^ lrr)) break;  // if lower slot full, exit, we can't add a new one
                llr = hlr;
                lrr = hrr;  // install new inner ranks, where they are new lows
            }
            // either we can ignore the new rank or we can consume it.  In either case pass on to the next one
            fs = FAV(fs)->fgh[0];
            f2 = FAV(fs)->valencefns[1];  // advance to the new function
        }
        return rank2ex(a, w, fs, llr, lrr, l, r, f2);
    } else
        return CALL2(
          f2, a, w, fs);  // pass in verb ranks to save a level of rank processing if not infinite.  Preserves inplacing
}
// Version for rank 0.  Call rank1ex0, pointing to the u"r
static A
jtrank20atom(J jt, A a, A w, A self) {
    A fs = FAV(self)->fgh[0];
    return (FAV(fs)->valencefns[1])(jt, a, w, fs);
}  // will be used only for no-frame executions.  Otherwise will be replaced by the flags loop
static A
jtrank20(J jt, A a, A w, A self) {
    return jtrank2ex0(jt, a, w, self, jtrank20atom);
}  // pass inplaceability through.
...
jtqq(J jt, A a, A w) {
    ...
    if (VERB & AT(w)) {
        ...
        // The floating-list is what we will call the v operand into rank?ex.  It holds the nominal verb ranks which may
        // be negative h is the integer version
        w = t;
    } else {
        ...
        v     = AV(t);
        hv[0] = v[2 == n];
        hv[0] = hv[0] > RMAX ? RMAX : hv[0];
        hv[0] = hv[0] < -RMAX ? -RMAX : hv[0];
        r[0]  = DR(hv[0]);
        hv[1] = v[3 == n];
        hv[1] = hv[1] > RMAX ? RMAX : hv[1];
        hv[1] = hv[1] < -RMAX ? -RMAX : hv[1];
        r[1]  = DR(hv[1]);
        hv[2] = v[n - 1];
        hv[2] = hv[2] > RMAX ? RMAX : hv[2];
        hv[2] = hv[2] < -RMAX ? -RMAX : hv[2];
        r[2]  = DR(hv[2]);
    }
    ...
    if (NOUN & AT(a)) {  // could be gerund"n or noun"n
        ...
        if (((hv[0] ^ RMAX) | (hv[1] ^ RMAX) | (hv[2] ^ RMAX)) && !((AR(a) ^ 1) | (AT(a) & (NOUN & ~BOX))) &&
            (ger = jtfxeachv(jt, 1LL, a))) {
            f1 = cycr1;
            f2 = cycr2;      // process this with the cyclic-gerund routines
            vf = VFLAGNONE;  // the cyclic processor does not inplace or IRS and we don't bother figuring out whether it
                             // is ASGSAFE
        } else {
            ...
            f2 = cons2;
            ACIPNO(
              a);  // use the constant routines for nouns; mark the constant non-inplaceable since it may be reused;
        ...
        // The flags for u indicate its IRS and atomic status.  If atomic (for monads only), ignore the rank, just point
        // to the action routine for the verb.  Otherwise, choose the appropriate rank routine, depending on whether the
        // verb supports IRS.  The IRS verbs may profitably support inplacing, so we enable it for them.
        vf = av->flag & (VASGSAFE | VJTFLGOK1 | VJTFLGOK2);  // inherit ASGSAFE from u, and inplacing
        // For monads: atomic verbs ignore rank, but they require the localuse field, so we can't just point the rank
        // verb at them; we use a passthrough routine instead.  Otherwise, if the verb supports IRS, go to the
        // appropriate routine depending on the sign of rank; otherwise we will be doing an explicit rank loop:
        // distinguish rank-0, quick rank (rank is positive and a is NOT a rankonly type that may need to be combined),
        // and all-purpose cases
        if (av->flag & VISATOMIC1) {
            f1 = jtrank10atom;
        } else {
            if (av->flag & VIRS1) {
                f1 = hv[0] >= 0 ? rank1i : rank1in;
            } else {
                f1 = hv[0] ? (hv[0] >= 0 && !(av->flag2 & VF2RANKONLY1) ? rank1q : rank1) : jtrank10;
                flag2 |= VF2RANKONLY1;
            }
        }
        // For dyad: atomic verbs take the rank from this block, so we take the action routine, and also the parameter
        // it needs; these parameters mean that only nonnegative rank can be accomodated; otherwise, use processor for
        // IRS (there is one for nonnegative, one for negative rank); if not IRS, there are processors for: rank 0;
        // nonneg ranks where fs is NOT a rank operator; general case
        if (av->flag & VFUSEDOK2 && (hv[1] | hv[2]) >= 0) {
            f2 = av->valencefns[1];
        } else if (av->flag & VIRS2) {
            f2 = (hv[1] | hv[2]) >= 0 ? rank2i : rank2in;
        } else {
            f2 = (hv[1] | hv[2]) ? ((hv[1] | hv[2]) >= 0 && !(av->flag2 & VF2RANKONLY2) ? rank2q : rank2) : jtrank20;
            flag2 |= VF2RANKONLY2;
        }
        // Test for special cases
        if (av->valencefns[1] == jtfslashatg && r[1] == 1 && r[2] == 1) {  // f@:g"1 1 where f and g are known atomic
            I isfork = av->id == CFORK;
            if (FAV(FAV(isfork ? av->fgh[1] : av->fgh[0])->fgh[0])->id == CPLUS &&
                FAV(isfork ? av->fgh[2] : av->fgh[1])->id == CSTAR) {
                // +/@:*"1 1 or ([: +/ *)"1 1 .  Use special rank-1 routine.  It supports IRS, but not inplacing
                // (fslashatg didn't inplace either)
                f2 = jtsumattymes1;
                vf |= VIRS2;
                flag2 &= ~VF2RANKONLY2;  // switch to new routine, which supports IRS
            }
        }
    }
