...
A jtrank2ex0(J jt,AD * RESTRICT a,AD * RESTRICT w,A fs,AF f2){F2PREFIP;PROLOG(0042);A virta,virtw,z;
 ...
 if(!(a && w)) return 0; ar=AR(a); wr=AR(w); if(!(ar+wr))return CALL2IP(f2,a,w,fs);   // if no frame, make just 1 call
...
static A rank1i(J jt,    A w,A self){F1PREFIP;if(!w) return 0;DECLF;  // this version when requested rank is positive
 I m=sv->localuse.lI4[0]; m=m>=AR(w)?~0:m; jt->ranks=(RANK2T)(m);  // install rank for called routine
 A z=CALL1IP(f1,w,fs);
 jt->ranks=(RANK2T)~0;  // reset rank to infinite
 return z;
}
static A rank1in(J jt,    A w,A self){F1PREFIP;if(!w) return 0;DECLF;  // this version when requested rank is negative
 I m=sv->localuse.lI4[0]+AR(w); m=m<0?0:m; jt->ranks=(RANK2T)(m);  // install rank for called routine
 A z=CALL1IP(f1,w,fs);
 jt->ranks=(RANK2T)~0;  // reset rank to infinite
 return z;
}
static A rank2i(J jt,A a,A w,A self){F2PREFIP;if(!w) return 0;DECLF;  // this version when requested rank is positive
...
static A rank2in(J jt,A a,A w,A self){F2PREFIP;if(!w) return 0;DECLF;  // this version when a requested rank is negative
