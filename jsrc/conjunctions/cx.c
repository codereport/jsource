...
#define DDSEP \
    0xa  // ASCII value used to mark line separator inside 9 : string.  Must have class CU so that it ends a final
         // comment
...
// sv->h is the A block for the [2][4] array of saved info for the definition; hv->[4] boxes of info for the current
// valence; line-> box 0 - tokens; x->box 1 - A block for control words; n=#control words; cw->array of control-word
// data, a CW struct for each
#define LINE(sv)                             \
    {                                        \
        A x;                                 \
...
// If the sentence passes a u/v into an operator, the current symbol table will become the prev and will have the u/v
// environment info If the sentence fails, we go into debug mode and don't return until the user releases us
#define parseline(z)                                                      \
    {                                                                     \
        C attnval = *jt->adbreakr;                                        \
        A *queue  = line + ci->i;                                         \
        I m       = ci->n;                                                \
        if (!attnval) {                                                   \
            if (!(gsfctdl & 16))                                          \
...
typedef struct {
    I4 d, t, e, b;
} TD;  // line numbers of catchd., catcht., end. and try.
#define WTD (sizeof(TD) / sizeof(I))
#define NTD 17 /* maximum nesting for try/catch */

static B
jtforinit(J jt, CDATA *cv, A t) {
    A x;
    C *s, *v;
    I k;
    ASSERT(t != 0, EVCTRL);
    RZ(ras(t));
    cv->t = t;       /* iteration array     */
    SETIC(t, cv->n); /* # of items in t     */
    cv->j = -1;      /* iteration index     */
    cv->x = 0;
    k     = AN(cv->line) - 5;
    cv->k = (I4)k;           /* length of item name; -1 if omitted (for.; for_. not allowed) */
...
static I
trypopgoto(TD *tdv, I tdi, I dest) {
    while (tdi && !BETWEENC(dest, tdv[tdi - 1].b, tdv[tdi - 1].e))
        --tdi;  // discard stack frame if structure does not include dest
    return tdi;
}
...
#define CHECKNOUN                                                                                                     \
    if (!(NOUN & AT(t))) { /* error, T block not creating noun */                                                     \
        ...
        if (i < SMAX) {                                                                                               \
            ...
            if (gsfctdl & 4) {                                                                                        \
                if (!--tdi) {                                                                                         \
                    jt->uflags.us.cx.cx_c.db = (UC)(gsfctdl >> 8);                                                    \
                    gsfctdl ^= 4;                                                                                     \
                }                                                                                                     \
            }                                                                                                         \
        } else                                                                                                        \
...
// If debug is running we have to check for a new line to run, after any execution with error or on any line in case the
// debugger interrupted something result is line to continue on
static I
debugnewi(I i, DC thisframe, A self) {
    if (thisframe) {
        DC siparent;  // if we can take debug input
        // debug mode was on when this execution started.  See if the execution pointer was changed by debug.
        if ((siparent = thisframe->dclnk) && siparent->dctype == DCCALL &&
            self == siparent->dcf) {                     // if prev stack frame is a call to here
...
jtxdefn(J jt, A a, A w, A self) {
    ...
    I gsfctdl = 0;     // flags: 1=locked 2=debug(& not locked) 4=tdi!=0 8=cd!=0 16=thisframe!=0 32=symtable was the
                       // original (i. e. AR(symtab)&LSYMINUSE) 256=original debug flag (must be highest bit)
    DC callframe = 0;  // pointer to the debug frame of the caller to this function (only if it's named), but 0 if we
                       // are not debugging
    ...
    DC thisframe = 0;                          // if we allocate a parser-stack frame, this is it
    {
        ...
        V *sv  = FAV(self);
        I sflg = sv->flag;  // fetch flags, which are the same even if VXOP is set
        A u, v;             // pointers to args
        // If this is adv/conj, it must be (1/2 : n) executed with no x or y.  Set uv then, and undefine x/y
        u = AT(self) & ADV + CONJ ? a : 0;
        v = AT(self) & ADV + CONJ ? w : 0;
        // saved for next rev   a=AT(self)&ADV+CONJ?0:a; w=AT(self)&ADV+CONJ?0:w;
        if (!(jt->uflags.us.cx.cx_us | (sflg & (VLOCK | VXOP | VTRY1 | VTRY2)))) {
            // Normal case of verbs. Read the info for the parsed definition, including control table and number of
            // lines
            LINE(sv);
            // Create symbol table for this execution.  If the original symbol table is not in use (rank unflagged), use
            // it; otherwise clone a copy of it.  We have to do this before we create the debug frame This code
            // duplicated below
            ...
            if (!(AR(locsym) & LSYMINUSE)) {
                AR(locsym) |= LSYMINUSE;
                gsfctdl |= 32;
            }  // remember if we are using the original symtab
            else {
                RZ(locsym = jtclonelocalsyms(jt, locsym));
            }
        } else {  // something special required
            // If this is a modifier-verb referring to x or y, set u, v to the modifier operands, and sv to the saved
            // modifier (f=type, g=compiled text).  The flags don't change
            if (sflg & VXOP) {
                u  = sv->fgh[0];
                v  = sv->fgh[2];
                sv = VAV(sv->fgh[1]);
            ...
            // if we are in debug mode, the call to this defn should be on the stack (unless debug was entered under
            // program control).  If it is, point to its stack frame, which functions as a flag to indicate that we are
            // debugging.  If the function is locked we ignore debug mode
            if (!(gsfctdl & 1) && jt->uflags.us.cx.cx_c.db) {
                if (jt->sitop && jt->sitop->dctype == DCCALL) {  // if current stack frame is a call
                    ...
                    if (sv->flag & VNAMED) {
                        callframe = jt->sitop;  // if this is a named (rather than anonymous call like 3 : 0"1), there
                                                // is a tight link with the caller.  Indicate that
                    }
                    ...
                    if (self == jt->sitop->dcf) {  // if the stack frame is for this exec
                        jt->sitop->dcloc = locsym;
                        jt->sitop->dcc   = hv[1];  // install info about the exec
                        // Use this out-of-the-way place to ensure that the compiler will not try to put for. and try.
                        // stuff into registers
                        forcetomemory(&tdi);
                        forcetomemory(&tdv);
                        forcetomemory(&cv);
                    }
                ...
                // With debug on, we will save pointers to the sentence being executed in the stack frame we just
                // allocated We will keep cxspecials set as long as savdcy is 0, i. e. in all levels where debug was set
                // at the start of the frame
                jt->cxspecials = 1;
            }

            // If the verb contains try., allocate a try-stack area for it.  Remember debug state coming in so we can
            // restore on exit
            if (sv->flag & VTRY1 + VTRY2) {
                A td;
                GAT0(td, INT, NTD * WTD, 1);
                tdv = (TD *)AV(td);
                gsfctdl |= jt->uflags.us.cx.cx_c.db << 8;
            }
        }
        // End of unusual processing
        SYMPUSHLOCAL(locsym);  // Chain the calling symbol table to this one

        // assignsym etc should never be set here; if it is, there must have been a pun-in-ASGSAFE that caused us to
        // mark a derived verb as ASGSAFE and it was later overwritten with an unsafe verb.  That would be a major mess;
        // we'll invest 2 stores in preventing it - still not a full fix, since invalid inplacing may have been done
        // already
        CLEARZOMBIE
        // Assign the special names x y m n u v.  Do this late in initialization because it would be bad to fail after
        // assigning to yx (memory leak would result) For low-rank short verbs, this takes a significant amount of time
        // using IS, because the name doesn't have bucket info and is not an assignment-in-place So, we short-circuit
        // the process by assigning directly to the name.  We take advantage of the fact that we know the order in which
        // the symbols were defined: y then x; and we know that insertions are made at the end; so we know the bucketx
        // for xy are 0 or maybe 1.  We have precalculated the buckets for each table size, so we can install the values
        // directly.
        // Assignments here are special.  At this point we know that the value is coming in from a different namespace,
        // and that it will be freed in that namespace.  Thus, (1) there is no need to realize a virtual block - we can
        // just assign it to x/y, knowing that it is not tying up a backer that could have been freed otherwise (we do
        // have to raise the usecount, but not by ras()); (2) if the input has been abandoned, we do not need to raise
        // the usecount here: we can just mark the arg non-inplaceable, usecount 1 and take advantage of assignment in
        // place here - in this case we must flag the name to suppress decrementing the usecount on reassignment or
        // final free.  In both cases we know the block will be freed by the caller. Virtual abandoned blocks are both
        // cases at once.  That's OK.
        ...
        if (w) {  // If y given, install it & incr usecount as in assignment.  Include the script index of the
                  // modification
            // If input is abandoned inplace and not the same as x, DO NOT increment usecount, but mark as abandoned and
            // make not-inplace.  Otherwise ra We can handle an abandoned argument only if it is direct or recursive,
            // since only those values can be assigned to a name
            if ((a != w) & SGNTO0(AC(w) & (((AT(w) ^ AFLAG(w)) & RECURSIBLE) - 1)) & ((I)jtinplace >> JTINPLACEWX)) {
                ybuckptr->flag = LPERMANENT | LWASABANDONED;
                AC(w) &= ~ACINPLACE;  // remember, blocks from every may be 0x8..2, and we must preserve the usecount
                                      // then as if we ra()d it
            } else
                ra(w);
            ybuckptr->val = w;
            ybuckptr->sn  = jt->slisti;
        }
        // for x (if given), slot is from the beginning of hashchain EXCEPT when that collides with y; then follow y's
        // chain We have verified that hardware CRC32 never results in collision, but the software hashes do (needs to
        // be confirmed on ARM CPU hardware CRC32C)
        if (a) {
            if (xbuckptr == ybuckptr) xbuckptr = xbuckptr->next + sympv;
            if ((a != w) & SGNTO0(AC(a) & (((AT(a) ^ AFLAG(a)) & RECURSIBLE) - 1)) & ((I)jtinplace >> JTINPLACEAX)) {
                xbuckptr->flag = LPERMANENT | LWASABANDONED;
                AC(a) &= ~ACINPLACE;
            } else
                ra(a);
            xbuckptr->val = a;
            xbuckptr->sn  = jt->slisti;
        }
    ...
    A z  = mtm;  // last B-block result; will become the result of the execution. z=0 is treated as an error condition
                 // inside the loop, so we have to init the result to i. 0 0
    ...
    while (1) {
        ...
        if (jt->cxspecials) {  // fast check to see if we have overhead functions to perform
            if (!(gsfctdl & (16 + 1)) && jt->uflags.us.cx.cx_c.db) {
                // If we haven't done so already, allocate an area to use for the SI entries for sentences executed
                // here, if needed.  We need a new area only if we are debugging.  Don't do it if locked. We have to
                // have 1 debug frame to hold parse-error information in, but it is allocated earlier if debug is off We
                // check before every sentence in case the user turns on debug in the middle of this definition NOTE:
                // this stack frame could be put on the C stack, but that would reduce the recursion limit because the
                // frame is pretty big If there is no calling stack frame we can't turn on debug mode because we can't
                // suspend If we are executing a recursive call to JDo we can't go into debug because we can't prompt
                DC d;
                for (d = jt->sitop; d && DCCALL != d->dctype; d = d->dclnk)
                    ; /* find bottommost call                 */
                if (d && jt->recurstate <
            ...
            if ((gsfctdl & 16) && jt->redefined) {
                DC siparent;
                A *hv;
                if ((siparent = thisframe->dclnk) && jt->redefined == siparent->dcn && DCCALL == siparent->dctype &&
                    self != siparent->dcf) {
                    self  = siparent->dcf;
                    V *sv = FAV(self);
                    LINE(sv);
                    ...
                    DO(
                      AN(hv[0]), if (AT(line[i]) & NAME) { NAV(line[i])->bucket = 0; });
                }
                jt->redefined = 0;
            }
            if (!((I)jt->redefined | (I)jt->pmctr | (I)(gsfctdl & 16)))
                jt->cxspecials = 0;  // if no more special work to do, close the gate
        }

        // Don't do the loop-exit test until debug has had the chance to update the execution line.  For example, we
        // might be asked to reexecute the last line of the definition
        if ((UI)i >= (UI)n) break;
        ...
        switch (cwtype = ci->type) {
            ...
            case CELSEIF:
                ...
                if ((UI)i >= (UI)n || (((cwtype = (ci = i + cw)->type) ^ CTBLOCK) + jt->cxspecials))
                    break;  // avoid indirect-branch overhead on the likely case
            case CASSERT:
            case CTBLOCK:
            tblockcase:
                ...
                if (ci->canend & 2)
                    ...
                    z = jtgc(jt, z, old);  // 2 means previous B can't be the result
                parseline(t);
                ...
                if (ci->type == CASSERT && jt->assert &&t && !(NOUN & AT(t) && all1(eq(num(1), t))))
                    t = jtpee(jt, line,
                            ...
                            EVASSERT,
                            gsfctdl << (BW - 2),
                            callframe);  // if assert., signal post-execution error if result not all 1s.  May go into
                                         // debug; sets to result after debug
                if (t != 0) {
                    ti = i, ++i;  // if no error, continue on
                    if ((UI)i < (UI)n && !(((cwtype = (ci = i + cw)->type) ^ CDO) + jt->cxspecials))
                        goto docase;  // avoid indirect-branch overhead on the likely case
                } else if ((gsfctdl & 16) && DB1 & jt->uflags.us.cx.cx_c.db)
                    ti = i,
                    i  = debugnewi(
                      i + 1,
                      thisframe,
                      self);  // error in debug mode: when coming out of debug, go to new line (there had better be one)
                else if (EVTHROW == jt->jerr) {
                    if (gsfctdl & 4 && (tdv + tdi - 1)->t) {
                        i = (tdv + tdi - 1)->t + 1;
                        RESETERR;
                    } else
                        BASSERT(0, EVTHROW);
                }  // if throw., and there is a catch., do so
                else {
                    i = ci->go;
                    if (i < SMAX) {
                        RESETERR;
                        ...
                        z = 0;
                }  // uncaught error: if we take error exit, we might not have protected z, which is not needed anyway;
                   // so clear it to prevent invalid use
                // if we are not taking the error exit, we still need to set z to a safe value since we might not have
                // protected it.  This is B1 try. if. error do. end. catch. return. end.
                break;
            case CDO:
            docase:
                ...
                {
                    A tt = t;
                    tt   = t ? t : mtv;  // missing t looks like '' which is true
                    //  Start by assuming condition is true; set to move to the next line then
                    ++i;
                    // Quick true cases are: nonexistent t; empty t; direct numeric t with low byte nonzero.  This gets
                    // most of the true.  We add in char types and BOX cause it's free (they are always true)
                    if (AN(tt) &&
                        (-(AT(tt) & (B01 | LIT | INT | FL | CMPX | C2T | C4T | BOX)) & -((I)CAV(tt)[0])) >= 0) {
                        I nexti = ci->go;  // C cond is false if (type direct or BOX) and (value not 0).  J cond is true
                                           // then.  Musn't fetch CAV[0] if AN==0
                        // here the type is indirect or the low byte is 0.  We must compare more
                        while (1) {  // 2 loops if sparse
                            if (AT(tt) & INT + B01) {
                                i = BIV0(tt) ? i : nexti;
                                break;
                            }  // INT and B01 are most common
                            if (AT(tt) & FL) {
                                i = DAV(tt)[0] ? i : nexti;
                                break;
                            }
                            if (AT(tt) & CMPX) {
                                i = DAV(tt)[0] || DAV(tt)[1] ? i : nexti;
                                break;
                            }
                            if (AT(tt) & (RAT | XNUM)) {
                                i = 1 < AN(XAV(tt)[0]) || IAV(XAV(tt)[0])[0] ? i : nexti;
                                break;
                            }
                            if (!(AT(tt) & NOUN)) { CHECKNOUN }  // will take error
                            // other types test true, which is how i is set
                            if (!(SPARSE & AT(tt))) break;
                            BZ(tt = jtdenseit(jt, tt));
                            if (AN(tt) == 0)
                                break;  // convert sparse to dense - this could make the length go to 0, in which case
                                        // true
                        }
                    }
                }
                t = 0;  // Indicate no T block, now that we have processed it
                if ((UI)i >= (UI)n || ((((cwtype = (ci = i + cw)->type) & 31) ^ CBBLOCK) + jt->cxspecials))
                    break;  // avoid indirect-branch overhead on the likely case
            ...
            dobblock:
                ...
                if (z != 0) {
                    bi = i;
                    i += (cwtype >> 5) + 1;  // go to next sentence, or to the one after that if it's harmless end.
                    if ((UI)i < (UI)n && !((((cwtype = (ci = i + cw)->type) & 31) ^ CBBLOCK) + jt->cxspecials))
                        goto dobblock;  // avoid indirect-branch overhead on the likely case
                    // BBLOCK is usually followed by another BBLOCK, but another important followon is END followed by
                    // BBLOCK.  BBLOCKEND means 'bblock followed by end that falls through', i. e. a bblock whose
                    // successor is i+2.  By handling that we process all sequences of if. T do. B end. B... without
                    // having to go through the switch; this means the switch will learn to go to the if.
                } else if ((gsfctdl & 16) && jt->uflags.us.cx.cx_c.db & (DB1)) {  // error in debug mode
                    z = mtm, bi = i,
                    i = debugnewi(
                      i + 1,
                      thisframe,
                      self);  // Remember the line w/error; fetch continuation line if any. it is OK to have jerr set if
                              // we are in debug mode, but z must be a harmless value to avoid error protecting it
                    // if the error is THROW, and there is a catcht. block, go there, otherwise pass the THROW up the
                    // line
                } else if (EVTHROW == jt->jerr) {
                    if (gsfctdl & 4 && (tdv + tdi - 1)->t) {
                        i = (tdv + tdi - 1)->t + 1;
                        ...
                        BASSERT(0, EVTHROW);  // z might not be protected if we hit error
                    // for other error, go to the error location; if that's out of range, keep the error; if not,
                    // it must be a try. block, so clear the error.  Pop the try. stack, and if it pops back to 0,
                    // restore debug mode (since we no longer have a try.) NOTE ERROR: if we are in a for. or select.,
                    // going to the catch. will leave the stack corrupted, with the for./select. structures hanging on.
                    // Solution would be to save the for/select stackpointer in the try. stack, so that when we go to
                    // the catch. we can cut the for/select stack back to where it was when the try. was encountered
                } else {
                    i = ci->go;
                    if (i < SMAX) {
                        RESETERR;
                    ...
                    }  // z might not have been protected: keep it safe. This is B1 try. error catch. return. end.
                }
                break;
            ...
            case CTRY:
                ...
                if (jt->uflags.us.cx.cx_c.db)
                    jt->uflags.us.cx.cx_c.db = (gsfctdl & 16) && (UC)(tdv + tdi)->d ? jt->dbuser : 0;
                ++tdi;
                ++i;
                gsfctdl |= 4;  // bump tdi pointer, set flag
                break;
            case CCATCH:
            case CCATCHD:
            case CCATCHT:
            ...
            case CSELECTN:
                ...
                if (!r)
                    if (gsfctdl & 8) {
                        ...
                        r = 9;
                        GAT0E(cd, INT, 9 * WCD, 1, i = -1; z = 0; continue);
                        ras(cd);
                        cv = (CDATA *)AV(cd) - 1;
                        gsfctdl |= 8;
                    }  // 9=r
            ...
            case CDOF:  // do. after for.
                ...
                if (!cv->t) {
                    ...
                    CHECKNOUN  // if t is not a noun, signal error on the last line executed in the T block
                      BZ(jtforinit(jt, cv, t));
                    t = 0;
                }
                ++cv->j;  // step to first (or next) iteration
                if (cv->x) {
                    A x;  // assign xyz and xyz_index for for_xyz.
                    if (!(ci->canend & 2))
                        BZ(
                          z = jtrat(
                            jt,
                ...
                if (cv->j < cv->n) {  // if there are more iterations to do...
                    ++i;
                    continue;  // advance to next line and process it
                }
            ...
            case CENDSEL:
                ...
                // Must jtrat(jt,) if the current result might be final result, in case it includes the variables we
                // will delete in unstack (this includes ONLY xyz_index, so perhaps we should avoid rat if stack empty
                // or xyz_index not used)
                if (!(ci->canend & 2)) BZ(z = jtrat(jt, z));
                jtunstackcv(jt, cv);
                --cv;
                ++r;
            ...
            case CCONTS:
                ...
                if (!(ci->canend & 2))
                    BZ(z = jtrat(jt, z));  // protect possible result from pop, if it might be the final result
                do {
                    I fin = cv->w == CSELECT;
                    jtunstackcv(jt, cv);
                    --cv;
            ...
            case CRETURN:
                ...
                i = ci->go;  // If there is a try stack, restore to initial debug state.  Probably safe to  do
                             // unconditionally
                if (gsfctdl & 4)
                    jt->uflags.us.cx.cx_c.db =
                      (UC)(gsfctdl >> 8);  // if we had an unfinished try. struct, restore original debug state
                break;
            case CCASE:
            case CFCASE:
                ...
                if (!cv->t) {
                    ...
                    CHECKNOUN  // if t is not a noun, signal error on the last line executed in the T block
                      BZ(ras(t));
                    cv->t = t;
                    t     = 0;  // protect t from free while we are comparing with it, save in stack
                }
                i = ci->go;  // Go to next sentence, which might be in the default case (if T block is empty)
                if ((UI)i < (UI)n && !(((cwtype = (ci = i + cw)->type) ^ CTBLOCK) + jt->cxspecials))
                    goto tblockcase;  // avoid indirect-branch overhead on the likely case, which is case. t-block do.
            ...
            case CDOSEL:              // do. after case. or fcase.
                ...
                if (t) {  // if t is not a noun, signal error on the last line executed in the T block
                    CHECKNOUN
                    if (!((AT(t) | AT(cv->t)) & BOX)) {
                        // if neither t nor cv is boxed, just compare for equality.  Boxed empty goes through the other
                        // path
                        if (!jtequ(jt, t, cv->t))
                            i =
                              ci->go;  // should perhaps take the scalar case specially & send it through singleton code
                    } else {
                        if (all0(jteps(jt, jtboxopen(jt, cv->t), jtboxopen(jt, t))))
                            i = ci->go;  // if case tests false, jump around bblock   test is cv +./@:,@:e. boxopen t
                    }
                    // Clear t to ensure that the next case./fcase. does not reuse this value
                    t = 0;
                }
                if ((UI)i < (UI)n && !((((cwtype = (ci = i + cw)->type) & 31) ^ CBBLOCK) + jt->cxspecials))
                    goto dobblock;  // avoid indirect-branch overhead on the likely  case. ... do. bblock
                break;
    ...
    // We still must not take an error exit in this runout.  We have to hang around to the end to restore symbol tables,
    // pointers, etc.
    ...
    if (z) {
        ...
        if (AT(z) & NOUN) {
            // If we are returning a virtual block, we are going to have to realize it.  This is because it might be
            // (indeed, probably is) backed by a local symbol that is going to be summarily freed by the
            // jtsymfreeha(jt,) below.  We could modify symfreeha to recognize when we are freeing z, but the case is
            // not common enough to be worth the trouble
            realizeifvirtual(z);
        } else if (AT(self) & ADV + CONJ) {  // non-noun result, but OK from adv/conj
            // if we are returning a non-noun, we have to cleanse it of any implicit locatives that refer to the symbol
            // table in use now. It is OK to refer to other symbol tables, since they will be removed if they try to
            // escape at higher lavels and in the meantime can be executed; but there is no way we could have a
            // reference to such an implied locative unless we also had a reference to the current table; so we replace
    ...
    if (!(gsfctdl & 8)) {
        // Normal path.  protect the result block and free everything allocated here, possibly including jt->locsyms if
        // it was cloned (it is on the stack now)
        z = EPILOGNORET(z);  // protect return value from being freed when the symbol table is.  Must also be before
                             // stack cleanup, in case the return value is xyz_index or the like
    } else {
        // Unusual path with an unclosed contruct (e. g. return. from inside for. loop).  We have to free up the for.
        // stack, but the return value might be one of the names to be deleted on the for. stack, so we must protect the
        // result before we pop the stack.  BUT, EPILOG frees all locally-allocated blocks, which might include the
        // symbol table that we need to pop from.  So we protect the symbol table during the cleanup of the result and
        // stack.
        ra(locsym);          // protect local syms
        z = EPILOGNORET(z);  // protect return value from being freed when the symbol table is.  Must also be before
                             // stack cleanup, in case the return value is xyz_index or the like
        CDATA *cvminus1 = (CDATA *)VAV(cd) - 1;
        while (cv != cvminus1) {
            jtunstackcv(jt, cv);
            --cv;
        }            // clean up any remnants left on the for/select stack
    ...
    // If we are using the original local symbol table, clear it (free all values, free non-permanent names) for next
    // use.  We know it hasn't been freed yet We detect original symbol table by rank LSYMINUSE - other symbol tables
    // are assigned rank 0. Tables are born with NAMEADDED off.  It gets set when a name is added.  Setting back to
    // initial state here, we clear NAMEADDED
    if (gsfctdl & 32) {
        AR(locsym) = LLOCALTABLE;
        jtsymfreeha(jt, locsym);
    }
    ...
    // See if EPILOG pushed a pointer to the block we are returning.  If it did, and the usecount we are returning is 1,
    // set this result as inplaceable and install the address of the tpop stack into AM (as is required for all
    // inplaceable blocks).  The the usecount is inplaceable 1, we don't do this, because it is possible that the AM
    // slot was inherited from higher up the stack. Note that we know we are not returning a virtual block here, so it
    // is OK to write to AM

    if (z != 0)
        if ((_ttop != jt->tnextpushp) == AC(z)) {
            AC(z)    = ACINPLACE | ACUC1;
            ABACK(z) = (A)_ttop;
        }  // AC can't be 0
    return z;
}
...
xv1(J jt, A w, A self) {
    A z;
    return df1(z, w, FAV(self)->fgh[0]);
}
static A
xv2(J jt, A a, A w, A self) {
    A z;
    return df2(z, a, w, FAV(self)->fgh[1]);
}
static A
xn1(J jt, A w, A self) {
...
jtxop2(J jt, A a, A w, A self) {
    ...
    RZ(ff = jtfdef(jt, 0,
                 ...
                 self,
                 w,
                 (VXOP | VFIX | VJTFLGOK1 | VJTFLGOK2) ^ FAV(self)->flag,
                 RMAX,
                 RMAX,
                 RMAX));
    return ff;
...
jtxop(J jt, A w) {
    ...
    for (i = 0; i < in; ++i) {
        ...
        if (AT(w) & NAME) {
            ...
            C *s = NAV(w)->s;
            if (n) {
                // Set flags if this is a special name, or an indirect locative referring to a special name in the last
                // position, or u./v.
...
jtcolon0(J jt, I deftype) {
    ...
    while (1) {
        ...
        if (deftype != 0)
            RZ(l = jtddtokens(jt, l, 8 + 2));  // if non-noun def, handle DDs, for explicit def, return string, allow
                                               // jtjgets(jt,).  Leave noun contents untouched
        ...
        if (p < q + m && ')' == *p) {
            while (p < q + m && ' ' == *++p)
                ;
            if (p >= m + q) break;
        }  // if ) with nothing else but blanks, stop
        ...
        if (isboxed) {
            ...
            while (AN(z) <= n + 1) {
                RZ(z = jtext(jt, 0, z));
                sb = AAV(z);
            }  // extend the result if necessary
            sb[n] = l;
            ++n;  // append the line, increment line number
        } else {
            while (AN(z) <= n + m) {
                RZ(z = jtext(jt, 0, z));
                s = CAV(z);
            ...
            n += m;
            s[n] = CLF;
            ++n;  // append LF at end of each line
        }
    }
...
jtsent12c(J jt, A w) {
    ...
    // Lines are separated by DDSEP, and there may be DDSEP embedded in strings.  Convert the whole thing to words,
    // which will leave the embedded DDSEP embedded; then split on the individual DDSEP tokens tokenize the lines.  Each
    // LF is its own token.
    A wil;
    RZ(wil = jtwordil(jt, w));
    ASSERT(AM(wil) >= 0, EVOPENQ) makewritable(wil);
    I wiln      = AS(wil)[0];
    I(*wilv)[2] = voidAV(wil);  // line index, and number of lines; array of (start,end+1) for each line
    // Compact the word-list to a line-list.  Go through the words looking for LF.  For each line, add an entry to the
    // line-list with all the characters except the LF
    ...
    while (i < wiln) {
        ...
        wilv[linex][0] = currlinest;
        wilv[linex][1] = wilv[i][0];  // add the line
        ++linex;
        currlinest = wilv[i][0] + 1;  // advance line pointer, skip over the LF to start of next line
...
static void
jtcalclocalbuckets(J jt, A t, LX *actstv, I actstn) {
    LX k;
    if (!(NAV(t)->flag &
          (NMLOC | NMILOC))) {  // don't store if we KNOW we won't be looking up in the local symbol table
        ...
        // search through the chain, looking for a match on name.  If we get a match, the bucket index is the one's
        // complement of the number of items compared before the match.  If we get no match, the bucket index is the
        // number of items compared (= the number of items in the chain)
        L *sympv = LAV0(jt->symp);
        for (k = actstv[tn->bucket]; k;
             ++compcount, k = sympv[k].next) {  // k chases the chain of symbols in selected bucket
            if (tn->m == NAV(sympv[k].name)->m && !memcmpne(tn->s, NAV(sympv[k].name)->s, tn->m)) {
                compcount = ~compcount;
                break;
            }
        }
...
A
jtcrelocalsyms(J jt, A l, A c, I type, I dyad, I flags) {
    A actst, *lv, pfst, t, wds;
    C *s;
    I j, ln;
    ...
    RZ(jtprobeis(jt, mnuvxynam[5], pfst));
    if (!(!dyad && (type >= 3 || (flags & VXOPR)))) { RZ(jtprobeis(jt, mnuvxynam[4], pfst)); }
    if (type < 3) {
        RZ(jtprobeis(jt, mnuvxynam[2], pfst));
        RZ(jtprobeis(jt, mnuvxynam[0], pfst));
    }
    if (type == 2) {
        RZ(jtprobeis(jt, mnuvxynam[3], pfst));
        RZ(jtprobeis(jt, mnuvxynam[1], pfst));
    }
    ...
    ln = AN(l);
    lv = AAV(l);                // Get # words, address of first box
    for (j = 1; j < ln; ++j) {  // start at 1 because we look at previous word
        t = lv[j - 1];          // t is the previous word
        // look for 'names' =./=: .  If found (and the names do not begin with `), replace the string with a special
        // form: a list of boxes where each box contains a name. This form can appear only in compiled definitions
        if (AT(lv[j]) & ASGN && AT(t) & LIT && AN(t) && CAV(t)[0] != CGRAVE) {
            ...
            if (AN(neww)) {                               // ignore blank string
                ...
                if (newt) {
                    t = lv[j - 1] = jtincorp(jt, newt);
                    AT(t) |= BOXMULTIASSIGN;
                } else
                    RESETERR  // if no error, mark the block as MULTIASSIGN type and save it in the compiled definition;
                              // also set as t for below.  If error, catch it later
            }
        }
        ...
        if ((AT(lv[j]) & ASGN + ASGNLOCAL) == (ASGN + ASGNLOCAL)) {  // local assignment
            if (AT(lv[j]) & ASGNTONAME) {                            // preceded by name?
                ...
                // LIT followed by =.  Probe each word.  Now that we support lists of NAMEs, this is used only for AR
                // assignments First, convert string to words
                s = CAV(t);  // s->1st character; remember if it is `
                if (wds = words(s[0] == CGRAVE ? jtstr(jt, AN(t) - 1, 1 + s) : t)) {
                    I kk;  // convert to words (discarding leading ` if present)
                    I wdsn  = AN(wds);
                    A *wdsv = AAV(wds), wnm;
                    for (kk = 0; kk < wdsn; ++kk) {
                    ...
                    RESETERR  // if invalid words, ignore - we don't catch it here
                ...
                A *tv = AAV(t);
                DO(AN(t), if (!(NAV(tv[i])->flag & (NMLOC | NMILOC))) RZ(jtprobeis(jt, tv[i], pfst));)
            }
    ...
    I cn    = AN(c);
    CW *cwv = (CW *)AV(c);          // Get # control words, address of first
    ...
    // Count the assigned names, and allocate a symbol table of the right size to hold them.  We won't worry too much
    // about collisions, since we will be assigning indexes in the definition. We choose the smallest feasible table to
    // reduce the expense of clearing it at the end of executing the verb
    I pfstn   = AN(pfst);
    LX *pfstv = LXAV0(pfst), pfx;
    I asgct   = 0;
    L *sympv  = LAV0(jt->symp);
    ...
    *(UI4 *)LXAV0(actst) =
      (UI4)((SYMHASH(NAV(mnuvxynam[4])->hash, AN(actst) - SYMLINFOSIZE) << 16) +
            SYMHASH(NAV(mnuvxynam[5])->hash,
                    AN(actst) -
                      SYMLINFOSIZE));  // get the yx bucket indexes for a table of this size, save in first hashchain
    ...
    RZ(jtprobeis(jt, mnuvxynam[5], actst));
    if (!(!dyad && (type >= 3 || (flags & VXOPR)))) { RZ(jtprobeis(jt, mnuvxynam[4], actst)); }
    for (j = 1; j < pfstn; ++j) {  // for each hashchain
        for (pfx = pfstv[j]; pfx; pfx = LAV0(jt->symp)[pfx].next) {
            L *newsym;
    ...
    I actstn   = AN(actst) - SYMLINFOSIZE;
    LX *actstv = LXAV0(actst);  // # hashchains in new symbol table, and pointer to hashchain table
    ...
    if (type >= 3 || flags & VXOPR) {  // If this is guaranteed to return a noun...
        for (j = 0; j < ln; ++j) {
            if (AT(t = lv[j]) & NAME) {
                ...
                A *tv = AAV(t);
                DO(AN(t), jtcalclocalbuckets(jt, tv[i], actstv, actstn);)  // install bucket info into boxed names
            }
        }
...
jtclonelocalsyms(J jt, A a) {
    ...
    for (j = SYMLINFOSIZE; j < an; ++j) {
        LX *zhbase = &zv[j];
        LX ahx     = av[j];
        LX ztx     = 0;  // hbase->chain base, hx=index of current element, tx is element to insert after
        while (ahx && (LAV0(jt->symp))[ahx].flag & LPERMANENT) {
            L *l;  // for each permanent entry...
            RZ(l = jtsymnew(jt, zhbase, ztx));
            A nm    = (LAV0(jt->symp))[ahx].name;
            l->name = nm;
            ras(l->name);  // point symbol table to the name block, and increment its use count accordingly
            l->flag =
              (LAV0(jt->symp))[ahx].flag & (LINFO | LPERMANENT);  // clear all but PERMANENT and INFO, in case we try to
                                                                  // delete the name (as in for_xyz. or 4!:55)
            ztx = ztx ? (LAV0(jt->symp))[ztx].next
                      : *zhbase;  // ztx=index to value we just added.  We avoid address calculation because of the
                                  // divide.  If we added at head, the added block is the new head; otherwise it's
                                  // pointed to by previous tail
...
A
jtcolon(J jt, A a, A w) {
    A d, h, *hv, m;
    B b;
    C *s;
    I flag = VFLAGNONE, n, p;
    if (VERB & AT(a) & AT(w)) {  // v : v case
        // If nested v : v, prune the tree
        if (CCOLON == FAV(a)->id && FAV(a)->fgh[0] && VERB & AT(FAV(a)->fgh[0]) && VERB & AT(FAV(a)->fgh[1]))
        ...
        return jtfdef(jt, 0,
                    ...
                    w,
                    0L,
                    ((FAV(a)->flag & FAV(w)->flag) & VASGSAFE),
                    mr(a),
                    lr(w),
                    rr(w));  // derived verb is ASGSAFE if both parents are
    }
    ...
    if (!n) {
        ra0(w);
        return w;
    }  // noun - return it.  Give it recursive usecount
    if ((C2T + C4T) & AT(w)) RZ(w = jtcvt(jt, LIT, w));
    I splitloc = -1;  // will hold line number of : line
    if (10 < n) {
        ASSERT(AT(w) & LIT, EVDOMAIN) s = CAV(w);
        p                               = AN(w);
        if (p && CLF == s[p - 1]) RZ(w = jtstr(jt, p - 1, s));
    }       // if tacit form, discard trailing LF
    else {  // not tacit translator - preparse the body
        // we want to get all forms to a common one: a list of boxed strings.  If we went through m : 0, we are in that
        // form already.  Convert strings
        if (!col0)
            if (BOX & AT(w)) {
                RZ(w = jtsent12b(jt, w))
            } else {
        ...
        if (AN(w) != 0)
            if (AN(AAV(w)[0]) && CAV(AAV(w)[0])[0] == ')') {
                // there is a control line.  parse it.  Cut to words
                A cwds = jtwordil(jt, AAV(w)[0]);
                RZ(cwds);
                ASSERT(AM(cwds) == 2, EVDOMAIN);  // must be exactly 2 words: ) and type
                ASSERT(((IAV(cwds)[1] - IAV(cwds)[0]) | (IAV(cwds)[3] - IAV(cwds)[2])) == 1,
                       EVDOMAIN);  // the ) and the next char must be 1-letter words
                C ctltype = CAV(AAV(w)[0])[IAV(
                  cwds)[2]];  // look at the second char, which must be one of acmdv*  (n is handled in ddtokens)
                I newn    = -1;
                newn      = ctltype == 'a' ? 1 : newn;
                newn      = ctltype == 'c' ? 2 : newn;
                newn      = ctltype == 'm' ? 3 : newn;
                newn      = ctltype == 'd' ? 4 : newn;
                newn      = ctltype == 'v' ? 3 : newn;
                newn      = ctltype == '*' ? 9 : newn;  // choose type based on char
        ...
        I mn = splitloc < 0 ? AN(w) : splitloc;
        I nn = splitloc < 0 ? 0 : AN(w) - splitloc - 1;
        RZ(m = jttake(jt, jtsc(jt, mn), w));
        RZ(d = jttake(jt, jtsc(jt, -nn), w));
        INCORP(m);
        INCORP(d);  // we are incorporating them into hv[]
        if (4 == n) {
            if ((-AN(m) & (AN(d) - 1)) < 0) d = m;
            m = mtv;
        }  //  for 4 :, make the single def given the dyadic one
        GAT0(h, BOX, 2 * HN, 1);
        hv = AAV(h);
    ...
    // The h argument is logically h[2][HN] where the boxes hold (parsed words, in a row);(info for each control
    // word);(original commented text (optional));(local symbol table) Non-noun results cannot become inputs to verbs,
    // so we do not force them to be recursive
    if ((1LL << n) & 0x206) {                                  // types 1, 2, 9
        ...
        if (n == 9) {
            I defflg = (fndflag & ((splitloc >> (BW - 1)) | -4)) | 1;
            CTLZI(defflg, n);
            n = (0x2143 >> (n << 2)) & 0xf;  // replace 9 by value depending on what was seen; if : seen, ignore x
            if (n == 4) {
                hv[HN]     = hv[0];
                hv[0]      = mtv;
                hv[HN + 1] = hv[1];
                ...
                hv[HN + 2] = hv[2];
                hv[2]      = mtv;
                flag       = (flag & ~VTRY2) + VTRY1;
            }  // if we created a dyadic verb, shift the monad over to the dyad and clear the monad, incl try flag
        }
        if (n <= 2) {                                        // adv or conj after autodetection
            ...
            // if there is only one valence defined, that will be the monad.  Swap it over to the dyad in two cases: (1)
            // it is a conjunction with uv only: the operands will be the two verbs; (2) it is an operator with a
            // reference to x
            if (((-AN(m)) & (AN(d) - 1) & (((fndflag - 5) & (1 - n)) | (5 - fndflag))) < 0) {
                A *u = hv, *v = hv + HN, x;
                DQ(HN, x = *u; *u++ = *v; *v++ = x;);
            }  // if not, it executes on uv only; if conjunction, make the default the 'dyad' by swapping monad/dyad
    ...
    // definition is executed, so that we wouldn't take up the space for library verbs; but since we don't explicitly
...
// input, for example if the string comes from (". y) or from an event.  If an unterminated DD is found when bit 2 is
// set, the call fails with control error
//
...
jtddtokens(J jt, A w, I env) {
    ...
    I(*wilv)
    [2] = voidAV(
      wil);  // cv=pointer to chars, nw=#words including final NB   wilv->[][2] array of indexes into wv word start/end
    ...
    for (firstddbgnx = ddschbgnx; firstddbgnx < nw; ++firstddbgnx) {
        US ch2 = *(US *)(wv + wilv[firstddbgnx][0]);
        ASSERT(!(ch2 == DDEND && (wilv[firstddbgnx][1] - wilv[firstddbgnx][0] == 2)), EVCTRL)
    ...
    while (firstddbgnx < nw) {
        ...
        I *fillv = &wilv[ddschbgnx][1];
        DQ(2 * (firstddbgnx - ddschbgnx) - 1, *fillv++ = wilv[firstddbgnx][0];)
        I ddendx = -1, ddbgnx = firstddbgnx;  // end/start of DD, indexes into wilv.  This will be the pair we process.
                                              // We advance ddbgnx and stop when we hit ddendx
        I scanstart = firstddbgnx;  // start looking for DDEND/nounDD at the first known DDBGN.  But if there turns out
                                    // to be no DDEND, advance start ptr to avoid rescanning
        while (1) {
            I i;  // loop till we find a complete DD
            for (i = scanstart; i < nw; ++i) {
                US ch2 = *(US *)(wv + wilv[i][0]);  // digraph for next word
                if (ch2 == DDEND && (wilv[i][1] - wilv[i][0] == 2)) {
                    ddendx = i;
                    break;
                }  // if end, break, we can process
                if (ch2 == DDBGN && (wilv[i][1] - wilv[i][0] == 2)) {
                    ...
                    fillv  = &wilv[firstddbgnx][1];
                    DQ(2 * (ddbgnx - firstddbgnx) - 1, *fillv++ = wilv[ddbgnx][0];)
                    if (AN(w) >= wilv[i][1] + 2 && wv[wilv[i][0] + 2] == ')' &&
                        wv[wilv[i][0] + 3] == 'n') {  // is noun DD?
            ...
            if (ddendx >= 0)
                break;  // we found an end, and we started at a begin, so we have a pair, or a noun DD if endx=0.  Go
                        // process it
            ...
            scanstart = AM(wil);  // Get # words, not including final NB.  We have looked at em all, so start next look
                                  // after all of them
            A neww = jtjgets(jt, "\001");  // fetch next line, in raw mode
            RE(0);
            ASSERT(neww != 0, EVCTRL);  // fail if jgets failed, or if it returned EOF - problem either way
            // join the new line onto the end of the old one (after discarding trailing NB in the old).  Must add an LF
            // character and a word for it
            w = jtapip(jtinplace, w, jtscc(jt, DDSEP));  // append a separator, which is all that remains of the
                                                         // original line   scaf use faux or constant block
            ...
            AS(wil)[0] = scanstart;
            AN(wil)    = 2 * scanstart;  // set AS and AN to discard comment
            A lfwd;
            fauxblockINT(lffaux, 2, 2);
            fauxINT(lfwd, lffaux, 2, 2) AS(lfwd)[0] = 2;
            AS(lfwd)[1]                             = 1;
            IAV(lfwd)[0]                            = oldchn - 1;
            IAV(lfwd)[1]                            = oldchn;
            ...
            A newwil;
            RZ(newwil = jtwordil(jt, neww));  // get index to new words
            ...
            I *nwv = IAV(newwil);
            DO(AN(newwil),
               *nwv++ += oldchn;)  // relocate all the new words so that they point to chars after the added LF
            RZ(wil = jtapip(jtinplace, wil, newwil));  // add new words after LF, giving old LF new
            if (savam >= 0) {
                AM(wil) = savam + scanstart;
            } else {
                AM(wil) = 0;
            }  // set total # words in combined list, not including final comment; remember if error scanning line
            wv   = CAV(w);
            nw   = AS(wil)[0];
            wilv = voidAV(wil);  // refresh pointer to word indexes, and length
        }

        if (ddendx == 0) {
            // ******* NOUN DD **********
            // we found a noun DD at ddbgnx: process it, replacing it with its string.  Look for delimiters on the first
            // line, or at the start of a later line
            ...
            // characters after nounstart cannot be examined as words, since they might have mismatched quotes.  Scan
            // them for }}
            if (nounstart < wn) {  // {{)n at EOL does not add LF
                for (enddelimx = nounstart; enddelimx < wn - 1; ++enddelimx)
                    if (wv[enddelimx] == (DDEND & 0xff) && wv[enddelimx + 1] == (DDEND >> 8)) break;
            }
            if (enddelimx >= wn - 1) {
                ASSERT(!(env & 4),
                       EVCTRL);  // Abort if we are not allowed to consume a new line (as for an event or ". y)
                ...
                while (1) {
                    ...
                    if (enddelimx < wn) {
                        w = jtapip(
                          jtinplace, w, jtscc(jt, DDSEP));  // append a separator   scaf use faux or constant block
                        jtinplace =
                          (J)((I)jtinplace | JTINPLACEW);  // after the first one, we can certainly inplace on top of w
                    }
                    ...
                    I oldchn = AN(w);  // len after adding LF, before adding new line
                    RZ(w =
                         jtapip(jtinplace, w, neww));  // join the new character list onto the old, inplace if possible
                    jtinplace =
                      (J)((I)jtinplace | JTINPLACEW);  // after the first one, we can certainly inplace on top of w
                    ...
                    if (AN(w) >= oldchn + 2 && wv[oldchn] == (DDEND & 0xff) && wv[oldchn + 1] == (DDEND >> 8)) {
                        enddelimx = oldchn;
                        break;
                    }
                }
            }
            // We have found the end delimiter, which starts at enddelimx.  We reconstitute the input line: we convert
            // the noun DD to a quoted string and append the unprocessed part of the last line.  For safety, we put
            // spaces around the string.  We rescan the combined line without trying to save the scan pointer, since the
            // case is rare
            A remnant;
            RZ(remnant =
            ...
            wv   = CAV(w);
            nw   = AS(wil)[0];
            wilv = voidAV(wil);  // cv=pointer to chars, nw=#words including final NB   wilv->[][2] array of indexes
                                 // into wv word start/end
            ddschbgnx = 0;       // start scan back at the beginning
        } else {
            // ********* NORMAL DD *******
            // We have found an innermost non-noun DD, running from ddbgnx to ddendx.  Convert it to ( m : 'string' )
            // form convert all the chars of the DD to a quoted string block
            A ddqu;
            RZ(ddqu = jtstrq(jt, wilv[ddendx][0] - wilv[ddbgnx + 1][0], wv + wilv[ddbgnx + 1][0]));
            // append the string for the start/end of DD
            I bodystart = AN(w), bodylen = AN(ddqu),
              trailstart = wilv[ddendx][1];  // start/len of body in w, and start of after-DD text
            RZ(ddqu = jtapip(jtinplace, ddqu, jtstr(jt, 7, ")( 9 : ")));
            // append the new stuff to w
            RZ(w = jtapip(jtinplace, w, ddqu));
            wv = CAV(w);  // refresh data pointer.  Number of words has not changed, nor have indexes
            // Replace ddbgnx and ddendx with the start/end strings.  Fill in the middle, if any, with everything in
            // between
            wilv[ddbgnx][0]     = AN(w) - 6;
            wilv[ddbgnx][1]     = AN(w);  //  ( 9 :
            wilv[ddbgnx + 1][0] = bodystart;
            fillv               = &wilv[ddbgnx + 1][1];
            DQ(2 * (ddendx - ddbgnx) - 1, *fillv++ = bodystart + bodylen + 1;)  // everything in between
            // continue the search.
            if (ddbgnx == firstddbgnx) {
                ddschbgnx =
                  ddendx + 1;  //   If this was not a nested DD, we can simply pick up the search after ddendx.
            } else {
                // Nested DD.  The characters below ddendx are out of order and there will be trouble if we try to
                // preserve the user's spacing by grouping them.  We must run the ending lines together, to save their
                // spacing, and then refresh w and wil to get the characters in order
                if (++ddendx < nw) {
                    wilv[ddendx][0] = trailstart;
        ...
        // We have replaced one DD with its equivalent explicit definition.  Rescan the line, starting at the first
        // location where DDBGN was seen
        for (firstddbgnx = ddschbgnx; firstddbgnx < nw; ++firstddbgnx) {
            US ch2 = *(US *)(wv + wilv[firstddbgnx][0]);
            ASSERT(!(ch2 == DDEND && (wilv[firstddbgnx][1] - wilv[firstddbgnx][0] == 2)), EVCTRL)
            if (ch2 == DDBGN && (wilv[firstddbgnx][1] - wilv[firstddbgnx][0] == 2)) break;
    ...
    ASSERT(AM(wil) >= 0,
           EVOPENQ);  // we have to make sure there is not an unscannable remnant at the end of the last line
