...
A jtevery(J jt, A w, A fs){A * RESTRICT wv,x,z,* RESTRICT zv;
 F1PREFIP;if(!w) return 0;RESETRANK;  // we claim to support IRS1 but really there's nothing to do for it
...
// apply f2 on items of a or w against the entirety of the other argument.  Pass on rank of f2 to reduce rank nesting
 ...
 A jteachr(J jt,A a,A w,A self){if(!(a && w && self)) return 0; I rcr=AR(w)-1<0?0:AR(w)-1; I rr=rr(self); rr=rcr<rr?rcr:rr; I lr=lr(self); lr=AR(a)<lr?AR(a):lr; return rank2ex(a,w,self,lr,rr,AR(a),rcr,FAV(self)->valencefns[1]);}
