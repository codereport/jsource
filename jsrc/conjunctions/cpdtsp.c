...
jtpdtspmv(J jt, A a, A w) {
    ...
    if (DENSE & AT(w)) {
        ...
        DQ(n, *u++ = *yv++; *v++ = *yv++;);
        ax = SPA(ap, x);
        RZ(wx = jtfrom(jt, yj, w));
    } else {
        v = AV(yj);
        DQ(n, yv++; *v++ = *yv++;);
        wp = PAV(w);
        RZ(b = jteps(jt, yj, SPA(wp, i)));
        bv     = BAV(b);
        ...
        RZ(ax = jtrepeat(jt, b, SPA(ap, x)));
        RZ(wx = jtfrom(jt, jtindexof(jt, SPA(wp, i), yj), SPA(wp, x)));
    }
    RZ(df2(x, yi, tymes(ax, wx), jtsldot(jt, jtslash(jt, ds(CPLUS)))));
    RZ(y = jtnub(jt, yi));
    RZ(g = jtgrade1(jt, y));
    GASPARSE(z, STYPE(AT(x)), 1, 1, AS(a));
    zp = PAV(z);
    SPB(zp, a, iv0);
...
jtpdtspvm(J jt, A a, A w) {
    ...
    if (DENSE & AT(a)) {
        ...
        RZ(ax = jtfrom(jt, jtindexof(jt, SPA(ap, i), yi), SPA(ap, x)));
        RZ(wx = jtrepeat(jt, b, SPA(wp, x)));
        RZ(x = tymes(ax, wx));
    }
    RZ(df2(y, yj, x, jtsldot(jt, jtslash(jt, ds(CPLUS)))));
    RZ(y = jtnub(jt, yj));
    RZ(g = jtgrade1(jt, y));
    GASPARSE(z, STYPE(AT(x)), 1, 1, 1 + AS(w));
    zp = PAV(z);
    SPB(zp, a, iv0);
    SPB(zp, e, jtscf(jt, 0.0));
    SPB(zp, i, jtfrom(jt, g, y));
...
static B
jtmmprep(J jt, P* p, I* n, I** iv, I* m, I** nv, D** xv) {
    A x;
    I j, k, q, *u, *v;
    x = SPA(p, x);
    if (!(FL & AT(x))) RZ(x = jtcvt(jt, FL, x));
    *xv = DAV(x);
    x   = SPA(p, i);
    *iv = u = AV(x);
    *n      = AN(x);
...
static B
jtmmharvest(J jt, I ii, I zjn, A zj, D* zyv, I* n, A* zi, A* zx) {
    A x;
    D *zxv, *zxv0;
    I j, m, p, *v, *ziv;
    m = MIN(*AS(*zi), *AS(*zx));
