...
static A
jttclosure(J jt, A a, A w) {
    A z;
    I an, *av, c, d, i, wn, wr, wt, *wv, *zv, *zz;
    wt = AT(w);
    wn = AN(w);
    wr = AR(w);
    if (B01 & wt) RZ(w = jtcvt(jt, INT, w));
    wv = AV(w);
    av = AV(a);
    an = AN(a);
    ...
    if (1 == wn) {        // just 1 result, which will be a list
        *zv++ = c = *wv;  // store first selection
        do {
            if (zv == zz) {
                i = zv - AV(z);
                ASSERT(i <= an, EVLIMIT) RZ(z = jtext(jt, 0, z));
                zv = AV(z) + i;
                zz = AV(z) + AN(z);
            }  // if we have more results than items in a, there must be a cycle - quit
            d = c;
            if ((UI)c >= (UI)an) {
                c += an;
                ASSERT((UI)c < (UI)an, EVINDEX);
            }
            *zv++ = c = av[c];  // d is prev selection, c is next.  Store c
        ...
        do {
            if (zv == zz) {
                i = zv - AV(z);
                ASSERT(i <= an * wn, EVLIMIT) RZ(z = jtext(jt, 0, z));
                zv = AV(z) + i;
                zz = AV(z) + AN(z);
            }  // break if there is a cycle
            resultdiff = 0;
            DQ(
              wn, d = c = zv[-wn]; if ((UI)c >= (UI)an) {
                  c += an;
                  ASSERT((UI)c < (UI)an, EVINDEX);
              } *zv++ = c = av[c];
              resultdiff |= c ^ d;);  // set diff if not a steady state
        } while (resultdiff);
        d = (zv - AV(z)) / wn - 1;
    }
    AS(z)[0] = d;
    AN(z)    = d * wn;
    MCISH(1 + AS(z), AS(w), wr);
    return z;
} /* {&a^:(<_) w */

static A
jtindexseqlim1(J jt, A w, A self) {
    A fs;
    fs = FAV(self)->fgh[0];  // {&x
...
static A
jtindexseqlim2(J jt, A a, A w, A self) {
    return 1 == AR(a) && AT(a) & INT && AN(w) && AT(w) & B01 + INT ? jttclosure(jt, a, w)
                                                                   : jtpowseqlim(jt, w, jtamp(jt, ds(CFROM), a));
} /* a {~^:(<_) w */

// u^:(<n) If n negative, take inverse of u; if v infinite, go to routine that checks for no change.  Otherwise convert
// to u^:(i.|n) and restart
static A
jtpowseq(J jt, A w, A self) {
    A fs, gs, x;
    I n = IMAX;
    V *sv;
    sv = FAV(self);
    ...
    x = AAV(gs)[0];
    if (!AR(x)) RE(n = jti0(jt, jtvib(jt, x)));
    if (0 > n) {
        RZ(fs = inv(fs));
        n = -n;
    }
...
#include "result.h"
    ...
    A n = sv->fgh[2];
    A rn;
    RZ(rn = jtravel(jt, n));  // n is powers, rn is ravel of n
    A gn;
    RZ(gn = jtgrade1(jt, rn));
    A p;
    RZ(p = jtnub(jt, jtfrom(jt, gn, rn)));  // gn is grade of power, p is sorted list of unique powers we want
    // find index of first nonneg power, remember, set scan pointer, set direction forward.  Set current power to 0.
    // Indic read of power needed
    I *pv  = IAV(p);
    I np   = AN(p);  // base of array of powers, and the number of them
    A z    = w;      // the next input/previous result
    I zpow = 0;      // The power corresponding to the value in z
    I pscan;
    for (pscan = 0; pscan < np && pv[pscan] < 0; ++pscan)
        ;              // scan pointer through powers, initialized to point to first nonnegative
...
#define ZZINSTALLFRAME(optr) *optr++ = np;
    ...
    A invfs;
    if (pv[0] < 0) RZ(invfs = inv(fs));  // if there is an inverse, look it up outside the loop so we don't free it
            ...
            if (!BETWEENO(pscan, 0, np)) {
                ...
                if (state & STATEPOSITIVEPOW) {
                    if ((pscan = pscan0 - 1) >= 0) {  // start on first negative power.  If there is one.
                        ...
                        fs   = invfs;
                        f1   = FAV(fs)->valencefns[0];  // switch over to the inverse function
                    }
                }
                if (!BETWEENO(pscan, 0, np)) break;  // when we exhaust the powers, quit the loop
            }
            neededpow = pv[pscan];
            pscan += state & STATEPOSITIVEPOW ? 1 : -1;
            state &= ~STATENEEDNEWPOW;  // take next power; advance to following; set no power needed till this consumed
        }
        if ((neededpow ^ REPSGN(neededpow)) <=
            (zpow ^ REPSGN(neededpow))) {  // check for closer to 0, based on direction
...
#include "result.h"
        ...
        } else {  // don't execute the verb willy-nilly because the store we just made may be the last  (we could check
                  // pscan and execute if we know another power is needed - but don't execute if zpow is infinite)
            // else execute verb, increment current power.  If there is an infinite power, see if result matches input,
            // set current power to infinite if so
            A oldz = z;
            RZ(z = CALL1(f1, z, fs));                   // run the verb
            zpow += state & STATEPOSITIVEPOW ? 1 : -1;  // increment the power corresponding to z
            if (state & STATEINFINITEPOW) {
                if (jtequ(jt, oldz, z)) zpow = state & STATEPOSITIVEPOW ? IMAX : -IMAX;
            }  // if no change, make z apply to everything.  Don't check unless _ power given
            if (!(zpow & 7)) {
                // Since we are not popping inside the loop (because each result must be reused even after its values
                ...
                if (zzbox) {
                    ...
                    AFLAG(zzbox) &= ~BOX;
                    DQ(AN(zzbox),
                       if (AAV(zzbox)[i]) tpush(AAV(zzbox)[i]);)  // mark zzbox nonrecursive; for each child, replace
                                                                  // the implied free with an explicit one on the stack
                }
            }
        }
    }
...
#include "result.h"
    ...
    if (pscan0) {
        A sneg;
        RZ(sneg = jtsc(jt, pscan0));
        RZ(p = apip(jtdrop(jt, sneg, p), jtreverse(jt, jttake(jt, sneg, p))));
    }
    // result is ($n) $ (p i. ,n) { result - avoid the reshape if n is a list, and avoid the from if (p i. ,n) is an
    // index vector
...
static A
jtpinf12(J jt, A a, A w, A self) {
    PROLOG(0340);
    A z;  // no reason to inplace, since w must be preserved for comparison, & a for reuse
    I i       = 0;
    I ismonad = (AT(w) >> VERBX) & 1;
    self      = ismonad ? w : self;  // the call shows the dyad; if it's really a monad, w->self, leave arg in a
    A fs      = FAV(self)->fgh[0];
    w         = ismonad ? fs : w;  // for monad, fs->w
    ...
    while (1) {
        ...
        oldw    = ismonad ? a : oldw;
        w       = ismonad ? w : z;
        a       = ismonad ? z : a;       // oldw=input w to f; save result for next loop overwriting a(monad) or w(dyad)
...
jtinv1(J jt, A w, A self) {
    ...
    RZ(i = inv((fs)));
    FDEPINC(1);
    z = (FAV(i)->valencefns[0])(FAV(i)->flag & VJTFLGOK1 ? jtinplace : jt, w, i);
    FDEPDEC(1);
    return z;
}  // was invrecur(jtfix(jt,fs))
static A
jtinvh1(J jt, A w, A self) {
    FPREFIP;
    DECLFGH;
    A z;
    FDEPINC(1);
    z = (FAV(hs)->valencefns[0])(jtinplace, w, hs);
    FDEPDEC(1);
    return z;
}
static A
jtinv2(J jt, A a, A w, A self) {
    DECLFG;
    A z;
    FDEPINC(1);
    df1(z, w, inv(jtamp(jt, a, fs)));
    FDEPDEC(1);
    STACKCHKOFL return z;
}  // the CHKOFL is to avoid tail recursion, which prevents a recursion loop from being broken
static A
jtinverr(J jt, A w, A self) {
    FPREFIP;
    ASSERT(0, EVDOMAIN);
...
// We catch the special cases 0  & 1 here, mostly for branch-prediction purposes.  All results of g1/g2 will be nouns,
// while most instances of u^:v (run through powop) have v as verb
...
// here for u^:v y
REFACTORME_CS1IP(
  jtpowv1, A u; A v; RZ(u = CALL1(g1, w, gs)); /* execute v */
  if (!AR(u) && (v = jtvib(jt, u)) && !(IAV(v)[0] & ~1)) {
...
// here for x u^:v y
REFACTORME_CS2IP(
  jtpowv2, A u; A v; RZ(u = CALL2(g2, a, w, gs)); /* execute v */
  if (!AR(u) && (v = jtvib(jt, u)) && !(IAV(v)[0] & ~1)) {
...
REFACTORME_CS2IP(
  jtpowv2a, jtinplace = (J)((I)jtinplace & ~JTINPLACEA); /* monads always have IP2 clear */
  A u;
  A v;
  fs = FAV(fs)->fgh[0];
  RZ(u = CALL2(g2, a, w, gs)); /* execute v */
  if (!AR(u) && (v = jtvib(jt, u)) && !(IAV(v)[0] & ~1)) {
      z = IAV(v)[0] ? (FAV(fs)->valencefns[0])(FAV(fs)->flag & VJTFLGOK1 ? jtinplace : jt, w, fs) : w;
  } else {
      RESETERR;
...
jtpowop(J jt, A a, A w, A self) {
    ...
    if (AT(w) & VERB) {
        ...
        v = FAV(a);
        b = ((v->id & ~1) == CATCO) && ID(v->fgh[1]) == CRIGHT;  // detect u@]^:v  (or @:)
        // The action routines are inplaceable; take ASGSAFE from u and v, inplaceability from u
        return CDERIV(CPOWOP,
                      jtpowv1cell,
                      b ? jtpowv2acell : jtpowv2cell,
                      (v->flag & FAV(w)->flag & VASGSAFE) + (v->flag & (VJTFLGOK1 | VJTFLGOK2)),
                      RMAX,
                      RMAX,
                      RMAX);
    }
    ...
    if (BOX & AT(w)) {
        ...
        if (!AR(w) && (x = AAV(w)[0], !AR(x) && NUMERIC & AT(x) || 1 == AR(x) && !AN(x))) {
            ...
            if ((!AN(x) || FL & AT(x) && inf == DAV(x)[0])) {
                if (CAMP == v->id && (CFROM == ID(v->fgh[0]) && (y = v->fgh[1], INT & AT(y) && 1 == AR(y)))) {
                    f1 = jtindexseqlim1;
                }  // {&b^:_ y
                else if (CTILDE == v->id && CFROM == ID(v->fgh[0])) {
                    f2 = jtindexseqlim2;
                }  // x {~^:_ y
            }
    ...
    if (((-(AT(w) & B01 + INT)) & ((AR(w) | ((UI)BIV0(w) >> 1)) - 1)) < 0)
        return a = BIV0(w) ? a : ds(CRIGHT);  //  u^:0 is like ],  u^:1 is like u   AR(w)==0 and B01|INT and BAV0=0 or 1
    RZ(hs = jtvib(jt, w));                    // hs=n coerced to integer
    AF f1 = jtply1;  // default routine for general array.  no reason to inplace this, since it has to keep the old
                     // value to check for changes
    ...
    if (!AR(w)) {    // input is an atom
        ...
        if ((IAV(hs)[0] << 1) == -2) {                                   //  u^:_1 or u^:_
            if (IAV(hs)[0] < 0) {                                        // u^:_1
                // if there are no names, calculate the monadic inverse and save it in h.  Inverse of the dyad, or the
                // monad if there are names, must wait until we get arguments
                A h = 0;
                f1  = jtinv1;
                if (nameless(a)) {
                    if (h = inv(a)) {
                ...
                }  // h must be valid for free.  If no names in w, take the inverse.  If it doesn't exist, fail the
                   // monad but keep the dyad going
        ...
        if (IAV(hs)[0] >= 0) {
            f1   = jtfpown;
            flag = FAV(a)->flag & VJTFLGOK1;
        }  // if nonneg atom, go to special routine for that, which supports inplace
    }
