...
static A
jtfitct1(J jt, A w, A self) {
    DECLFG;
    FPREFIP;
    A z;
    PUSHCCT(FAV(self)->localuse.lD) z = CALL1IP(f1, w, fs);
    POPCCT return z;
}  // lD has the complementary ct

#define fitctvector(name, vector)                   \
    static A name(J jt, A a, A w, A self) {         \
        DECLFG;                                     \
        FPREFIP;                                    \
        A z;                                        \
        PUSHCCT(FAV(self)->localuse.lD) z = vector; \
        POPCCT return z;                            \
    }
fitctvector(jtfitct2, CALL2IP(f2, a, w, fs)) fitctvector(jtfitcteq, jtatomic2(jtinplace, a, w, fs))
...
jtfitct(J jt, A a, A w, I cno) {
    ...
    A fn = fdef(0,
                ...
                w,
                0L,
                sv->flag & (VIRS1 | VIRS2 | VJTFLGOK1 | VJTFLGOK2 | VISATOMIC1),
                (I)(sv->mr),
                lrv(sv),
                rrv(sv));  // preserve INPLACE flags
    RZ(fn);
    FAV(fn)->localuse.lD = 1.0 - d;
    return fn;  // save the fit value in this verb
}

static A
jtfitexp2(J jt, A a, A w, A self) {
    F2RANK(0, 0, jtfitexp2, self);
    ASSERT(0 <= jti0(jt, w) && !jt->jerr, EVDOMAIN);
    A z;
    return jtaslash(jt, CSTAR, plus(a, df2(z, jtiota(jt, w), FAV(self)->fgh[1], jtslash(jt, ds(CSTAR)))));
} /* a ^!.s w */

static A
jtfitpoly2(J jt, A a, A w, A self) {
    I j;
    ...
    return jtaslash(
      ...
      tymes(
        a,
        jtascan(jt, CSTAR, jtshift1(jt, plus(w, df2(z, IX(SETIC(a, j)), FAV(self)->fgh[1], jtslash(jt, ds(CSTAR))))))));
} /* a p.!.s w */

static A
jtfitfill1(J jt, A w, A self) {
    DECLFG;
    FPREFIP;
    A z;
    jt->fill = gs;
    ...
    jt->fill = 0;
    return z;
}  // gs cannot be virtual
static A
jtfitfill2(J jt, A a, A w, A self) {
    DECLFG;
    FPREFIP;
    A z;
    jt->fill = gs;
...
jtfit(J jt, A a, A w) {
    ...
    switch (sv->id) {
        ...
        case CPOWOP:                                           // support for #^:_1!.n
            ...
            if (c == CPOUND) {
                ASSERT(!AR(w), EVRANK);
                return CDERIV(CFIT, 0, jtfitfill2, VFLAGNONE, m, l, r);
            }  // CPOWOP has no VIRS
        ...
        case CCOMDOT:
        case CLAMIN:
        case CRAZE: ASSERT(!AR(w), EVRANK); /* fall thru */
        case CROT:
        case CDOLLAR:
            ...
            return CDERIV(CFIT,
                          jtfitfill1,
                          jtfitfill2,
                          sv->flag & (VIRS1 | VIRS2 | VJTFLGOK1 | VJTFLGOK2),
                          m,
                          l,
                          r);  // preserve INPLACE flags
        case CTHORN:
