...
static A
jtipprep(J jt, A a, A w, I zt, I* pm, I* pn, I* pp) {
    A z = mark;
    I *as, ar, ar1, m, mn, n, p, *ws, wr, wr1;
    ar                           = AR(a);
    as                           = AS(a);
    ar1                          = ar - 1 >= 0 ? ar - 1 : 0;
    PRODX(m, ar1, as, 1)* pm     = m;  // m=# 1-cells of a.  It could overflow, if there are no atoms
    wr                           = AR(w);
    ws                           = AS(w);
    wr1                          = wr - 1 >= 0 ? wr - 1 : 0;
    PRODX(n, wr1, ws + 1, 1)* pn = n;
    DPMULDE(m, n, mn);  // n=#atoms in item of w; mn = #atoms in result
    I t = AS(w)[0];
    p   = wr ? t : 1;
    t   = AS(a)[ar1];
    p   = ar ? t : p;
    *pp = p;  // if a is an array, the length of a 1-cell; otherwise, the number of items of w
...
#define PDTBY(Tz, Tw, INC)                                                                                         \
    {                                                                                                              \
        ...
        v = wv = (Tw*)AV(w);                                                                                       \
        zv     = (Tz*)AV(z);                                                                                       \
        memset(&zero, C0, sizeof(Tz));                                                                             \
        if (1 == n)                                                                                                \
            DQ(m, v = wv; c = zero; DQ(p, if (*u++) INC(c, *v); ++v;); *zv++ = c;)                                 \
        else                                                                                                       \
            DQ(m, v = wv; memset(zv, C0, zk); DQ(p, x = zv; if (*u++) DQ(n, INC(*x, *v); ++x; ++v;) else v += n;); \
               zv += n;);                                                                                          \
    }
...
#define PDTXB(Tz, Ta, INC, INIT)                                                                                  \
    {                                                                                                             \
        ...
        memset(&zero, C0, sizeof(Tz));                                                                            \
        if (1 == n)                                                                                               \
            DQ(m, v = wv; c = zero; DQ(p, if (*v++) INC(c, *u); ++u;); *zv++ = c;)                                \
        else                                                                                                      \
            DQ(m, v = wv; memset(zv, C0, zk); DQ(p, x = zv; INIT; DQ(n, if (*v++) INC(*x, c); ++x;);); zv += n;); \
    }

static A
jtpdtby(J jt, A a, A w) {
    A z;
    B b, *u, *v, *wv;
    C er = 0;
    I at, m, n, p, t, wt, zk;
    ...
    switch (CTTZ(t)) {
        ...
        case CMPXX:
            if (at & B01) PDTBY(Z, Z, ZINC) else PDTXB(Z, Z, ZINC, c = *u++);
            break;
        case FLX:
            if (at & B01) PDTBY(D, D, DINC) else PDTXB(D, D, DINC, c = *u++);
            break;
        case INTX:
...
I
cachedmmult(J jt, D* av, D* wv, D* zv, I m, I n, I p, I flgs) {
    D c[(CACHEHEIGHT + 1) * CACHEWIDTH + (CACHEHEIGHT + 1) * OPHEIGHT * OPWIDTH * 2 +
        2 * CACHELINESIZE / sizeof(D)];  // 2 in case complex
    // m is # 1-cells of a
    ...
    for (; w0rem > 0; w0rem -= CACHEWIDTH, w0base += CACHEWIDTH, z0base += CACHEWIDTH) {
        // process each 16x64 section of w, adding each result to the columns of z.  Each section goes through a
        // different set of 16/32 columns of a
        D* a1base = av;
        D* w1base = w0base;
        D* z1base = z0base;
        ...
        for (; w1rem > 0; w1rem -= CACHEHEIGHT, a1base += CACHEHEIGHT << (flgs & FLGCMP), w1base += CACHEHEIGHT * n) {
            ...
            for (i = MIN(CACHEHEIGHT, w1rem), cvx = cvw; i; --i) {
                ...
                // prefetch it
                for (j = 0; j < MIN(CACHEWIDTH, w0rem); ++j) {
                    D fv = *w1x;
                    if (flgs & FLGINT) fv = (D) * (I*)w1x;
                    *cvx++ = fv;
                    w1x++;
                }                                          // move the valid data
                for (; j < CACHEWIDTH; ++j) *cvx++ = 0.0;  // fill the rest with 0
            }
            // Because of loop unrolling, we fetch and multiply one extra value in each cache column.  We make sure
            // those values are 0 to avoid NaN errors
            ...
            for (; a2rem > 0; a2rem -= OPHEIGHT, a2base0 += OPHEIGHT * p, z2base += OPHEIGHT * n) {
                ...
                // If the second row of a is off the end of the data, we mustn't fetch it - switch the pointer to a row
                // of 1s so it won't give NaN error on multiplying by infinity
                ...
                for (; a3rem > 0; a3rem -= OPWIDTH, c3base += OPWIDTH, z3base += OPWIDTH) {
                    ...
                    z00 = z3base[0];
                    if (a3rem > 3) {
                        z01 = z3base[1], z02 = z3base[2], z03 = z3base[3];
                        if (a2rem > 1) z10 = z3base[n], z11 = z3base[n + 1], z12 = z3base[n + 2], z13 = z3base[n + 3];
                    } else {
                        if (a3rem > 1) {
                            z01 = z3base[1];
                            if (a3rem > 2) z02 = z3base[2];
                        };
                        if (a2rem > 1) {
                            z10 = z3base[n];
                            if (a3rem > 1) z11 = z3base[n + 1];
                            if (a3rem > 2) z12 = z3base[n + 2];
                        }
                    }
                    ...
                    z3base[0] = z00;
                    if (a3rem > 3) {
                        z3base[1] = z01, z3base[2] = z02, z3base[3] = z03;
                        if (a2rem > 1) z3base[n] = z10, z3base[n + 1] = z11, z3base[n + 2] = z12, z3base[n + 3] = z13;
                    } else {
                        if (a3rem > 1) {
                            z3base[1] = z01;
                            if (a3rem > 2) z3base[2] = z02;
                        };
                        if (a2rem > 1) {
                            z3base[n] = z10;
                            if (a3rem > 1) {
                                z3base[n + 1] = z11;
                                if (a3rem > 2) z3base[n + 2] = z12;
                            }
                        }
                    }
                }
...
jtpdt(J jt, A a, A w) {
    ...
    if (((at | wt) & XNUM + RAT) != 0)
        return df2(
          ...
          jtatop(jt,
                 jtslash(jt, ds(CPLUS)),
                 jtqq(jt, ds(CSTAR), jtv2(jt, 1L, AR(w)))));  // On indirect numeric, execute as +/@(*"(1,(wr)))
    if (B01 & (at | wt) && TYPESNE(at, wt) && ((ar - 1) | (wr - 1) | (AN(a) - 1) | (AN(w) - 1)) >= 0)
        return jtpdtby(jt, a, w);  // If exactly one arg is boolean, handle separately
    {
        t = maxtyped(at, wt);
        if (!TYPESEQ(t, AT(a))) { RZ(a = jtcvt(jt, t, a)); }
        if (!TYPESEQ(t, AT(w))) { RZ(w = jtcvt(jt, t, w)); }
    }  // convert args to compatible precisions, changing a and w if needed.  B01 if both empty
    ...
    if (!p) {
        ...
        return z;
    }  // if dot-products are all 0 length, set them all to 0
    // If either arg is atomic, reshape it to a list
    if (!ar != !wr) {
        if (ar) RZ(w = jtreshape(jt, jtsc(jt, p), w)) else RZ(a = jtreshape(jt, jtsc(jt, p), a));
    }
    p1 = p - 1;
    ...
    switch (CTTZNOFLAG(t)) {
        ...
        case INTX: {
            ...
            */
            // INT product is problematic, because it is used for many internal purposes, such as #. and indexing of {
            // and m} .  For these uses, one argument (usually w) has only one item, a list that is reused.  So, we
            // check for that case; if found we go through faster code that just performs vector inner products,
            ...
            if (n == 1) {
                ...
                DQ(
                  ...
                  wv++;)
                if (!er) {
                    // w fits in 32 bits.  Try to accumulate the products.  If we can be sure that the total will not
                    // exceed 32 bits unless an a-value does, do the fastest loop
                    zv = AV(z);
                    av = AV(a);
                    if ((UI)(p * tot) < (UI)0x100000000) {  // integer overflow
                        ...
                        DQ(m, I tot = 0; wv = AV(w);
                           DQ(p, I mpcnd = *av++; I prod = mpcnd * *wv++; if (mpcnd != (I4)mpcnd) goto oflo1;
                              tot += prod;)* zv++ = tot;)
                    } else {
                        // w has some big numbers, so we have to check each individual product
                        DQ(m, I tot = 0; wv = AV(w); DQ(p, I mpcnd = *av++; I prod = mpcnd * *wv++;
                                                        if (mpcnd != (I4)mpcnd || prod != (I4)prod) goto oflo1;
                                                        tot += prod;)* zv++ = tot;)
                    }
                    AT(z) = INT;
                    break;
            ...
            oflo1:
                ...
                zv = AV(z);
                av = AV(a);
                DQ(m, I lp; I tot = 0; wv = AV(w);
                   DQ(p, DPMULD(*av++, *wv++, lp, goto oflo2;) I oc = (~tot) ^ lp; tot += lp; lp ^= tot;
                      if (XANDY(oc, lp) < 0) goto oflo2;)* zv++ = tot;)
                AT(z) = INT;
                break;
            oflo2:
                ...
                if (m)
                    RZ(jtsumattymesprods(
                      ...
                      1,
                      m,
                      voidAV(z)));  // use +/@:*"1 .  Exchange w and a because a is the repeated arg in
                                    // jtsumattymesprods.  If error, clear z (should not happen here)
            } else {
                // full matrix products
                RZ(a = jtcvt(jt, FL, a));
                RZ(w = jtcvt(jt, FL, w));
                cachedmmult(jt, DAV(a), DAV(w), DAV(z), m, n, p, 0);  // Do our matrix multiply - converting   TUNE
                // If the result has a value that has been truncated, we should keep it as a float.  Unfortunately,
                ...
                if (!i) {
                    AT(z) = INT;
                    for (zv = DAV(z), i = AN(z); i; --i, ++zv) *(I*)zv = (I)*zv;
                }  // if not, convert all to integer
            }
        } break;
        case FLX: {
            I smallprob;
            // As for INT, handle the cases where n=1 separately, because they are used internally & don't require as
            // much setup as matrix multiply
            NAN0;
            if (n == 1) {
                if (m)
                    RZ(jtsumattymesprods(jt,
                                         FL,
                                         voidAV(w),
                                         voidAV(a),
                                         p,
                                         ...
                                         1,
                                         m,
                                         voidAV(z)));  // use +/@:*"1 .  Exchange w and a because a is the repeated arg
                                                       // in jtsumattymesprods.  If error, clear z
                smallprob = 0;                         // Don't compute it again
            } else {
                // not single column.  Choose the algorithm to use
                I probsize = (m - 1) * n *
                             (IL)p;  // This is proportional to the number of multiply-adds.  We use it to select the
                                     // implementation.  If m==1 we are doing dot-products; no gain from fancy code then
                if (!(smallprob =
                        (m <= 4 ||
                         probsize <
                           1000LL))) {  // if small problem, avoid the startup overhead of the matrix version  TUNE
                    cachedmmult(jt,
                                DAV(a),
                                DAV(w),
                                DAV(z),
                                ...
                                n,
                                p,
                                ((AFLAG(a) >> (AFUPPERTRIX - FLGAUTRIX)) & FLGAUTRI) |
                                  ((AFLAG(w) >> (AFUPPERTRIX - FLGWUTRIX)) &
                                   FLGWUTRI));  // Do our one-core matrix multiply - real   TUNE this is 160x160 times
                                                // 160x160.  Tell routine if uppertri
                }
            }
            // If there was a floating-point error, retry it the old way in case it was _ * 0
            if (smallprob || NANTEST) {
                D c, s, t, *u, *v, *wv, *x, *zv;
                u = DAV(a);
                v = wv = DAV(w);
                zv     = DAV(z);
                NAN0;
                if (1 == n) {
                    DQ(m, v = wv; c = 0.0; DQ(p, s = *u++; t = *v++; c += TYMES(s, t);); *zv++ = c;);
                } else
                    for (i = 0; i < m; ++i, v = wv, zv += n) {
                        x = zv;
                        if (c = *u++) {
                            if (INF(c)) DQ(n, *x++ = TYMES(*v, c); ++v;) else DQ(n, *x++ = c * *v++;);
                        } else {
                            v += n;
                            DQ(n, *x++ = 0.0;);
                        }
        ...
        case CMPXX: {
            ...
            I probsize =
              m * n *
              (IL)p;  // This is proportional to the number of multiply-adds.  We use it to select the implementation
            ...
            if (!smallprob) {               // use old-fashioned way if small.  16b3.4 comes though here
                smallprob =
                  1 ^ cachedmmult(jt,
                                  ...
                                  p * 2,
                                  ((AFLAG(a) >> (AFUPPERTRIX - FLGAUTRIX)) & FLGAUTRI) |
                                    ((AFLAG(w) >> (AFUPPERTRIX - FLGWUTRIX)) & FLGWUTRI) |
                                    FLGCMP);  // Do the fast matrix multiply - complex.  Change widths to widths in D
                                              // atoms, not complex atoms  TUNE  this is 130x130 times 130x130
            }
            if (smallprob || NANTEST) {
                Z c, *u, *v, *wv, *x, *zv;
                ...
                NAN0;
                if (1 == n)
                    DQ(m, v = wv; c = zeroZ; DQ(p, c.re += ZRE(*u, *v); c.im += ZIM(*u, *v); ++u; ++v;); *zv++ = c;)
                else
                    for (i = 0; i < m; ++i, v = wv, zv += n) {
                        x = zv;
                        c = *u++;
                        DQ(n, x->re = ZRE(c, *v); x->im = ZIM(c, *v); ++x; ++v;);
...
jtipbx(J jt, A a, A w, C c, C d) {
    ...
    wc  = AR(w) ? n : 0;
    q   = (n - 1) >> LGSZI;
    r = (-n) & (SZI - 1);  // ana = 1 if a is not atomic; wc = stride between items of w; q=#fullwords to proc, r=#bytes
                           // of last one NOT to proc
                           // Set c0 & c1 to classify the g operation
                           // See if we can use special techniques
#define Q9(f, c0, c1) ((I)(((c1) << 2) + (c0)) << (((f)-CSTARCO) << 2))
#define PMSK                                                                                                      \
    Q9(CSTARCO, IPBX1, IPBXNW) + Q9(CPLUSCO, IPBXNW, IPBX0) + Q9(CSTAR, IPBX0, IPBXW) + Q9(CPLUS, 0, 0) +         \
      Q9(CPLUSDOT, IPBXW, IPBX1) + Q9(CSTARDOT, IPBX0, IPBXW) + Q9(CEQ, IPBXNW, IPBXW) + Q9(CNE, IPBXW, IPBXNW) + \
      Q9(CLT, IPBXW, IPBX0) + Q9(CLE, IPBX1, IPBXW) + Q9(CGE, IPBXNW, IPBX1) + Q9(CGT, IPBX0, IPBXNW) +           \
      Q9(CEBAR, 0, 0) + Q9(CEPS, 0, 0) + Q9(CMIN, IPBX0, IPBXW) + Q9(CMAX, IPBXW, IPBX1)
    I pmsk = 0;
    pmsk   = BETWEENC(d, CSTARCO, CMAX) ? PMSK : pmsk;
    pmsk &= -(B01 & AT(w));
    c0 = (pmsk >> ((d - CSTARCO) << 2)) & 3;
    c1 = (pmsk >> (((d - CSTARCO) << 2) + 2)) & 3;  // get control bits if known fn, (0,0) if unknown
    ...
    I esat = 2, eskip = 2;  // if byte of a equals esat, the entire result-cell is set to its limit value; if it equals
                            // eskip, the innerproduct is skipped
    if (c == CPLUSDOT && (c0 == IPBX1 || c1 == IPBX1) || c == CSTARDOT && (c0 == IPBX0 || c1 == IPBX0)) {
        // +./ . [+. <: >: *:}   *./ . [*. < > +:]   a byte of a can saturate the entire result-cell: see which value
        // does that
        esat =
          c == CPLUSDOT ? c1 == IPBX1 : c1 == IPBX0;  // esat==1 if +./ . (+. >:)   or *./ . (< +:) where x=1 overrides
                                                      // y (producing 1);  if esat=0, x=0 overrides y (producing 1)
    } else if (c == CPLUSDOT && (c0 == IPBX0 || c1 == IPBX0) || c == CSTARDOT && (c0 == IPBX1 || c1 == IPBX1) ||
               c == CNE && (c0 == IPBX0 || c1 == IPBX0)) {
        // [+. ~:]/ . [*. < > +:]   *./ . [+. <: >: *:]  a byte of a can guarantee the current innerproduct has no
        // effect
        eskip = c == CSTARDOT ? c1 == IPBX1 : c1 == IPBX0;  // eskip==1 if  (+. ~:)/ . (+:)   *./ . (+. >:)  where x=1
                                                            // has no effect; if esat=0, x=0 has no effect
    }
...
jtdotprod(J jt, A a, A w, A self) {
    ...
    return df2(
      ...
      a,
      w,
      jtatop(jt, fs, jtqq(jt, gs, jtv2(jt, r == RMAX ? r : 1 + r, RMAX))));  // inner product according to the Dic
}
...
jtdet(J jt, A w, A self) {
    ...
    s = AS(w);
    A z;
    if (h && 1 < r && 2 == s[r - 1] && s[r - 2] == s[r - 1]) return df1(z, w, h);
    F1RANK(2, jtdet, self);
    c = 2 > r ? 1 : s[1];
    return !c               ? df1(z, mtv, jtslash(jt, gs))
           : 1 == c         ? CALL1(f1, jtravel(jt, w), fs)
           : h && c == s[0] ? jtgaussdet(jt, w)
                            : jtdetxm(jt, w, self);
}
