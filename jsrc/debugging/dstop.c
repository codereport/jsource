...
jtdbstop(J jt, DC d, I i) {
    ...
    if (i == d->dcstop) {
        d->dcstop = -2;
        return 0;
    } /* not stopping if already stopped at the same place */
    if (!(jt->dbstops)) return 0;
    s = CAV(jtstr0(jt, jt->dbstops));
    sprintf(nw, FMTI, i);
    a  = d->dca;
    n  = d->dcm;
    t  = NAV(a)->s;
...
jtdbstopq(J jt, A w) {
    ASSERTMTV(w);
    return jt->dbstops ? jt->dbstops : mtv;
}
/* 13!:2  query stops */
