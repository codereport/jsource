...
static void jteputl(J jt,A w){jtep(jt,AN(w),CAV(w)); jteputc(jt,CLF);}
...
static void jteputq(J jt,A w,I nflag){C q=CQUOTE,*s;
 if(jtequ(jt,ds(CALP),w))jteputs(jt," a."+!nflag);
 else{
  jteputc(jt,q);
  s=CAV(w); DO(AN(w), jteputc(jt,s[i]); if(q==s[i])jteputc(jt,q););
  jteputc(jt,q);
...
static void jtdspell(J jt,C id,A w,I nflag){C c,s[5];
 if(id==CFCONS){if(nflag)jteputc(jt,' '); jteputv(jt,FAV(w)->fgh[2]); jteputc(jt,':');}
 else{
  s[0]=' '; s[4]=0;
  spellit(id,1+s);
  c=s[1]; 
...
static I jtdisp(J jt,A w,I nflag){B b=1&&AT(w)&NAME+NUMERIC;
 ...
 case BOXX:
  if(!(AT(w)&BOXMULTIASSIGN)){jteputs(jt," a:"+!nflag); break;}
  // If this is an array of names, turn it back into a character string with spaces between
  else{w=jtcurtail(jt,jtraze(jt,every2(jtevery(jt,w,(A)&sfn0overself),chrspace,(A)&sfn0overself)));}  // }: (string&.> names) ,&.> ' '  then fall through to display it
...
static void jtseeparse(J jt,DC d){A*v;I m;
 ...
 DO(d->dcn, if(i==m)jteputs(jt,"    "); nflag=jtdisp(jt,v[i],nflag););  // display tokens with spaces before error
...
void jtdebdisp(J jt,DC d){A*x,y;I e,t;
 ...
 t=d->dctype;
 if(e&&!jt->etxn&&(t==DCPARSE||t==DCCALL)){x=e+AAV(jt->evm); jtdhead(jt,0,0L); jteputl(jt,*x);}  // if error, display error header
 switch(t){
  case DCPARSE:  jtdhead(jt,3,d); jtseeparse(jt,d); if(NETX==jt->etxn)--jt->etxn; jteputc(jt,CLF); break;
  case DCCALL:   jtdhead(jt,0,d); jtseecall(jt,d);  jteputc(jt,CLF); break;
  case DCSCRIPT: jtdhead(jt,0,d); jtefmt(jt,"[-"FMTI"] ", d->dcn-1); 
                 if(0<=d->dcm){y=*(d->dcm+AAV(jt->slist)); jtep(jt,AN(y),CAV(y));}
                 jteputc(jt,CLF); break;
}}
...
static B jtdebsi1(J jt,DC d){I t;
 ...
 ASSERTMTV(w);
 if(d){if(DCCALL!=d->dctype)d=d->dclnk; while(d){jtdebdisp(jt,d); d=d->dclnk;}}
 return mtm;
...
static void jtjsigstr(J jt,I e,I n,C*s){
 ...
 jtdhead(jt,0,0L);
 if(jt->uflags.us.cx.cx_c.db&&!jtspc(jt)){jteputs(jt,"ws full (can not suspend)"); jteputc(jt,CLF); jt->uflags.us.cx.cx_c.db=0;}
 jtep(jt,n,s);
 if(jt->curname){if(!jt->uflags.us.cx.cx_c.glock){jteputs(jt,": "); jtep(jt,AN(jt->curname),NAV(jt->curname)->s);} jt->curname=0;}
 jteputc(jt,CLF);
...
void jtjsignal3(J jt,I e,A w,I j){
 ...
 if(!jt->uflags.us.cx.cx_c.glock){
  if(e==EVCTRL){jtdhead(jt,3,0L); jtefmt(jt,"["FMTI"]",j); jteputl(jt,w);}
  else{
   jtdhead(jt,3,0L); jteputl(jt,w);
   jtdhead(jt,3,0L); DQ(j, jteputc(jt,' ');); jteputc(jt,'^'); jteputc(jt,CLF);
  }
...
static A jtdbsig(J jt,A a,A w){I e;
 RE(0);
 if(!AN(w))return mtm;
 RZ(w=jtvi(jt,w)); e=AV(w)[0]; 
 ASSERT(1<=e,EVDOMAIN);
 ASSERT(e<=255,EVLIMIT);
 if(a||e>NEVM){if(!a)a=mtv; RZ(a=jtvs(jt,a)); jtjsig(jt,e,a);} else jtjsignal(jt,e);
 return 0;
}    
 ...
 A jtdberr(J jt, A w){ASSERTMTV(w); return jtsc(jt,jt->jerr1);}           /* 13!:11 last error number   */
