...
static A jtdfrep(J jt, A w){ return NOUN&AT(w)?w:jtlrep(jt,w);}

static SYMWALK(jtdloc,A,BOX,5,2,1,{RZ(*zv++=jtincorp(jt,jtsfn(jt,0,d->name))); RZ(*zv++=jtincorp(jt,jtdfrep(jt,d->val)));})
 ...
 *zv++=jtincorp(jt,q);                                  /* 6 argument list            */
 if(si->dcloc){RZ(y=jtdloc(jt,si->dcloc)); RZ(*zv++=jtincorp(jt,jtgrade2(jt,y,jtope(jt,IRS1(y,0L,1L,jthead,z)))));}
 else         RZ(*zv++=jtincorp(jt,jtiota(jt,jtv2(jt,0L,2L))));   /* 7 locals                   */  // empty so cannot be readonly
 ...
 RZ(y=jtfrom(jt,jtscind(jt,IRS1(z,0L,1L,jthead,zz)),jtover(jt,jtsnl(jt,mtv),ds(CACE))));  // get script index for each line of stack; then fetch the name, or a: if no name
 yv=AAV(y); zv=5+AAV(z);
