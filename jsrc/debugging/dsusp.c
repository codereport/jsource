...
static void
movesentencetosi(J jt, void *wds, I nwds, I errwd) {
    if (jt->sitop && jt->sitop->dctype == DCPARSE) {
        jt->sitop->dcy  = (A)wds;
        jt->sitop->dcn  = (I)nwds;
        jt->sitop->dcix = (I)errwd;
    }
}
static void
movecurrtoktosi(J jt) {
    if (jt->sitop && jt->sitop->dctype == DCPARSE) { jt->sitop->dcix = jt->parserstackframe.parsercurrtok; }
}
void
moveparseinfotosi(J jt) {
    movesentencetosi(
...
DC
jtdeba(J jt, C t, void *x, void *y, A fs) {
    DC d;
    {
        A q;
        GAT0(q, LIT, sizeof(DST), 1);
        d = (DC)AV(q);
    }
    memset(d, C0, sizeof(DST));
    if (jt->sitop && t != DCJUNK)
        moveparseinfotosi(
          jt);  // if we are creating a space between normal and suspension, don't modify the normal stack
    d->dctype = t;
    d->dclnk  = jt->sitop;
    jt->sitop = d;
    switch (t) {
...
lnumcw(I j, A w) {
    ...
    if (0 > j)
        ...
        u = (CW *)AV(w);
        DO(AN(w), if (j <= u[i].source) return i;) return IMAX / 2;
    }
...
lnumsi(DC d) {
    ...
    if (c = d->dcc) {
        i = d->dcix;
        return (MIN(i, AN(c) - 1) + (CW *)AV(c))->source;
    } else
        return 0;
...
suspset(DC d) {
    ...
    if (d->dcc) {
        RZ(e);
        e->dcsusp = 1;
    }  // if explicit, set susp on line - there should always be a following frame, but if not do nothing
    else
...
jtsusp(J jt) {
    ...
    // and it will create a stack frame for its result.  CREATION of this stack frame will overwrite the current
    ...
    if (jt->dbssexec) {
        RESETERR;
        jtimmex(jt, jt->dbssexec);
        jttpop(jt, old);
    } else if (jt->dbtrap) {
        RESETERR;
        jtimmex(jt, jt->dbtrap);
        jttpop(jt, old);
    }
    while (jt->dbsusact == SUSCONT) {
        ...
        if ((inp = jtjgets(jt, "      ")) == 0)
            ...
            jtimmex(jt, inp);  // read and execute a line, but exit debug if error reading line
        jttpop(jt, old);
    }
    if (jt->dbuser) {
...
// Result is the value that will be used for the failing sentence.  This should not be 0 unless there is an error,
// because jtxdefn requires nonzero z during normal operation
static A
jtdebug(J jt) {
    A z = 0;
    C e;
    ...
    if (jt->dbssd) {
        jt->dbssd->dcss = 0;
        jt->dbssd       = 0;
    }  // clear previous single-step state - should do at end instead
    ...
    jtsusp(jt);
    switch (jt->dbsusact) {
        case SUSRUN: DGOTO(d, d->dcix); break;
        case SUSRET:
            DGOTO(d, -1) z = jt->dbresult;
            jt->dbresult   = 0;
            break;
        case SUSJUMP: DGOTO(d, lnumcw(jt->dbjump, d->dcc)) break;
        case SUSCLEAR:
            jt->jerr = e;
            c        = jt->sitop;
            while (c) {
                if (DCCALL == c->dctype) DGOTO(c, -1) c = c->dclnk;
            }
            break;
    }
...
// post-execution error.  Used to signal an error on sentences whose result is bad only in context, i. e. non-nouns or
// assertions we reconstruct conditions at the beginning of the parse, and set an error on token 1.
A
jtpee(J jt, A *queue, CW *ci, I err, I lk, DC c) {
    A z = 0;
    ASSERT(lk <= 0, err);  //  locked fn is totally opaque, with no stack.  Exit with 0 result, indicating error
    jt->parserstackframe.parserqueue    = queue + ci->i;
    jt->parserstackframe.parserqueuelen = (I4)ci->n;
    jt->parserstackframe.parsercurrtok  = 1;  // unless locked, indicate failing-sentence info
    jtjsignal(jt, err);                       // signal the requested error
    // enter debug mode if that is enabled
    if (c && jt->uflags.us.cx.cx_c.db) {
        jt->sitop->dcj = jt->jerr;
        z              = jtdebug(jt);
        jt->sitop->dcj = 0;
    }  //  d is PARSE type; set d->dcj=err#; d->dcn must remain # tokens jtdebz(jt);  not sure why we change previous
       //  frame
    if (jt->jerr) z = 0;
    return z;  // if we entered debug, the error may have been cleared.  If not, clear the result.  Return debug result,
               // which is result to use or 0 to indicate jump
}
...
A
jtparsex(J jt, A *queue, I m, CW *ci, DC c) {
    A z;
    B s;
    movesentencetosi(jt, queue, m, 0);  // install sentence-to-be-executed for stop purposes
    if (s = jtdbstop(jt, c, ci->source)) {
        z = 0;
        jtjsignal(jt, EVSTOP);
    } else {
        z = jtparsea(jt, queue, m);
    }
    // If we hit a stop, or if we hit an error outside of try./catch., enter debug mode.  But if debug mode is off now,
    // we must have just executed 13!:0]0, and we should continue on outside of debug mode.  Error processing filled the
    // current si line with the info from the parse
    if (!z && jt->uflags.us.cx.cx_c.db) {
        DC t   = jt->sitop->dclnk;
        t->dcj = jt->sitop->dcj = jt->jerr;
        z                       = jtdebug(jt);
        t->dcj                  = 0;
    }  //  d is PARSE type; set d->dcj=err#; d->dcn must remain # tokens
    return z;
}

A
jtdbunquote(J jt, A a, A w, A self, L *stabent) {
    A t, z;
    B b = 0, s;
    DC d;
    V *sv;
    ...
    if (CCOLON == sv->id &&
        ...
        do {
            ...
            // off now, we must have just executed 13!:8]0, and we should continue on outside of debug mode
            if (!z && jt->uflags.us.cx.cx_c.db) {
                d->dcj = jt->jerr;
                movecurrtoktosi(jt);
                z = jtdebug(jt);
                if (self != jt->sitop->dcf) self = jt->sitop->dcf;
            }
        ...
        } while (d->dcnewlineno && d->dcix != -1);  // if suspension tries to reexecute a line other than -1 (which
                                                    // means 'exit'), reexecute the tacit definition
    }
...
jtdbrr(J jt, A a, A w) {
    ...
    RE(0);
    d = jt->sitop;
    while (d && DCCALL != d->dctype) d = d->dclnk;
...
A
jtdbrr1(J jt, A w) {
    return jtdbrr(jt, 0L, w);
} /* 13!:9   re-run with arg(s) */
A
jtdbrr2(J jt, A a, A w) {
    return jtdbrr(jt, a, w);
}

A
jtdbtrapq(J jt, A w) {
    ASSERTMTV(w);
    return jt->dbtrap ? jt->dbtrap : mtv;
}
/* 13!:14 query trap */
