...
static const UC typesizes[32] = {
  TPSZ(B01),  TPSZ(LIT),  TPSZ(INT),  TPSZ(FL),         TPSZ(CMPX),        TPSZ(BOX),        TPSZ(XNUM),
  TPSZ(RAT),  TPSZ(SB01), TPSZ(SLIT), TPSZ(SINT),       TPSZ(SFL),         TPSZ(SCMPX),      TPSZ(SBOX),
  TPSZ(SBT),  TPSZ(C2T),  TPSZ(C4T),  TPSZ(ASGN),       TPSZ(MARK),        TPSZ(NAME),       TPSZ(SYMB),
  TPSZ(CONW), TPSZ(LPAR), TPSZ(RPAR), [ADVX] = INTSIZE, [CONJX] = INTSIZE, [VERBX] = INTSIZE  // note ACV sizes are in
                                                                                              // INTs
};
...
jtmakename(J jt, C *s) {
    ...
    m = strlen(s);
    GATV0(z, NAME, m, 1);
    zv = NAV(z);  // Use GATV because GA doesn't support NAME type
    memcpy(zv->s, s, m);
    *(m + zv->s) = 0;
    zv->m        = (UC)m;
    zv->bucket   = 0;
...
jtglobinit(J jt) {
    ...
    RZ(y = jtrifvs(jt, jtstr(jt, 1L, "z")));
    ACX(y);
    AS(y)[0] = BUCKETXLOC(1, "z");  // for paths, the shape holds the bucketx
    GA(x, BOX, 1, 1, 0);
    ACX(x);
    AAV(x)[0] = y;
    zpath     = x;
    AFLAG(zpath) |= (AT(zpath) & TRAVERSIBLE);  // ensure that traversible types in pst are marked traversible, so
                                                // tpush/ra/fa will not recur on them
    ...
    // take all the permanent blocks off the tpop stack so that we don't decrement their usecount.  All blocks allocated
    // here must be permanent
...
jtjinit2(J jt) {
    ...
    memcpy(jt->typepriority,
           typepriority,
           sizeof(jt->typepriority));  // required for ma.  Repeated for each thread in jtinit3
    memcpy(jt->prioritytype,
           prioritytype,
           sizeof(jt->prioritytype));  // required for ma.  Repeated for each thread in jtinit3
    jt->tssbase = tod();
    jtmeminit(jt);
    jtsesminit(jt);
    jtevinit(jt);
    ...
    // jttpop(jt,) isn't called during initialization.  So, to keep the memory auditor happy, we reset ttop so that it
