...
// NANFLAG is used internally as a special value that is not generated by any normal operations.  It is represented in
// integer form,
...
#define JTWILLBEOPENEDX \
    4  // result of this exec will be opened immediately, so it can contain virtual references to an input to the
       // current verb
...
#define JTALIGNBDY \
    8192  // jt is aligned on this boundary - all lower bits are 0 (the value is the size of an SDRAM page, to avoid row
          // precharges while accessing jt)
...
#define AS(x) \
    ((x)->s)  // Because s is an array, AS(x) is a pointer to the shape, which is in s.  The shape is stored in the
              // fixed position s.
...
#define WP(t, n, r)                 \
    (SMMAH + r + (1 && t & LAST0) + \
     (((t & NAME ? sizeof(NM) : 0) + ((n) << bplg(t)) + SZI - 1) >> LGSZI))  // # I to allocate
...
#define IAV0(x) \
    ((I*)((C*)(x) + AKXR(0)))  // integer in a stack- or heap-allocated atom (rank 0 - used for internal tables)
#define BAV0(x) ((C*)((C*)(x) + AKXR(0)))  // Boolean when rank is 0 - fixed position (known to avoid segfault)
#define LXAV0(x)     \
    ((LX*)((C*)(x) + \
           AKXR(     \
             0)))  // Symbol when rank is 0 - fixed position (for SYMB hash tables).  Note AK() is used in SYMB tables
...
#define BIV0(w)  \
    (IAV(w)[0] & \
     (1 - ((AT(w) & INT) >> (INTX - 1))))  // the first (presumably only) value in w, when w is an INT or B01 type
...
#define PARTOFSPEECHEQ(x, y)                                                                       \
    (((((x) | (RESV & -((x)&NOUN))) ^ ((y) | (RESV & -((x)&NOUN)))) & RESV + CONJ + VERB + ADV) == \
     0)  // using RESV to hold NOUN status, verify parts-of-speech the same
#define PARTOFSPEECHEQACV(x, y) \
    ((((x) ^ (y)) & RESV + CONJ + VERB + ADV) == 0)  // using RESV to hold NOUN status, verify parts-of-speech the same
...
#define ACINPLACE \
    ((I)((UI)1 << ACINPLACEX))  // set when this block CAN be used in inplace operations.  Always the sign bit.
...
#define ACINCR(a) \
    if (!ACISPERM(AC(a))) (AC(a) = (AC(a) + 1) & ~ACINPLACE)
#define ACX(a) \
    { AC(a) = ACPERMANENT; }
...
#define ASGNINPLACESGN(s, w)                                                                   \
    (((s)&AC(w)) < 0 || ((s) & (AC(w) - 2)) < 0 && jt->assignsym && jt->assignsym->val == w && \
                          (!(AFLAG(w) & AFRO + AFNVR) ||                                       \
                           (!(AFLAG(w) & AFRO) && jtnotonupperstack(jt, w))))  // OK to inplace ordinary operation
#define ASGNINPLACESGNNJA(s, w)                                                                             \
    (((s)&AC(w)) < 0 || (((s) & (AC(w) - 2)) < 0 || (((s) & (AC(w) - 3) & SGNIF(AFLAG(w), AFNJAX)) < 0)) && \
                          jt->assignsym && jt->assignsym->val == w &&                                       \
                          (!(AFLAG(w) & AFRO + AFNVR) ||                                                    \
                           (!(AFLAG(w) & AFRO) && jtnotonupperstack(jt, w))))  // OK to inplace ordinary operation
// define virtreqd and set it to 0 to start   scaf no LIT B01 C2T etc
// This is used in apip.  We must ALWAYS allow inplacing for NJA types, but for ordinary inplacing we don't bother if
// the number of atoms of w pushes a over a power-of-2 boundary We don't try to help the non-NVR case because NJAs will
// always be globals and thus have NVR set
#define EXTENDINPLACENJA(a, w)                                                                          \
    (((AC(a) & (((AN(a) + AN(w)) ^ AN(a)) - AN(a))) < 0) ||                                             \
     ((((AC(a) - 2) & (((AN(a) + AN(w)) ^ AN(a)) - AN(a))) < 0) || (AC(a) == 2 && AFLAG(a) & AFNJA)) && \
       ((jt->assignsym && jt->assignsym->val == a && !(AFLAG(a) & AFRO)) ||                             \
        (!jt->assignsym && (virtreqd = 1, !(AFLAG(a) & (AFRO | AFVIRTUAL))))) &&                        \
       jtnotonupperstack(jt, a))  // OK to inplace ordinary operation
...
#define AFNVRX 8
#define AFNVR \
    ((I)1 << AFNVRX)  // This value is on the parser's execution stack, and must not be freed until it is removed.
                      // Stacked values start as NVR+UNFREED
...
#define AFUNIFORMITEMS \
    ((I)1              \
     << AFUNIFORMITEMSX)  // It is known that this boxed array has contents whose items are of uniform shape and type
#define AFNVRUNFREEDX 18
#define AFNVRUNFREED \
    ((I)1 << AFNVRUNFREEDX)  // This value does NOT need to be freed by the parser, even though it was stacked
#define AFVIRTUALX 17        // matches C2TX
#define AFVIRTUAL \
    ((I)1 << AFVIRTUALX)  // this block is a VIRTUAL block: a subsequence of another block.  The data pointer points to
                          // the actual data, and the
// m field points to the start of the block containing the actual data.  A VIRTUAL block cannot be incorporated into
// another block, and it cannot be assigned, unless it is 'realized' by creating another block and copying the data.  We
// realize whenever we call ra() on the block, except during the EPILOG, where we don't realize the block unless the
// real block is about to be freed.  Like PERMANENT blocks, VIRTUAL blocks are always recursive so that fa() will not
// recur.  Virtual blocks are always freed from tpop.  Since it cannot be copied or realized, the virtual block always
// has usecount of ACUC1 or ACUC1+ACINPLACE.  EXCEPTION: for the initial assignment to x/y in an explicit definition, we
// allow assigning a virtual block, because we know that the block and its backer are allocated in a higher level and
// can never be freed until the explicit definition finishes.  In this case it is safe to increment the usecount of the
// virtual block - not to ra() the block, which would try to recur.  It is OK to fa() the block on exit or on
// reassignment, because that will just decrement the usecount of the nonrecursive block. VIRTUAL blocks are normally
// not inplaceable (since they are by definition aliased to another block), but the temporary UNINCORPORABLE blocks
// created by partitioning modifers to track cells may be inplaceable, and a virtual block whose backer has been
// abandoned may be marked inplaceable as well. NOTE: AFVIRTUALX must be higher than any RECURSIBLENOUN type (for test
// in result.h)
#define AFUNINCORPABLEX 16  // matches SBTX
#define AFUNINCORPABLE \
    ((I)1              \
     << AFUNINCORPABLEX)    // (used in result.h) this block is a virtual block used for subarray tracking and must not
                            // ever be put into a boxed array, even if WILLBEOPENED is set, because it changes
#define AFVIRTUALBOXEDX 19  // matches XDX
#define AFVIRTUALBOXED \
    ((I)1 << AFVIRTUALBOXEDX)  // this block (created in result.h) is an array that is about to be opened, and thus may
                               // contain virtual blocks as elements
#define AFPRISTINEX 21         // matches ASGN
#define AFPRISTINE \
    ((I)1 << AFPRISTINEX)  // meaningful only for BOX type.  This block's contents were made entirely of DIRECT
                           // inplaceable or PERMANENT values, and thus can be
// inplaced by &.> .  If any of the contents are taken out, the PRISTINE flag must be cleared, unless the block is never
// going to be used again (i. e. is inplaceable). When a VIRTUAL block is created, it inherits the PRISTINE status of
// its backer; if the block is modified or a value escapes by address, PRISTINE status is cleared in the backer. If a
// PRISTINE virtual block is realized, the backer must become non-PRISTINE (because its contents are escaping). If a
// PRISTINE block is incorporated, it must lose PRISTINE status because it is no longer possible to know whether
// contents may have been fetched while the block was incorporated. MUST BE GREATER THAN ANY DIRECT FLAG
...
#define LNAMEADDED \
    LPERMANENT  // Set in rank when a new name is added to the local symbol table.  We transfer the bit from the L flags
                // to the rank-flag
#define LLOCALTABLE \
    16  // Set in rank of all local symbol tables.  This indicates that the first hashchain holds x/y info and should
        // not be freed as a symbol
...
// canend - Indicates that the most-recent B-block result can (1) or can't (2) become the result of the running
...
#define SYMLINFOSIZE \
    1  // Number of symbol-table entries that DO NOT contain symbol chains, but instead are LINFO entries
...
// FOR EXECUTING LOCAL SYMBOL TABLES: AK() points to the active global symbol table, AM() points to the calling local
// symbol table. In all local symbol tables, the first 'hashchain' has the symbol offsets of x/y
...
#define LWASABANDONED \
    ((I)1 << LWASABANDONEDX)  // set if the name was assigned from an abandoned value, and we DID NOT raise the usecount
                              // of the value (we will have changed INPLACE to ACUC1, though).
// when the name is reassigned or deleted, we must refrain from fa(), and if the value still has AC=ACUC1, we should
// revert it to inplaceable so that the parser will free it immediately This value passes into AFLAGS and must not
// overlap anything there
...
#define CALLSTACKDELETE \
    256  // the given locale must be deleted, and this is the earliest place on the stack that refers to it
...
#define pushcallstack1(t, v)                             \
    {                                                    \
        ASSERT(jt->callstacknext < jt->fcalln, EVSTACK); \
        pushcallstack(jt->callstacknext, (t), (v));      \
    }
#define pushcallstack1d(t, v)                            \
    {                                                    \
        FDEPDEC(d);                                      \
        ASSERT(jt->callstacknext < jt->fcalln, EVSTACK); \
        pushcallstack(jt->callstacknext, (t), (v));      \
...
// m:    length of non-locale part of name note 255-byte limit! (AN holds the length of the entire name including the
// locative)
...
#define SPBV(p, a, v, x)          \
    {                             \
        ...
        INCORP(v);                \
        (p)->a = (C*)v - (C*)(p); \
    }  // store x into component (a); return if x is 0.  a is one of aeix.  Result in v
#define SPB(p, ZWa, x)           \
    {                            \
        A ZWo = (x);             \
        SPBV((p), ZWa, ZWo, (x)) \
    }  // store x into component (a); return if x is 0.  a is one of aeix
...
typedef struct {
    ...
    UI invalidlo;  // the start of the area that does not have constrained values because it was used for a bit table
                   // (as an index into hashtable, rounded up to multiple of I for endian reasons)
    UI invalidhi;  // the end+1 of the area used for a bit table  (as an index into hash table, rounded up to multiple
                   // of I)
    I datamin;     // (for small-range use) minimum value in the data
    UI datarange;  // (for small-range use) range+1 of the data
    // 11 words here; 7 words of header (we allocate no shape); 2 words of memory-allocation header; so this block is 20
    // words after the memory allocation, and thus is on the same 256-bit boundary as the allocation.
...
// the localuse fields are not freed or counted for space, as the f/g/h fields are.  It is for local optimizations only.
// We put if first so that the rest of the block, which is used more, is in a single cacheline.  Local uses are: for
// ATOMIC ops, lvp[] is pointer to the VA/UA block for adocv [dyad then monad] for name references, lvp[0] is pointer to
// last resolution for FIT conj, the CCT data for RANK conj, lI4[0-2] has the signed ranks for Fold final operator,
// lfns[1] has pointer to the dyadic EP of the handler (xdefn or unquote) For cyclic iterators, lI has the index of the
// next gerund to execute for u;.n where n is nonzero, lvp[0] holds n.  u/. also goes through this code for reductions
// (u/ u/\ u/\.) lvp[1] points to the VA block for u for u&.[:]v, lvp[0] points to the verb whose inverse is needed for
// x <;.0 y  and  x (<;.0~ -~/"2)~ y, lpf.parm is ~0 for first, 0 for second, and func points to failover routine for
// dyads ; (,<) ,&[:]<  lclr[0] indicates which function
...
#define ID(f)                                                            \
    FAV(AT(f ? f : (A)(validitymask + 12)) & FUNC ? f : (A)validitymask) \
      ->id  // can be branchless, if compiler can manage it
#define VFLAGNONE 0L
...
#define VFKEYSLASHTX \
    0  // (in f/.) set if f is u/ where u is one of + <. >., indicating which types are OK for atoms (can't handle int
       // ovfl)
...
#define VFUSEDOK2 \
    ((I)(1L << 30))  // this block can be executed by passing in another block (containing rank) whose fgh[0] points to
                     // the native block for this primitive
...
#define VF2RANKONLY1X \
    13  // set if this verb is u"r, not special-cased (i. e. function points to rank processing).  The rank may be
        // subsumed into a higher rank before we see arguments.
...
} PRIM;  // two cachelines exactly in 64-bit
...
typedef struct {
    VARPSF f;
    I cv;
} VARPS;  // for reduce/prefix/suffix
...
typedef struct {
    // Because the parse decode looks mostly at pt, make that the first thing so that it is always aligned to the
    // beginning of whatever block is usd to copy one stack element to another.
