...
I jtthv(J jt,A w,I n,C*s){A t;B ov=0;C buf[WZ],*x,*y=s;I k,n4=n-4,p,wd,wn,wt;FMTFUN fmt;
 ...
 case XNUMX: case RATX:
  RZ(t=thxqe(w)); p=AN(t); if(ov=n<p)p=n4;
  memcpy(y,AV(t),p); y+=p; break;
 case B01X:
...
static A jtthdx1(J jt,DX y){A x,z;B b;C*s,s1[2+XBASEN],s2[20];I e,n,p,p1,p2,*v;
 ...
 if(b)*s++=CSIGN; *s++=*s1; if(1<p1){*s++='.';
     memcpy(s,1+s1,p1-1); s+=p1-1;}
 DQ(n-1, --v; I j=*v; j=b?-j:j; sprintf(s,FMTI04,j); s+=XBASEN;);
 memcpy(s,s2,p2);
 return z;
}
  ...
  // No internal newlines are possible unless the original w was character type (in boxes, they were changed to space)
   ...
   case 1:
    ...
    // Otherwise just move fast
        ...
        memcpy(zv,v,c1); zv+=c1; v+=c1;    if(c1<c){v+=c-c1; DDD(zv);}}
    break;
   default: {C4 *u=(C4*)v,x;
