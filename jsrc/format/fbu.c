...
extrawidth(C4 w) {
    ...
    static const UINT widechars[] = {
      0x1100,
      0x100115F,  // Hangul - note the first value is wired into jttwidthf16 below
      0x231A,
      0x100231B,  // Hourglass
      0x2329,
      0x100232A,  // Angle Brackets
      0x23E9,
      0x10023EC,  // Double triangles
      0x23F0,     // Hourglass
      0x23F3,     // Hourglass
      0x25FD,
      0x10025FE,  // Small square
      0x2614,
      0x1002615,  // Umbrellas
      0x2648,
      0x1002653,  // Astrological
      0x267F,     // Wheelchair
      0x2693,     // Anchor
      0x26A1,     // High Voltage
      0x26AA,
      0x10026AB,  // Medium white circle
      0x26BD,
      0x10026BE,  // Soccer ball, baseball
      0x26C4,
      0x10026C5,  // Snowman
      0x26CE,     // Ophiuchus
      0x26D4,     // No entry
      ...
      0x2705,     // check mark
      0x270A,
      0x100270B,  // Victory fist
      0x2728,     // sparkles
      0x274C,     // cross mark
      0x274E,     // negative cross mark
      0x2753,
      0x1002755,  // Question marks
      ...
      0x100FAFF,  // CJK
      0xFE10,
      0x100FE19,  // Punctuation
      0xFE30,
      0x100FE6B,  // Punctuation
      0xFF01,
      0x100FF60,  // Punctuation
      0xFFE0,
      0x100FFE6,  // Symbols
      0x16FE0,    // Lm         TANGUT ITERATION MARK
      0x17000,
      0x10187EC,  // Lo  [6125] TANGUT IDEOGRAPH-17000..TANGUT IDEOGRAPH-187EC
      0x18800,
      0x1018AF2,  // Lo   [755] TANGUT COMPONENT-001..TANGUT COMPONENT-755
      0x1B000,
      0x101B001,  // Lo     [2] KATAKANA LETTER ARCHAIC E..HIRAGANA LETTER ARCHAIC YE
      0x1F004,    // So         MAHJONG TILE RED DRAGON
      0x1F0CF,    // So         PLAYING CARD BLACK JOKER
      0x1F18E,    // So         NEGATIVE SQUARED AB
      0x1F191,
      0x101F19A,  // So    [10] SQUARED CL..SQUARED VS
      0x1F200,
      0x101F202,  // So     [3] SQUARE HIRAGANA HOKA..SQUARED KATAKANA SA
      0x1F210,
      0x101F23B,  // So    [44] SQUARED CJK UNIFIED IDEOGRAPH-624B..SQUARED CJK UNIFIED IDEOGRAPH-914D
      0x1F240,
      0x101F248,  // So     [9] TORTOISE SHELL BRACKETED CJK UNIFIED IDEOGRAPH-672C..TORTOISE SHELL BRACKETED CJK
                  // UNIFIED IDEOGRAPH-6557
      0x1F250,
      0x101F251,  // So     [2] CIRCLED IDEOGRAPH ADVANTAGE..CIRCLED IDEOGRAPH ACCEPT
      0x1F300,
      0x101F320,  // So    [33] CYCLONE..SHOOTING STAR
      0x1F32D,
      0x101F335,  // So     [9] HOT DOG..CACTUS
      0x1F337,
      0x101F37C,  // So    [70] TULIP..BABY BOTTLE
      0x1F37E,
      0x101F393,  // So    [22] BOTTLE WITH POPPING CORK..GRADUATION CAP
      0x1F3A0,
      0x101F3CA,  // So    [43] CAROUSEL HORSE..SWIMMER
      0x1F3CF,
      0x101F3D3,  // So     [5] CRICKET BAT AND BALL..TABLE TENNIS PADDLE AND BALL
      0x1F3E0,
      0x101F3F0,  // So    [17] HOUSE BUILDING..EUROPEAN CASTLE
      0x1F3F4,    // So         WAVING BLACK FLAG
      0x1F3F8,
      0x101F3FA,  // So     [3] BADMINTON RACQUET AND SHUTTLECOCK..AMPHORA
      0x1F3FB,
      0x101F3FF,  // Sk     [5] EMOJI MODIFIER FITZPATRICK TYPE-1-2..EMOJI MODIFIER FITZPATRICK TYPE-6
      0x1F400,
      0x101F43E,  // So    [63] RAT..PAW PRINTS
      0x1F440,    // So         EYES
      0x1F442,
      0x101F4FC,  // So   [187] EAR..VIDEOCASSETTE
      0x1F4FF,
      0x101F53D,  // So    [63] PRAYER BEADS..DOWN-POINTING SMALL RED TRIANGLE
      0x1F54B,
      0x101F54E,  // So     [4] KAABA..MENORAH WITH NINE BRANCHES
      0x1F550,
      0x101F567,  // So    [24] CLOCK FACE ONE OCLOCK..CLOCK FACE TWELVE-THIRTY
      0x1F57A,    // So         MAN DANCING
      0x1F595,
      0x101F596,  // So     [2] REVERSED HAND WITH MIDDLE FINGER EXTENDED..RAISED HAND WITH PART BETWEEN MIDDLE AND RING
                  // FINGERS
      0x1F5A4,    // So         BLACK HEART
      0x1F5FB,
      0x101F5FF,  // So     [5] MOUNT FUJI..MOYAI
      0x1F600,
      0x101F64F,  // So    [80] GRINNING FACE..PERSON WITH FOLDED HANDS
      0x1F680,
      0x101F6C5,  // So    [70] ROCKET..LEFT LUGGAGE
      0x1F6CC,    // So         SLEEPING ACCOMMODATION
      0x1F6D0,
      0x101F6D2,  // So     [3] PLACE OF WORSHIP..SHOPPING TROLLEY
      0x1F6EB,
      0x101F6EC,  // So     [2] AIRPLANE DEPARTURE..AIRPLANE ARRIVING
      0x1F6F4,
      0x101F6F6,  // So     [3] SCOOTER..CANOE
      0x1F910,
      0x101F91E,  // So    [15] ZIPPER-MOUTH FACE..HAND WITH INDEX AND MIDDLE FINGERS CROSSED
      0x1F920,
      0x101F927,  // So     [8] FACE WITH COWBOY HAT..SNEEZING FACE
      0x1F930,    // So         PREGNANT WOMAN
      0x1F933,
      0x101F93E,  // So    [12] SELFIE..HANDBALL
      0x1F940,
      0x101F94B,  // So    [12] WILTED FLOWER..MARTIAL ARTS UNIFORM
      0x1F950,
      0x101F95E,  // So    [15] CROISSANT..PANCAKES
      0x1F980,
      0x101F991,  // So    [18] CRAB..SQUID
      0x1F9C0,    // So         CHEESE WEDGE
      0x20000,
      0x102FFFD,  // All undesignated code points in Planes 2 and 3, whether inside or
      0x30000,
      0x103FFFD  // outside of allocated blocks, default to "W":
    };
    ...
    I l = 0, r = sizeof(widechars) / sizeof(widechars[0]) -
                 1;  // left index, right index.  w is >= widechars[l] and widechars[r]
    while (l <= r) {
        I m = (l + r) >> 1;
        if (w == (widechars[m] & 0xffffff)) {
            l = m;
            break;
        }
        if (w > (widechars[m] & 0xffffff))
            l = m + 1;
        else if (w < (widechars[m] & 0xffffff))
            r = m - 1;
    }
    // widechars[l] is the first entry >= w.  If it is an end-of-interval, or it is equal to w, w is in an interval:
    // return 1 then else 0
    return (widechars[l] & 0x1000000) || widechars[l] == w;
}

static I
wtomnullsize(US* src, I srcn) {
    US w, w1;
    I nignulls = 0;
    I r        = 0;
    I eatnull  = 1;
    while (srcn--) {
        w = *src++;
        if (nignulls && (--nignulls, w == 0))
            ;  // If we should ignore a null, do so, decrement count of ignored nulls
        else if (w <= 0x7f)
            r++;
        else if (w <= 0x7ff) {
            r += 2;
            ...
            r += 3;
            if (eatnull)
                nignulls = extrawidth(w);  // extra nulls only possible in upper pages.  See if there are any
            else {
                if (!srcn) {
                    // isolated surrogate
                    r += 3;
                    ...
                    if (w >= 0xdc00 || w1 <= 0xdbff || w1 >= 0xe000)  // incorrect high/low surrogate
                    {
                        ...
                        if (eatnull)
                            nignulls =
                              extrawidth(t);  // extra nulls only possible in upper pages.  See if there are any
                        src++;
                        srcn--;  // next code unit of surrogate pair
                    }
                }
            }
        }
...
wtownull(J jt, A w) {
    US *wv, *zv;
    I n, i, nignulls = 0, naddednulls = 0;
    A z;
    n = AN(w);
    if (!n) return w;
    wv = USAV(w);
    ...
    for (i = 0; i < n; ++i) {
        if (nignulls && (--nignulls, wv[i] == 0))
            --naddednulls;  // If an added null is already present, remove it from the count
        else if (wv[i] >= 0x1100)
            naddednulls += nignulls = extrawidth(wv[i]);
    }
    ...
    GATV0(z, C2T, n + naddednulls, 1);
    zv       = USAV(z);
    nignulls = 0;
    for (i = 0; i < n; ++i) {
        if (nignulls && (--nignulls, wv[i] == 0))
            ;  // If an added null is already present, skip the copy
        else {
            *zv++ = wv[i];
            if (wv[i] >= 0x1100 && (nignulls = extrawidth(wv[i]))) { *zv++ = 0; }
        }  // copy the char; if doublewide, copy the null for it
    }
    return z;
}
...
A
RoutineA(J jt, A w, A prxthornuni) {
    A z;
    I n, t, q, q1, b = 0;
    UC* wv;
...
A
RoutineC(J jt, A w, A prxthornuni) {
    A z;
    I n, t, q, b = 0;
    C4* wv;
    ASSERT(1 >= AR(w), EVRANK);
    ...
    DQ(
      n,
      if ((UI4)(*wv - 0xd800) <= (UI4)(0xdfff - 0xd800)) {
          b = 1;
          break;
      };
      wv++;);
...
A
RoutineD(J jt, A w, A prxthornuni) {
    A z;
    I n, t, q, b = 0;
    C4* c4v;
    ASSERT(1 >= AR(w), EVRANK);
...
// nul are ignored
I
stringdisplaywidth(J jt, I c2, void* src, I nsrc) {
    I n = nsrc, q;
    A c4;
    C4* u;
    PROLOG(0000);
    switch (c2) {
        default:
            ...
            mtou(src, nsrc, u);
            n = q;
            DO(q, if (u[i]) n += extrawidth(u[i]); else n--;);
            break;
        case 1:
            q = wtousize(src, nsrc);
            q = (q < 0) ? -q : q;
