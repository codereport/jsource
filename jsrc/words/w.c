...
// If there are mismatched quotes, AM(z) is set to -1 and the number of valid tokens is in AS(z)[0]
 A jtwordil(J jt, A w){A z;I s,i,m,n,nv,*x;UC*v;
...
A jtenqueue(J jt,A a,A w,I env){A*v,*x,y,z;B b;C d,e,p,*s,*wi;I i,n,*u,wl;UC c;
 ...
 for(i=0;i<n;i++,x++){  // for each word
   ...
   *x=y;   // install the value
   ...
   if((p&~CA)==0){ASSERTN(jtvnm(jt,wl,wi),EVILNAME,jtnfs(jt,wl,wi)); RZ(*x=jtnfs(jt,wl,wi)); if((env==2)&&(NAV(*x)->flag&NMXY)){AT(*x)|=NAMEBYVALUE;}  // starts with alphabetic, make it a name, error if invalid name
...
#define EMIT0c(T,j,i,r,c)   {CHKJ(j); p=(i)-(j); EXTZ(T,1); RZ(*u++=jtstr(jt,p,(j)+wv));}
...
#define EMIT0x(T,j,i,r,c)   {CHKJ(j); p=(i)-(j); EXTZ(T,1); GA(x,t0,p*wm,wr,AS(w0));  \
                                AS(x)[0]=p; memcpy(AV(x),wv0+wk*(j),wk*p); *u++=x;}
#define EMIT1(T,j,i,r,c)    {CHKJ(j); p=(i)-(j);            cc=(j)+wv; DQ(p, *u++=*cc++;);}
#define EMIT1x(T,j,i,r,c)   {CHKJ(j); p=wk*((i)-(j));       memcpy(u,wv0+j*wk,p); u+=p;}
...
static A jtfsmdo(J jt,I f,A s,A m,I*ijrd,A w,A w0){A x,z;C*cc,*wv0;
 ...
 A jtfsmvfya(J jt, A w){PROLOG(0099);A a,*av,m,s,x,z,*zv;I an,c,e,f,ijrd[4],k,p,q,*sv,*v;
 RZ(a=w);
...
static A jtfsm0(J jt,A a,A w,C chka){PROLOG(0100);A*av,m,s,x,w0=w;B b;I c,f,*ijrd,k,n,p,q,*v;
 ...
 A jtfsmfx(J jt,    A w,A self){ return fsm0(FAV(self)->fgh[0],w,0);}
     /* x&;: y */
