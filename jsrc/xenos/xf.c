...
jtrdns(J jt, F f) {
    ...
    if (tr == (U)n)
        ...
        GATV0(z, LIT, (I)tr, 1);
        memcpy(CAV(z), CAV(za), tr);
    }
    return z;
} /* read entire file stream (non-seekable) */

A
jtrd(J jt, F f, I j, I n) {
    A z;
    C *x;
    I p      = 0;
    size_t q = 1;
...
static B
jtwa(J jt, F f, I j, A w) {
    C *x;
    I n, p = 0;
    size_t q = 1;
    RZ(f && w);
    n = AN(w) * (C4T & AT(w) ? 4 : C2T & AT(w) ? 2 : 1);
    x = CAV(w);
...
jtjferase(J jt, A w) {
    ...
    RE(h = jtfnum(jt, w));
    if (h) {
        RZ(y = jtstr0(jt, jtfname(jt, jtsc(jt, h))))
    } else
