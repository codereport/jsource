...
/* Return mode_t formatted into 11-character buffer supplied by the caller.  The last byte of the buffer is the string
 * terminator \0 */
static C *
modebuf(mode_t m, C *b) {
    C c;
    I t = m;
...
static int
ismatch(J jt, C *pat, C *name, struct stat *dirstatbuf, C *diratts, C *dirmode, C *dirrwx, C *dirnamebuf, C *dirbase) {
    strcpy(dirbase, name);
    if (stat(dirnamebuf, dirstatbuf)) return 0;
...
static A
jtdir1(J jt, struct dirent *f, struct stat *dirstatbuf, C *diratts, C *dirmode, C *dirrwx) {
    A z, *zv;
    C *s, att[16];
    I n, ts[6], i, m, sz;
    S x;
    struct tm tmr, *tm = &tmr;
...
jtjdir(J jt, A w) {
    ...
    C dirmode[11];
    C dirrwx[3];
    struct stat dirstatbuf[3];  // for some reason there were 2 dummy blocks reserved after the buffer for 32-bit Linux.
                                // Preserve that
     ...
     */
    sprintf(dirnamebuf, "%s/", dir);
    C *dirbase = dirnamebuf + strlen(dirnamebuf);
    f          = readdir(DP);
    GATV0(z, BOX, n, 1);
    zv = AAV(z);
    while (f) {
