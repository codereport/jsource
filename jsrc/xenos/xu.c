        ...
        if (w <= 0x7f)
            ...
            if (!srcn) {
                ...
                if (w >= 0xdc00 || w1 <= 0xdbff || w1 >= 0xe000) {
                    ...
                    r += 4;
                    src++;
                    srcn--;  // skip the next code unit of surrogate pair
                }
            }
        }
    }
...
A
jttoutf8a(J jt, A w, A prxthornuni) {
    A z;
    I n, t, q;  // prxthornuni is unused
    PROLOG(0000);
    ASSERT(1 >= AR(w), EVRANK);
    n = AN(w);
    t = AT(w);
