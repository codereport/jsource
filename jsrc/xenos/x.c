...
#define SDERIV(id, f1, f2, flag, m, l, r)                                    \
    jtfdef(jt, 0,                                                            \
         ...
         a,                                                                  \
         w,                                                                  \
         jtv2(jt, (I)(f1 ? f1 : jtdomainerr1), (I)(f2 ? f2 : jtdomainerr2)), \
         (flag),                                                             \
         (I)m,                                                               \
         (I)l,                                                               \
         (I)r)
...
static A
secf1(J jt, A w, A self) {
    FPREFIP;
    A h = FAV(self)->fgh[2];
    ASSERT(!jt->seclev, EVSECURE);
    return CALL1IP((AF)AV(h)[0], w, self);
}
static A
secf2(J jt, A a, A w, A self) {
    FPREFIP;
    A h = FAV(self)->fgh[2];
    ASSERT(!jt->seclev, EVSECURE);
    return CALL2IP((AF)AV(h)[1], a, w, self);
}

// undocumented 13!:80 functions, used to test condrange
static A
jtfindrange(J jt, A a, A w, A self) {
    I *av  = AV(a);
    CR rng = condrange(AV(w), AN(w), av[0], av[1], av[2]);
    return jtv2(jt, rng.min, rng.range);
}  // 13!:80
static A
jtfindrange4(J jt, A a, A w, A self) {
    I *av  = AV(a);
    CR rng = condrange4(C4AV(w), AN(w), av[0], av[1], av[2]);
    return jtv2(jt, rng.min, rng.range);
}  // 13!:81
static A
jtfindrange2(J jt, A a, A w, A self) {
    I *av  = AV(a);
...
jtforeign(J jt, A a, A w) {
    ...
    switch (XC(p, q)) {
        ...
        case XC(2, 3):
            return SDERIV(CIBEAM, jtjwait, 0, VASGSAFE, 0, RMAX, RMAX);
        ...
        case XC(2, 8):
            return SDERIV(CIBEAM, jtcerrno, 0, VASGSAFE, RMAX, RMAX, RMAX);
        ...
        case XC(6, 5):
            return CDERIV(CIBEAM,
                          jtpeekdata,
                          0,
                          VASGSAFE,
                          RMAX,
                          RMAX,
                          RMAX);  // turn on to provide a J window into the running system
        ...
        case XC(9, 63):
            return CDERIV(CIBEAM, jtdirectdefs, 0, VASGSAFE, RMAX, RMAX, RMAX);  // scaf
        ...
        case XC(128, 8):
            return CDERIV(CIBEAM, jtqhash12, jtqhash12, VASGSAFE | VJTFLGOK1 | VJTFLGOK2, RMAX, RMAX, RMAX);
...
void
double_trick(D a, D b, D c, D d, D e, D f, D g, D h) {
    ;
}
