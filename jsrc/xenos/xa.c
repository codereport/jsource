...
A
jtdirectdefq(J jt, A w) {
    ASSERTMTV(w);
    return jtscb(jt, jt->directdef);
}  // scaf 9!:62

A
jtdirectdefs(J jt, A w) {
    B b;
    RE(b = jtb0(jt, w));
    jt->directdef = b;
    return mtm;
...
jtboxq(J jt, A w) {
    ASSERTMTV(w);
    return jtstr(jt, sizeof(jt->bx), jt->bx);
}
...
jtevms(J jt, A w) {
    ...
    *tv++ = mtv;
    wv    = AAV(w);
    DQ(NEVM, RZ(*tv = jtca(jt, jtvs(jt, *wv))); CAV(*tv)[AN(*tv)] = 0; ++tv;
       ++wv;);  // NUL-terminate.  ca to make sure there's room
    ras(t);
    fa(jt->evm);
    jt->evm = t;
    return mtv;
...
jtposq(J jt, A w) {
    ASSERTMTV(w);
    return jtv2(jt, jt->pos[0], jt->pos[1]);
}
