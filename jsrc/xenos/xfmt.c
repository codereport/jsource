 ...
 for(i=0;i<n;i++) {
  ...
  if(l>0 && l<*il) memset(cv,SUBs,l);  // can't dereference il if l==0.  If field too short, fill with user's * character
   ...
   if(*bits&BITSf) {  // value is _ __ _. (and therefore FL type)
    if(mI && *bits&BITS__)memcpy(cv, cI, nI);  // NULL has priority i<xx>
    else if(mD) memcpy(cv, cD, nD);  // if d<xx> given, use it
    ...
    if(mB) memcpy(cv, cB, nB);  // if b<xx> given, use it
    else {
     if(mPQ) { memcpy(cv, cP, nP); memcpy(cv+*il-nQ, cQ, nQ); }  // if p<xx> or q<xx> given, move in those fields
     RZ(sprintfI(cv+nP,*il-nP-nQ,d,0,subs));  // format 0 into the field, skipping p/q
    }
   } else if(mI && t&INT && *iv==IMIN){memcpy(cv, cI, nI);  // if we are checking for NULL, that overrides exponential
    ...
    if     (dtmp< 0 && mMN) { memcpy(cv, cM, nM); memcpy(cv+*il-nN, cN, nN); }  // if negative & pref/suff given, move them in
    else if(dtmp< 0       ) { *cv=SUBm;                              }   // if other negative, use the specified - sign
    else if(dtmp>=0 && mPQ) { memcpy(cv, cP, nP); memcpy(cv+*il-nQ, cQ, nQ); }  // if nonnegative & pref/suff given, move them in
   } else {
    if(UNSAFE(t)==INT){  // integer to be displayed in fixed-point
     ...
     if     (*iv < 0 && mMN) { memcpy(cv, cM, nM); memcpy(cv+*il-nN, cN, nN); }  // install pref/suff as above
     else if(*iv < 0       ) { *cv=SUBm;                              }
     else if(*iv>= 0 && mPQ) { memcpy(cv, cP, nP); memcpy(cv+*il-nQ, cQ, nQ); }
    }else if(UNSAFE(t)<INT){  // B01
     if(mPQ) { memcpy(cv, cP, nP); memcpy(cv+*il-nQ, cQ, nQ); }
     ...
     if     (*dv < 0 && mMN) { memcpy(cv, cM, nM); memcpy(cv+*il-nN, cN, nN); }
     else if(*dv < 0       ) { *cv=SUBm;                              }
     else if(*dv>= 0 && mPQ) { memcpy(cv, cP, nP); memcpy(cv+*il-nQ, cQ, nQ); }
    }
   }
  }
 ...
 *omode=0;
 ...
 if(lvl=level(w)){A*wv=AAV(w),x; 
  ...
  A z; return df2(z,jtreitem(jt,shape(jt,w),a),w,jtamp(jt,jtforeign(jt,num(8),num(0)), ds(COPE)));
 } else {
