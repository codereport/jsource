...
jtoblique(J jt, A w, A self) {
    ...
    A xn;
    RZ(xn = IX(n));
    RZ(x = ATOMIC2(jt, xm, xn, rkblk, 0L, 1L, CPLUS));
    AR(x)    = 1;
    AS(x)[0] = AN(x);
    ...
    // if((m==0 || n==0) && (m+n>0)){return jtreitem(jt,jtsc(jt,m+n-1),x);}  This change withdrawn pending further
    // deliberation
    return z;
}
...
jtobqfslash(J jt, A w, A self) {
    ...
    wt = AT(w);
    wv = CAV(w);
    if ((-AN(w) & (1 - r) & -(DENSE & wt)) >= 0)
        return jtoblique(
          jt, w, self);  // revert to default if rank<2, empty, or sparse.  This implies m/n below are non0
    y  = FAV(self)->fgh[0];
    y  = FAV(y)->fgh[0];
    id = FAV(y)->id;
    m  = s[0];
    ...
    if (((1 - m) & (1 - n)) >= 0) {
        ...
        AS(z)[0] = d;
        memcpy(AV(z), wv, AN(w) << bplg(wt));
        return z;
    }  // m=1 or n=1, return item of input
    if (wt & FL + CMPX) NAN0;
    if (1 == c) switch (OBQCASE(CTTZ(wt), id)) {
            ...
            case OBQCASE(CMPXX, CPLUS): OBQLOOP(Z, Z, wt, x = *u, x = jtzplus(jt, x, *u)); break;
            case OBQCASE(XNUMX, CMAX): OBQLOOP(X, X, wt, x = *u, x = 1 == jtxcompare(jt, x, *u) ? x : *u); break;
            case OBQCASE(XNUMX, CMIN): OBQLOOP(X, X, wt, x = *u, x = 1 == jtxcompare(jt, x, *u) ? *u : x); break;
            case OBQCASE(XNUMX, CPLUS): OBQLOOP(X, X, wt, x = *u, x = jtxplus(jt, x, *u)); break;
            ...
            case OBQCASE(RATX, CPLUS): OBQLOOP(Q, Q, wt, x = *u, x = jtqplus(jt, x, *u)); break;
...
A
jtpolymult(J jt, A a, A w, A self) {
    A f, g, z;
    B b = 0;
    C *av, c, d, *wv;
    ...
    if ((-m & (AR(a) - 2) & -n & (AR(w) - 2)) >= 0)
        return jtobqfslash(jt, df2(z, a, w, g), f);  // if empty, or not atoms/lists, do general code.  Never happens.
    ...
    switch (PMCASE(CTTZ(t), c, d)) {
        ...
        case PMCASE(CMPXX, CPLUS, CSTAR):
            PMLOOP(Z, Z, CMPX, x = jtztymes(jt, *u--, *v++), x = jtzplus(jt, x, jtztymes(jt, *u--, *v++)));
            break;
        case PMCASE(XNUMX, CPLUS, CSTAR):
            PMLOOP(X, X, XNUM, x = jtxtymes(jt, *u--, *v++), x = jtxplus(jt, x, jtxtymes(jt, *u--, *v++)));
            break;
        case PMCASE(RATX, CPLUS, CSTAR):
            PMLOOP(Q, Q, RAT, x = jtqtymes(jt, *u--, *v++), x = jtqplus(jt, x, jtqtymes(jt, *u--, *v++)));
            break;
        ...
        case PMCASE(INTX, CPLUS, CSTAR):
            ...
            // here for +//.@(*/)
            {
                ...
                for (i = 0; i < zn; ++i) {
                    ...
                    p    = MIN(1 + i, zn - i);
                    p    = MIN(p, k);
                    I rc = ((AHDR2FN *)adocv.f)((I)1, p, u, v, yv, jt);
                    if (rc & 255) jtjsignal(jt, rc);
                    if (255 & (rc = ((AHDRRFN *)adocvsum.f)((I)1, p, (I)1, yv, zv, jt))) jtjsignal(jt, rc);
                    ++zv;
                }
                if (EWOV <= jt->jerr) {
                    RESETERR;
                    PMLOOP(I, D, FL, x = *u-- * (D)*v++, x += *u-- * (D)*v++);
                }  // erroneous fa(z) removed; any error >= EWOV will be an overflow
                break;
            }
    }
    if (t & FL + CMPX) NAN1;
...
jtkeysp(J jt, A a, A w, A self) {
    ...
    {
        I t2;
        ASSERT(SETIC(a, n) == SETIC(w, t2), EVLENGTH);
    }  // verify agreement.  n is # items of a
...
jtkey(J jt, A a, A w, A self) {
    ...
    // allocate and run the small-range table and use it to rearrange the input.  The small-range variant is signaled by
    // the LSB of the result of indexofsub being set.
    ...
    // if this is a supported f//., handle it without calling cut.  We can take it if the flag says f//. and the rank is
    // >1 or the type is one we can do cheaply: B01/INT/FL
    if (SZI == SZD &&
        FAV(self)->flag & VFKEYSLASHT) {  // f//. where f is + >. <. mean   Implementation requires SZI==SZD
        // start of f//. on special cases
        if ((((AR(w) - 2) | ((AT(w) & CMPX + XNUM + RAT) - 1)) & ((AT(w) & FAV(self)->flag & VFKEYSLASHT) - 1)) >=
            0) {  // rank>=2 and extended, or type we can handle locally  flag is B01+INT+FL for <. >., B01+FL for + (we
                  // don't handle int ovfl)
            ...
            I ztoride = (0x0e010000 >> ((4 * keyslashfn + ((AT(w) >> INTX) & 3)) << 1)) &
                        3;  // z type override. 00=none, 01=B to I 10=B to FL 11=I to FL  bits are opFI, values are (0)
                            // 0 3 2 (0) 0 x 1 (0) 0 0 0 (0) 0 0 0
            ztoride   = AT(w) & FL + INT + B01 ? ztoride : 0;  // no override for other types
            I zt      = AT(w) ^ (((((INT + FL) << 12) + ((B01 + FL) << 8) + ((B01 + INT) << 4)) >> (ztoride << 2)) &
                            0xf);          // switch result precision as needed
            I celllen = cellatoms << bplg(zt);  // length of a cell of z, in bytes
#define RTNCASE(r, t) ((r)*4 + (((t) >> INTX) & 3))
            I routineid;
            VA2 adocv;  // case index to use
            if (AR(w) <= 1) {
                // partitions have rank 1, so operations are on atoms.  set the index
                routineid =
                  RTNCASE(keyslashfn, zt) ^
                  ztoride;  // min, max are normal.  + is B01 (BtoI) or FL.  mean is B01 (BtoF), INT (ItoF), or FL
            } else {
                // partitions are arrays.  Operate on them in the output area
                routineid = RTNCASE(
                  ztoride,
                  INT + FL);  // special case index for action routine: 3 for normal (for <. >.), 7 for BtoI (for +), 11
                              // for BtoF (for mean), 15 for ItoF (for mean) 17 = FL mean 16 for other mean e. g. INTX
                I meanrtn = 16 + ((zt >> FLX) & 1);  // 17 if FL result, 16 if not
                routineid = keyslashfn == routineid
                              ? meanrtn
                              : routineid;  // 'other mean': only way they can be = is if both are 3.  Switch to 16/17,
            ...
            if (!((I)ai & 1)) {
                ...
                GA(z, zt, nfrets * cellatoms, AR(w), AS(w));
                AS(z)[0] = nfrets;
                if (AN(z) == 0) return z;  // avoid calls with empty args
                if (keyslashfn == 3) {
                    GA(freq,
                       zt & FL ? FL : INT,
                       nfrets,
                       1,
                       0);  // allocate place for divisor - INT if result may be XNUM/RAT
                }
...
#define KSLGLP(wincr, zincr, pwvinit, pwvupd)                                                                          \
    {                                                                                                                  \
        ...
        for (indexx = 0, indexn = AN(ai), wv = voidAV(w); indexx != indexn; ++indexx, wv = (C *)wv + (wincr)) {        \
            ...
            if (nextx >= indexx) {                                                                                     \
                /* new partition.  Allocate it sequentially, move the data to be the initial result value, and replace \
                 * the index with the pointer to the partition */                                                      \
            ...
            if (nextx >= indexx) {                                                                                     \
                ...
                switch (routineid) {
                    case 3: {
                        I rc;
                        KSLGLP(
                          celllen, celllen, memcpy(partition, wv, celllen);
                          , ASSERT(EVOK == (rc = ((AHDR2FN *)adocv.f)(1, cellatoms, wv, partition, partition, jt)), rc))
                        break;
                    }  // <. >.
                    case 7: {
                        I rc;
                        KSLGLP(
                          cellatoms,
                          celllen,
                          DO(cellatoms, ((I *)partition)[i] = ((C *)wv)[i];),
                          ASSERT(EVOK == (rc = ((AHDR2FN *)adocv.f)(1, cellatoms, wv, partition, partition, jt)), rc))
                        break;
                    }  // + B01
                    case 11: {
                        ...
                        KSLGLPMEAN(
                          cellatoms,
                          celllen,
                          DO(cellatoms, ((D *)partition)[i] = ((B *)wv)[i];),
                          ASSERT(EVOK == (rc = ((AHDR2FN *)adocv.f)(1, cellatoms, wv, partition, partition, jt)), rc),
                          D)
                        break;
                    }  // mean B01
                    case 15: {
                        ...
                        KSLGLPMEAN(
                          celllen,
                          celllen,
                          DO(cellatoms, ((D *)partition)[i] = ((I *)wv)[i];),
                          ASSERT(EVOK == (rc = ((AHDR2FN *)adocv.f)(1, cellatoms, wv, partition, partition, jt)), rc),
                          D)
                        break;
                    }  // mean INT
                    ...
                    case RTNCASE(0, B01):
                        KSLGLP(1, 1, *(C *)partition = *(C *)wv;, *(C *)partition &= *(B *)wv;) break;  // <.
                    case RTNCASE(0, INT):
                        KSLGLP(SZI, SZI, *(I *)partition = *(I *)wv;, I p0 = *(I *)partition; I w0 = *(I *)wv;
                               p0              = p0 < w0 ? p0 : w0;
                               *(I *)partition = p0;)
                        break;  // <.
                    case RTNCASE(0, FL):
                        KSLGLP(SZD, SZD, *(D *)partition = *(D *)wv;, D *p0 = partition;
                               p0              = *(D *)partition < *(D *)wv ? p0 : wv;
                               *(D *)partition = *p0;)
                        break;  // <.
                    case RTNCASE(1, B01):
                        KSLGLP(1, 1, *(C *)partition = *(C *)wv;, *(C *)partition |= *(B *)wv;) break;  // <.
                    ...
                    case RTNCASE(1, FL):
                        ...
                        break;  // >.
                    case RTNCASE(2, B01):
                        KSLGLP(1, SZI, *(I *)partition = *(B *)wv;, *(I *)partition += *(B *)wv;)
                        break;  // +  the input is B01 but the result is INT
                    case RTNCASE(2, FL):
                        NAN0;
                        KSLGLP(SZD, SZD, *(D *)partition = *(D *)wv;, *(D *)partition += *(D *)wv;) NAN1 break;
                    case RTNCASE(3, B01):
                        NAN0;
                        KSLGLPMEAN(1, SZD, *(D *)partition = *(B *)wv;, *(D *)partition += *(B *)wv;, D) NAN1 break;
                    case RTNCASE(3, INT):
                        NAN0;
                        KSLGLPMEAN(SZI, SZD, *(D *)partition = *(I *)wv;, *(D *)partition += *(I *)wv;, D) NAN1 break;
                    case RTNCASE(3, FL):
                        NAN0;
                        KSLGLPMEAN(SZD, SZD, *(D *)partition = *(D *)wv;, *(D *)partition += *(D *)wv;, D) NAN1 break;
                }
            } else {
                // small-range processing of f//.
                // indexofsub detected that small-range processing is in order.  Information about the range is secreted
                // in fields of a
                ai        = (A)((I)ai - 1);
                I k       = AN(ai);
                I datamin = AK(ai);
                I p       = AM(ai);  // get size of an item in bytes, smallest item, range+1
                // allocate a tally area and clear it to ~0
                A ftbl;
                GATV0(ftbl, INT, p, 1);
                void **ftblv = voidAV(ftbl);
                memset(ftblv, ~0, p << LGSZI);
                // pass through the inputs, setting ftblv to 0 if there is a partition, and counting the number of
                // partitions
                ...
                I *av    = IAV(a);
                DQ(nitems, void *of = ftblv[*av & valmsk]; ftblv[*av & valmsk] = 0; nparts += (I)of & 1;
                   av = (I *)((I)av + k);)  // count partitions and set pointers there to 0
                // allocate the result area(s)
                GA(z, zt, nparts * cellatoms, AR(w), AS(w));
                AS(z)[0] = nparts;
                if (AN(z) == 0) return z;  // avoid calls with empty args
                if (keyslashfn == 3) {
                    GA(freq,
                ...
                // pass through the inputs again, accumulating the result.  First time we encounter a partition,
                // initialize its pointer and the result cell loop through the index, copying the first value and
...
#define KSLSRLPMEAN(wincr, zincr, pwvinit, pwvupd, freqt)                                                              \
        ...
        for (av = IAV(a), wv = voidAV(w); nitems; av = (I *)((I)av + k), wv = (C *)wv + (wincr), --nitems) {           \
            ...
            if (!partitionx) {                                                                                         \
                ...
                switch (routineid) {
                    ...
                    case RTNCASE(0, B01):
                        KSLSRLP(1, 1, *(C *)partition = *(C *)wv;, *(C *)partition &= *(B *)wv;) break;  // <.
                    case RTNCASE(0, INT):
                        KSLSRLP(SZI, SZI, *(I *)partition = *(I *)wv;, I p0 = *(I *)partition; I w0 = *(I *)wv;
                                p0              = p0 < w0 ? p0 : w0;
                                *(I *)partition = p0;)
                        break;  // <.
                    case RTNCASE(0, FL):
                        KSLSRLP(SZD, SZD, *(D *)partition = *(D *)wv;, D *p0 = partition;
                                p0              = *(D *)partition < *(D *)wv ? p0 : wv;
                                *(D *)partition = *p0;)
                        break;  // <.
                    case RTNCASE(1, B01):
                    ...
                    case RTNCASE(1, FL):
                        ...
                        break;  // >.
                    case RTNCASE(2, B01):
                        KSLSRLP(1, SZD, *(I *)partition = *(B *)wv;, *(I *)partition += *(B *)wv;)
                        break;  // +  the input is B01 but the result is INT
                    case RTNCASE(2, FL):
                        NAN0;
                        KSLSRLP(SZD, SZD, *(D *)partition = *(D *)wv;, *(D *)partition += *(D *)wv;) NAN1 break;
                    case RTNCASE(3, B01):
                        NAN0;
                        KSLSRLPMEAN(1, SZD, *(D *)partition = *(B *)wv;, *(D *)partition += *(B *)wv;, D) NAN1 break;
                    case RTNCASE(3, INT):
                        NAN0;
                        KSLSRLPMEAN(SZI, SZD, *(D *)partition = *(I *)wv;, *(D *)partition += *(I *)wv;, D) NAN1 break;
                    case RTNCASE(3, FL):
                        NAN0;
                        KSLSRLPMEAN(SZD, SZD, *(D *)partition = *(D *)wv;, *(D *)partition += *(D *)wv;, D) NAN1 break;
                }
            }
            // Finally, if this was mean, divide the total by the frequency
            if (((FAV(self)->flag & VFKEYSLASHF) >> VFKEYSLASHFX) == 3) {
                z = divideAW(z, freq);
            }  // always inplaceable
            EPILOG(z);
    ...
    // Allocate the area for the reordered copy of the input.  Do these calls early to free up registers for the main
    // loop
    ...
    GA(wperm,
       ...
       AN(w),
       AR(w),
       AS(w));  // Note we could avoid initialization of indirect types, since we are filling it all
    I celllen = cellatoms << bplg(AT(w));  // length of a cell of w, in bytes
    // Once we know the number of frets, we can allocate the fret area.  If the number is small, we can use the canned
    // area on the C stack. The max # bytes needed is 4*((max # partitions of size>=256) clamped at # frets) + #frets If
    // y is inplaceable we can probably use it to store the frets, since it is copied sequentially.  The requirements
    // are: The fret area must start at least 5 bytes before the data in y, to leave space for the first fret before the
    // first cell of y is moved (i. e. rank>0 for 64-bit, >1 for 32-bit) The cells must be at least 5 bytes long so that
    // the frets don't overrun them The cells must be in the actual fret area (following the shape), i. e not virtual or
    // NJA The cells must be DIRECT type so that it is OK to write garbage over them We are taking advantage here of the
    // fact that the shape comes before the cells in a standard GA noun We don't have to worry about a==w because a has
    ...
    if (!((I)ai & 1)) {
        // NOT small-range processing: go through the index+size table to create the frets and reordered data for
        // passing to cut
        nfrets        = AM(ai);  // fetch # frets before we possibly clone ai
        I maxfretsize = (nitems >> 8);
        maxfretsize   = maxfretsize < nfrets ? nfrets : maxfretsize;
        maxfretsize   = 4 * maxfretsize + nfrets + 1;  // max # bytes needed for frets, if some are long
        if ((UI)maxfretsize < sizeof(localfrets) - NORMAH * SZI) {
            frets     = (A)localfrets;
            AT(frets) = 0;
        }  // Cut tests the type field - only.  If debug, the flag also
        else if ((I)jtinplace & (I)((AFLAG(w) & (AFVIRTUAL | AFNJA)) == 0) &
                 ((UI)((-(I)(AT(w) & DIRECT)) & AC(w) & (4 - celllen) & ((I)(SZI == 4) - AR(w))) >>
                  (BW - 1 - JTINPLACEWX)))
            frets = w;
        else
            GATV0(frets, LIT, maxfretsize, 0);  // 1 byte per fret is adequate, since we have padding
        fretp = CUTFRETFRETS(frets);  // Place where we will store the fret-lengths.  They are 1 byte normally, or 5
                                      // bytes for groups longer than 254

        makewritable(ai);  // we modify the size+index info to be running endptrs into the reorder area
        // pass through the input, incrementing each reference
        I *av = IAV(ai);  // av->a data
        // Now each item av[i] is either (1) smaller than i, which means that it is extending a previous key; or (2)
        // greater than i, which means it starts a new partition whose length is a[i]-i.  Process the values in order,
        ...
        for (i = 0, nextpartitionx = (I)IAV(wperm); i < nitems; ++i) {
            ...
            if (avvalue < i) {                    // this value extends its partition
                ...
                // start of new partition.  Figure out the length; out new partition; replace length with starting
                // pointer; Use length to advance partition pointer
                avvalue -= i;  // length of partition
                if (avvalue < 255)
                    *fretp++ = (UC)avvalue;
                else {
                    *fretp++      = 255;
                    *(UI4 *)fretp = (UI4)avvalue;
                    fretp += SZUI4;
                }
                ...
                avvalue =
                  i;  // shift meaning of avvalue from length to index, where the partition pointer will be stored
            }

            av[avvalue] = (I)partitionptr + celllen;  // store updated end-of-partition after move
            JMCR(partitionptr, wv, celllen, loop1, 1, endmask);
            wv = (I *)((C *)wv + celllen);  // Don't overwrite, since we are scatter-writing
        }
    } else {
        I *av;  // running pointer through the inputs
        // indexofsub detected that small-range processing is in order.  Information about the range is secreted in
        // fields of a
        ai        = (A)((I)ai - 1);
        I k       = AN(ai);
        I datamin = AK(ai);
        I p       = AM(ai);  // get size of an item, smallest item, range+1
        ...
        av     = IAV(a);
        DQ(nitems, I tval = ftblv[*av & valmsk]; ftblv[*av & valmsk] = tval - 1; nfrets -= SGNTO0(tval);
           av = (I *)((I)av + k);)  // build (negative) frequency table; sub 1 for each non-fret
        // now that we have the number of freats, we can allocate the fret block
        I maxfretsize = (nitems >> 8);
        maxfretsize   = maxfretsize < nfrets ? nfrets : maxfretsize;
        maxfretsize   = 4 * maxfretsize + nfrets + 1;  // max # bytes needed for frets
        if ((UI)maxfretsize < sizeof(localfrets) - NORMAH * SZI) {
            frets     = (A)localfrets;
            AT(frets) = 0;
        }  // Cut tests the type field - only; for memaudit we need flag too
        else if ((I)jtinplace & (I)((AFLAG(w) & (AFVIRTUAL | AFNJA)) == 0) &
                 ((UI)((-(I)(AT(w) & DIRECT)) & AC(w) & (4 - celllen) & ((I)(SZI == 4) - AR(w))) >>
                  (BW - 1 - JTINPLACEWX)))
            frets = w;
        ...
        // pass through the inputs again.  If we encounter a negative value, allocate the next fret.  Copy the next item
        // and advance that partition's fret pointer Now each item ftblv[av[i]] is either (1) nonnegative, which means
        // that it is extending a previous key; or (2) negative, which means it starts a new partition whose length is
        // -ftblv[av[i]].  Process the values in order, creating partitions as they come up, and moving the data for
        // each input value in turn, reading in order and scatter-writing.
        ...
        for (av = IAV(a), i = nitems, nextpartitionx = 0; i; --i) {
            ...
            *slotaddr       = partitionndx + 1;                        // store updated next-in-partition after move
            ...
            JMCR(partitionptr, wv, celllen, loop2, 1, endmask);
            wv = (I *)((C *)wv + celllen);  // Don't overwrite, since we are scatter-writing
    ...
    // Frets are calculated and w is reordered.  Call cut to finish the job.  We have to store the count and length of
    // the frets
    ...
    // Transfer pristinity of w to wperm so we can see if it went away, but only if w is pristine inplaceable.  We want
    // to leave wperm pristine so it can be used, but it's pristine only is w is zombie.  We sacrifice pristinity to
    // inplaceability
    ...
    z = jtcut2((J)(intptr_t)((I)jt + ((FAV(self)->flag & VGERL)
                                        ? 0
                                        : (FAV(FAV(self)->fgh[0])->flag >> (VJTFLGOK1X - JTINPLACEWX)) & JTINPLACEW)),
               frets,
               wperm,
               self);
    // If the operation turned off pristinity of wperm, do the same for w.  Remember that pristinity only matters if the
    // block is inplaceable
...
A
jtkeybox(J jt, A a, A w, A self) {
    FPREFIP;
    PROLOG(0009);
    A ai, z = 0;
    I nitems;
    if ((SPARSE & AT(a)) != 0)
        return (AT(w) & NOUN ? (AF)jtkeysp : (AF)jthook1cell)(jt, a, w, self);  // if sparse, go handle it
    SETIC(a, nitems);                                                           // nitems is # items in a and w
    I cellatoms,
      celllen;  // number of atoms in an item of w, and the number of bytes therein.  celllen is negative for the monad
    struct AD fauxw;  // needed only for (</. i.@#) but must stay in scope
    if ((AT(w) & NOUN) != 0) {
        // dyad: </.
        I t2;
        ASSERT(nitems == SETIC(w, t2), EVLENGTH);  // verify agreement
        PROD(cellatoms, AR(w) - 1, AS(w) + 1);     // length of a cell of w, in atoms
        celllen = cellatoms << bplg(AT(w));        // length of a cell of w, in bytes
    } else {
        // monad: (<./ i.@#) .  Create a suitable w and switch to it
        cellatoms = 1;
        celllen   = -1;  // len and flag of cells from the synthetic index vector
        // Fill in enough of fauxw to stand in for the vector.  We need type=INT and rank=1, and the shape needs to be a
        // fetchable address but the value doesn't matter. The data address returned by IAV needs to be 0
    ...
    if (!((I)ai & 1)) {
        ...
        GATV0(z, BOX, nboxes, 1);
        if (nboxes == 0) { return z; }  // allocate result, and exit if empty for comp ease below
        // boxes will be in AAV(z), in order.  Details of hijacking tnextpushp are discussed in jtbox().
        // We have to do it a little differently here, because when we allocate a block the contents come in piecemeal
        // and we have no convenient time to INCORPRA the new contents.  So instead we leave the outer box
        // non-recursive, and rely on the EPILOG to make it recursive, including recurring on the contents of the boxes.
        // To make this work we install an AC of 0 in the allocated boxes, because they have no tpop outstanding.  The
        // EPILOG will raise that to 1.
        pushxsave      = jt->tnextpushp;
        ...
        for (i = 0; i < nitems; ++i) {
            ...
            if (avvalue < i) {                    // this value extends its partition
                ...
                // pointer; Use length to advance partition pointer
                avvalue -= i;  // length of partition
                GAE(y, wt, cellatoms * avvalue, yr, AS(w), break);
                AC(y) = 0;
                AS(y)
                [0] = avvalue;  // allocate a region for the boxed data and set usecount to 0 since it is not on the
                                // tstack.  EPILOG will raise it to 1
            ...
            if (celllen >= 0) {
                ...
                JMCR(partitionptr, wv, celllen, loop3, 1, endmask);
                wv = (I *)((C *)wv + celllen);  // Don't overwrite, since we are scatter-writing
        ...
        // This is tricky, because we want the slot to hold either the number of slots (i. e. size of box) or the next
        // address to store into for the slot. But the address can be anything - ah, but not quite.  The address can't
        // be inside an allocated block, for example it can't be inside a. So we initialize the table to the address of
        // the start of the data of a, and any number in the range (a,AN(a)+a] will be known to hold a count, not a
        // pointer NOTE that we know the items of a are not empty, so the prohibited address space is big enough
        I freqminval = (I)voidAV(a) + 1;  // values from here to here+nitems-1 are frequencies
        {
            I mm = freqminval - 1;
            mvc(p << LGSZI, ftblv, SZI, &mm);
        }                                                     // init table to '0' values
        I valmsk = (UI)~0LL >> (((-k) & (SZI - 1)) << LGBB);  // mask to leave the k lowest bytes valid
        ftblv -= datamin;                                     // bias starting addr so that values hit the table
        I nboxes = 0;                                         // initialize fret counter, incremented for each fret
        av       = IAV(a);
        DQ(nitems, I tval = ftblv[*av & valmsk]; ftblv[*av & valmsk] = ++tval; nboxes += tval == freqminval;
           av = (I *)((I)av + k);)  // build (negative) frequency table; sub 1 for each non-fret
        // allocate the result, which will be recursive, and set up to fill it with blocks off the tstack
        GATV0(z, BOX, nboxes, 1);
        if (nboxes == 0) { return z; }  // allocate result, and exit if empty for comp ease below
        // boxes will be in AAV(z), in order.  Details discussed in jtbox().
        // We will later make the block recursive usecount, and we will set all the initial usecounts to 1 since they
        // are not on the tpop stack
        pushxsave      = jt->tnextpushp;
        jt->tnextpushp = AAV(z);  // save tstack info before allocation
        // **** MUST NOT FAIL FROM HERE UNTIL THE END, WHERE THE ALLOCATION SYSTEM CAN BE RESTORED ****
        // pass through the inputs again.  If we encounter a frequency value, allocate the next box.  Copy the next item
        ...
        for (av = IAV(a), i = nitems; i; --i) {
            ...
            if ((UI)(avvalue - freqminval) >= (UI)nitems) {  // this value extends its partition
                ...
                [0] = avvalue;  // allocate a region for the boxed data and set usecount to 0 since it is not on the
                                // tstack.  EPILOG will raise it to 1
                partitionptr =
                  (I *)IAV(y);  // start the data for the partition at the beginning of the allocated box's data
            }
            ...
            if (celllen >= 0) {
                *slotaddr = (I)partitionptr + celllen;  // store updated end-of-partition after move
                // copy the data to the end of its partition and advance the partition pointer
                JMCR(partitionptr, wv, celllen, loop4, 1, endmask);
                wv = (I *)((C *)wv + celllen);          // Don't overwrite, since we are scatter-writing
            } else {                                    // flag for (<./ i.@#)
...
jtkeytallysp(J jt, A w) {
    ...
    v = AV(z);
    DQ(j, *v++ = *u++;);
    *v++ = SETIC(w, k) - bsum(c, BAV(b));
    DQ(d - j, *v++ = *u++;);
    EPILOG(z);
} /* x #/.y , sparse x */

static A
jtkeytally(J jt, A a, A w, A self) {
    FPREFIP;
    PROLOG(0016);
    A z, q;
    I at, j, k, n, r, s, *qv, *u, *v;
    SETIC(a, n);
    at = AT(a);
    ...
    if ((I)ai & 1) {  // if small-range
        ...
        I *av = IAV(a);
        DQ(n, I of = ftblv[*av & valmsk]; ftblv[*av & valmsk] = of + 1; nparts += SGNTO0(of);
           av = (I *)((I)av + k);)  // count partitions and set pointers there to 0
    ...
    // pass through the table, writing out every value that starts a new partition.  The partition size is encoded in
    // the value
    I i   = 0;
    I *av = IAV(ai);
    I *zv = IAV(z);
...
A
jtkeyheadtally(J jt, A a, A w, A self) {
    FPREFIP;
    PROLOG(0017);
    A f, q, x, y, z;
    I b;
    I at, *av, k, n, r, *qv, *u, *v, wt, *zv;
    SETIC(a, n);
    wt = AT(w);
    if ((AT(w) & NOUN) != 0) {
        // dyad: </.
    ...
    av = AV(a);
    f  = FAV(f)->fgh[0];
    b  = CHEAD == ID(f);  // f-> left tine of (#,{.)  b is 1 for {.,#  0 for #,{.  i. e. index of tally
    if ((AT(a) & B01) != 0)
        if (1 >= AR(a) && !(wt & VERB)) {
            B *p = (B *)av;
            I i, j, m;  // first special case: boolean list/atom, for which we can handle all types (except VERB)
            B *c = memchr(p, 1 ^ *p, n);
            i    = c - p;
            i    = c ? i : 0;
            j    = i;
            i    = *p ? 0 : i;
            j    = *p ? j : 0;  // i=index of first 1, j=index of first 0 (0 if not found)
            k    = bsum(n, p);
            m    = i + j ? 1 : 0;  // k=# 1s  m is 1 if there are 0s and 1s
            GATV0(x, INT, m + 1, 1);
            v    = AV(x);
            v[m] = i + j;
            v[0] = 0;  // 0=index of first item (always 0); 1 if it exists is the other
            GATV0(y, INT, m + 1, 1);
            v = AV(y);
            j = n - k;
            k = i ? j : k;
            k &= -m;
            v[0] = k;
            v[m] = n - k;  // if 1st value is 0, complement k; if only 1 value, clear k
            if (!(wt & VERB)) RZ(x = jtfrom(jt, x, w));
            w = x;
            x = b ? x : y;
            y = b ? y : w;
            return jtstitch(jt, x, y);  // select using index (unless i.@#, then keep index); set order & ,.
        }
    ...
    if (wt & B01 + INT + FL + VERB) {
        ...
        // We then allocate and run the small-range table and use it to rearrange the input.  The small-range variant is
        // signaled by the LSB of the result of indexofsub being set.
        if ((I)ai & 1) {  // if small-range
            // we should do small-range processing.  Extract the info
            ai        = (A)((I)ai - 1);
            I k       = AN(ai);
            ...
            DQ(n, I of = ftblv[*av & valmsk]; ftblv[*av & valmsk] = of + 1; nparts += SGNTO0(of);
               av = (I *)((I)av + k);)  // count partitions and set pointers there to 0
            // allocate result area
            GA(z, MAX(wt & ~VERB, INT), nparts * 2, 2, 0);
            AS(z)[0] = nparts;
            AS(z)[1] = 2;  // output area: one per partition
            // pass the input again, copying out the values.  Use branches to avoid the long carried dependency
            I i = 0;
            av  = IAV(a);  // item index and input scan pointer
            if (wt & INT) {
                I *wv = IAV(w);
                I *zv = IAV(z);
                while (1) {
            ...
            AS(z)[0] = nparts;
            AS(z)[1] = 2;  // output area: one per partition
            // pass through the table, writing out every value that starts a new partition.  The partition size is
            // encoded in the value
            I i   = 0;
            I *av = IAV(ai);  // item index and input scan pointer
            if (wt & INT) {
                I *wv = IAV(w);
                I *zv = IAV(z);
                while (1) {
...
jtsldot(J jt, A w) {
    ...
    AF f1 = jtoblique, f2;
    C c, d, e;
    I flag = VJTFLGOK1 | VJTFLGOK2;
    V *v;
    // NOTE: u/. is processed using the code for u;.1 and passing the self for /. into the cut verb.  So, the self
    // produced by /. and ;.1 must be the same as far as flags etc.
    if (NOUN & AT(w)) {
        flag |= VGERL;
        RZ(h = jtfxeachv(jt, 1L, w));
    }
    v = VAV(w);
    switch (ID(w)) {  // no default for f2: every path must set it
        ...
        case CSLASH:
            f2 = jtkey;
            if (AT(v->fgh[0]) & VERB && FAV(v->fgh[0])->flag & VISATOMIC2) {  // f//.  if f is atomic2
                f1 = jtobqfslash;
                flag &= ~VJTFLGOK1;
...
#define keyslashvalues(w) CCM(w, CPLUS) + CCM(w, CMIN) + CCM(w, CMAX)
                CCMWDS(keyslash) CCMCAND(keyslash, cand, FAV(v->fgh[0])->id) if (CCMTST(cand, FAV(v->fgh[0])->id)) {
                    I op = FAV(v->fgh[0])->id & 1;
                    op   = FAV(v->fgh[0])->id == CPLUS ? 2 : op;  // 0=<. 1=>. 2=+
                    flag += (((((2 << VFKEYSLASHFX) + ((FL + B01) << VFKEYSLASHTX)) << 16) +
                              (((1 << VFKEYSLASHFX) + ((FL + INT + B01) << VFKEYSLASHTX)) << 8) +
                              ((0 << VFKEYSLASHFX) + ((FL + INT + B01) << VFKEYSLASHTX))) >>
                             (op << 3)) &
                            (VFKEYSLASHT + VFKEYSLASHF);  // get flag bits
                }
            }
            break;
        case CBOX: f2 = jtkeybox; break;  // </.
        case CFORK:
            if (v->valencefns[0] == (AF)jtmean) {
                flag += (3 << VFKEYSLASHFX) + ((FL + INT + B01) << VFKEYSLASHTX);  // (+/%#)/., treated as f//.
            } else {
                c = ID(v->fgh[0]);
                d = ID(v->fgh[1]);
                e = ID(v->fgh[2]);
                if (((c ^ e) == (CHEAD ^ CPOUND)) && d == CCOMMA && (c == CHEAD || c == CPOUND)) {
                    f2 = jtkeyheadtally;
                    break;
                }
            }
