...
static B
jtiaddr(J jt, A z, A ind, A* i1, A* i2) {
    ...
    RZ(q = gt(jtsc(jt, h), a));
    y = SPA(zp, i);
    if (!all1(q)) RZ(y = jtrepeatr(jt, q, y)); /* indexed cols of index mat   */
    m  = *AS(y);
    yv = AV(y);
    RZ(ai = IX(h));
    RZ(as = jtless(jt, IX(AR(z)), a));
    u = AV(as);
    n = AN(as); /* dense axes                  */
    GATV0(t, INT, n, 1);
...
jtzpadn(J jt, A z, A ind, B ip) {
    ...
    d = 0;
    DO(h, if (b[i])++ d;);
    RZ(i1 = d < h ? jtrepeatr(jt, t, ind) : ind);
    if (2 != AR(ind)) RZ(i1 = d ? jtreshape(jt, jtv2(jt, AN(i1) / d, d), i1) : mtm);
    RZ(t = gt(jtsc(jt, h), a));
    RZ(y1 = all1(t) ? y : jtrepeatr(jt, t, y));
    ...
    if (m = *AS(p)) { /* new cells being added */
        ...
        RZ(t = shape(jt, x));
        *AV(t) = m;
        RZ(x = jtfrom(jt, q, jtover(jt, x, jtreshape(jt, t, SPA(zp, e)))));
...
jtastdn(J jt, A a, A z, A ind) {
    ...
    n  = *(AS(ind) + ir - 1);
    d  = (ir - 1) + (zr - n);
    ASSERT(ar <= d, EVRANK);  // n=shape of item of i; d is # unindexed axes
    GATV0(s, INT, d, 1);
    v = AV(s);
    MCISH(v, AS(ind), ir - 1);
    MCISH(v + ir - 1, zs + n, zr - n);
...
A
jtamne(J jt, A a, A z, A ind, B ip) {
    A i1, i2, x, y;
    C *u, *v;
    I *iv, *jv, k, m, n, vk, xk;
    P* zp;
...
A
jtamna(J jt, A a, A z, A ind, B ip) {
    A i1, i2, x;
    C *u, *v;
    I *iv, *jv, k, n, vk, xk;
    P* zp;
    ...
    jv = AV(i2);
    if (AR(a))
        DO(n, mvc(vk, v + xk * iv[i] + k * jv[i], vk, u); u += vk;)
    else
...
A
jtamnsp(J jt, A a, A z, A ind, B ip) {
    A i1, i2, t;
    C *ev, *u, *v, *vv;
    ...
    for (i = 0; i < n; ++i) {
        ...
        for (j = 0; j < c; ++j) {
            q = r;
            DQ(r, --q; ++dv[q]; if (dv[q] < s[q]) break; dv[q] = 0;);
            q = 1;
            while (p < m) {
