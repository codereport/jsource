...
I
bw1111insI(I d, I n, I m, UI* RESTRICTI x, UI* RESTRICTI z, J jt) {
    I k = SZI * m * d;
    memset(z, CFF, k);
    return EVOK;
}
static I
bw1111insC(I d, I n, I m, UC* RESTRICTI x, UC* RESTRICTI z, J jt) {
    I k = m * d;
    memset(z, CFF, k);
    return EVOK;
}
...
I
bw0011insI(I d, I n, I m, UI* RESTRICTI x, UI* RESTRICTI z, J jt) {
    I k = d * (n - 1);
    DQ(m, DQ(d, *z++ = *x++;); x += k;);
    return EVOK;
}
static I
bw0011insC(I d, I n, I m, UC* RESTRICTI x, UC* RESTRICTI z, J jt) {
    I k = d * (n - 1);
    DQ(m, DQ(d, *z++ = *x++;); x += k;);
    return EVOK;
}
...
I
bw1100insI(I d, I n, I m, UI* RESTRICTI x, UI* RESTRICTI z, J jt) {
    I k = d * (n - 1);
    if (1 < n) DQ(m, DQ(d, *z++ = ~*x++;); x += k;) else memcpy(z, x, SZI * m * d);
    return EVOK;
}
static I
bw1100insC(I d, I n, I m, UC* RESTRICTI x, UC* RESTRICTI z, J jt) {
    I k = d * (n - 1);
    if (1 < n) DQ(m, DQ(d, *z++ = ~*x++;); x += k;) else memcpy(z, x, m * d);
    return EVOK;
}

I
bw0101insI(I d, I n, I m, UI* RESTRICTI x, UI* RESTRICTI z, J jt) {
    I k = d * (n - 1);
    x += k;
    DQ(m, DQ(d, *z++ = *x++;); x += k;);
    return EVOK;
...
static I
bw0101insC(I d, I n, I m, UC* RESTRICTI x, UC* RESTRICTI z, J jt) {
    I k = d * (n - 1);
    x += k;
    DQ(m, DQ(d, *z++ = *x++;); x += k;);
    return EVOK;
...
I
bw1010insI(I d, I n, I m, UI* RESTRICTI x, UI* RESTRICTI z, J jt) {
    I k  = d * (n - 1);
    UI t = (n & 1) - 1;
    x += k;
    DQ(m, DQ(d, *z++ = t ^ *x++;); x += k;);
...
static I
bw1010insC(I d, I n, I m, UC* RESTRICTI x, UC* RESTRICTI z, J jt) {
    I k  = d * (n - 1);
    UC t = (UC)((n & 1) - 1);
    x += k;
    DQ(m, DQ(d, *z++ = t ^ *x++;); x += k;);
...
#define BITWISE(f, T, op)                                                                      \
    A f(J jt, A a, A w) {                                                                      \
        ...
        GATV(z, INT, AN(AR(a) > AR(w) ? a : w), MAX(AR(a), AR(w)), AS(AR(a) > AR(w) ? a : w)); \
        zv = (T*)AV(z);                                                                        \
        if (!AN(z)) return z;                                                                  \
        if (AR(a) == AR(w))                                                                    \
            DQ(AN(a), x = *av++; y = *wv++; *zv++ = op(x, y);)                                 \
        else if (AR(a) < AR(w))                                                                \
            DQ(AN(a), x = *av++; DQ(AN(w) / AN(a), y = *wv++; *zv++ = op(x, y););)             \
        else                                                                                   \
            DQ(AN(w), y = *wv++; DQ(AN(a) / AN(w), x = *av++; *zv++ = op(x, y);););            \
        RE(0);                                                                                 \
        return z;                                                                              \
    }
...
static AHDR2FN* bwI[16] = {(AHDR2FN*)bw0000II,
                           (AHDR2FN*)bw0001II,
                           (AHDR2FN*)bw0010II,
                           (AHDR2FN*)bw0011II,
                           (AHDR2FN*)bw0100II,
...
A
jtbitwisechar(J jt, A a, A w, A self) {
    DECLFG;
    A *p, x, y, z;
    B b;
    I j, m, n, zn;
    ...
    x = a;
    y = w;
    if ((-AN(a) & -AN(w) & -(AT(a) & AT(w)) & LIT) >= 0)
        return jtfrom(
          jt, df2(z, jtindexof(jt, ds(CALP), a), jtindexof(jt, ds(CALP), w), fs), ds(CALP));  // empty or not LIT
    b  = AR(a) <= AR(w);
    zn = AN(b ? w : a);
    m  = AN(b ? a : w);
    n  = zn / m;
    ...
    if (1 == n) {
        ado = bwI[j];
        m   = (m + SZI - 1) >> LGSZI;
    } else if ((-AR(a) & -AR(w) & -(n & (SZI - 1))) >= 0) {
        ado = bwI[j];
        n   = (n + SZI - 1) >> LGSZI;
        p   = b ? &x : &y;
        A zz;
        RZ(*p = IRS2(jtsc(jt, SZI), *p, 0L, 0L, 0L, jtrepeat, zz));
    }  // a atom, w atom, or multiple of SZI
    else
        ado = bwC[j];
    n ^= -b;
...
B
jtbitwisecharamp(J jt, UC* t, I n, UC* wv, UC* zv) {
    I p;
    UC c, i, j, *pv, s[256];
    AHDR2FN* ado;
    i = t[0];
    ...
    if (i == 0) {
        c   = j;
        ado = (AHDR2FN*)bw0001II;
    } else if (j == i) {
        c   = i;
        ado = (AHDR2FN*)bw0011II;
    } else if (j == 255) {
        c   = i;
        ado = (AHDR2FN*)bw0111II;
    } else if (j == 255 - i) {
        c   = i;
        ado = (AHDR2FN*)bw0110II;
    } else if (j == 0) {
        c   = i;
        ado = (AHDR2FN*)bw0010II;
    } else if (i == 255) {
        c   = j;
        ado = (AHDR2FN*)bw1011II;
    } else
        return 0;
    pv = (UC*)&p;
    DO(SZI, pv[i] = c;);
    ado((I)(256 / SZI), (I)1, AV(ds(CALP)), pv, s, jt);
    if (memcmpne(s, t, 256L)) return 0;
    ado((n + SZI - 1) >> LGSZI, (I)1, wv, pv, zv, jt);
    zv[n] = 0;
    return 1;
}  // kludge this should be scrapped in favor of wordlong ops

static AHDRRFN* bwinsC[16] = {(AHDRRFN*)bw0000insC,
                              (AHDRRFN*)bw0001insC,
                              (AHDRRFN*)bw0010insC,
                              (AHDRRFN*)bw0011insC,
                              (AHDRRFN*)bw0100insC,
...
static AHDRRFN* bwinsI[16] = {(AHDRRFN*)bw0000insI,
                              (AHDRRFN*)bw0001insI,
                              (AHDRRFN*)bw0010insI,
                              (AHDRRFN*)bw0011insI,
                              (AHDRRFN*)bw0100insI,
...
A
jtbitwiseinsertchar(J jt, A w, A self) {
    A fs, z;
    I d, j, n, r, wn, wr, zatoms;
    UC *u, *v, *wv, x, *zv;
    AHDRRFN* ado;
    wr = AR(w);
    wn = AN(w);
    ...
    z  = VAV(self)->fgh[0];
    fs = VAV(z)->fgh[0];
    if ((-(wn) & (SZI - n) & SGNIF(AT(w), LITX)) >= 0)
        return jtfrom(jt, df1(z, jtindexof(jt, ds(CALP), w), fs), ds(CALP));  // revert if not wn!=0 & n>SZI & LIT
    PROD(d, wr - 1, AS(w) + 1);
    zatoms = d;
    wv     = CAV(w);
    j      = jti0(jt, VAV(fs)->fgh[1]) - 16;
    ado    = (AHDRRFN*)bwinsC[j];  // d=#atoms in an item of a cell.  There is only 1 cell here (rank _)
    if (1 == wr) switch (j) {      // d==1 here
            ...
            case 7:
            case 9:
                ado = (AHDRRFN*)bwinsI[j];
                n   = n >> LGSZI;  // this gets # full words in the list arg.  Handle as ints.  Remnant handled below
        }
    else if (0 == (d & (SZI - 1))) {
        ado = (AHDRRFN*)bwinsI[j];
        d >>= LGSZI;
    }  // if #atoms are a word multiple, switch to handling ints
    GATV(z, LIT, zatoms, wr - 1, 1 + AS(w));
    zv = CAV(z);
    ado(d, n, 1L, wv, zv, jt);
    if (1 == wr) {
        r = wn - (n << LGSZI);
        ...
        switch (j) {  // Handle the remnant for fullword ops
            case 1:
                DQ(SZI - 1, x = BW0001(x, *v); ++v;);
                DQ(r, x = BW0001(x, *u); ++u;);
                break;
            case 6:
                DQ(SZI - 1, x = BW0110(x, *v); ++v;);
                DQ(r, x = BW0110(x, *u); ++u;);
                break;
            case 7:
                DQ(SZI - 1, x = BW0111(x, *v); ++v;);
                DQ(r, x = BW0111(x, *u); ++u;);
                break;
            case 9:
                DQ(SZI - 1, x = BW1001(x, *v); ++v;);
                DQ(r, x = BW1001(x, *u); ++u;);
                break;
        }
        *(I*)zv = 0;
