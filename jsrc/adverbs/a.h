...
#define CS1IPext(lpclass,cellclass,f,exp,x) cellclass DF1(f##cell){F1PREFIP;DECLFG;A z;PROLOG(x); exp; EPILOG(z);} lpclass DF1(f){PREF1(f##cell); return f##cell(jt,w,self);}
#define CS2IP(cellclass,class,f,exp,x) cellclass DF2(f##cell){F2PREFIP;DECLFG;A z;PROLOG(x); exp; EPILOG(z);} class DF2(f){PREF2(f##cell); return f##cell(jt,a,w,self);}
...
#define ASSERTNN(a,w)   ASSERT(!(VERB&(AT(a)|AT(w))),EVDOMAIN)
