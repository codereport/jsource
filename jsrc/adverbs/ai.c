...
static A jtexpandf(J jt,    A w,A self){A f; if(!(w && self)) return 0; f=FAV(self)->fgh[0]; return expand(VAV(f)->fgh[0],w);}
...
A jtinv(J jt, A w, I recur){A f,ff,g;B b,nf,ng,vf,vg;C id;I p,q;V*v;
 if(!w) return 0; STACKCHKOFL  // make sure we don't have a recursion loop through inv
