...
jtinvfork(J jt, A w) {
    ...
    V* v;
    v = FAV(w);
    RZ(f = jtunname(jt, v->fgh[0]));
    g = v->fgh[1];
    RZ(h = jtunname(jt, v->fgh[2]));
...
static A
jtexpandf(J jt, A w, A self) {
    A f;
    f = FAV(self)->fgh[0];
    return jtexpand(jt, VAV(f)->fgh[0], w);
}

static A
jtexpandg(J jt, A w, A self) {
    A f, g, z;
    V* v;
    f        = FAV(self)->fgh[0];
    v        = FAV(f);
    g        = v->fgh[1];
    jt->fill = FAV(g)->fgh[1];
    z        = jtexpand(jt, v->fgh[0], w);
    jt->fill = 0;  // elements of FAV cannot be virtual
    return z;
}
...
jtbminv(J jt, A w) {
    A *wv, x, z = w;
    I i, j, m, r, *s, t = 0, *u, **v, *y, wn, wr, *ws;
    ASSERT(0, EVNONCE);
    ...
    for (i = 0; i < wn; ++i) {
        ...
        if (2 > r)
            ...
            DO(wr, y = v[i] + u[i]; if (0 > *y)* y = s[i]; else ASSERT(*y == s[i], EVLENGTH););
        j = wr;
        while (1) {
            --j;
            ++u[j];
    ...
    if (!z) {
        ...
        for (i = 0; i < wn; ++i) {
            ...
            if (2 > AR(x)) {
                DO(wr, hv[i] = *(v[i] + u[i]););
                RZ(zv[i] = jtdiag(jt, x, jtreshape(jt, h, f)));
            }
            j = wr - 1;
            while (1) {
                --j;
    ...
    DO(wr, A t = z; RZ(df1(z, t, jtslash(jt, jtunder(jt, jtqq(jt, ds(CCOMMA), jtsc(jt, wr - i)), ds(COPE))))););
    return jtope(jt, z);
...
jtinvamp(J jt, A w) {
    ...
    x  = nf ? f : g;
    c  = ID(h);
    u  = VAV(h);  // h=verb arg, x=noun arg
    switch (c) {
        case CPLUS: return jtamp(jt, jtnegate(jt, x), h);
        case CSTAR: return jtamp(jt, jtrecip(jt, x), h);
        ...
        case CJDOT:
            return nf ? jtatop(jt, invrecur(ds(CJDOT)), jtamp(jt, ds(CMINUS), x))
                      : jtamp(jt, ds(CMINUS), jtjdot1(jt, x));
        case CRDOT:
            return nf ? jtatop(jt, invrecur(ds(CRDOT)), jtamp(jt, ds(CDIV), x)) : jtamp(jt, ds(CDIV), jtrdot1(jt, x));
        case CLBRACE: return nf ? jtamp(jt, jtpinv(jt, x), h) : jtamp(jt, x, ds(CIOTA));
        case COBVERSE: ff = FAV(h)->fgh[1]; return jtamp(jt, nf ? x : ff, nf ? ff : x);
        case CPDERIV:
            if (nf && !AR(x)) return ds(CPDERIV);
            break;                                                                  // only atom&p.. is invertible
        case CXCO: RE(n = jti0(jt, x)); ASSERT(n && BETWEENC(n, -2, 2), EVDOMAIN);  // fall through to create (-x)&u
        case CROT:                                                                  // fall through to create (-x)&u
        case CCIRCLE:                                                               // fall through to create (-x)&u
        case CSPARSE:
            if (nf) return jtamp(jt, jtnegate(jt, x), h);
            break;
        case CABASE:
            if (nf) return jtamp(jt, x, ds(CBASE));
            break;
        case CIOTA:
            if (nf) return jtamp(jt, ds(CLBRACE), x);
            break;
        case CTHORN:
            if (nf) return ds(CEXEC);
            break;
        case CTILDE:
            if (ff = FAV(h)->fgh[0], VERB & AT(ff))
                return jtinvamp(jt, jtamp(jt, nf ? ff : x, nf ? x : ff));
            else {
                ff = jtunname(jt, h);
                return jtinvamp(jt, jtamp(jt, nf ? x : ff, nf ? ff : x));
            }
        case CSCO:
            ASSERT(nf != 0, EVDOMAIN);
            RE(n = jti0(jt, x));
            ASSERT(n && BETWEENC(n, -6, 6), EVDOMAIN);
            return jtamp(jt, jtsc(jt, -n), h);
        case CUCO:
            ASSERT(nf != 0, EVDOMAIN);
            RE(n = jti0(jt, x));
            ASSERT(BETWEENC(n, 1, 8) & !BETWEENC(n, 5, 6), EVDOMAIN);
            return jtamp(jt, jtsc(jt, -(1 ^ (-n))), h);
        case CCANT: ASSERT(nf != 0, EVDOMAIN); return jtobverse(jt, jteva(jt, x, "] |:~ u C.^:_1 i.@#@$"), w);
        case CPCO:
            if (nf) {
                RE(n = jti0(jt, x));
                switch (n) {
                    case -4:
                    case 4: return jtamp(jt, jtnegate(jt, x), h);
                    case -1: return ds(CPCO);
                    case 2: return jtobverse(jt, jteval(jt, "*/@(^/)\"2"), w);
                    case 3: return jteval(jt, "*/");
                }
            }
            break;
        case CQCO:
        ...
        case CPOWOP:
            if (VGERL & u->flag) {
                ff = *(1 + AAV(u->fgh[2]));
                return jtamp(jt, nf ? x : ff, nf ? ff : x);
            }
            break;
        case CCOMMA:
            SETIC(x, n);
            return jtobverse(jt, 1 == n ? ds(nf ? CDROP : CCTAIL) : jtamp(jt, jtsc(jt, nf ? n : -n), ds(CDROP)), w);
        case CBASE:
            if (!nf) break;
        ...
        case CATOMIC:
            if (ng) {
                ...
                return jtobverse(jt, jtatop(jt, f, jtamp(jt, x, ds(CIOTA))), w);
            }  // fall through to common obverse (?)
        case CCYCLE:
            if (nf && AR(x) <= (c == CCYCLE)) return jtobverse(jt, jteva(jt, w, "/:@u@(i.@#) { ]"), w);
            break;
        case CDROP:
            if (!(nf && 1 >= AR(x))) break;
            RZ(x = jtcvt(jt, INT, x));
            RZ(y = jteps(jt, jtv2(jt, -1L, 1L), signum(x)));
            yv = CAV(y);
            f  = jtamp(jt, mag(x), ds(CPLUS));
            g  = 1 == AN(x) ? ds(CPOUND) : jtatop(jt, jtamp(jt, tally(jt, x), ds(CTAKE)), ds(CDOLLAR));
            h  = !yv[1] ? f : jtatop(jt, !yv[0] ? ds(CMINUS) : jtamp(jt, jtnegate(jt, signum(x)), ds(CSTAR)), f);
            return jtobverse(jt, jthook(jt, jtswap(jt, ds(CTAKE)), jtatop(jt, h, g)), w);
        case CDOMINO:
            if (!(2 == AR(x) && *AS(x) == *(1 + AS(x)))) break;
            ff = jteval(jt, "+/ .*");
            return nf ? jtatop(jt, h, jtamp(jt, ff, jtminv(jt, x))) : jtamp(jt, x, ff);
        case CDOT:
            if (ip(h, CPLUS, CSTAR)) {
                ASSERT(2 == AR(x), EVRANK);
                ASSERT(AS(x)[0] == AS(x)[1], EVLENGTH);
                return nf ? jtamp(jt, ds(CDOMINO), x) : jtamp(jt, h, jtminv(jt, x));
            }
            break;
        case CQQ:
        ...
        case CBSLASH:
            if (nf && (n = jti0(jt, x), 0 > n) && (d = ID(u->fgh[0]), (d & -2) == CLEFT))
                return jtslash(jt, ds(CCOMMA));  // LEFT || RIGHT
            break;
        case CIBEAM:
            x = FAV(h)->fgh[0];
            y = FAV(h)->fgh[1];
            if (NOUN & AT(x) && jtequ(jt, x, num(3)) && NOUN & AT(y)) {
                RE(n = jti0(jt, f));
                if (all1(jteps(jt, y, jtv2(jt, 4L, 5L)))) {
                    ASSERT(n && BETWEENC(n, -2, 2), EVDOMAIN);
                    return jtamp(jt, jtsc(jt, -n), g);
                }
                if (all1(jteps(jt, y, jtv2(jt, 1L, 3L)))) {
                    ASSERT(0 == n || 1 == n || 10 == n || 11 == n, EVDOMAIN);
                    return jtforeign(jt, x, num(2));
                }
            }
            break;
        case CBDOT:
            RE(n = jti0(jt, x));
            switch (jti0(jt, FAV(h)->fgh[1])) {
                case 22:
                case 25: return w;
                case 19:
                case 28:
                ...
                case 32:
                case 33:
                case 34: ASSERT(nf != 0, EVDOMAIN); return jtamp(jt, jtnegate(jt, x), h);
            }
            break;
        case CPOLY:
...
static const C simpleinv[128] = {
  [CDIV & 127] = CDIV,     [CPLUS & 127] = CPLUS,   [CMINUS & 127] = CMINUS,  [CLEFT & 127] = CLEFT,
  [CRIGHT & 127] = CRIGHT, [CREV & 127] = CREV,     [CCANT & 127] = CCANT,    [CPOLY & 127] = CPOLY,
  [CNOT & 127] = CNOT,     [CGRADE & 127] = CGRADE, [CCYCLE & 127] = CCYCLE,  [CDOMINO & 127] = CDOMINO,
  [COPE & 127] = CBOX,     [CBOX & 127] = COPE,     [CLOG & 127] = CEXP,      [CEXP & 127] = CLOG,
  [CGE & 127] = CLE,       [CLE & 127] = CGE,       [CHALVE & 127] = CPLUSCO, [CPLUSCO & 127] = CHALVE,
  [CSQRT & 127] = CSTARCO, [CSTARCO & 127] = CSQRT, [CHEAD & 127] = CLAMIN,   [CLAMIN & 127] = CHEAD,
...
jtinv(J jt, A w, I recur) {
    ...
    id = ID(w);
    v  = FAV(w);  // id=pseudochar for w, v->verb info
#define simpleinvvalues(w)                                                                                             \
    CCM(w, CDIV) + CCM(w, CPLUS) + CCM(w, CMINUS) + CCM(w, CLEFT) + CCM(w, CRIGHT) + CCM(w, CREV) + CCM(w, CCANT) +    \
      CCM(w, CPOLY) + CCM(w, CNOT) + CCM(w, CGRADE) + CCM(w, CCYCLE) + CCM(w, CDOMINO) + CCM(w, COPE) + CCM(w, CLOG) + \
      CCM(w, CBOX) + CCM(w, CEXP) + CCM(w, CGE) + CCM(w, CLE) + CCM(w, CHALVE) + CCM(w, CPLUSCO) + CCM(w, CSQRT) +     \
      CCM(w, CSTARCO) + CCM(w, CHEAD) + CCM(w, CLAMIN) + CCM(w, CABASE) + CCM(w, CBASE) + CCM(w, CTHORN) +             \
      CCM(w, CEXEC)
    ...
    vg = g && !ng;
    switch (id) {
        case CCIRCLE: return jteval(jt, "1p_1&*");
        case CJDOT: return jteval(jt, "0j_1&*");
        case CRDOT: return jteval(jt, "%&0j1@^.");
        case CPLUSDOT: return jteval(jt, "j./\"1\"_ :. +.");
        case CSTARDOT: return jteval(jt, "r./\"1\"_ :. *.");
        case CDGRADE: return jteval(jt, "/:@|.");
        case CWORDS: return jteval(jt, "}:@;@(,&' '&.>\"1) :. ;:");
        case CBANG:
        ...
        case CFORK: return jtinvfork(jt, w);
        case CAMP:
            if (nf != ng) {
                A z = jtinvamp(jt, w);
                if (nf ^ ng) return z;
            }  // may fall through... but avoid tail-recursion so we get out of loop
        case CAT:
            if (vf && vg) return jtatop(jt, invrecur(g), invrecur(f));
            break;
        case CAMPCO:
        case CATCO:
            if (vf && vg) return jtatco(jt, invrecur(g), invrecur(f));
            break;
        case CSLASH:
            if (CSTAR == ID(f)) return ds(CQCO);
            break;
        case CQQ:
            if (vf) return jtqq(jt, invrecur(f), g);
            break;
        case COBVERSE:
            if (vf && vg) return jtobverse(jt, g, f);
            break;  // if defined obverse, return it
        ...
        case CPOWOP:
            ...
            break;
        case CTILDE:
            if (nf) return invrecur(jtsymbrd(jt, f));  // name~ - resolve name & try again
            switch (ID(f)) {                           // inverses for reflexive monads
                case CPLUS: return ds(CHALVE);
                case CSTAR: return ds(CSQRT);
                case CJDOT: return jteval(jt, "0.5j_0.5&*");
                case CLAMIN: return jteval(jt, "{. :. (,:~)");
                case CSEMICO: return jteval(jt, ">@{. :. (;~)");
                case CCOMMA: return jteval(jt, "<.@-:@# {. ] :. (,~)");
                case CEXP: return jteval(jt, "3 : '(- -&b@(*^.) % >:@^.)^:_ ]1>.b=.^.y' \" 0 :. (^~)");
            }
            break;
        case CBSLASH:
        case CBSDOT:
            if (CSLASH == ID(f) && (ff = FAV(f)->fgh[0], ff && VERB & AT(ff))) {  //  ff/\  or ff/\.
                b = id == CBSDOT;
                switch (ID(ff)) {
                    case CPLUS: return jtobverse(jt, jteval(jt, b ? "- 1&(|.!.0)" : " - |.!.0"), w);
                    case CSTAR: return jtobverse(jt, jteval(jt, b ? "% 1&(|.!.1)" : " % |.!.1"), w);
                    case CEQ: return jtobverse(jt, jteval(jt, b ? "= 1&(|.!.1)" : " = |.!.1"), w);
                    case CNE: return jtobverse(jt, jteval(jt, b ? "~:1&(|.!.0)" : " ~:|.!.0"), w);
                    case CMINUS: return jtobverse(jt, jteval(jt, b ? "+ 1&(|.!.0)" : "(- |.!.0) *\"_1 $&1 _1@#"), w);
                    case CDIV: return jtobverse(jt, jteval(jt, b ? "* 1&(|.!.1)" : "(% |.!.1) ^\"_1 $&1 _1@#"), w);
                }
            }
            break;
        case CCUT:
        ...
        case CIBEAM:
            ...
            break;
        case CHOOK:
            if (BOTHEQ8(ID(f), ID(g), CFROM, CEQ)) return jteval(jt, "i.\"1&1");
            break;
    }
...
jtiden(J jt, A w) {
    ...
    switch (v->id) {
        ...
        case CUNDER:;
            A t;
            df1(x, df1(t, mtv, jtiden(jt, f)), inv(g));
            break;
        case CAT:
...
#define INV1 (BMK(1) + BMK(9) + BMK(11) + BMK(13) + BMK(17) + BMK(27) + BMK(29))
            if (bt < 32 && ((INVM1 | INV0 | INV1) & (1LL << bt))) {
                I bi;
                bi = -1;
                bi = INV0 & (1LL << bt) ? 0 : bi;
                bi = INV1 & (1LL << bt) ? 1 : bi;
