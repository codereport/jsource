...
A
jtdfs1(J jt, A w, A self) {
    FPREFIP;
    A s = jt->sf, z;
    RZ(self);
    z      = CALL1IP(FAV(self)->valencefns[0], w, jt->sf = self);
    jt->sf = s;
    return z;
}
A
jtdfs2(J jt, A a, A w, A self) {
    FPREFIP;
    A s = jt->sf, z;
    RZ(self);
    z      = CALL2IP(FAV(self)->valencefns[1], a, w, jt->sf = self);
    jt->sf = s;
    return z;
}
...
jtself1(J jt, A w) {
    A z;
    FDEPINC(d = 0);
    STACKCHKOFL df1(z, w, jt->sf);
    FDEPDEC(d);
    forcetomemory(w);
    return z;
}
A
jtself2(J jt, A a, A w) {
    A z;
    FDEPINC(d = 0);
    STACKCHKOFL df2(z, a, w, jt->sf);
    FDEPDEC(d);
    forcetomemory(w);
    return z;
}
...
A
jtdomainerr1(J jt, A w) {
    ASSERT(0, EVDOMAIN);
}
A
jtdomainerr2(J jt, A a, A w) {
    ASSERT(0, EVDOMAIN);
}
...
A
jtfdef(J jt, I flag2, C id, I t, AF f1, AF f2, A fs, A gs, A hs, I flag, I m, I l, I r) {
    A z;
    V *v;
    RE(0);
    GAT0(z, INT, (VERBSIZE + SZI - 1) >> LGSZI, 0);
    v     = FAV(z);  // allocate as fixed size, and as INT to avoid clearing the area, which will be all filled in
    AT(z) = t;       // install actual type
    if (fs) INCORP(fs);
    if (gs) INCORP(gs);
    if (hs) INCORP(hs);                                                 // indicate fgh are about to be incorporated
...
B
jtboxatop(J jt, A w) {
    return 1 & boxat(FAV(w)->fgh[0], RMAX, RMAX, RMAX);
}
...
I
boxat(A x, I m, I l, I r) {
    C c;
    V *v;
    if (!x) return 0;
    v = FAV(x);
    c = v->id;                    // x->fgh[0], v->value, c=id of f
    if (!COMPOSE(c)) return 0;    // Return if not @ @: & &:
    if (CBOX == ID(v->fgh[0])) {  // if u is <...
        if (c == CATCO) return 3;
        if (c == CAMPCO) return 1;  // @: is always good, and &: for monad
        I res = 0;
        if (v->fgh[1] && VERB & AT(v->fgh[1])) {
            res |= FAV(v->fgh[1])->mr >= m;  // monad ok if rank big enough  for either @ or &
            res |= (c == CAT) && lr(v->fgh[1]) >= (UI)l && rr(v->fgh[1]) >= (UI)r
                     ? 2
                     : 0;  // dyad for @ only, if rank big enough
        }
        return res;
    }
    return 0;
...
atoplr(A w) {
    ...
    C id = v->id;
    if ((v->id & -2) == CATCO) id = FAV(v->fgh[1])->id;  // @ @:
    return (id - (CLEFT - 1)) & REPSGN((CLEFT - 1) - (I)id) &
           REPSGN((I)id - (CRIGHT + 1));  // LEFT->1, RIGHT->2 punning with JINPLACEW/A; but 0 if not LEFT or RIGHT
}
