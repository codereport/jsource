...
I jtfdep(J jt,A w){A f,g;I d=0,k;V*v;
 ...
 A jtfdepadv(J jt, A w){if(!w) return 0; ASSERT(VERB&AT(w),EVDOMAIN); return sc(fdep(w));}
#endif
...
B jtboxatop(J jt,A w){if(!w) return 0; return 1&boxat(FAV(w)->fgh[0],RMAX,RMAX,RMAX);}
