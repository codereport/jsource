...
#define VDONE(T, PAR)                                          \
    {                                                          \
        I q = n / sizeof(T);                                   \
        T s, *y = (T*)x;                                       \
        DQ(m, s = 0; DQ(q, s ^= *y++;); PAR; *z++ = b == pc;); \
    }

static void
vdone(I m, I n, B* x, B* z, B pc) {
    B b, *u;
    if (1 == m) {
        UI s, *xi;
        s  = 0;
        ...
        DQ(SZI, b ^= *u++;);
        *z = b == pc;
    } else if (0 == (n & (sizeof(UI) - 1)))
        VDONE(UI, PARITYW)
    else if (0 == (n & (sizeof(UINT) - 1)))
        VDONE(UINT, PARITY4)
    else if (0 == (n & (sizeof(US) - 1)))
        VDONE(US, PARITY2)
    else
        DQ(m, b = 0; DQ(n, b ^= *x++;); *z++ = b == pc;);
}
...
#define REDUCEBFX(f, pfx, ipfx, spfx, bpfx, vdo)               \
    I f(I d, I n, I m, B* RESTRICTI x, B* RESTRICTI z, J jt) { \
        ...
        if (0 == d % sizeof(UI))                               \
            RBFXLOOP(UI, pfx)                                  \
        else if (0 == d % sizeof(UINT))                        \
            RBFXLOOP(UINT, ipfx)                               \
        else if (0 == d % sizeof(US))                          \
            RBFXLOOP(US, spfx)                                 \
        else                                                   \
            RBFXODDSIZE(pfx, bpfx)                             \
        return EVOK;                                           \
    } /* non-commutative */

REDUCECFX(eqinsB, EQ, IEQ, SEQ, BEQ, vdone(m, n, x, z, (B)(n & 1)))
REDUCECFX(neinsB, NE, INE, SNE, BNE, vdone(m, n, x, z, 1))
REDUCECFX(orinsB, OR, IOR, SOR, BOR, {
    I c = d * n;
    DQ(m, *z++ = 1 && memchr(x, C1, n); x += c;)
})
REDUCECFX( andinsB, AND, IAND, SAND, BAND, {
    I c = d * n; DQ(m, *z++=!  memchr(x,C0,n);                         x+=c;}))
REDUCEBFX(  ltinsB, LT,  ILT,  SLT,  BLT,  {
    I c = d * n;
    DQ(m, *z++ = *(x + n - 1) && !memchr(x, C1, n - 1) ? 1 : 0; x += c;)})
REDUCEBFX(  leinsB, LE,  ILE,  SLE,  BLE,  {
    I c = d * n;
    DQ(m, *z++ = !*(x + n - 1) && !memchr(x, C0, n - 1) ? 0 : 1; x += c;)})
REDUCEBFX(  gtinsB, GT,  IGT,  SGT,  BGT,  {
    I c = d * n;
    DQ(m, y = memchr(x, C0, n); *z++ = 1 && (y ? 1 & (y - x) : 1 & n); x += c;)})
REDUCEBFX(  geinsB, GE,  IGE,  SGE,  BGE,  {
    I c = d * n;
    DQ(m, y = memchr(x, C1, n); *z++ = !(y ? 1 & (y - x) : 1 & n); x += c;)})
REDUCEBFX( norinsB, NOR, INOR, SNOR, BNOR, {
    I c = d * n;
    DQ(m, y = memchr(x, C1, n); d = y ? y - x : n; *z++ = (1 & d) == d < n - 1; x += c;)})
REDUCEBFX(nandinsB, NAND,INAND,SNAND,BNAND,{
    I c = d * n;
    DQ(m, y = memchr(x, C0, n); d = y ? y - x : n; *z++ = (1 & d) != d < n - 1; x += c;)})
...
REDUCCPFX(tymesinsO, D, I, TYMESO) 
 ...
 I plusinsD(I d,I n,I m,D* RESTRICTI x,D* RESTRICTI z,J jt){
    ...
    if (d == 1) {
        x += m * n;
        z += m;
        DQ(m, D v0 = 0.0; D v1 = 0.0; if (((n + 1) & 3) == 0) v1 = *--x; D v2 = 0.0; if (n & 2) v2 = *--x; D v3 = 0.0;
           if (n & 3) v3 = *--x;
           DQ(n >> 2, v0 = PLUS(*--x, v0); v1 = PLUS(*--x, v1); v2 = PLUS(*--x, v2); v3 = PLUS(*--x, v3););
           v0 += v1;
           v2 += v3;
           *--z = v0 + v2;)
    } else {
        ...
        for (i = 0; i < m; ++i, z -= d) {
            ...
            y = x;
            x -= d;
            if (255 & (rc = plusDD(1, d, x, y, z, jt))) return rc;
            x -= d;
...
jtredg(J jt, A w, A self) {
    ...
    fauxblock(virtafaux);
    fauxvirtual(a, virtafaux, w, r - 1, ACUC1);  // allocate UNINCORPORABLE block
    // wfaux will hold the result from the iterations.  Init to value of last cell
    // Allocate fauxvirtual arg for the first cell, so it can be inplaceable/pristine if needed (tail returned a virtual
    // block, which messed things up for high rank)
    fauxblock(virtwfaux);
    A wfaux;
    fauxvirtual(wfaux, virtwfaux, w, r - 1, ACUC1);  // allocate UNINCORPORABLE block, mark inplaceable - used only once
                                                     // finish filling the virt block
    A* old = jt->tnextpushp;  // save stack mark for subsequent frees.  We keep the a argument over the calls, but allow
                              // the w to be deleted
    // fill in the shape, offset, and item-count of the virtual block
    I k;
    PROD(k, r - 1, AS(w) + 1);  // k=#atoms of cell of w
    AN(wfaux) = k;
    AN(a)     = k;
    k <<= bplg(AT(w));  // k now=length of input cell in bytes, where it will remain
    AK(wfaux) += (n - 1) * k;
    AK(a) += (n - 2) * k;
    MCISH(AS(wfaux), AS(w) + 1, r - 1);
    MCISH(AS(a), AS(w) + 1, r - 1);  // make the virtual block look like the tail, except for the offset
    // Calculate inplaceability.  We can inplace the left arg, which is always virtual, if w is inplaceable and (w is
    // direct or fs is &.>) We include contextual inplaceability (from jtinplace) here because if the block is returned,
    // its pristinity will be checked if it is inplaceable.  Thus we do not want to call a faux argument inplaceable if
    // it really isn't.  This gives us leeway with jtinplace itself
    I aipok = (SGNIF((I)jtinplace & (((AT(w) & TYPEVIPOK) != 0) | f2 == jtevery2self), JTINPLACEWX) & AC(w)) +
              ACUC1;  // requires JTINPLACEWX==0.  This is 1 or 8..1
    // We can inplace the right arg the first time if it is direct inplaceable, and always after that (assuming it is an
    // inplaceable result). and the input jtinplace.  We turn off WILLBEOPENED status in jtinplace for the callee.
    AC(wfaux) = aipok;  // first cell is inplaceable if second is
    jtinplace = (J)(intptr_t)(((I)jt) + (JTINPLACEW + JTINPLACEA) * ((FAV(fs)->flag >> (VJTFLGOK2X - JTINPLACEWX)) &
                                                                     JTINPLACEW));  // all items are used only once

    // We need to free memory in case the called routine leaves it unfreed (that's bad form & we shouldn't expect it),
    // and also to free the result of the previous iteration.  We don't want to free every time, though, because that
    // does ra() on w which could be a costly traversal if it's a nonrecusive recursible type. As a compromise we free
    // every few iterations: at least one per 8 iterations, and at least 8 times through the process
#define LGMINGCS 3       // lg2 of minimum number of times we call gc
#define MINGCINTERVAL 8  // max spacing between frees
    I freedist  = MIN((n + ((1 << LGMINGCS) - 1)) >> LGMINGCS, MINGCINTERVAL);
    I freephase = freedist;
    i           = n - 1;
    while (1) {         // for each cell except the last
        ...
        if (--freephase == 0) {
            wfaux     = jtgc(jt, wfaux, old);
            freephase = freedist;
        }  // free the buffers we allocated, except for the result
        // move to next input cell
        AK(a) -= k;
    }
    // At the end of all this, it is possible that the result is the original first or last cell, resting in its
    // original virtual block. In that case, we have to realize it, so that we don't let the fauxvirtual block escape
...
static A
jtredsp1a(J jt, C id, A z, A e, I n, I r, I* s) {
    A t;
    B b, p = 0;
    D d = 1;
    switch (id) {
...
static A
jtredsp1(J jt, A w, A self, C id, VARPSF ado, I cv, I f, I r, I zt) {
    A e, x, z;
    I m, n;
    P* wp;
    wp = PAV(w);
    ...
    GA(z, zt, 1, 0, 0);
    if (n) {
        I rc = ((AHDRRFN*)ado)(1L, n, 1L, AV(x), AV(z), jt);
        if (255 & rc) jtjsignal(jt, rc);
        RE(0);
        if (m == n) return z;
    }
...
static A
jtredspd(J jt, A w, A self, C id, VARPSF ado, I cv, I f, I r, I zt) {
    A a, e, x, z, zx;
    ...
    MCISH(xf + AS(zx), 1 + xf + s, xr - 1);
    I rc = ((AHDRRFN*)ado)(c / n, n, m, AV(x), AV(zx), jt);
    if (255 & rc) jtjsignal(jt, rc);
    RE(0);
    switch (id) {
        case CPLUS:
    ...
    if (TYPESNE(AT(e), AT(zx))) {
        ...
        if (TYPESNE(t, AT(zx))) RZ(zx = jtcvt(jt, t, zx));
    }
    wr = AR(w);
    ws = AS(w);
    GASPARSE(z, STYPE(AT(zx)), 1, wr - 1, ws);
    if (1 < wr) MCISH(f + AS(z), f + 1 + ws, wr - 1);
    zp = PAV(z);
    RZ(a = jtca(jt, a));
    v = AV(a);
...
static B
jtredspsprep(J jt, C id, I f, I zt, A a, A e, A x, A y, I* zm, I** zdv, B** zpv, I** zqv, C** zxxv, A* zsn) {
    A d, p, q, sn = 0, xx;
    B* pv;
    C* xxv;
    ...
    yc  = v[1];
    yr1 = yr - 1;
    RZ(d = jtgrade1(jt, eq(a, jtsc(jt, f))));
    dv = AV(d);
    DO(
      AN(a), if (i != dv[i]) {
          RZ(q = jtgrade1p(jt, d, y));
          qv = AV(q);
          break;
      });
    GATV0(p, B01, yr, 1);
...
static B
jtredspse(J jt, C id, I wm, I xt, A e, A zx, A sn, A* ze, A* zzx) {
    A b;
    B nz;
    I t, zt;
    RZ(b = ne(jfalse, sn));
    ...
    switch (id) {
        case CPLUS:
            if (nz) RZ(zx = plus(zx, tymes(e, sn)));
            RZ(e = tymes(e, jtsc(jt, wm)));
            break;
        case CSTAR:
            if (nz) RZ(zx = tymes(zx, jtbcvt(jt, 1, expn2(e, sn))));
...
static A
jtredsps(J jt, A w, A self, C id, VARPSF ado, I cv, I f, I r, I zt) {
    ...
    for (i = 0; i < m; ++i) {
        ...
        if (1 < n) {
            if (qv) {
                u = xxv;
                DO(n, memcpy(u, xv + xk * v[i], xk); u += xk;);
            }
            I rc = ((AHDRRFN*)ado)(xc, n, 1L, qv ? xxv : xv, zv, jt);
            if (255 & rc) jtjsignal(jt, rc);
            RE(0);
        } else if (zk == xk)
            memcpy(zv, qv ? xv + xk * *v : xv, xk);
        else {
            if (!x1) GA(x1, xt, xc, 1, 0);
            memcpy(AV(x1), qv ? xv + xk * *v : xv, xk);
            RZ(y = jtcvt(jt, zt, x1));
            memcpy(zv, AV(y), zk);
        }
        zv += zk;
    ...
    v = AV(a1);
    n = 0;
    DO(AN(a), if (f != v[i]) v[n++] = v[i] - (I)(f < v[i]););
    GASPARSE(z, STYPE(AT(zx)), 1, wr - 1, ws);
...
static A
jtreducesp(J jt, A w, A self) {
    A a, g, z;
    B b;
    I f, n, r, *v, wn, wr, *ws, wt, zt;
    P* wp;
    J jtinplace = jt;
    wr          = AR(w);
    r           = (RANKT)jt->ranks;
...
static A
jtreduce(J jt, A w, A self) {
    A z;
    I d, f, m, n, r, t, wr, *ws, zt;
    FPREFIP;
    ...
    r = wr < r ? wr : r;
    f = wr - r;
    SETICFR(w, f, r, n);  // no RESETRANK
    // Handle the special cases: neutrals, single items
    if (n > 1) {
        // Normal case, >1 item.
        // The case of empty w is interesting, because the #cells, and the #atoms in an item of a cell, may both
        // overflow if n=0.  But we have handled that case above.  If n is not 0, there may be other zeros in the shape
        // that allow an overflow when an infix of the shape is multiplied; but that won't matter because the other 0
        // will guarantee that there are no atoms written
        I wt = AT(w);
        wt   = AN(w) ? wt : B01;  // Treat empty as Boolean type
        ...
        // Calculate m: #cells of w to operate on; d: #atoms in an item of a cell of w cell of w (a cell to which u is
        // applied); zn: #atoms in result
        ...
        zt = rtype(adocv.cv);
        GA(z, zt, m * d, MAX(0, wr - 1), ws);
        if (1 < r) MCISH(f + AS(z), f + 1 + ws, r - 1);  // allocate, and install shape
        ...
        // EWOV1 means that there was an overflow on a single result, which was calculated accurately and stored as a D.
        // So in that case all we have to do is change the type of the result.
        if (255 & rc) {
            if (jt->jerr == EWOV1) {
                AT(z) = FL;
                return z;
                ...
                jtjsignal(jt, rc);
                return rc >= EWOV ? IRS1(w, self, r, jtreduce, z) : 0;
            }
        } else {
        ...
        return jthead(jt, w);  // reduce on single items - ranks are still set
    else
        return jtred0(jt, w, self);  // 0 items - neutrals - ranks are still set

} /* f/"r w main control */

static A
jtredcatsp(J jt, A w, A z, I r) {
    A a, q, x, y;
    B* b;
    ...
    GATV0(q, INT, n - (I)(c && d), 1);
    v = AV(q);
    DO(wr, if (b[i])* v++ = i - (I)(i > f););
    SPB(zp, a, q);
    if (c && d) { /* sparse sparse */
        SPB(zp, x, jtca(jt, x));
        SPB(zp, i, jtrepeatr(jt, ne(a, jtsc(jt, f)), y));
        q = SPA(zp, i);  // allow for virtualization of SPB
        v = j + AV(q);
        u = j + AV(y);
        ...
        *v = u[0] * u[1];
        ...
        v = j + AV(y);
        if (c)
            DO(m, k = p * *v; DO(e, *v = k + i; v += n;);)
        else
            DO(m, k = *v; DO(e, *v = k + p * i; v += n;););
        RZ(q = jtgrade1(jt, y));
        RZ(y = jtfrom(jt, q, y));
        RZ(x = jtfrom(jt, q, x));
        SPB(zp, x, x);
...
jtredcatcell(J jt, A w, I r) {
    ...
    if ((ASGNINPLACESGN(SGNIF((I)jtinplace, JTINPLACEWX) & (-r), w) &&
         !(AFLAG(w) & AFUNINCORPABLE))) {  // inplace allowed, usecount is right
        // operation is loosely inplaceable.  Just shorten the shape to frame,(#atoms in cell).  We do this here rather
        // than relying on the self-virtual-block code in virtual() because we can do it for indirect types also, since
        // we know we are not changing the number of atoms
        z     = w;
    ...
    I m;
    PROD(m, wr - r, AS(w));  // # cells being strung together
    AS(z)[0] = m;
    MCISH(AS(z) + 1, AS(w) + wr - r, r);  // install # cells and then shift the last r axes back
    return z;
}

A
jtredcat(J jt, A w, A self) {
    A z;
    B b;
    I f, r, *s, *v, wr;
    FPREFIP;
    ...
    s  = AS(w);
    RESETRANK;
    b = 1 == r && 1 == s[f];  // special case: ,/ on last axis which has length 1: in that case, the rules say the axis
                              // disappears (because of the way ,/ works on length-1 lists)
    ...
    if (!(SPARSE & AT(w))) {
        RZ(z = jtvirtual(jtinplace, w, 0, wr - 1));
        AN(z) = AN(w);  // Allocate the block.  Then move in AN and shape
        I* zs = AS(z);
        MCISH(zs, s, f);
        if (!b) {
            DPMULDE(s[f], s[f + 1], zs[f]);
            MCISH(zs + f + 1, s + f + 2, r - 2);
        }
        return z;
    } else {
...
jtredsemi(J jt, A w, A self) {
    ...
    if (BOX & AT(w))
        return jtredg(jt, w, self);  // the old way failed because it did not mimic scalar replication; revert to the
                                     // long way.  ranks are still set
    else {
        A z;
        return IRS1(w, 0L, r - 1, jtbox, z);
    }  // unboxed, just box the cells
} /* ;/"r w */

static A
jtredstitch(J jt, A w, A self) {
    A c, y;
    I f, n, r, *s, *v, wr;
    wr = AR(w);
    r  = (RANKT)jt->ranks;
    ...
    if (1 == r) {
        if (2 == n) return w;
        A z1, z2, z3;
        RZ(IRS2(num(-2), w, 0L, 0L, 1L, jtdrop, z1));
        RZ(IRS2(num(-2), w, 0L, 0L, 1L, jttake, z2));
    ...
    v[f]     = f + 1;
    v[f + 1] = f;
    RZ(y = jtcant2(jt, c, w));  // transpose last 2 axes
    if ((SPARSE & AT(w)) != 0) {
        A x;
        GATV0(x, INT, f + r - 1, 1);
        v = AV(x);
        MCISH(v, AS(y), f + 1);
        DPMULDE(s[f], s[f + 2], v[f + 1]);
        MCISH(v + f + 2, s + 3 + f, r - 3);
...
jtredstiteach(J jt, A w, A self) {
    ...
    SETIC(y, p);
    t = AT(y);
    DO(n, y = wv[i]; r = AR(y); if (!((((r - 1) & -2) == 0) && p == SETIC(y, n) && TYPESEQ(t, AT(y)))) return jtreduce(
         jt, w, self););  // rank 1 or 2, rows match, equal types
    return jtbox(jt, jtrazeh(jt, w));
} /* ,.&.>/ w */

static A
jtredcateach(J jt, A w, A self) {
    A *u, *v, *wv, x, *xv, z, *zv;
    I f, m, mn, n, r, wr, *ws, zm, zn;
    I n1 = 0, n2 = 0;
    wr = AR(w);
    ws = AS(w);
    r  = (RANKT)jt->ranks;
    ...
    PROD(m, r - 1, ws + f + 1);
    mn = m * n;
    GATV(z, BOX, zn, wr - 1, ws);
    MCISH(AS(z) + f, ws + f + 1, r - 1);
    GATV0(x, BOX, n, 1);
    xv = AAV(x);
    zv = AAV(z);
    wv = AAV(w);
    DO(zm, u = wv; DO(m, v = u++; DO(n, xv[i] = *v; v += m;); A Zz; RZ(Zz = jtraze(jt, x)); INCORP(Zz); *zv++ = Zz;);
       wv += mn;);  // no need to incorp *v since it's already boxed
    return z;
} /* ,&.>/"r w */

static A
jtoprod(J jt, A a, A w, A self) {
    A z;
    return df2(z, a, w, FAV(self)->fgh[2]);
}  // x u/ y - transfer to the u"lr,_ verb (precalculated)
...
jtslash(J jt, A w) {
    ...
    if (NOUN & AT(w))
        return jtevger(
          jt, w, jtsc(jt, GINSERT));  // treat m/ as m;.6.  This means that a node with CSLASH never contains gerund u
    v = FAV(w);
    switch (v->id) {  // select the monadic case
        ...
        default:
            f1   = jtreduce;
            flag = (v->flag & VJTFLGOK2) >> (VJTFLGOK2X - VJTFLGOK1X);
...
A
jtaslash(J jt, C c, A w) {
    RZ(w);
    A z;
    return df1(z, w, jtslash(jt, ds(c)));
}
A
...
jtatab(J jt, C c, A a, A w) {
    A z;
    return df2(z, a, w, jtslash(jt, ds(c)));
}

A
jtmean(J jt, A w, A self) {
    I wr = AR(w);
    I r  = (RANKT)jt->ranks;
    r    = wr < r ? wr : r;
    I n  = AS(w)[wr - r];
...
static A
jtfoldx(J jt, A a, A w, A self) {
    FPREFIP;  // this stands in place of jtxdefn, which inplaces
    // see if this is monad or dyad
    I foldflag = ((~AT(w)) >> (VERBX - 3)) & 8;  // flags: dyad mult fwd rev  if w is not conj, this must be a dyad call
    self       = foldflag ? self : w;
    w          = foldflag ? w : a;
    a          = foldflag ? a : mtv;  // if monad, it's w self garbage,  move to '' w self
    ...
    jt->foldrunning = 1;
    A z             = (*(FAV(self)->localuse.lfns[1]))(jt, a, w, self);
    jt->foldrunning = foldrunning;
