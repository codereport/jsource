...
jtistd1(J jt, A z, A ind) {
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (BOX & AT(x)) {
            ...
            RZ(x = jtpind(jt, d, x));
            jv[i] = x;
        }  // INCORP not needed probably, since this use is transient
    }
    return j;
} /* convert index list ind into standard form */

static A
jtastd1(J jt, A a, A z, A ind) {
    A *iv, q, r, s, s1, *sv, x;
    B b;
...
static A
jtssel(J jt, A z, A ind) {
    A a, *iv, p, q, x, y;
    B *b;
    I *av, c, i, j, m, n, *u, *v, *yv;
    P *zp;
    zp = PAV(z);
    y  = SPA(zp, i);
    v  = AS(y);
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (x != ds(CACE)) {
            ...
            RZ(p = jteps(jt, q, 1 < AR(x) ? jtravel(jt, x) : x));
            b = BAV(p);
        }
    }
    return p;
...
static B
jtipart(J jt, A z, A ind, A *i1, A *i2) {
    A *iv, p, *pv, q, *qv, x;
    B *b;
    I c, d, n;
    P *zp;
...
jtdcube(J jt, A z, A i2) {
    ...
    m  = 1;
    y  = iv[n - 1];
    if (y == ds(CACE)) RZ(y = IX(s[n - 1]));
    for (i = n - 2; 0 <= i; --i) {
        m *= s[1 + i];
        x = iv[i];
        A t;
        RZ(t = tymes(jtsc(jt, m), x == ds(CACE) ? IX(s[i]) : x));
...
jtscuba(J jt, A z, A i1, B u) {
    ...
    for (j = n - 1; 0 <= j; --j) {
        x = iv[j];
        if (x == ds(CACE)) RZ(x = IX(s[v[j]])) else {
                if (1 < AR(x)) RZ(x = jtravel(jt, x));
                if (u) RZ(x = jtnub(jt, x));
            }
        c = AN(x);
        if (q) {
            d = *AS(q);
            RZ(q = jtstitch(jt, jtrepeat(jt, jtsc(jt, d), x), jtreitem(jt, jtsc(jt, c * d), q)));
        } else
            RZ(q = jtreshape(jt, jtv2(jt, c, 1L), x));
    }
    return q;
} /* index cube relative to sparse axes; 1=u iff unique (remove duplicates) */

static A
jtscubb(J jt, A z, A i1) {
    A a, q, x, y;
    I c, d, h, j, *s, *v, *xv;
    P *zp;
...
jtscubc(J jt, A z, A i1, A p) {
    ...
    v  = AV(a);
    DO(h, d *= sv[i] = u[v[n + i]];);
    RZ(y = jtrepeat(jt, p, SPA(zp, i)));
    m = *AS(y);
    RZ(y1 = jttake(jt, jtv2(jt, m, n), y));
    v = AV(y1);
    GATV0(q, B01, m, 1);
    qv = BAV(q);
    if (m) {
...
jtzpad1(J jt, A z, A t, B ip) {
    ...
    if (m = *AS(t)) { /* new cells being added */
        ...
        RZ(s = shape(jt, x0));
        *AV(s) = m;
        RZ(x = jtfrom(jt, q, jtover(jt, x0, jtreshape(jt, s, SPA(zp, e)))));
        ...
        if (jt->assignsym && jt->assignsym->val == z) {
            RZ(ras(y));
            RZ(ras(x));
            fa(y0);
            fa(x0);
...
jtam1e(J jt, A a, A z, A ind, B ip) {
    ...
    s  = AS(x);
    k  = bpnoun(AT(x));
    xk = k * jtprod(jt, r - (n + 1), s + (n + 1));
    vk = k * jtprod(jt, r - 1, s + 1);
    if (!n)
        DO(m, if (pv[i]) mvc(xk, v, k, u); v += vk;)
    else {
        RZ(i2 = jtdcube(jt, z, i2));
        iv = AV(i2);
        n  = AN(i2);
...
jtam1a(J jt, A a, A z, A ind, B ip) {
    ...
    s  = AS(x);
    k  = bpnoun(AT(x));
    xk = k * jtprod(jt, r - 1 - n, s + 1 + n);
    vk = k * jtprod(jt, r - 1, s + 1);
    uk = !ar ? k : n ? xk : vk;
    u  = CAV(a);
    xv = v = CAV(x);
    RZ(t = jtiindx(jt, z, i1));
    iv = AV(t);
    m  = AN(t);
    if (!n && !m) {
        a1 = SPA(zp, a);
    ...
    if (!n)
        DO(m, mvc(vk, v + vk * iv[i], uk, u); if (ar) u += uk;)
    else if (m)
        DO(m, v = xv + vk * iv[i]; DO(c, mvc(xk, v + xk * jv[i], uk, u); if (ar) u += uk;);)
    else
        DO(*AS(x), v += vk; DO(c, mvc(xk, v + xk * jv[i], uk, u); if (ar) u += uk;););
    return z;
