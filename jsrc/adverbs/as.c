...
#define SUFFIXPFX(f, Tz, Tx, pfx, vecfn, retstmt)                                \
    I f(I d, I n, I m, Tx* RESTRICTI x, Tz* RESTRICTI z, J jt) {                 \
        ...
        z += m * d * n;                                                          \
        if (d == 1)                                                              \
            DQ(m, *--z = v = *--x; DQ(n - 1, --x; --z; *z = v = pfx(*x, v);))    \
        else {                                                                   \
            for (i = 0; i < m; ++i) {                                            \
                DQ(d, *--z = *--x;);                                             \
...
#define SUFFICPFX(f, Tz, Tx, pfx)                                                   \
    I f(I d, I n, I m, Tx* RESTRICTI x, Tz* RESTRICTI z, J jt) {                    \
        ...
        z += m * d * n;                                                             \
        if (d == 1)                                                                 \
            DQ(m, *--z = v = (Tz) * --x; DQ(n - 1, --x; --z; *z = v = pfx(*x, v);)) \
        else {                                                                      \
            for (i = 0; i < m; ++i) {                                               \
                y = z;                                                              \
                DQ(d, *--z = (Tz) * --x;);                                          \
    ...
    I f(I d, I n, I m, I* RESTRICTI x, I* RESTRICTI z, J jt) {                 \
        ...
        if (d == 1) {                                                          \
            if (1 == n)                                                        \
                ...
                DQ(m, z = zz -= d * n; x = xx -= d * n; fs1(n, z, x);)         \
        } else {                                                               \
            for (i = 0; i < m; ++i) {                                          \
                DQ(d, *--zz = *--xx;);                                         \
...
#define SUFFIXBFX(f, pfx, ipfx, spfx, bpfx, vexp)                                               \
    I f(I d, I n, I m, B* RESTRICTI x, B* RESTRICTI z, J jt) {                                  \
        ...
        z += m * d * n;                                                                         \
        if (1 == d) {                                                                           \
            DQ(m, *--z = v = *--x; DQ(n - 1, --x; --z; *z = v = vexp;));                        \
            return EVOK;                                                                        \
        }                                                                                       \
        if (0 == (d & (sizeof(UI) - 1))) {                                                      \
            SUFFIXBFXLOOP(UI, pfx);                                                             \
            return EVOK;                                                                        \
        }                                                                                       \
        if (0 == (d & (sizeof(UI4) - 1))) {                                                     \
            SUFFIXBFXLOOP(UINT, ipfx);                                                          \
            return EVOK;                                                                        \
        }                                                                                       \
        if (0 == (d & (sizeof(US) - 1))) {                                                      \
            SUFFIXBFXLOOP(US, spfx);                                                            \
            return EVOK;                                                                        \
        }                                                                                       \
...
jtgsuffix(J jt, A w, A self) {
    ...
    zv     = AAV(z);
    I imod = 0;
    DO(n, imod = (imod == m) ? 0 : imod; RZ(zv[i] = df1(h, jtdrop(jt, jtsc(jt, i), w), hv[imod])); ++imod;);
    return jtope(jt, z);
} /* g\."r w for gerund g */

static A
jtssg(J jt, A w, A self) {
    FPREFIP;
    PROLOG(0020);
    A a, z;
    I i, n, r, wr;
...
#define ZZFLAGWORD state
    ...
    A fs = FAV(FAV(self)->fgh[0])->fgh[0];
    AF f2 =
      FAV(fs)
        ->valencefns[1];  // self = f/\.   FAV(self)->fgh[0] = f/  FAV(FAV(self)->fgh[0])->fgh[0] = f   fetch dyad for f
    // Set BOXATOP if appropriate.  Since {:y is always the last cell, BOXATOP is allowed only when the rank of w is 1,
    // meaning that
    // {:y is a single box, just like the other results.  Also require that w be boxed, lest we make the first z-cell
    // invalid
    state = ZZFLAGINITSTATE | (REPSGN(wr - 2) & (AT(w) >> (BOXX - ZZFLAGBOXATOPX)) &
                               ((FAV(fs)->flag2 & VF2BOXATOP2) >>
                                (VF2BOXATOP2X - ZZFLAGBOXATOPX)));  // If rank OK, extract flag.  Rank cannot be 0.
                                                                    // Don't touch fs yet, since we might not loop
    state &=
      ~((FAV(fs)->flag2 & VF2ATOPOPEN2W) >> (VF2ATOPOPEN2WX - ZZFLAGBOXATOPX));  // We don't handle &.> here; ignore it

    // We cannot honor WILLBEOPENED, because the same box that goes into the result must also be released into the next
    // application of f. Without WILLBEOPENED, there is no need to set COUNTITEMS
// not checked state |= (-state) & (I)jtinplace & JTCOUNTITEMS; // remember if this verb is followed by ; - only if we
...
#define ZZWILLBEOPENEDNEVER 1
    ...
    fauxblock(virtafaux);
    fauxvirtual(a, virtafaux, w, r - 1, ACUC1);
    ...
    // Allocate fauxvirtual arg for the first cell, so it can be inplaceable/pristine if needed (tail returned a
    // noninplaceable virtual block, which messed things up for high rank)
    fauxblock(virtwfaux);
    fauxvirtual(z, virtwfaux, w, r - 1, ACUC1);  // allocate UNINCORPORABLE block, mark inplaceable - used only once
    ...
    AK(z) += (n - 1) * k;
    AK(a) += (n - 1) * k;
    MCISH(AS(z), AS(w) + 1, r - 1);
    MCISH(AS(a), AS(w) + 1, r - 1);  // a points to tail; it will be decremented before first use
    ...
    state |= (UI)(SGNIF((I)jtinplace, JTINPLACEWX) & ~((AT(w) & TYPEVIPOK) - (f2 != jtevery2self)) & AC(w)) >>
             (BW - 1 - ZZFLAGVIRTAINPLACEX);  // requires JTINPLACEWX==0.  Single flag bit
    ...
    AC(z)     = ACUC1 + ((state & ZZFLAGVIRTAINPLACE)
                     << (ACINPLACEX - ZZFLAGVIRTAINPLACEX));  // first cell is inplaceable if second is
...
#define ZZPOPNEVER \
    1  // we mustn't TPOP after copying the result atoms, because they are reused.  This will leave the memory used for
       // type-conversions unclaimed.

    // We have to dance a bit for BOXATOP verbs, because the result comes back unboxed, but it has to be put into a box
    // to be fed into the next iteration.  This is still a saving, because we can use the same box to point to each
    // successive result. Exception: if the reusable box gets incorporated, it is no longer reusable and must be
    // reallocated.  We will use the original z box, which will NEVER be virtual because it is an atom whenever BOXATOP
    // is set, as the starting pointer to the prev boxed result
    A boxedz = z;
    z        = AAV(z)[0];
    z = (state & ZZFLAGBOXATOP) ? z : boxedz;  // init current pointer for the temp box; if BOXATOP, use >{:y as the
                                               // first (to-be-boxed) result (always safe to fetch 1 value from z)
...
#define ZZINSTALLFRAME(optr) *optr++ = n;
    ...
    for (i = 0; i < n; ++i) {                      // loop through items, noting that the first is the tail itself
        ...
        // have escaped into the running result
...
#include "result.h"
        ...
        if (state & ZZFLAGBOXATOP) {
            // If boxedz itself has been incorporated into the result, we have to reallocate it.  We don't need the
            // usual check for z==boxedz, because we know we INCORPed z into the boxed result, so if it was the same as
            // boxedz, the usecount of boxedz was incremented then
            if (!ACIPISOK(boxedz)) GAT0(boxedz, BOX, 1, 0);  // reallocate boxedz if needed
            ACIPNO(z);
            AAV(boxedz)[0] = z;
            z = boxedz;  // incorporate z into boxedz; point boxedz to the previous result, and make that the new
                         // argument for next time
        }
        ...
        AC(a) = ACUC1 + ((state & ZZFLAGVIRTAINPLACE)
...
A
jtscansp(J jt, A w, A self, AF sf) {
    A e, ee, x, z;
    B* b;
    I f, m, j, r, t, wr;
    P *wp, *zp;
    ...
    GASPARSE(z, STYPE(t), 1, wr + !m, AS(w));
    if (!m) *(wr + AS(z)) = 1;
    zp = PAV(z);
    SPB(zp, e, e);
    SPB(zp, x, x);
...
static A
jtsscan(J jt, A w, A self) {
    A y, z;
    I d, f, m, n, r, t, wn, wr, *ws, wt;
    FPREFIP;
    if (!w) return 0;
    wt = AT(w);
    ...
    I* nn = &ws[f];
    nn    = r ? nn : &I1mem;
    n     = *nn;                // will not be used if WN==0, so PROD ok.  n is # items along the selected rank
    ...
    VARPS adocv;
    varps(adocv, self, wt, 2);                          // analyze f - get suffix routine
    ...
    if (rc & 255) {
        jtjsignal(jt, rc);
        return jt->jerr >= EWOV ? IRS1(w, self, r, jtsscan, z) : 0;
    } else
        return adocv.cv & VRI + VRD ? jtcvz(jt, adocv.cv, z) : z;
} /* f/\."r w main control */

static A
jtomask(J jt, A a, A w) {
    A c, r, x, y;
    I m, n, p;
    RE(m = jti0(jt, a));
    p = ABS(m);
...
jtgoutfix(J jt, A a, A w, A self) {
    ...
    zv     = AAV(z);
    I imod = 0;
    DO(n, imod = (imod == m) ? 0 : imod; RZ(zv[i] = df1(h, jtrepeat(jt, jtfrom(jt, jtsc(jt, i), x), w), hv[imod]));
       ++imod;);
    return jtope(jt, z);
}
...
static A
jtofxassoc(J jt, A a, A w, A self) {
    A f, i, j, p, s, x, z;
    C id, *zv;
    I c, d, k, kc, m, r, t;
    V* v;
    ...
    SETIC(w, m);
    RE(k = jti0(jt, a));
    c  = ABS(k);  // m = # items in w; k is value of a; c is # items per suffix
    f  = FAV(self)->fgh[0];
    x  = FAV(f)->fgh[0];
    v  = FAV(x);
    ...
    RZ(p = jtfrom(
         jt, i, df1(z, w, jtbslash(jt, f))));  // p is i { u\ w; that is, the totals of the prefixes after the first
    RZ(s = jtfrom(jt, j, df1(z, w, jtbsdot(jt, f))));  // s is j { u\. w; totals of suffixes except the last
    // We need to make sure that p, s, and (p f s) all have the same type.  This is problematic, since we don't actually
    // see the type of (p f s) which is encoded in cv below.  But since this case is limited to atomic associative
    // verbs, we know that if p and s have the same type, p f s will also, except that it might overflow, which we will
    // detect after we run it.
    //
    ...
    if (!TYPESEQ(AT(p), AT(s))) {
        rc = EWOV;
    } else {
        I klg;  // simulate overflow if different precisions - will convert everything to float
        r                               = AR(p);
        ...
        GA(z, t, c * (1 + d), r, AS(p));
        AS(z)[0] = 1 + d;
        zv       = CAV(z);      // allocate result assuming no overflow
        memcpy(zv, AV(s), kc);  // first cell is {.s, i. e. all but the first infix
        rc = (1 < d) ? ((AHDR2FN*)adocv.f)((I)1, c * (d - 1), AV(p), kc + CAV(s), zv + kc, jt) : EVOK;
          /* (}:p) f (}.s), with result stored into the result area */  // don't call with 0 length!
...
jtbsdot(J jt, A w) {
    ...
    switch (v->id) {
        ...
        case CSLASH:       // v is f/, but not when f is a gerund
            ...
            f  = v->fgh[0];
            flag |= (FAV(f)->flag & VJTFLGOK2) >> (VJTFLGOK2X - VJTFLGOK1X);
            id = ID(f);
            if (id == CBDOT) {
                f = VAV(f)->fgh[1];
                if (INT & AT(f) && !AR(f)) id = (C)AV(f)[0];
            }
...
#define xassocvalues(w)                                                                                   \
    CCM(w, CSTAR) + CCM(w, CMAX) + CCM(w, CMIN) + CCM(w, CPLUSDOT) + CCM(w, CSTARDOT) + CCM(w, CBW0000) + \
      CCM(w, CBW0001) + CCM(w, CBW0011) + CCM(w, CBW0101) + CCM(w, CBW0111) + CCM(w, CBW1111)
