...
static A jtlcpb(J jt,B b,A w){FPREFIP;A z=w;B p;C c,*v,*wv,*zv;I n;
 ...
 if(b){GATV0(z,LIT,2+n,1); zv=CAV(z); *zv='('; memcpy(1+zv,wv,n); zv[1+n]=')';}
 return z;
}

static A jtlcpx(J jt,A w){FPREFIP; return parfn(jtinplace,jtlp(jt,w),w);}

static A jtltiea(J jt,A w,A *ltext){FPREFIP;A t,*v,*wv,x,y;B b;C c;I n;
 n=AN(w); wv=AAV(w);  RZ(t=jtspellout(jt,CGRAVE));
 GATV0(y,BOX,n+n,1); v=AAV(y);
 DO(n, *v++=i?t:mtv; x=wv[i]; c=ID(x); RZ(x=lrr(x)); 
     b=BETWEENC(c,CHOOK,CFORK)||i&&jtlp(jt,x); RZ(*v++=parfn(jtinplace,b,x)););
 return jtraze(jt,y);
}

static A jtltieb(J jt,A w,A *ltext){FPREFIP;A pt,t,*v,*wv,x,y;B b;C c,*s;I n;
 n=AN(w); wv=AAV(w);  RZ(t=jtspellout(jt,CGRAVE)); RZ(pt=jtover(jt,jtscc(jt,')'),t));
 GATV0(y,BOX,n+n,1); v=AAV(y);
...
static A jtlsh(J jt,A w,A *ltext){FPREFIP;return apip(jtthorn1(jt,shape(jt,w)),jtspellout(jt,CDOLLAR));}

// return something to turn a list into the shape:
static A jtlshape(J jt,A w,A *ltext){FPREFIP;I r,*s;
 r=AR(w); s=AS(w);
 return 2==r&&(1==s[0]||1==s[1]) ? jtspellout(jt,(C)(1==s[1]?CCOMDOT:CLAMIN)) : !r ? mtv :
     1<r ? lsh(w) : 1<AN(w) ? mtv : jtspellout(jt,CCOMMA);
}
...
static A jtlchar(J jt,A w,A *ltext){FPREFIP;A y;B b,p=1,r1;C c,d,*u,*v;I j,k,m,n;
 ...
 if(0<=j&&r1&&!memcmpne(v+j,AV(ds(CALP)),m)){
  ...
  return jtlp(jt,y)?jtover(jt,jtcstr(jt,"a.,~"),y):jtover(jt,y,jtcstr(jt,",a."));  // use rest,a. or a.,~rest depending on () needs
 }
 if(r1&&m==n&&(y=jticap(jt,ne(w,ds(CALP))))&&m>AN(y)){  // if 256-byte string, see where it differs from a.
  if(1==AN(y))RZ(y=jthead(jt,y));
  return jtover(jt,jtover(jt,jtcstr(jt,"a. "),jtlcpx(jt,lnum(y))),jtover(jt,jtcstr(jt,"}~"),lchar(jtfrom(jt,y,w))));   // use diff indx} a. or the like
 }
 ...
 if(!p){  // if the string contains a nonprintable, represent it as nums { a.
  ...
  if(r1&&n<m&&(!k||k==m-n)&&jtequ(jt,y,apv(n,k,1L)))return jtover(jt,jtthorn1(jt,jtsc(jt,d?-n:n)),jtcstr(jt,"{.a."));
  RZ(y=lnum(y));
...
A jtdecorate(J jt,A w,I t){
 ...
 if(t&FL){
  ...
  if(needdot){w=jtover(jt,w,jtscc(jt,'.')); RZ(w=jtmkwris(jt,w)); DQ(AN(w) , if(CAV(w)[i]==' ')return w;  if(CAV(w)[i]=='e'){C f='.'; C *s=&CAV(w)[i]; DO(AN(w)-i, C ff=s[i]; s[i]=f; f=ff;)}) }
 }else if(t&INT){
 // integer: if the string contains nothing but one-digit 0/1 values, insert '0' before last number
  ...
  if(l==0){I ls=0; DQ(AN(w), if(CAV(w)[i]==' ') ls=i;); w=ls?jtdinsert(jt,w,jtscc(jt,'0'),ls+1):jtover(jt,jtscc(jt,'0'),w);}
 }else if(t&XNUM) w=jtdcapp(jt, w,'x',jtscc(jt,'x')); // extended: make sure there is an x somewhere in the string, else put 'x' at end
 else if(t&RAT) w=jtdcapp(jt, w,'r',jtcstr(jt,"r1")); // rational: make sure there is an r somewhere in the string, else put 'r1' at end
 else if(t&CMPX) w=jtdcapp(jt, w,'j',jtcstr(jt,"j0")); // complex: make sure there is a j somewhere in the string, else put "j0" at end
 return w;
}
 ...
 if(7<n||1<n&&1<AR(w)){
  ...
  if(p){
   ...
   if(p=!(jtequ(jt,b,jtsc(jt,n-1))&&jtequ(jt,d,num(-1)))){
    if     (!jtequ(jt,b,num(0)   )){v[0]=lnum1(b); v[1]=jtspellout(jt,CPLUS);}
    if     ( jtequ(jt,d,num(-1))) v[1]=jtspellout(jt,CMINUS);
    else if(!jtequ(jt,d,num(1)    )){v[2]=lnum1(d); v[3]=jtspellout(jt,CSTAR);}
   }
   v[4]=jtspellout(jt,CIOTA); v[5]=jtthorn1(jt,p?shape(jt,w):jtnegate(jt,shape(jt,w)));
   RE(y); return jtraze(jt,y);
  }
...
static A jtlsparse(J jt,A w,A *ltext){FPREFIP;A a,e,q,t,x,y,z;B ba,be,bn;I j,r,*v;P*p;
 ...
 be=!(AT(w)&SFL&&0==*DAV(e));
 if(be)RZ(z=jtover(jt,lnoun(e),jtcstr(jt,SB01&AT(w)?"":SINT&AT(w)?"+-~2":SFL&AT(w)?"+-~2.1":"+-~2j1")));
 if(be||ba){
 ...
 }else RZ(t=jtover(jt,jtlcpx(jt,lnoun(jthead(jt,x))),jtcstr(jt," a:}"))); 
...
static A jtlnoun0(J jt,A w,A *ltext){FPREFIP;A s,x;B r1;
 r1=1==AR(w); RZ(s=jtthorn1(jt,shape(jt,w)));
 switch(CTTZ(AT(w))){
...
static A jtlsymb(J jt,C c,A w,A *ltext){FPREFIP;A t;C buf[20],d,*s;I*u;V*v=FAV(w);
 ...
 }else{RZ(t=jtspella(jt,w)); if(AN(t)==1&&(CAV(t)[0]=='{'||CAV(t)[0]=='}')){RZ(t=jtmkwris(jt,t)); AS(t)[0]=AN(t)=2; CAV(t)[1]=' '; }}  // add trailing space to { } to avoid DD codes
 d=cf(t);
...
// ?t tells whether () is needed around the f/g/h component
 if(1<=n){f=av[0]; t=fs; c=ID(t); ft=BETWEENC(c,CHOOK,CADVF)||(b||id==CFORK)&&NOUN&AT(t)&&jtlp(jt,f);}  // f: () if it's hook fork && or noun left end of nvv or n (op)
 if(2<=n){g=av[1]; t=gs; c=ID(t); gt=VERB&AT(w)    ?BETWEENC(c,CHOOK,CFORK):jtlp(jt,g);}
 if(3<=n){h=av[2]; t=v->fgh[2]; c=ID(t); ht=VERB&AT(w)&&!b?c==CHOOK          :jtlp(jt,h);}
...
static A jtlrr(J jt,A w,A self,A *ltext){FPREFIP;A hs,t,*tv;C id;I fl,m;V*v;
 ...
 if(evoke(w)){RZ(w=jtsfne(jt,w)); if(FUNC&AT(w))w=lrr(w); return w;}  // keep named verb as a string, UNLESS it is NMDOT, in which case use the (f.'d) verb value
 ...
 if(1<m)RZ(tv[1]=jtincorp(jt,fl&VGERR?tiefn(jtinplace,jtfxeach(jt,gs,(A)&jtfxself[!!self]),ltext):lrr(gs)));  // fill in g if present
 if(0<m)RZ(tv[0]=jtincorp(jt,fl&VGERL?tiefn(jtinplace,jtfxeach(jt,fs,(A)&jtfxself[!!self]),ltext):lrr(fs)));  // fill in f (always present)
 return linsert(t,w);
}
...
// jt->lcp and jt->ltie are routines for handling adding enclosing () and handling `
 A jtlrep(J jt, A w){PROLOG(0056);A z;A ltextb=0, *ltext=&ltextb;
...
// jt->lcp and jt->ltie are routines for handling adding enclosing () and handling `
 A jtprep(J jt, A w){PROLOG(0057);A z;A ltextb=0, *ltext=&ltextb;
