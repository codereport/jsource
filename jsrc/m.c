...
// only protect the top level, because if the named value is incorporated at a lower level its usecount must be >1.
 A jtrat(J jt, A w){if(!w) return 0; ras(w); tpush(w); return w;}  // recursive.  w can be zero only if explicit definition had a failing sentence

A jtras(J jt, AD * RESTRICT w) { if(!w) return 0; realizeifvirtual(w); ra(w); return w; }  // subroutine version of ra() to save space
A jtra00s(J jt, AD * RESTRICT w) { if(!w) return 0; ra00(w,AT(w)); return w; }  // subroutine version of ra00() to save space
A jtrifvs(J jt, AD * RESTRICT w) { if(!w) return 0; realizeifvirtual(w); return w; }  // subroutine version of rifv() to save space and be an rvalue
A jtmkwris(J jt, AD * RESTRICT w) { if(!w) return 0; makewritable(w); return w; }  // subroutine version of makewritable() to save space and be an rvalue
...
A jtext(J jt,B b,A w){A z;I c,k,m,m1,t;
 if(!w) return 0;                               /* assume AR(w)&&AN(w)    */
