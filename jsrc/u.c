...
void mvc(I m,void*z,I n,void*w){I p=n,r;static I k=sizeof(D);  // ???
 ...
 memcpy(z,w,MIN(p,m)); while(m>p){r=m-p;
     memcpy(p+(C*)z,z,MIN(p,r)); p+=p;}
}
...
A jtstr(J jt,I n,C*s){A z; GATV0(z,LIT,n,1);
    memcpy(AV(z),s,n); return z;}
...
// w is a LIT string; result is a new block with the same string, with terminating NUL added
 A jtstr0(J jt, A w){A z;C*x;I n; if(!w) return 0; ASSERT(LIT&AT(w),EVDOMAIN); n=AN(w); GATV0(z,LIT,n+1,1); x=CAV(z);
     memcpy(x,AV(w),n); x[n]=0; return z;}
...
A jtvec(J jt,I t,I n,void*v){A z; GA(z,t,n,1,0);
    memcpy(AV(z),v,n<<bplg(t)); return z;}
