...
jtmult(J jt, I x, I y) {
    I z;
    I const lm   = 0x00000000ffffffffLL;
    I const hm   = 0xffffffff00000000LL;
    I const lmsb = 0x0000000080000000LL;
    I const hlsb = 0x0000000100000000;
    ...
    // if each argument fits in signed-32, do signed multiply; no overflow possible
    else if (!(hm & ((x + 0x80000000LL) | (y + 0x80000000LL))))
        z = x * y;
    else {
        ...
        if (((hm & (x + (x & hlsb))) && (hm & (y + (y & hlsb)))) || ((x == hm) && (y == hm))) {
            if (jt) jtjsignal(jt, EVLIMIT);
            return 0;
        }

        // otherwise, do full 64-bit multiply.  Don't convert neg to pos, because the audit for positive fails on MAXNEG
        // * 1
...
jtaii(J jt, A w) {
    I m;
    PROD(m, AR(w) - 1, 1 + AS(w));
    return m;
}

// return A-block for b+m*i.n
A
jtapv(J jt, I n, I b, I m) {
    A z;
    ...
    if (m == 1 && b >= IOTAVECBEGIN && b + n <= IOTAVECLEN + IOTAVECBEGIN) {
        ...
        AS(z)[0] = n;
        AN(z)    = n;
        AK(z)    = (C *)(iotavec + b - IOTAVECBEGIN) - (C *)z;
        AC(z)    = ACUC1;
...
cf(A w) {
    if (!w) return 0;
    return CAV(w)[0];
}  // first character in a character array

C
cl(A w) {
    if (!w) return 0;
    return CAV(w)[AN(w) - 1];
}  // last character in a character array

// A block for null-terminated C string, with a trailing NUL (which is not included in the AN of the string)
A
jtcstr(J jt, C *s) {
    A z;
    RZ(z = jtrifvs(jt, jtstr(jt, (I)strlen(s), s)));
    CAV(z)[AN(z)] = 0;
    return z;
}  // used only for initialization, so ensure real string returned.  The string has only the non-NUL, but add a trailing
   // NUL.  There's always room.

// Return 1 iff w is the evocation of a name.  w must be a FUNC
B
evoke(A w) {
    V *v = FAV(w);
    return CTILDE == v->id && v->fgh[0] && NAME & AT(v->fgh[0]);
}
...
jti0(J jt, A w) {
    ...
    if (AT(w) & FL) {
        D d    = DAV(w)[0];
        D e    = jround(d);
        I cval = (I)e;  // FL without call to cvt
        ...
        cval = d < (D)-IMAX ? -IMAX : cval;
        cval = d >= FLIMAX ? IMAX : cval;
        ASSERT(!AR(w), EVRANK);
        return cval;  // too-large values don't convert, handle separately
    }
    if (!(w = jtvi(jt, w))) return 0;
    ASSERT(!AR(w), EVRANK);
    return IAV(w)[0];
...
jtifb(J jt, I n, B *RESTRICT b) {
    ...
    I zbase = 0;
    UI *wvv = (UI *)b;
    UI bits = *wvv++;  // prime the pipeline for top of loop
...
I
jtmaxtype(J jt, I s, I t) {
    // If values differ and are both nonzero...
    I resultbit =
      jt->prioritytype[MAX(jt->typepriority[CTTZ(s)], jt->typepriority[CTTZ(t)])];  // Get the higher-priority type
    if (((s | t) & SPARSE) != 0) {
        ASSERT(!((s | t) & (C2T | C4T | XNUM | RAT | SBT)), EVDOMAIN);
        return (I)1 << (resultbit + SB01X - B01X);
    }                          // If either operand sparse, return sparse version
    return (I)1 << resultbit;  // otherwise, return normal version
}

// Copy m bytes from w to z, repeating every n bytes if n<m
void
mvc(I m, void *z, I n, void *w) {
    I p        = n, r;
    static I k = sizeof(D);  // ???
...
jtodom(J jt, I r, I n, I *RESTRICT s) {
    ...
    if (1 == n)
        ...
        I *zvo = zv - 1;
        DQ(n, *zv++ = 0;)  // init first row, point zvo to before the first value
        DQ(
          m - 1, I dontadd = 0; zv = zvo + n; zvo = zv + n; DQ(
            n, I out = *zv + 1 + dontadd; dontadd = REPSGN(out - s[i]); *zvo = out & dontadd; --zv;
            --zvo;))  // add 1 first time, & continue as long as there is carry.  Once dontadd goes to -1 it stays there
    }
    return z;
}
...
jtstrq(J jt, I n, C *s) {
    A z;
    I qc = 2;
    DO(n, qc += s[i] == '\'';) GATV0(z, LIT, n + qc, 1);
    C *zv                                                       = CAV(z);
    *zv++                                                       = '\'';
    DO(n, C c = s[i]; if (c == '\'') *zv++ = c; *zv++ = c;) *zv = '\'';
    return z;
}
...
jtvi(J jt, A w) {
    if (!w) return 0;
    return INT & AT(w) ? w : jtcvt(jt, INT, w);
}

// Audit w to ensure valid integer value(s).  Error if non-integral.  Result is A block for integer array.  Infinities
// converted to IMAX/-IMAX
A
jtvib(J jt, A w) {
    A z;
    D d, e, *wv;
    ...
    switch (UNSAFE(AT(w))) {
        default:
            ...
            for (i = 0; i < n; ++i) {
                ...
                cval  = d < (D)-IMAX ? -IMAX : cval;
                cval  = d >= FLIMAX ? IMAX : cval;
                zv[i] = cval;  // too-large values don't convert, handle separately
            }
            break;
        case XNUM:
...
jtvs(J jt, A w) {
    ...
    return LIT & AT(w) ? w : jtcvt(jt, LIT, w);
}
/* verify string */
...
jtvslit(J jt, A w) {
    ASSERT(1 >= AR(w), EVRANK);
    return LIT & AT(w) ? w : (C2T + C4T) & AT(w) ? jttoutf8(jt, w) : jtcvt(jt, LIT, w);
}
/* verify string */
