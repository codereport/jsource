...
// when they are running under debug.  Pseudo-named functions are created by namerefop.  We need to run them here so they get the debug side-effects of having a name.
 ...
 if(callstackx!=jt->callstacknext){  // normal case, with no stack, bypasses all this
  ...
  if(((jt->callstack[callstackx].type^CALLSTACKPOPLOCALE) | ((callstackx+1)^jt->callstacknext))==0) {   // jt->callstack[callstackx].type==CALLSTACKPOPLOCALE && callstackx+1==jt->callstacknext
...
A jtnameref(J jt,A w,A locsyms){
 return jtnamerefacv(jt,w,jtsyrd(jt,w,locsyms));  // get the symbol-table slot for the name (don't store the locale-name); return its 'value'
