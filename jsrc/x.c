...
static DF1(secf1){F1PREFIP; A h=FAV(self)->fgh[2]; ASSERT(!jt->seclev,EVSECURE); R CALL1IP((AF)AV(h)[0],  w,self);}
static DF2(secf2){F2PREFIP; A h=FAV(self)->fgh[2]; ASSERT(!jt->seclev,EVSECURE); R CALL2IP((AF)AV(h)[1],a,w,self);}
...
// (2,9) reserved for messages from host OS
  ...
  case XC(4,8):   R CDERIV(CIBEAM, jtscnl,       0,            VASGSAFE,RMAX,0,   0   );
  ...
  case XC(9,32):  R CDERIV(CIBEAM, jttlimq,      0,            VASGSAFE,RMAX,RMAX,RMAX); 
  case XC(9,33):  R CDERIV(CIBEAM, jttlims,      0,            VASGSAFE,RMAX,RMAX,RMAX); 
